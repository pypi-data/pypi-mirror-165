...
# print('import time:',time.time()-st1)
def read_pointsabtribution_x(Primitive=None,x=None,key=0):
    xml_p = Primitive.from_xml(x)
    svg_sp = SVG([xml_p])

    # 每一个path单独处理
    svg_sp = svg_sp.to_path().simplify_arcs()
    svg_path_fillcolor=list([svg_sp.svg_path_groups[i].fill for i in range(len(svg_sp.svg_path_groups))])
    svg_sp_tensor = svg_sp.to_tensor()
    s_indx = list(np.where(svg_sp_tensor[:, 0] == 0)[0]) + \
             [svg_sp_tensor.shape[0]]
    # s_indx = np.where(svg_sp.to_tensor()[:, 0] == 0)[0]
    # s_indx = np.append(s_indx,svg_sp.to_tensor().shape[0])
    # print('#indx#time used:', time.time() - st)
    s_hz = []
    # 是否是M开头Z结尾
    if len(s_indx) >= 3:
        in0 = np.array(s_indx[1:-1]) - 1
        s_hz = svg_sp_tensor[:, 0][np.array(in0)] == 6
        s_hz = list(np.array(s_hz).astype(np.int))
    if svg_sp_tensor[:, 0][-1] == 6:
        s_hz.append(1)
    else:
        s_hz.append(0)

    svg_path_indx=s_indx
    svg_path_hz=s_hz
    svg_path_tensor=svg_sp_tensor
    # print('svg_path_tensor',svg_path_tensor.shape)

    return {'key':key,'value':[xml_p,svg_sp,svg_path_fillcolor,svg_path_indx,svg_path_hz,svg_path_tensor]}

# 继承多线程类
class svgFuncThread(threading.Thread):
    def __init__(self, *kwargs):
        threading.Thread.__init__(self)    # 实例化父类的构造方法
        self.kwargs = kwargs    # 接受参数，用来传递
        self.resultList = []    # 用来存返回值

    # 自动执行
    def run(self):
      # 写你的逻辑代码, 给属性赋值
      self.resultList  = read_pointsabtribution_x(*self.kwargs)
...
def from_str(svg_str: str):
    ...
    for tag, Primitive in primitives.items():
        ...
        #     thread_child = svgFuncThread(Primitive, x,key)
        #     key+=1
        #
        #     # 开启线程
        #     thread_child.start()
        ...
        #     t.join()  # 等待线程执行完毕
        #
        #     # t.resultList 就是存储的返回值
        #     text.append(t.resultList)
        #
        # def funckey(d):
        #     return d['key']
        #
        # text.sort(key=funckey)  # 如果要重新排序
        #
        # svg_path_fillcolor=[]
        # svg_path_indx=[]
        # svg_path_hz=[]
        # svg_path_tensor=[]
        # for subtext in text:
        #     subvalue=subtext['value']
        #     svg_path_fillcolor.append(subvalue[2])
        #     svg_path_indx.append(subvalue[3])
        #     svg_path_hz.append(subvalue[4])
        #     svg_path_tensor.append(subvalue[5])
        # return svg_path_fillcolor,svg_path_indx,svg_path_hz,svg_path_tensor,domTree

            st = time.time()
            xml_p=Primitive.from_xml(x)
            svg_xmn_p = SVG([xml_p])
            print('read used time:', time.time() - st)
            svg_path_groups.append(xml_p)
            SVG_list.append(svg_xmn_p)
            # print('read used time:',time.time()-st)
...
def from_str_deepread(svg_str: str,apply='normal'):
    ...
    for tag, Primitive in primitives.items():
        for i,x in enumerate(rootNode.getElementsByTagName(tag)):
            ...
            svg_path_tensor.append(svg_sp_tensor)
...
def get_points_color_insvg(svg_path,p_n=10,filetype='url'):
    ...
    # svg_path_fillcolor,svg_path_indx,svg_path_hz,svg_path_tensor,domTree = from_str(svg_str)
    ...
    for svg_sp in svg_list:
        ...
        # s_indx = list(np.where(svg_sp_tensor[:, 0] == 0)[0]) + \
        #          [svg_sp_tensor.shape[0]]
        s_indx = np.where(svg_sp_tensor[:, 0] == 0)[0]
        s_indx = np.append(s_indx,svg_sp_tensor.shape[0])
        print('#indx#time used:', time.time() - st)
        s_hz = []
        # 是否是M开头Z结尾
        if len(s_indx) >= 3:
...
def main(svgpath=None,sample_n=10,filetype='path',c_svg=None,csvg_filetype='path',arch_per=0.5,pos='up',fix='top', mode='arch',file_path = None):
    ...
    if 'arch' in mode:
        ...
        print('simpleread time:', time.time() - st)
...
if __name__ == "__main__":
    ...
    svg,xml = main(svgpath=r"C:\Users\25790\Downloads\x(3).svg",c_svg='<svg viewBox="0 0 184.95 57.76" xmlns="http://www.w3.org/2000/svg"><path d="m184.95 57.76-92.48-9-92.47 9v-57.76l92.47 9 92.48-9z"/></svg>',
                   file_path=r'C:\Users\25790\Downloads\result22.svg', filetype='path',csvg_filetype='string',mode='random',pos='up',fix='negative',arch_per=0.3,sample_n=2)
    # print(time.time()-st1)
    # print(xml)
