...
class SVGPath:
    def __init__(self, path_commands: List[SVGCommand] = None, origin: Point = None, closed=False, filling=Filling.OUTLINE, translate=[0,0], rotate=0, scale=1):
    ...
    def from_xml(x: minidom.Element):
        ...
        transform = not x.hasAttribute("transform") or not x.getAttribute("transform") == "none"
        if transform == True:
            transform = x.getAttribute("transform")
            transform = re.split(r'[(,)\s]', transform)
            transform = np.array(transform)
            transform = transform[transform != '']
            transform = list(transform)
            if 'translate' in transform:
                translate_idx = transform.index('translate')
                translate = transform[translate_idx+1:translate_idx+3]
            else:
                translate = [0, 0]
            if 'rotate' in transform:
                rotate_idx = transform.index('rotate')
                rotate = transform[rotate_idx+1]
            else:
                rotate = 0
            if 'scale' in transform:
                scale_idx = transform.index('scale')
                scale = transform[scale_idx+1]
            else:
                scale = 1
        ...
        s = x.getAttribute('d')
        return SVGPath.from_str(s, fill=fill, filling=filling, translate=translate, rotate=rotate, scale=scale)

    @staticmethod
    def from_str(s: str, fill=False, filling=Filling.OUTLINE, add_closing=False, translate=[0,0], rotate=0, scale=1):
        path_commands = []
        ...
        return SVGPath.from_commands(path_commands, fill=fill, filling=filling, add_closing=add_closing, translate=translate, rotate=rotate, scale=scale)
    ...
    @staticmethod
    def from_commands(path_commands: List[SVGCommand], fill=False, filling=Filling.OUTLINE, add_closing=False, allow_empty=False, translate=[0,0], rotate=0, scale=1):
        from .svg_primitive import SVGPathGroup
        ...
        for command in path_commands:
            if isinstance(command, SVGCommandMove):
                ...
                svg_path = SVGPath([], command.start_pos.copy(), filling=filling, translate=translate, rotate=rotate, scale=scale)
            else:
        ...
        if svg_path is not None and (allow_empty or svg_path.path_commands):  # SVGPath contains at least one command
            ...
            if not svg_path.path_commands:
                svg_path.path_commands.append(empty_command)
            svg_paths.append(svg_path)
        return SVGPathGroup(svg_paths, fill=fill, translate=translate, rotate=rotate, scale=scale)
