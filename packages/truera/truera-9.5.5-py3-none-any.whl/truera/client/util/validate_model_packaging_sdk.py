...
def validate_packaged_python_model(
    ...
    if is_regression_model:
        if not isinstance(ys_pred, np.ndarray):
            logger.info(MALFORMED_MODEL_PREDS_MSG)
            raise AssertionError(
                f"Predict call on model outputted object of type {type(ys_pred)}, expected np.ndarray for regression model!"
            )

        if ys_pred.shape != (1,):
            logger.info(MALFORMED_MODEL_PREDS_MSG)
            raise AssertionError(
                f"Predict call on model outputted np.ndarray of shape {ys_pred.shape}, expected shape (1,) for regression model!"
            )
