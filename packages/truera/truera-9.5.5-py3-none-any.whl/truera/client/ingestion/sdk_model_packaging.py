...
class PythonModelPackager(ABC):
    ...
    def __init__(
        self, model: Any, model_output_type: str, python_version: str,
    ...
    def _dump_pickle_and_save_model(
        ...
        with open(pickle_file_temp_location, "wb") as pickle_file_temp:
            if additional_modules is None:
                additional_modules = []
            try:
                for curr in additional_modules:
                    cloudpickle.register_pickle_by_value(curr)
                cloudpickle.dump(self.model, pickle_file_temp)
            finally:
                for curr in additional_modules:
                    try:
        ...
        try:
            if transformer is not None:
                ...
                with open(
                    transform_pickle_file_temp_location, "wb"
                ) as pickle_file_temp:
                    cloudpickle.dump(transformer, pickle_file_temp)
        except Exception as e:
            logger.error(f"Failure while handling transform: {e}")
            pass
...
class XgBoostModelPackager(PythonModelPackager):
    ...
    def __init__(
        ...
        super().__init__(
            model, model_output_type, python_version,
    ...
    def module_package_mapping(
        ...
        if module.startswith("xgboost"):
            ...
            LightGBMModelPackager(
                model, self.model_output_type, self.python_version,
