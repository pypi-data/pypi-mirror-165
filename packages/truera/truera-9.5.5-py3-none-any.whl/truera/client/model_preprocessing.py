...
class FrameworkSupportSpec:
    ...
    def __init__(self, name, root_framework, output_kinds, is_framework):
        super().__init__()
        self.name = name
...
class PipDependencyParser:
    ...
    def _parse_pip_deps(self):
        ...
        for dep_str in self.pip_dep_list:
            ...
            package_name, version_str = dep_str_sliced
    ...
    def add_dependency(self, package_name: str, version_number: str):
        if package_name in self.package_names:
            raise ValueError(
                f"Already specified dependency with package name {package_name}!"
            )
...
BUILT_IN_FRAMEWORK_WRAPPER_MAP = {
    ...
    "tensorflow":
        FrameworkSupportSpec("tensorflow", "tensorflow", ["classification"],
                             True),
    "pytorch":
        FrameworkSupportSpec("pytorch", "pytorch", ["classification"], True),
    "pyspark":
...
def prepare_python_model_folder(
    ...
    if framework_wrapper in BUILT_IN_FRAMEWORK_WRAPPER_MAP.keys():
        wrapper_file_source_name = BUILT_IN_FRAMEWORK_WRAPPER_MAP[
            framework_wrapper
        ].root_framework + "_" + model_output_type + "_predict_wrapper.py"
        wrapper_file_name = framework_wrapper + "_" + model_output_type + "_predict_wrapper.py"
    ...
    if conda_file is not None:
        ...
        if pip_requirements_file:
            with open(pip_requirements_file) as f:
                ...
                dependencies = ""
        conda = CondaTemplate(
            dependencies, pip_dependencies, python_version=python_version
        )
