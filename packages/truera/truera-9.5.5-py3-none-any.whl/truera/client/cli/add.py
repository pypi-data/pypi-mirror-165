...
def nn_project(
    project_name, score_type, input_data_format, connection_string, log_level
):
...
def _create_nn_project(
    ...
    with cl.ArtifactInteractionClient(ar_client, ds_client, logger) as client:
        project = client.create_project(
            project_name, score_type, input_data_format=input_data_format
        )
        output_dict = {
...
@click.option("--log_level", default=None, help=cli_logger.log_level_message)
def project(project_name, score_type, connection_string, log_level):
    UncaughtExceptionHandler.wrap(
        _create_project, project_name, score_type, connection_string, log_level
    )


def _create_project(project_name, score_type, connection_string, log_level):
    logger = cli_logger.create_cli_logger(log_level)
    ar_client = cli_utils.get_artifact_repo_client(connection_string, logger)
    ds_client = cli_utils.get_data_service_client(connection_string, logger)

    with cl.ArtifactInteractionClient(ar_client, ds_client, logger) as client:
        project = client.create_project(project_name, score_type)
        output_dict = {
            "project_name": project_name,
            "project_id": project.id,
            "score_type": score_type
...
def _add_explanation_cache(
    ...
    with cl.ArtifactInteractionClient(ar_client, ds_client, logger) as client:
        ...
        explanation_cache = project.create_explanation_cache(
            model_name, data_collection_name, split_name,
            path_to_explanation_cache, score_type, model_output_type,
            client_name, client_version
        )
