...
class ArtifactRepoClient:
    ...
    def create_cache_metadata(
        ...
        Args:
            model_id: model that created predictions
            project_id: project model resides in
            data_collection_name: data collection for the model 
            split_name: data split for the model
            location: location of the cache in ArtifactRepo
            format: format of the input file (eg csv or parquet)
        ...
        req = self._build_create_cache_request(
            project_id, model_id, data_collection_name, split_name,
            background_data_split_name, location, format, score_type, row_count,
            cache_type, client_name, client_version
        )

        try:
            response = self.communicator.put_metadata(req)
        except AlreadyExistsError as e:
            message = f"The cache entity for model id {model_id} of type {cache_type} already exists."
            self.log(message)
            e.message = message
            raise e
        return response.id
    ...
    def _build_create_project_metadata_request(
        self, project_name: str, score_type: str, input_data_format: str
    ):
    ...
    def _build_create_cache_request(
        ...
        background_data_split_name, location, format, score_type, row_count,
        cache_type: CacheEnum, client_name, client_version
    ):
        ...
        proto_cache_type = {
            CacheEnum.EXPLANATION:
                md_pb.CacheType.EXPLANATION_CACHE,
            CacheEnum.MODEL_PREDICTION:
                md_pb.CacheType.MODEL_PREDICTION_CACHE,
            CacheEnum.PARTIAL_DEPENDENCY:
                md_pb.CacheType.PARTIAL_DEPENDENCE_PLOT_CACHE,
        }[cache_type]
