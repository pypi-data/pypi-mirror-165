...
class LayerAnchor(Enum):
    ...
    def to_string(i):
        if i is None:
            return None
        elif i == LayerAnchor.IN:
            return "in"
        elif i == LayerAnchor.OUT:
            return "out"
        else:
...
class RNNAttributionConfiguration(AttributionConfiguration):
    ...
    input_layer (required): str, The name of the layer to run attributions.
        Can also be a `truera.client.nn.client_configs.Layer.IN`.
    ...
    output_layer (required): str, The name of the final layer that is being explained when examining model outputs.
        Can also be a `truera.client.nn.client_configs.Layer.OUT`.
    n_time_step_output (required): int, The number of output timesteps.
    n_output_neurons (required): int, The number of output classes.

    Optional Parameters
    =========================
    n_baseline_records (optional): int, The number of records to to generate a baseline for attributions.
        Higher numbers may increase reliability of attributions.
        If unspecified, a Truera selected default will be used.

    internal_layer (optional): str, The name of an intermediate layer that is being explained when examining internal neuron outputs.
        This is used to explain neurons in a network.
    n_internal_neurons (optional): int, The number of neurons in the internal tensor.

    input_anchor (optional): client_configs.LayerAnchor, Determines the tensor to run attributions.
        It can be the `in` or `out` tensor of the `input_layer`. Defaults to `out`
    output_anchor (optional): client_configs.LayerAnchor, determines the output tensor to explain.
        It can be the `in` or `out` tensor of the `output_layer`. Defaults to `out`
    internal_anchor (optional): client_configs.LayerAnchor, Determines the internal tensor to run attributions.
        It can be the `in` or `out` tensor of the `internal_layer`. Defaults to `out`

    input_dimension_order (optional): list/tuple[Dimension], The order of which input feature dimensions are expected.
    ...
    output_dimension_order (optional): list/tuple[Dimension], The order of which input feature dimensions are expected.
    ...
    internal_dimension_order (optional): list/tuple[Dimension], The order of which input feature dimensions are expected.
    ...
    use_cuda (optional): bool, Whether to use CUDA for accelerated computing. Defaults to True
    use_training_mode (optional): bool, Determines to set the model to training mode which could cause nondeterministic model behavior. Defaults to False
    """
    forward_padded: bool
...
class NLPAttributionConfiguration(AttributionConfiguration):
    ...
    ref_token (optional): str, Used for integrated gradients baseline. Typically a mask or informationless token.
    resolution (optional): int, Used for integrated gradients baseline. The number of interpolation points to use from baseline to value.
    rebatch_size (optional): int, Used for memory constraints with integrated gradients. Resolution will expand the inputsize multiplicatively. 
        If memory issues arise, choose rebatch size to bring the expanded batchsize back down.

    embedding_anchor (optional): client_configs.LayerAnchor, Determines the tensor to run attributions.
    ...
    shuffle_data (optional): bool, A flag on whether to shuffle the data.
    use_cuda (optional): bool, Whether to use CUDA for accelerated computing. Defaults to True
    use_training_mode (optional): bool, Determines to set the model to training mode which could cause nondeterministic model behavior. Defaults to False
    """
    n_metrics_records: int
    embedding_layer: Union[str, Layer]
    output_layer: Union[str, Layer]
    n_output_neurons: int
    ...
    def input_layer(self):
        # For compatibility with attrconfig checks
        return self.embedding_layer

    @property
    def input_anchor(self):
        # For compatibility with attrconfig checks
        return self.embedding_anchor
