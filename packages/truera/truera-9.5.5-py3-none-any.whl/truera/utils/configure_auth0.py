...
class TruEraDeployment:
    ...
    def auth0_client_name(self) -> str:
        return f"{self.deployment_id}-client"
...
class Auth0CreateConnectionBody:
    ...
    # TODO: Add field to disable signup. May not be necessary depending on New vs. Classic Login
...
def build_client_oidc_config(
    ...
    config_lst = [
        ...
        f'&client_id={client.client_id}"', 'export TRUERA_KONG_OIDC_REALM="/"\n'
    ]
    return "\n".join(config_lst)
...
class Auth0Initializer():
    ...
    def __init__(self, auth0_config: Auth0Config, logger: Logger) -> None:
        ...
        self.managment_token = self._get_management_token()
        self.auth0 = Auth0(
            domain=self.auth0_config.domain, token=self.managment_token
        )
    ...
    def get_or_create_connection(
        ...
        create_connection_body = Auth0CreateConnectionBody(name=conn_name)
