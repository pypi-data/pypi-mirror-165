...
TRUERA_USERMANAGMENT_CLIENT_CREDS_FILENAME = "truera-usermanagement-client.json"
...
class TrueraConfiguration:
    ...
    def usermanagement_client_creds_filepath(self) -> str:
        return os.path.join(
            self.truera_secrets_home, TRUERA_USERMANAGMENT_CLIENT_CREDS_FILENAME
        )
...
class KeycloakCommunicator():
    ...
    def get_admin_role(self) -> KeylcoakUserRole:
        url = f"{self.connection_string}/admin/realms/master/roles/admin"
        resp = self._make_req(url, method="GET")
        return KeylcoakUserRole(resp.json()["id"], resp.json()["name"])

    def grant_role_to_user(
        self, role: KeylcoakUserRole, user: KeycloakUserInfo
    ):
...
def add_keycloak_user_as_truera_admin(
    ...
    if response.status_code != 200:
        logger.warning(
            f"Failed to add keycloak user '{truera_keycloak_admin_user}' as truera admin"
        )
