...
class HttpServer(Worker):
    ...
    def _configure(self, sock: socket.socket = None, ext_router: ExtRouter = None,
        ...
        if mdw_manager:
            self.mdw_manager.debug = self.debug
    ...
    async def broadcasts_rt(self, message: GenericIMessage):
        promises = []
...
def create_rt_subscribes(*workers: HttpServer,
    ...
    for worker in workers:
        worker.channel.add_event_listener(event_type, worker.broadcasts_rt, **kwargs)
