...
function xhr(url, cb){
    ...
    xhr.onreadystatechange = function(){
        if (xhr.readyState === XMLHttpRequest.DONE) cb(xhr);
    }
    xhr.open("GET", url);
    xhr.send();
}
...
xhr("/screenshot?"+(new Date).getTime(), function(_this){
    let taken = (new Date).getTime()-stime;
    setInterval(get_screenshot, taken+500);
});
</script>'''
