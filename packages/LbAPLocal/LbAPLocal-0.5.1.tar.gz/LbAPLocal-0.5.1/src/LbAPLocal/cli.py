...
import click
import requests
from LbAPCommon import get_api_token, parse_yaml, render_yaml, validate_yaml
...
def wipe_token_cache(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    from LbAPCommon import wipe_token_cache

    wipe_token_cache(LBAPI_CLIENT_ID)
    click.secho("Token cache wiped successfully", fg="green")
    ctx.exit()
...
@click.option(
    "--wipe-cache",
    is_flag=True,
    callback=wipe_token_cache,
    expose_value=False,
    is_eager=True,
)
def login():
    """Login to the Analysis Productions API"""
    token = get_api_token(LBAPI_CLIENT_ID, f"{LBAPI_HOST}/user")
    r = requests.get(f"{LBAPI_HOST}/user", headers={"Authorization": f"Bearer {token}"})
    r.raise_for_status()
...
@click.argument(
    "clone_type", type=click.Choice(["ssh", "https", "krb5"]), default="ssh", nargs=1
)
def clone(
    version: str,
    working_group: str,
    ...
    Specify clone_type as either ssh, https or krb5."""
    if clone_type not in ["ssh", "https", "krb5"]:
        raise click.ClickException(f"{clone_type} is not a valid cloning option!")
    ...
    if os.path.isdir("./AnalysisProductions"):
        ...
        _checkout(
            version, working_group, production_name, branch_name, allow_deletes=True
        )
    else:
        raise click.ClickException(
