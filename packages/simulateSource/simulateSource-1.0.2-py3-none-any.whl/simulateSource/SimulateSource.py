...
from SingleSimulateParams import SingleSimulateParams
from BLETransmissionConstant import BLETransmissionConstant
from SerialCommunication import TransDevice
from Packet import BLEPacket
from TraceSimulateParams import TraceSimulateParams
...
class SimulateSource(TransDevice.ITransDeviceInterface):
    ...
    # readList = queue.Queue(maxsize=1000)
    ...
    def __init__(self, com):
        self.com = com
        self.list = []
        self.transDevice = TransDevice(self.com, 115200, 0.5)
    ...
    def _startSimulate(self):
        msg = self.packet.getCmdTran(BLETransmissionConstant.START_SIMULATE, None)
    ...
    def _stopSimulate(self):
        msg = self.packet.getCmdTran(BLETransmissionConstant.STOP_SIMULATE, None)
        self.transDevice.sendMessage(msg)
        # print("msg",msg)
        while True:
            # print("msg")
    ...
    def setSignalGain(self, gainIndB):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.SET_OUTPUT_SIGNAL_GAIN, gainIndB)
        self.transDevice.sendMessage(msg)
        self.cmdPacket = self.cmdQueue.get()
        while True:
            i += 1
    ...
    def setSimulateSceneTime(self, startTime):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.SET_SIMULATE_SCENE_TIME, startTime)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def enableStaticScene(self, staticEnable):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.ENABLE_STATIC_SCENE, staticEnable)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def enableLocalEPHData(self , ephLocalEn):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.ENABLE_LOCAL_EPH_DATA, ephLocalEn)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def _setSimulateSourceDisplaySpeed(self, speed):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.SET_SIMULATESOURCE_DISPLAY_SPEED, speed)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def setRFTestState(self, setRF):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.SET_RF_TEST_STATE, setRF)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def _queryDeviceState(self):
        ...
        while True:
            time.sleep(10)
            msg = packet.getCmdTran(BLETransmissionConstant.QUERY_DEVICE_STATE, None)
            self.transDevice.sendMessage(msg)
            # print("状态请求", msg)
            self.cmdPacket = self.cmdQueue.get()
    ...
    def responseDeviceTranceRequestFrame(self):
        msg = self.packet.getCmdTran(BLETransmissionConstant.RESPONSE_DEVICE_TRACE_REQUEST_FRAME, None)
        self.transDevice.sendMessage(msg)

    # 重置星历数据index值
    def resetEphTranIndex(self, index):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.RESET_EPHTRAN_INDEX, index)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def resetTraceTran(self, index):
        i = 0
        msg = self.packet.getCmdTran(BLETransmissionConstant.RESET_TRACETRAN_INDEX, index)
        self.transDevice.sendMessage(msg)
        while True:
            i += 1
            time.sleep(0.2)
    ...
    def setSingleSimulateParams(self, singleSimulateParams):
        ...
        # if singleSimulateParams.longitude > 180:
        #     self.traceMsgError = True
        # elif singleSimulateParams.latitude > 90:
        #     self.traceMsgError = True
    ...
    def startSingleModeSimulate(self):
        ...
        self.transDevice.sendMessage(msg)
        time.sleep(2)
        ENABLE_STATIC_payload = [BLETransmissionConstant.ENABLE_STATIC]
    ...
    def startTraceModeSimulate(self):
        self._stopSimulate()
        # 读取参数并发送对应命令
        # self.readMessage(self.traceSimulateParams.fileName)
        ...
        simulateDisplaySpeed = self.traceSimulateParams.simulateDisplaySpeed
        self._setSimulateSourceDisplaySpeed([simulateDisplaySpeed])
        # # 回调开始轨迹模拟初始化
        ...
        while True:
            ...
            if self.tensor:
                ...
                if not self.readList.empty():
                    if i >=300:
                        # 回调发送超时
                        self.sendTrackTimeOut = True
                        if self.simulateSourceInterface:
                            self.simulateSourceInterface.onListenSendTrackTimeOut(self.sendTrackTimeOut)
                        i -= 300
                        # self.queryDeviceState()
                        self.sendTrackTimeOut = False
                else:
                    # 队列为空则回调数据发送完成
    ...
    def tensorCheck(self):
        ...
        while True:
            # print("222")
            time.sleep(0.1)
            if self.tensorcheck == True:
                # 队列判空操作
                if not self.readList.empty():
                    self.tensorList = self.readList.get()
                    print(float(self.tensorList[14:28]),
                                          float(self.tensorList[:13]),
                                          float(self.tensorList[29:35]))
                    ...
                    msg = self.packet.getTensor(i, self.traceSimulateParams.longitude,
    ...
    def parsePacket(self):
        while True:
            self.recvPacket = self.recvQueue.get()
