...
class PemBank:
    ...
    def check(self, checkType: PEM_CHECK_TYPE, showCheck: bool = True) -> bool:
        if checkType in [
            PEM_CHECK_TYPE.CHECK_TYPE_SSL_SERVER_IDENTITY,
            PEM_CHECK_TYPE.CHECK_TYPE_SSL_CLIENT_IDENTITY,
        ]:
            return self._checkTrustChain(showCheck)
    ...
    def _checkTrustChain(self, showCheck: bool) -> bool:
        ...
        if len(self.trustChains) != 1:
            # no 1 and only 1 chain of trust is established
            return False
    ...
    def _checkNumberLimits(
        ...
        if exceptions:
            logger.exception(
                "pem bundle '%s' is malformed",
                self._pemSimpleBank.bundleFilePath,
            )
            raise
...
class PemBundleParser:
    ...
    def _loadPemBundleFile(self, pemFilePath):
        ...
        if not pemFile.is_file() or not pemFile.exists():
            logger.error("'%s' is not found", pemFilePath)
...
class PemViewer:
    ...
    def log(self, pemBundleName: str):
        ...
        for pemInfo in self._inspections.values():
            summary = [
                f"{key}: '{value}'" for key, value in pemInfo.summary.items()
            ]
