...
class LinearConstraint:
    ...
    def build_polytope_sampler_constraint(self) -> [Tuple[Tensor, Tensor, float]]:
        ...
        """
        # How to handle this index when the continuous variable is after a categorical with descriptors?! Need to count the number of descriptors?
        # Can we somehow use the headings + descriptors attribute that we used for the continuous permutation feature importance?!
    ...
    def bool_evaluate_constraint(self, X: np.ndarray) -> np.ndarray:
        ...
        Evaluate whether the equality or inequality linear constraint is met and returns a boolean
...
class CategoricalConstraint:
    ...
    def bool_evaluate_constraint(self, X: np.ndarray, descriptor_to_name: bool = True) -> np.ndarray:
        """
        ...
        X: np.ndarray
            The X array to be evaluated. Can be more than one row of X-values
        descriptor_to_name: bool
            If descriptors are used to describe the categorical variables in the X array, this needs to be set to True
...
class ConstraintsList:
    ...
    def bool_evaluate_constraints(self, X: np.ndarray, descriptor_to_name: bool = True) -> np.ndarray:
        pass_fail = np.zeros((X.shape[0], self.n_constr))

        # Needed for all constraints other than CategoricalConstraint because it ensures that the index is correct.
        # Index must refer to the index when categoricalwithdescriptors have descriptors present (not names)
        ...
        for constr_index, constr in enumerate(self.constraints):
            if not isinstance(constr, CategoricalConstraint):
                ...
                pass_fail[:, constr_index] = constr.bool_evaluate_constraint(X, descriptor_to_name)
