...
@pytest.fixture
def apinfo_cache(tmp_path, mocked_responses, patched_get_api_token):
...
def test_fetch(mocked_responses, patched_get_api_token):
    pi = fetch_ap_info("SL", "RDs")
    assert len(pi.PFNs()) == 58


def test_fetch_withdate(mocked_responses, patched_get_api_token):
    ap_date = datetime.datetime(2021, 12, 1, 9, 0, 0).isoformat()
...
def test_cache(mocked_responses, tmp_path, patched_get_api_token):
    cachedir = tmp_path / "cache"
    os.makedirs(cachedir)
...
def test_cache_withdate(mocked_responses, tmp_path, patched_get_api_token):
    ap_date = datetime.datetime(2021, 12, 1, 9, 0, 0).isoformat()
