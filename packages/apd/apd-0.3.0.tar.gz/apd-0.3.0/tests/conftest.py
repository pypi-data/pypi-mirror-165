...
@pytest.fixture
def patched_get_api_token(monkeypatch):
    def patched_get_api_token(app, url):
        return ""

    monkeypatch.setattr(apd.auth, "get_api_token", patched_get_api_token)
    yield


@pytest.fixture
def apinfo_cache(tmp_path, mocked_responses, monkeypatch, patched_get_api_token):
