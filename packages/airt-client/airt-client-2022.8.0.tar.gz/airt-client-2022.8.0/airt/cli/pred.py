...
def to_s3(
    ...
    if quiet:
        status.wait()
        typer.echo(
            f"Results for Prediction uuid: {pred.uuid} are successfully pushed to the s3 bucket."
        )
    else:
        typer.echo(
...
def to_azure_blob_storage(
    ...
    credential: Optional[str] = typer.Option(
        None,
        help="Azure Blob Storage credential. If **None** (default value), then the value from **AZURE_STORAGE_CREDENTIAL** environment variable will be used.",
    ),
    quiet: bool = typer.Option(
        False,
        "--quiet",
    ...
    if quiet:
        status.wait()
        typer.echo(
            f"Results for Prediction uuid: {pred.uuid} are successfully pushed to the Azure Blob Storage."
        )
    else:
        typer.echo(
...
def to_local(
    ...
    if quiet:
        ...
        typer.echo(
            f"Results for Prediction uuid: {pred.uuid} has been successfully downloaded."
        )
    else:
...
def to_mysql(
    ...
    if quiet:
        status.wait()
        typer.echo(
            f"Results for Prediction uuid: {pred.uuid} are successfully pushed to the mysql database."
        )
    else:
        typer.echo(
...
def to_clickhouse(
    ...
    if quiet:
        status.wait()
        typer.echo(
            f"Results for Prediction uuid: {pred.uuid} are successfully pushed to the clickhouse database."
        )
    else:
        typer.echo(
