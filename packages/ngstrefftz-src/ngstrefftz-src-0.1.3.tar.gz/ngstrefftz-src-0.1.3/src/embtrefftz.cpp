...
namespace ngcomp
{
                                       ...
                                       )
    {
        ...
        ma->IterateElements(VOL,lh,[&](auto ei, LocalHeap & mlh)
        {
            ...
            if(stats) singular_values[ei.Nr()] = Vector<SCAL>(elmat.Diag());
        ...
        if(stats){
            //Singular value avg
            size_t maxdofs = 0;
            for( auto v : singular_values ) maxdofs=max(maxdofs, v.Size());
            Vector<SCAL> sing_val_avg(maxdofs);
            sing_val_avg = 0;
            for( auto v : singular_values ) if(v.Size()>0) sing_val_avg += v;
            ...
            for(auto &v : sing_val_var) v=sqrt(v);
...
void ExportEmbTrefftz(py::module m)
{
    m.def("TrefftzEmbedding", [] (shared_ptr<ngfem::SumOfIntegrals> bf,
                            ...
                            )
          {
              ...
              if(fes->IsComplex()){
                  ...
                  auto P = ngcomp::EmbTrefftz<Complex>(bf,fes,lf,eps,test_fes,tndof,&stats);
                  if(pystats) for(auto const& x : stats) (*pystats)[py::cast(x.first)]=py::cast(x.second);
                  return P;
              }
              else {
                  std::map<std::string,ngcomp::Vector<double>> stats;
                  auto P = ngcomp::EmbTrefftz<double>(bf,fes,lf,eps,test_fes,tndof,&stats);
                  if(pystats) for(auto const& x : stats) (*pystats)[py::cast(x.first)]=py::cast(x.second);
                  return P;
              }
          }, R"mydelimiter(
    ...
    py::arg("bf"), py::arg("fes"), py::arg("lf"), py::arg("eps")=0, py::arg("test_fes")=nullptr, py::arg("tndof")=0, py::arg("stats_dict")=py::none());
    ...
    m.def("TrefftzEmbedding", [] (shared_ptr<ngfem::SumOfIntegrals> bf,
                                 ...
                                 ) -> shared_ptr<ngcomp::BaseMatrix>
          {
              ...
              {
                  std::map<std::string,ngcomp::Vector<Complex>> stats;
                  auto P = std::get<0>(ngcomp::EmbTrefftz<Complex>(bf,fes,nullptr,eps,test_fes,tndof,&stats));
                  if(pystats) for(auto const& x : stats) (*pystats)[py::cast(x.first)]=py::cast(x.second);
                  return P;
              }
              else {
                  std::map<std::string,ngcomp::Vector<double>> stats;
                  auto P = std::get<0>(ngcomp::EmbTrefftz<double>(bf,fes,nullptr,eps,test_fes,tndof,&stats));
                  if(pystats) for(auto const& x : stats) (*pystats)[py::cast(x.first)]=py::cast(x.second);
                  return P;
              }
          }, R"mydelimiter(
    ...
    py::arg("bf"), py::arg("fes"), py::arg("eps")=0, py::arg("test_fes")=nullptr, py::arg("tndof")=0, py::arg("stats_dict")=py::none());
