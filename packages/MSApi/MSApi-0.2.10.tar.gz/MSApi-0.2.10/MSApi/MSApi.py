...
class MSApi(MSLowApi):
    ...
    @classmethod
    def load_document(cls, object: ObjectMS, template: Template, extension: EPrintExtension, **kwargs):

        request = f"/entity/{object.get_typename()}/{object.get_id()}/export"
        request_json = {
            'template': {
                'meta': template.get_meta().get_json()
            },
        ...
        response = cls.auch_post(request, json=request_json, **kwargs)
        if response.status_code == 303:
            url = response.json().get('Location')
            file_response = requests.get(url)
            data = file_response.content
