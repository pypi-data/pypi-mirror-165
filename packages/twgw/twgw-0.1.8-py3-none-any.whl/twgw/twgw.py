...
def mqtt_client(g_info, client_id, b_type):
    ...
    def on_message(client, userdata, message):
        ...
        """
        try:
            g_info["info"]["client"][client_id]["on_message"](message)
        except Exception:
...
def sub_pulse(g_info, client_id):
    ...
    try:
        ...
        if g_info["info"]["pulse"]["level"] == "unusual":
            ...
            pulse_publish(g_info, client_id)
        else:
...
def pulse_publish(g_info, client_id):
    ...
    try:
        ...
        g_info["info"]["client"][client_id]["con"].publish(g_info["info"]["debug"]["p_topic"], 
