...
class AUC:
    ...
    def active_packages(self):
        ...
        try:
            ...
            logging.error(f"Get call to ActivePackages endpoint resulted in a ConnectionError \n {ex}")

    @property
    def edt_is_installed(self):
        try:
            edt_install_status = Clumper(self.active_packages).keep(lambda d: 'EDT Framework' in d["Description"]).keep(
                lambda d: d['Status'] == "Success").blob

        except KeyError as ex:
            logging.error(f"EDT Framework was not found in ActivePackages \n{ex}")
            edt_install_status = []
        if edt_install_status:
            return True
        try:
            if edt_update := regobj.HKLM.SOFTWARE.WOW6432Node.get_subkey('AGCO Corporation').EDT['UpdateNumber'].data:
                return True
        except (AttributeError, KeyError) as e:
            logging.error(f'UpdateNumber was not present in registry. It does not appear that EDT is currently '
                          f'installed.\n {e}')
    ...
    def client_status(self):
        ...
        if r.status_code == 200:
            logging.debug("AUC's client status is refreshed.")
    ...
    def download(self):
        ...
        try:
            ...
            try:
                ...
                logging.error(f'Unable to download the AUC client \n {ex}')
        except Exception as e:
            logging.error(f'The link to download the latest AUC is down \n{e}')
    ...
    def restart(self):
        logging.info('Attempting to restart AUC')
...
class EDT:
    ...
    def get_license_id(self, voucher):
        logging.info(f'Attempting to get license id using the voucher code: {voucher}')
    ...
    def add_services_to_license(self):
        ...
        logging.debug('Attempting to add AuthCode to License for Dev environment')
...
def status_watcher(vouchertype, auc, edt):
    ...
    while idle_counter <= 4:
        auc.client_status
        if auc.auc_state in {"ReadyToInstall", "InstallingNeedsResume"}:
            auc.install_now()
            continue
        ...
        if auc.auc_state == "Installing":
            auc.active_packages
            if auc.edt_is_installed and not edt.voucher:
