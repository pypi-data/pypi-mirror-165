...
class App(wx.App):
  ...
  def OnInit(self):
    ...
    self.use_get_graph = hasattr(Hardware, "GetGraph")
    self.use_get_smeter = hasattr(Hardware, "GetSmeter")
  ...
  def MakeButtons(self, frame, gbs):
    ...
    self.smeter_menu.Append('S-meter 1', self.OnSmeterMeterA)
    self.smeter_menu.Append('S-meter 5', self.OnSmeterMeterB)
    self.smeter_menu.Append('Frequency 2',  self.OnSmeterFrequencyA)
    self.smeter_menu.Append('Frequency 10', self.OnSmeterFrequencyB)
    self.smeter_menu.Append('Audio 1', self.OnSmeterAudioA)
    self.smeter_menu.Append('Audio 5', self.OnSmeterAudioB)
  ...
  def OnReadSound(self):	# called at frequent intervals
    ...
    if self.screen == self.scope:
      ...
      if self.use_get_graph:
        data = Hardware.GetGraph(0, 1.0, 0)
      else:
      ...
      if self.use_get_graph:
        data = Hardware.GetGraph(1, self.zoom, float(self.zoom_deltaf))	# get FFT data and discard
      else:
      ...
      if self.use_get_graph:
        data = Hardware.GetGraph(1, self.zoom, float(self.zoom_deltaf))	# get FFT data
      else:
        data = QS.get_graph(1, self.zoom, float(self.zoom_deltaf))	# get FFT data
      if data:
        #T('')
        if self.use_get_smeter:
