...
SELECT TOP (1000) [locked]
  ...
  -- 0.4
    ...
    def test_compute_isda_upfront(self):
        ...
        self.sdate = datetime.datetime(2022, 8, 31)
        ...
        for credit_spread, recovery_rate in zip(self.credit_spread_list, self.recovery_rate_list):
            ...
            upfront_charge_dirty, upfront_charge_clean, status, duration_in_milliseconds = f
            ai = upfront_charge_dirty - upfront_charge_clean
            ...
            ai *= 1 if self.is_buy_protection else -1

            "pv_dirty {pv_dirty * 1e6:,.2f} "
                  f"pv_clean {pv_clean * 1e6:,.2f} """
            ...
            print(f"credit_spread {credit_spread * 10000.:,.2f} "
                  ...
                  f"accrued {ai * 1e6:,.2f} "
                  f"clean_settlement_amount {(upfront_charge_dirty - ai) * 1e6:,.2f} "
                  f"dirty_settlement_amount: {(upfront_charge_clean) * 1e6:,.2f} ")
        ...
        self.assertAlmostEqual(ai * 1e6, -24333.3333333333)
