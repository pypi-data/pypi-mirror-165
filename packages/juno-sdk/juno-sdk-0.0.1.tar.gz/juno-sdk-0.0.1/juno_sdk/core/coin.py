...
class Coin(JSONSerializable):
    ...
    denom: str = attr.ib()
    """Coin's denomination, only ``ujuno``."""
    ...
    def from_str(cls, string: str) -> Coin:
        ...
        ``283923ujuno`` (``int``-Coin) or ``23920.23020ujuno`` (:class:`Dec`-Coin).

        >>> int_coin = Coin.from_str("230920ujuno")
        >>> int_coin.denom
        'ujuno'
        >>> int_coin.amount
        230920
        >>> dec_coin = Coin.from_str("203922.223ujuno")
        >>> dec_coin.denom
        'ujuno'
        >>> dec_coin.amount
        Dec('203922.223')
