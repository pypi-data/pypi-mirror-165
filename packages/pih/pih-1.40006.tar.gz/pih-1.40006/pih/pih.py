...
from pih.collection import ActionValue, FieldItem, FieldItemList, FullName, LogCommand, LoginPasswordPair, Mark, MarkGroup, MarkGroupStatistics, ParamItem, PasswordSettings, PrinterADInformation, PrinterReport, Result, TimeTrackingItem, TimeTrackingByDate, TimeTrackingResultItem, User, UserContainer
...
class PIH:
    ...
    class RESULT:
        ...
        class PRINTER:
            ...
            @staticmethod
            def report(redirect_to_log: bool = True) -> List:
        ...
        class TIME_TRACKING:
            ...
            @staticmethod
            def time_tracking_today(tab_number: str = None) -> Result[List[TimeTrackingByDate]]:
                return PIH.RESULT.TIME_TRACKING.make(tab_number=tab_number)

            @staticmethod
            def in_period(day_start: int = 1, day_end: int = None, month: int = None, tab_number: str = None) -> Result[List[TimeTrackingByDate]]:
                now: datetime = datetime.now()
                if month is not None:
                    now = now.replace(month = month)
                start_date: datetime = now.replace(hour=0, minute=0, second=0)
                end_date: datetime = None
                now_day: int = start_date.day
                if day_start < 0:
                    day_start = now_day + day_start
                if day_start < 1:
                    day_start = 1
                start_date = start_date.replace(day=day_start)
                end_date = now.replace(
                        day=now.day if day_end is None else day_end , hour=23, minute=59, second=59)
                return PIH.RESULT.TIME_TRACKING.make(start_date, end_date, tab_number)

            @staticmethod
            def make(start_date: datetime = None, end_date: datetime = None, tab_number: str = None) -> Result[List[TimeTrackingByDate]]:
                now: datetime = None
                ...
                time_tracking_list: List[TimeTrackingItem] = DataTools.to_result(
                    RPC_COMMANDS.MARK.get_time_tracking(start_date, end_date, tab_number), TimeTrackingItem).data
                result_data = {}
                for time_tracking_item in time_tracking_list:
                    date: str = time_tracking_item.TimeVal.split("T")[0]
                    tab_number: str = time_tracking_item.TabNumber
                    if date not in result_data:
                        result_data[date] = {}
                    if tab_number not in result_data[date]:
                        result_data[date][tab_number] = []
                    result_data[date][tab_number].append(time_tracking_item)
                result: List[TimeTrackingByDate] = []
                for date in result_data:
                    result_item: TimeTrackingByDate = TimeTrackingByDate(date)
                    result.append(result_item)
                    for user_item in result_data[date]:
                        time_tracking_data: List[TimeTrackingItem] = result_data[date][user_item]
                        time_tracking_item: TimeTrackingItem = time_tracking_data[0]
                        time_tracking_item2: TimeTrackingItem = None
                        if len(time_tracking_data) > 1:
                            time_tracking_item2 = time_tracking_data[1]
                        time_tracking_start_item: TimeTrackingItem = None
                        time_tracking_end_item: TimeTrackingItem = None
                        if time_tracking_item.Mode == 1:
                            time_tracking_start_item = time_tracking_item
                        if time_tracking_item.Mode == 2:
                            time_tracking_end_item = time_tracking_item
                        if time_tracking_item2 is not None:
                            if time_tracking_item2.Mode == 1:
                                time_tracking_start_item = time_tracking_item2
                            if time_tracking_item2.Mode == 2:
                                time_tracking_end_item = time_tracking_item2
                        duration: str = "--"
                        if time_tracking_start_item is not None:
                            if time_tracking_end_item is not None:
                                duration = str(timedelta(seconds=(datetime.fromisoformat(time_tracking_end_item.TimeVal) - datetime.fromisoformat(time_tracking_start_item.TimeVal)).seconds))
                        result_item.list.append(
                            TimeTrackingResultItem(time_tracking_item.FullName, time_tracking_item.TabNumber, None if time_tracking_start_item is None else time_tracking_start_item.TimeVal.split("T")[1], None if time_tracking_end_item is None else time_tracking_end_item.TimeVal.split("T")[1], duration))
        ...
        class USER:
            ...
            def by_job_position(job_position: CONST.AD.JobPositions) -> Result[List[User]]:
                return DataTools.to_result(RPC_COMMANDS.USER.get_user_by_job_position(job_position), User)
    ...
    class CHECK:
        ...
        class USER:
            ...
            def is_acive(user: User) -> bool:
                 return user.distinguishedName.find(CONST.AD.DN_ACTIVE_CONTAINER_DN) != -1
    ...
    def _log_command(command_name: LogCommands, parameters:Tuple = None) -> bool:
    ...
    class LOG_COMMAND:
        ...
        @staticmethod
        def login() -> Any:
            return PIH._log_command(LogCommands.LOG_IN, (PIH.OS.get_login(), PIH.OS.get_computer_name()))


        @staticmethod
        def printer_report(printer_name: str, location: str) -> Any:
            return PIH._log_command(LogCommands.PRINTER_REPORT, (printer_name, location))
    ...
    class LOG_COMMAND:
        ...
        @staticmethod
        def hr_notify_about_new_employee(new_employee_user: User) -> bool:
            hr_user: User = ResultTools.get_first(PIH.RESULT.USER.by_job_position(CONST.AD.JobPositions.HR))
            return PIH._log_command(LogCommands.HR_NOTIFY_ABOUT_NEW_EMPLOYEE, (FullNameTool.to_given_name_string(FullNameTool.from_string(hr_user.name)), 
            new_employee_user.name, new_employee_user.mail))

        @staticmethod
        def printer_report(name: str, location: str, report: str) -> bool:
            return PIH._log_command(LogCommands.PRINTER_REPORT, (name, location, report))
    ...
    class ACTION:
        ...
        class TIME_TRACKING:
            ...
            def report(path: str, start_date: datetime, end_date: datetime, tab_number: str = None) -> bool:
                ...
                lan_adress: str = f"//{PIH.OS.get_computer_name()}/"
                if path[1] == ":":
