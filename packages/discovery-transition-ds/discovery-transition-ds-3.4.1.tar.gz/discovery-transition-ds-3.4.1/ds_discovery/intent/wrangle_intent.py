...
class WrangleIntentModel(AbstractBuilderIntentModel):
    ...
    def correlate_missing_analysis(self, canonical: Any, header: str, analysis_list: list, strict_typing: bool=None,
                                   category_limit: int=None, seed: int=None, save_intent: bool=None,
                                   column_name: [int, str]=None, intent_order: int=None, replace_intent: bool=None,
                                   remove_duplicates: bool=None):
        """ imputes missing continuous or discrete data with a weighted distribution based on the analysis of the
        given analysis list which should contain an associative tree to the target header

        :param canonical: a pd.DataFrame as the reference dataframe
        :param header: the target header
        :param analysis_list: a list structure of columns to select for association
        :param strict_typing: (optional) stops objects and string types being seen as categories
        :param category_limit: (optional) a global cap on categories captured. zero value returns no limits
        :param seed: seed: (optional) a seed value for the random function: default to None
        :param save_intent: (optional) if the intent contract should be saved to the property manager
        ...
        # set the seed and call the method
        seed = self._seed(seed=seed)
        return self._correlate_missing_analysis(seed=seed, **params)
