...
def _get_axes(
    X: np.ndarray, Y: np.ndarray, ax: Optional[GeoAxesSubplot], **kwargs,
) -> GeoAxesSubplot:
    if ax is None:
...
def full_domain(
    ...
    try:
        ...
        data = grid["data"][0]
    im = ax.pcolormesh(X, Y, data, vmin=vmin, vmax=vmax, cmap=cmap, shading="gouraud",)
    ...
    animation = FuncAnimation(
        ax.get_figure(), _update, frames=frames, interval=1000 / fps,
    )
    return animation
...
def plot_traj(
    ...
    size = _plot_style.pop("markersize")
...
def _normalize_kwargs(kwargs: dict[str, str], kind: str = "patch") -> dict[str, str]:
    ...
    if kind == "line2d":
        ...
        long_names = dict(
            c="color", ls="linestyle", lw="linewidth", ec="edgecolor", fc="facecolor",
        )
