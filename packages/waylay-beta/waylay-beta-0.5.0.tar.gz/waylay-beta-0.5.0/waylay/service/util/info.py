    ...
    links       filter on the documentation link names
        ...
        def action_doc(action: WaylayAction) -> str:
            return "\n".join(f'<div>{doc}</div>' for doc in [
                action.description,
                format_pydoc(action.arguments_doc),
                format_pydoc(action.returns_doc),
                format_pydoc(action.sdk_action_doc),
                *[
                    wrapped_doc
                    for wrapped_action in getattr(action, 'wrapped_actions', [])
                    for wrapped_doc in [
                        'Can use the following REST action:',
                        format_rest_action(wrapped_action.rest_action_doc),
                        wrapped_action.description,
                        format_pydoc(wrapped_action.arguments_doc),
                        format_pydoc(wrapped_action.returns_doc),
                        format_pydoc(wrapped_action.sdk_action_doc),
                    ]
                ],
                format_rest_action(action.rest_action_doc),
