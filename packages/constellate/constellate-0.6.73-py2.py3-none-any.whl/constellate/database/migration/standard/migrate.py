...
from typing_extensions import Awaitable, AsyncGenerator, AsyncIterator
...
def _find_missing_migrations(
    ...
    def _render_plan(plan: _MigrationPlan) -> str:
        header = f"{action.value.capitalize()} migration plan {'applied' if plan.applied else 'to be applied'}:"
        ...
        with mkf_tmpfs() as f:
            try:
                ...
                migration_path2 = "no Graphiz plan rendered. Graphiz likely not installed/reachable from system path"
    ...
    # Find (if present) the migration file common to the applied set of files and yet to be applied set of files
