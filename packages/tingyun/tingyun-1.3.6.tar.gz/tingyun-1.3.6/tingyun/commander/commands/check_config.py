...
config_properties = os.environ.get('TINGYUN_CONFIG_PROPERTIES')
...
class Command(object):
    ...
    def execute(self, args):
        ...
        if (len(args) > 0) and len(args) != 1 and config_properties is None:
        ...
        env_msg = ''
        if 0 == len(args):
            env_msg = "please set the environment variable(%s) for agent config file or config properties." % ENV_CONFIG_FILE

        if not os.path.exists(config_file) and not config_properties:
        ...
        if not _config_parser.read(config_file):
            if not config_properties:
                errors.append(
                    "Errors: unable to read the config file or the config properties. "
                    "please check the file permission or set the config properties.")
                self.internal_tips(errors)
                raise CommandlineParametersException()
            _config_parser.add_section('tingyun')
            for item in config_properties.split(';'):
                _config_parser.set('tingyun', item.split('=')[0], item.split('=')[1])

        # check the config detail.
        if config_file:
            print("Use config file: %s" % config_file)
        if config_properties:
            print('Use config properties: %s' % config_properties)
