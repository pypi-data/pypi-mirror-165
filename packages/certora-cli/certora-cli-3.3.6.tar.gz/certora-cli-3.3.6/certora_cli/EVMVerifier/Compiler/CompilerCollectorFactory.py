...
def get_relevant_solc(file_name: str, solc: str, solc_mappings: Dict[str, str]) -> str:
    """
    @param file_name: the contract that we are working on
    ...
    @return: string of the solc we want to run on this contract
    """
    if file_name in solc_mappings:
        base = solc_mappings[file_name]
    else:
        base = solc
...
def get_extra_solc_args(contract: str, solc_args: list, optimize_map: Dict[str, str]) -> str:
    """
    ...
    @param contract: the contract that we are working on
    ...
    if contract in optimize_map:
        num_runs = optimize_map[contract]
...
class CompilerCollectorFactory:
    ...
    def get_compiler_collector(self, path: Path) -> CompilerCollector:
        ...
        if str(path).endswith(".vy"):
            ...
            version = self.__get_solc_version(path.name)
            return CompilerCollectorSol(version, get_extra_solc_args(path.name, self._solc_args, self._optimize_map))
        else:
            raise RuntimeError(f'expected {path} to represent a Solidity or Vyper file')

    def __get_solc_version(self, contract_file: str) -> Tuple[int, int, int]:
        """
        @param contract_file: the contract that we are working on
        @return: the running solc version
        """
        solc_logger.debug(f"visiting contract file {contract_file}")
        solc_path = get_relevant_solc(contract_file, self._solc, self._solc_mappings)
