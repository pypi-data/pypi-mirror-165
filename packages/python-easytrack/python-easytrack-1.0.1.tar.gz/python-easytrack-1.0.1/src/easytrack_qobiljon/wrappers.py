...
class DataTable:
	...
	def __connect():
		...
		DataTable.con = pg2.connect(database=notnull(getenv('POSTGRES_DBNAME')),
	...
	def insert(participant: models.Participant, data_source: models.DataSource, ts: dt, val: Dict) -> None:
		...
		cur = DataTable.con.cursor()
		cur.execute(
		    f'insert into data.{table_name}(data_source_id, ts, val) values (%s,%s,%s)',
		    (  # noqa
	...
	def select_next_k(participant: models.Participant, data_source: models.DataSource, from_ts: dt, limit: int) -> List[DataRecord]:
		...
		cur.execute(
		    f'select data_source_id, ts, val from data.{table_name} where data_source_id = %s and ts >= %s limit %s',
		    (  # noqa
	...
	def select_range(participant: models.Participant, data_source: models.DataSource, from_ts: dt, till_ts: dt) -> List[DataRecord]:
		...
		cur: pg2_extras.DictCursor = DataTable.con.cursor(cursor_factory=pg2_extras.DictCursor)
		cur.execute(
		    f'select data_source_id, ts, val from data.{table_name} where data_source_id = %s and ts >= %s and ts < %s',
		    (  # noqa
...
class ParticipantStats:
	...
	def __init__(self, participant: models.Participant):
		...
		for data_source in sorted(data_sources, key=lambda x: x.name):
			latest_hourly_stats: models.HourlyStats = models.HourlyStats.filter(participant=participant, data_source=data_source).order_by(models.HourlyStats.ts, models.HourlyStats.ts.desc()).limit(1)

			self.per_data_source_stats[data_source] = DataSourceStats(data_source=data_source, amount_of_samples=latest_hourly_stats.amounts,
