...
class BflbIotTool(object):
    ...
    def bl60x_mfg_ota_xz_gen(self, ota_path, chipname="bl60x", chiptype="bl60x", cpu_type=None, fw_len=0):
        ...
        if len(fw_ota_bin_xz) > fw_len:
            bflb_utils.printf("Warn: fw1 xz bin size is overflow with partition table, don't create ota bin")
            return False
                ...
                if bin_d_list[i]:
                    if bin_d_list[i] is not None and len(bin_d_list[i]) > 0:
                        if 'factory_addr' in pt_parcel:
                            bl_ro_device_tree.bl_ro_params_device_tree(bin_d_list[i],
        ...
        if yocboot is True:
            if 'fw_yocboot_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    whole_img_len = offset + os.path.getsize(
                        chipnamedir + "/img_create_iot/img_yocboot.bin")
                    whole_img_data = bl_create_flash_default_data(whole_img_len)
        ...
        if dtb is True:
            if 'dtb_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    whole_img_len = offset + os.path.getsize(
                        chipnamedir + "/img_create_iot/img_dtb.bin")
                    whole_img_data = bl_create_flash_default_data(whole_img_len)
                ...
                if boot2 is not None and len(boot2) > 0:
                    ...
                    if config["check_box"]['sign']:
                        if "boot2_security" in pt_parcel:
                            ...
                            temp_sign = True                    
                    self.bl60x_fw_boot_head_gen(True, config['param']["chip_xtal"], self.bh_cfg_file,
                                           temp_encrypt, temp_sign, chipname, chiptype, cpu_type, boot2_en)
                    f_org = os.path.join(chip_path, chipname, "img_create_iot", "img_create_cfg_boot2.conf")
                    f = os.path.join(chip_path, chipname, "img_create_iot", "img_create_cfg_boot2.ini")
                    if os.path.isfile(f) is False:
                        shutil.copyfile(f_org, f)
                    ...
                    self.bl60x_image_gen(chipname, chiptype, cpu_type, "boot2", boot2,
                                    temp_aes_key, temp_aes_iv, 
                                    temp_publickey, temp_privatekey, 
                                    f)
                    boot2_d = True
        elif chiptype == "bl702" or chiptype == "bl702l":
            self.bl60x_fw_boot_head_gen(True, config['param']["chip_xtal"], self.bh_cfg_file,
                    ...
                    if media_sign == 1 or media_sign == 2 or config["check_box"][name+'_download'] is True:
                        if name + '_addr' in pt_parcel:
                            if chiptype == "bl808":
                                ...
                                cfg_t.read(self.bh_cfg_file)
                                cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel[name+'_addr']+0x1000))
                                cfg_t.write(self.bh_cfg_file)
                            else:
                                if media_sign == 2:
                                    media_path = os.path.join(chip_path, chipname, "img_create_iot", "media.bin")
    ...
    def flasher_download_cfg_ini_gen_old(self, chipname, chiptype, cpu_type, config):
            ...
            if config["check_box"]['yocboot_download'] is True:
                if 'fw_yocboot_addr' in pt_parcel:
                    if chiptype == "bl808":
                        ...
                        cfg_t.read(self.bh_cfg_file)
                        cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel['fw_yocboot_addr']+0x1000))
                        cfg_t.write(self.bh_cfg_file)
                    else:
            ...
            if config["check_box"]['dtb_download'] is True:
                if 'dtb_addr' in pt_parcel:
                    if chiptype == "bl808":
                        ...
                        cfg_t.read(self.bh_cfg_file)
                        cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel['dtb_addr']+0x1000))
                        cfg_t.write(self.bh_cfg_file)
                    else:
        ...
        if config["check_box"]['mfg_download'] is True:
            if 'mfg_addr' in pt_parcel:
                if chiptype == "bl808":
                    ...
                    cfg_t.read(self.bh_cfg_file)
                    cfg_t.set(bootheader_section_name, 'group_image_offset', "0x%x" % (pt_parcel['mfg_addr']+0x1000))
                    cfg_t.write(self.bh_cfg_file)
                else:
        ...
        return self.bl60x_mfg_flasher_eflash_loader_cfg(chipname, chiptype, bin_d, boot2_d, ro_params_d,
                                                        ...
                                                        kv_bin_d, yocboot_bin_d, dtb_bin_d)
...
def iot_download_cmd(args):
    ...
    if not args.pt:
        if chiptype == "bl808":
            config["input_path"]["pt_table_bin_input"] = os.path.join(chip_path, chipname, "partition/partition_cfg_8M.toml") 
        else:
            config["input_path"]["pt_table_bin_input"] = os.path.join(chip_path, chipname, "partition/partition_cfg_2M.toml")
    else:
