...
def _interpolate(raster, x, y, resolution, extent=None, nodata=-1, vmin=1e-6):
    ...
    print(f"Output raster shape is ({height},{width})")
...
class AshDisperseResult:
    ...
    def to_netcdf(self, filename="AshDisperse.nc"):
        da = xa.Dataset(
            data_vars=dict(
        ...
        da.to_netcdf(path=filename, mode="w", format="NETCDF4")
    ...
    def write_gtiff(self, raster, x, y, outname, nodata=-1, vmin=1e-6, resolution=None):
        ...
        print(f"Writing data to {outname}")
        if os.path.isfile(outname):
            print(f"WARNING: {outname} already exists and will be replaced")
    ...
    def plot_iso_conc_for_grain_class(self, grain_i, conc):
        ...
        if np.nanmax(C) < conc:
            print(f"In plotIsoConc, no concentration values in excess of {conc}.")
            return
