...
class SolverParameters:
    ...
    def describe(self):
        ...
        print(f"  Dimensionless domain size in x, domX = {self.domX}")
        print(f"  Dimensionless domain size in y, domY = {self.domY}")
        print(
            f"  Minimum resolution in z, minN = {self.minN} (minN_log2 = {self.minN_log2})"
        )
        print(
            f"  Maximum resolution in z, maxN = {self.maxN} (maxN_log2 = {self.maxN_log2})"
        )
        print(f"  Number of Chebyshev iterates = {self.chebIts}")
        print(f"  Tolerance for Chebyshev series, epsilon = {self.epsilon}")
        print(f"  Resolution in x, Nx = {self.Nx} (Nx_log2 = {self.Nx_log2})")
        print(f"  Resolution in y, Ny = {self.Ny} (Ny_log2 = {self.Ny_log2})")
        print("********************")
