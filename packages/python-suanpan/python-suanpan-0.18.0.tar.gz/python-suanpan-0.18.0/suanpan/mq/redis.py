...
class MQ(object):
    ...
    def sendMessage(
        ...
        if self._p2p:
            for target, data4in in self._sendP2PMessages(data):
                target_queue = f'{queue}-{target}'
                logger.debug(f'send p2p message to {target}: {data4in}')
                self.client.xadd(
                    target_queue, data4in, id=messageID, maxlen=maxlen, approximate=(not trimImmediately)
                )
        else:
    ...
    def _getP2PConnections(self):
        ...
        for port in node.metadata['def'].ports:
            if port.ioType == 'out':
                ...
                self._outputConnections[port_id] = connections

    def _sendP2PMessages(self, data):
        for port_id, connections in self._outputConnections.items():
            for tgt in connections:
                d = data.get(port_id)
                if d:
                    data4in = {"id": data['request_id'], tgt.port: d, "extra": data["extra"]}
                    yield tgt.process, data4in
