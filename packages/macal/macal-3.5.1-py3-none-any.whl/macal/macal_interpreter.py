            ...
            for param, instr in zip(func.args.params, instruction.params.params):
                ...
                if param.token.token_type == ParserNodeTypes.VARIABLE:
                    ...
                    if instr.token.token_type != LexTokenTypes.Identifier:
                        raise RuntimeError(f"Illegal literal ({instr.token.token_type}), only variables are allowed in this function. @({instr.token.token_line},{instr.token.token_offset})")
    ...
    def interpret_foreach_loop(self, instruction, scope: MacalScope):
        ...
        if isinstance(instruction.expr, ast_Variable):
            ...
            if var_type != VariableTypes.STRING and var_type != VariableTypes.ARRAY and var_type != VariableTypes.RECORD and var_type != VariableTypes.PARAMS:
                raise RuntimeError(f"Foreach: Invalid variable type ({var_type}), only string, array and record are allowed. ({loop_var.name} @{instruction.token.token_line},{instruction.token.token_offset}).")
            walker = None
            ...
            raise RuntimeError(f"Foreach: Incompatible operand type error (@{instruction.token.token_line},{instruction.token.token_offset}).")
