...
class EmpireClient(utinni.EmpireApiClient):
    ...
    async def generate_payload(self, deploy_arguments: dict) -> str:
        ...
        listener_name = deploy_arguments[co.LISTENER_NAME]
        stager_type = deploy_arguments[co.STAGER_TYPE]

        listener_port = deploy_arguments.get(co.LISTENER_PORT, 80)
        listener_type = deploy_arguments.get(co.LISTENER_TYPE, "http")
    ...
    async def execute_on_agent(self, arguments) -> dict:
        ...
        if module_name is not None:
            ...
            try:
                ...
                return {co.RETURN_CODE: -2,
                        co.STD_ERR: f"Shell command execution timed out, check that empire agent '{agent_name}' "
                                    f"is alive"}
        else:
