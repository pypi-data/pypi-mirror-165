    ...
    def _threaded_consumer(self, thread_id: int) -> None:
        ...
        while self.is_running():
            try:
                ...
                time.sleep(5)  # Wait a few seconds before trying to create a new channel, connection might be lost

            except Exception as ex:  # If any uncaught exception occurs, consumer will still work
                logger.logger.debug("Threaded consumer encountered an error.", thread_id=thread_id, error=str(ex))
