...
async def present(
    ...
    **Autogenerated function**
    ...
    Args:
        ...
        delegations(List, optional): An array of references to the delegations on the subnet.
         Defaults to None
        ...
        .. code-block:: sls
            ...
            my-subnet:
              azure.virtual_networks.subnets.present:
                ...
                - enforce_private_link_endpoint_network_policies: Enabled/Disabled
                - delegations:
                    - name: my-delegation-1
                      service: Microsoft.StoragePool/diskPools
                - enforce_private_link_service_network_policies: Enabled/Disabled
                - service_endpoints:
                    - Microsoft.Sql
                    - Microsoft.Storage
    ...
    if not response_get["result"]:
        if response_get["status"] == 404:
            ...
            if ctx.get("test", False):
                ...
                result["comment"].append(
                    f"Would create azure.virtual_networks.subnets '{name}'"
                )
                return result

            else:
                # PUT operation to create a resource.
                payload = hub.tool.azure.virtual_networks.subnets.convert_present_to_raw_subnets(
                    address_prefix=address_prefix,
                    enforce_private_link_endpoint_network_policies=enforce_private_link_endpoint_network_policies,
                    delegations=delegations,
                    enforce_private_link_service_network_policies=enforce_private_link_service_network_policies,
                ...
                if not response_put["result"]:
                    hub.log.debug(
                        f"Could not create azure.virtual_networks.subnets '{name}' {response_put['comment']} {response_put['ret']}"
                    )
                ...
                result["comment"].append(
                    f"Created azure.virtual_networks.subnets '{name}'"
                )
                return result
        else:
            hub.log.debug(
                f"Could not get azure.virtual_networks.subnets {response_get['comment']} {response_get['ret']}"
            )
            result["result"] = False
        ...
        new_payload = hub.tool.azure.virtual_networks.subnets.update_subnets_payload(
            existing_resource,
            {
                "address_prefix": address_prefix,
                "enforce_private_link_endpoint_network_policies": enforce_private_link_endpoint_network_policies,
        ...
        if ctx.get("test", False):
            if new_payload["ret"] is None:
                ...
                result["comment"].append(
                    f"azure.virtual_networks.subnets '{name}' has no property need to be updated."
                )
            else:
                result[
                    "new_state"
                ...
                result["comment"].append(
                    f"Would update azure.virtual_networks.subnets '{name}'"
                )
            return result
        ...
        if not response_put["result"]:
            hub.log.debug(
                f"Could not update azure.virtual_networks.subnets {response_put['comment']} {response_put['ret']}"
            )
            result["result"] = False
            result["comment"] = [response_put["comment"], response_put["ret"]]
            return result
        ...
        result["comment"].append(f"Updated azure.virtual_networks.subnets '{name}'")
        return result
...
async def absent(
    ...
    **Autogenerated function**
    ...
    if response_get["result"]:
        ...
        if ctx.get("test", False):
            result["comment"].append(
                f"Would delete azure.virtual_networks.subnets '{name}'"
            )
            return result
        ...
        if not response_delete["result"]:
            hub.log.debug(
                f"Could not delete azure.virtual_networks.subnets{response_delete['comment']} {response_delete['ret']}"
            )
            result["result"] = False
            result["comment"] = [response_delete["comment"], response_delete["ret"]]
            return result

        result["comment"].append(f"Deleted azure.virtual_networks.subnets '{name}'")
        return result
    elif response_get["status"] == 404:
        # If Azure returns 'Not Found' error, it means the resource has been absent.
        result["comment"].append(
            f"azure.virtual_networks.subnets '{name}' already absent"
        )
        return result
    else:
        hub.log.debug(
            f"Could not get azure.virtual_networks.subnets '{name}' {response_get['comment']} {response_get['ret']}"
        )
        result["result"] = False
        result["comment"] = [response_get["comment"], response_get["ret"]]
    return result
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    **Autogenerated function**
    ...
    async for vnet_page_result in hub.tool.azure.request.paginate(
        ...
        vnet_list = vnet_page_result.get("value")
        if vnet_list:
            for vnet in vnet_list:
                ...
                if response_get["result"]:
                    ...
                    for resource in resource_list:
                        ...
                        resource_translated = hub.tool.azure.virtual_networks.subnets.convert_raw_subnets_to_present(
                            resource=resource,
                            idem_resource_name=resource["id"],
                            resource_id=resource["id"],
                            **uri_parameter_values,
                        )
                        result[resource["id"]] = {
                            f"azure.virtual_networks.subnets.present": [
