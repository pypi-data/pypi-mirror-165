...
class Contact(Entity):
    ''' Singleton-класс для работы с контактами '''

    async def add(self, contact_data):  # USE !!!!
        ''' Метод для добавление нового контакта '''
        response = await super().post(f'{self.fulldomain}/api/v4/contacts', contact_data)
        if response['result']:
            response['data'] = response['data']['_embedded']['contacts']
        return response

    async def add_links(self, post_data):  # USE !!!!
        response = await super().post(f'{self.fulldomain}/api/v4/contacts/link', post_data)
        if response['result']:
            response['data'] = response['data']['_embedded']['links']
        return response

    async def edit(self, edit_data, contact_id=None):  # USE !!!!
        ''' Метод для изменения данных контакта '''
        url = f'{self.fulldomain}/api/v4/contacts'
        if contact_id:
            url += f'/{contact_id}'
        response = await super().patch(url, edit_data)
        if response['result'] and not contact_id:
            response['data'] = response['data']['_embedded']['contacts']
        return response
    ...
    async def get_custom_fields(self, params={}):  # USE !!!!
        response = await super().get(f'{self.fulldomain}/api/v4/contacts/custom_fields', params)
        if response['result']:
