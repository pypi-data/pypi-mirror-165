...
# 读取交易日日历
def load_calendar(start_date=None, end_date=None, freq='', db_path='', table='index_ts'):
...
def load_funds_data1(
        fund_list, first_date=None, end_date=None, freq='w', fillna=True,
        db_path='', cal_db_path='', drop_1=False, print_info=False,
):
    if first_date is None:
        first_date = datetime.now().date()
...
def load_funds_alpha(
        ...
        db_path='', cal_db_path='', index_table='index_ts', print_info=False
):
    cal = load_calendar(end_date=end_date, db_path=cal_db_path)
    cal_weekly = load_calendar_extra(freq='w', end_date=end_date, db_path=cal_db_path)
