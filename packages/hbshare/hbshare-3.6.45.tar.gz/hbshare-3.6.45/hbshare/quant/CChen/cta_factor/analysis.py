...
def cta_factor_line(cta_factors, start_date, end_date, table=None, sql_path=None, freq=''):
    if table is None:
        table = 'cta_index'
    ...
    cal = load_calendar(
        start_date=start_date, end_date=end_date, freq=freq, db_path=sql_path
    )
    data_all = pd.read_sql_query(
        ...
        + ' and FACTOR in ' + str(tuple(cta_factors.keys()))
        + ' order by TDATE',
        sql_path
    )
...
if __name__ == '__main__':
    ...
    endDate = datetime(2022, 7, 1).date()
