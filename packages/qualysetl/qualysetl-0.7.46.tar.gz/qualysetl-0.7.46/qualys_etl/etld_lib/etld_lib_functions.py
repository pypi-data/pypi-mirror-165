...
def log_system_information(logger_method=print, data_directory="/opt/qetl/users"):
    logger_method(
        f'SYS STAT: CPU percent usage={psutil.cpu_percent(interval=0.5)}, '
        f'frequency='
        f'{get_cpu_frequency()}%, '
        f'count='
        f'{get_cpu_count()}, '
...
def if_ram_space_size_less_than_8_gig_log_warning(logger_method=print):
    ram_size_total_in_mb = int(get_ram_size_total_in_mb())
    if ram_size_total_in_mb < 8100:
        logger_method(f'SYS WARNING: DISK size usage < 8 Gig.  Consider increasing RAM to 16 Gig for ETL')
