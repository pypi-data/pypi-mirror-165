...
static PyObject * polyclip_intersection_area (PyObject *self, PyObject *args)
{
    ...
    int parsing_ok = PyArg_ParseTuple(args, "O!iO!i",
    ...
    if ((PyArray_AsCArray(&subject_obj, (void *) &subject_arr, dims, subject_nd, subject_descr) < 0)| // or
...
static PyObject * get_weights_for_hpixs (PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    // Our input is mostly numpy ndarrays. The conversion Python <=> C is really verbose, but it works.
    ...
    if ((PyArray_AsCArray(&hpix_polys_obj, (void *) &hpix_polys_arr,
                          hpix_polys_dims, hpix_polys_nd, hpix_polys_descr) < 0)) {
        PyErr_SetString(PyExc_TypeError, "Error getting C array for hpix_polys.");
        return NULL;
    }
    if ((PyArray_AsCArray(&hpix_tally_obj, (void *) &hpix_tally_arr,
                          hpix_tally_dims, hpix_tally_nd, hpix_tally_descr) < 0)) {
        PyErr_SetString(PyExc_TypeError, "Error getting C array for hpix_tally.");
        return NULL;
    }
    if ((PyArray_AsCArray(&wcs_image_obj, (void *) &wcs_image_arr,
                          wcs_image_dims, wcs_image_nd, wcs_image_descr) < 0)) {
        PyErr_SetString(PyExc_TypeError, "Error getting C array for wcs_image.");
        return NULL;
    }

    // Prepare the output. Three same-sized lists, fit for a sparse matrice initialization.
    ...
    // This poses problems, notably on the poles in full-sky projections, where we may like to have 5 sided polygons
    // in the future for better accuracy. The idea of this is to do the memory allocation only once and reuse
    // those polygons and vectors on each loop with different values. There's probably a way to do this and support 5-sided polygons too.
    ...
    for (hpix_id = 0 ; hpix_id < hpix_cnt ; hpix_id++) {
        ...
        // The hpix_tally keeps track for each healpixel of how many polygons were packed.
        ...
        for (k = 0 ; k < hpix_tally_arr[hpix_id] ; k++) {
            ...
            // Those are the WCS array indices boundaries, min inclusive, max exclusive, for each healpixel.
            ...
            for (x_ind=x_min; x_ind<x_max; x_ind++){
                for (y_ind=y_min; y_ind<y_max; y_ind++){
                    ...
                    // Compute the area of the intersection between the polygons. The clipper is the WCS poly
                    // as we may rely on optimizations due  to its simple form of a square aligned with the axes.
                    clipped_poly = poly_clip(hpix_poly, wcs_poly);
                    area = poly_area(clipped_poly);

                    if (area > 0.0) {
                        dbg_wcs_count++;
                        // The pixels intersect, let's add them to our output. darray stands for dynamic array,
                        // which is a naive but effective way of using arrays whose size we don't know in advance.
        ...
        printf("  Done healpixel #%ld/%ld (%ld%%) with %d WCS pixels (area=%f).\n", hpix_id+1, hpix_cnt,
                    100 * (hpix_id+1) / hpix_cnt, dbg_wcs_count, poly_area(hpix_poly));
    ...
    // I don't get why we don't need that. Who's going to clean up ?
    // Py_DECREF(w_pylist);
