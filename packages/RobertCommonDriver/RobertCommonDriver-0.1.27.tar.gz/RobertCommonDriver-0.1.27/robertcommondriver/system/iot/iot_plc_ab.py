...
class AllenBradleyTools:
    ...
    def parse_address(address: str, type: int):
        ...
        try:
            ...
            for i, tag_name in enumerate(tag_names):
                ...
                if index_first > 0 and index_second > 0 and index_second > index_first:
    ...
    def extract_actual_data(response: bytearray, is_read: bool):
        ...
        has_more_data = False
        data_type = 0
        count = IOTBaseCommon.DataTransform.unpack_bytes(response[38:40], '<H')
        if IOTBaseCommon.DataTransform.unpack_bytes(response[40:44], '<i') == 0x8A:
            offset = 44
