...
class loadprojectini:
    ...
    def set_labelling_checkboxes(self, setting: str=None):
        if setting == 'advanced':
            self.bool_from_scratch.set(False)
            self.bool_continue_existing.set(False)
            self.bool_advanced_labelling.set(True)
            self.cb_from_scratch.config(state=DISABLED)
            self.cb_continoue.config(state=DISABLED)
            self.cb_advanced.config(state=NORMAL)
