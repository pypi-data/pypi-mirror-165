...
from simba.read_config_unit_tests import (read_config_entry,
                                          ...
                                          check_int)
from simba.misc_tools import get_video_meta_data, find_video_of_file, get_fn_ext
import simba
from tkinter import *
...
class LabellingInterface(object):
    def __init__(self,
                 ...
                 setting: str='pseudo'): #from_scratch, #advanced, #pseudo
        ...
        self.frm_log_ini = os.path.join(os.path.dirname(self.config_path), 'logs', "lastframe_log.ini")
        self.video_path = find_video_of_file(video_dir=self.videos_dir_path, filename=self.video_name)
        ...
        if setting == 'from_scratch':
            ...
            for target in self.target_lst:
                ...
                df_targets[target] = [1 if x > threshold[target] else 0 for x in df[[probabilityColumnName]].to_numpy()]
        ...
        if isinstance(data_df, str):
            check_file_exist_and_readable(file_path=self.data_df)
            self.data_df = read_df(self.data_df, self.file_type)
        else:
            pass
        ...
        if setting == 'from_scratch':
            self.main_window.title('SIMBA ANNOTATION INTERFACE (ANNOTATING FROM SCRATCH)')
        ...
        save = Button(self.main_window, text="Save and advance to the next frame", command=lambda: self.save_checkboxes())
        ...
        self.generate = Button(self.main_window, text="Save Annotations", command=lambda: self.save_results())
    ...
    def save_results(self):
        ...
        print('Annotation file for video {} saved.'.format(self.video_name))
        if not os.path.exists(self.frm_log_ini):
            f = open(self.frm_log_ini, "w+")
            f.write('[Last saved frames]\n')
            f.close()
        self.config.set('Last saved frames', str(self.video_name), str(self.current_frm_n))
        with open(self.frm_log_ini, 'w') as configfile: self.config.write(configfile)


test = LabellingInterface(config_path='/Users/simon/Desktop/troubleshooting/train_model_project/project_folder/project_config.ini',
                          file_path='/Users/simon/Desktop/troubleshooting/train_model_project/project_folder/videos_2/Together_1.avi',
