                ...
                for boundary in iter_boundaries(field):
                    ...
                    if wrap_width is not None and cur + w > wrap_width:
                        ...
                        line_extents = partial_extents(
                            field_extents, breakpoint, prev_boundary
                        )
...
class TTFormatter(object):
    ...
    def __init__(self, wrap_width, tab_width=8, tab_char=" ", ambiguous_as_wide=False):
        self._lines = [""]
    ...
    def tab_char(self, value):
        if east_asian_width(value) not in ("N", "Na", "H"):
            raise ValueError(
                """only a narrow code point is available for 
                             tab_char"""
            )
