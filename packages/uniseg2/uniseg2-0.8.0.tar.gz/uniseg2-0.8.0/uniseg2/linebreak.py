...
def _preprocess_boundaries(s):
    r"""
    (internal) Preprocess LB9: X CM* -> X
...
def line_break_breakables(s, legacy=False):
    ...
    for i, (pos, lb) in enumerate(primitive_boundaries):
        next_pos, __ = (
            primitive_boundaries[i + 1]
            if i < len(primitive_boundaries) - 1
            else (len(s), None)
        )
        ...
        # LB14
            ...
            do_break = False
        # LB15
        elif (prev_prev_lb == QU and prev_lb == SP and lb == OP) or (
            prev_lb == QU and lb == OP
        ):
            do_break = False
        # LB16
        elif (prev_prev_lb in (CL, CP) and prev_lb == SP and lb == NS) or (
            prev_lb in (CL, CP) and lb == NS
        ):
            do_break = False
        # LB17
        elif (prev_prev_lb == B2 and prev_lb == SP and lb == B2) or (
            prev_lb == B2 and lb == B2
        ):
            do_break = False
        # LB18
        elif prev_lb == SP:
        ...
        # LB26
            ...
            (prev_lb == JL and lb in (JL, JV, H2, H3))
            or (prev_lb in (JV, H2) and lb in (JV, JT))
            or (prev_lb in (JT, H3) and lb == JT)
        ):
            do_break = False
        # LB27
        elif (prev_lb in (JL, JV, JT, H2, H3) and lb in (IN, PO)) or (
            prev_lb == PR and lb in (JL, JV, JT, H2, H3)
        ):
            do_break = False
        # LB28
        ...
        # LB29
            ...
            do_break = False
        # LB30
        elif (prev_lb in (AL, HL, NU) and lb == OP) or (
            prev_lb == CP and lb in (AL, HL, NU)
        ):
            do_break = False
        # LB30a
        elif prev_lb == lb == RI:
