...
class LoadEdges(MMSEG_LoadAnnotations):
    ...
    def __call__(self, results):
        ...
        if results.get("binary", None):
            ...
            # FIXME: only supports RGB inputs (need to change to binary or tif for more classes)
            # FIXME: for tif, we need to implement 32bit transforms
            img_bytes = self.file_client.get(filename)
            gt_semantic_edge = (
                mmcv.imfrombytes(
                    img_bytes,
            ...
            if self.format:
                ...
                # outputs (c, h, w)
                gt_semantic_edge = np.ascontiguousarray(
                    gt_semantic_edge.transpose(2, 0, 1)
                )
