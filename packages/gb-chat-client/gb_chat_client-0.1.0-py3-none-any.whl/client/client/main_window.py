...
class ClientMainWindow(QMainWindow):
    ...
    def history_list_update(self):
        ...
        for i in range(start_index, length):
            ...
            if item[1] == 'in':
                ...
                mess.setBackground(QBrush(QColor(255, 245, 225)))
                ...
                mess.setBackground(QBrush(QColor(223, 232, 204)))
                self.history_model.appendRow(mess)
    ...
    def set_active_user(self):
        try:
            self.current_chat_key = self.transport.key_request(
                self.current_chat)
            logger.debug(f'Public key loaded for {self.current_chat}')
            ...
            self.current_chat_key = None
            self.encryptor = None
            logger.debug(f'Unable to load key for {self.current_chat}')

        if not self.current_chat_key:
            self.messages.warning(
                self, 'Error', 'No encryption key for this user.')
            return
        ...
        try:
            ...
            if err.errno:
                self.messages.critical(
                    self, 'Error', 'Connection with the server is lost.')
                self.close()
            ...
            logger.info(f'{new_contact} successfully added.')
            self.messages.information(
                self, 'Ok', 'Contact successfully added.')
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            self.messages.information(self, 'Ok', 'Contact successfully deleted.')
            item.close()
    ...
    def send_message(self):
        ...
        try:
            ...
            logger.debug(
                f'Message sent for {self.current_chat}: {message_text}')
            self.history_list_update()
    ...
    def message(self, message):
        ...
        try:
            ...
            self.messages.warning(
                self, 'Error', 'Unable to decode message.')
            return
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                if self.messages.question(
                    self,
                    'New message',
                    f'Got new message from {sender}, open chat?',
                    QMessageBox.Yes,
