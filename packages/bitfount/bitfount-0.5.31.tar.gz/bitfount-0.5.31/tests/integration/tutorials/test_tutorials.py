...
TUTORIALS: Final[Dict[int, str]] = {
    ...
    11: "11_running_a_segmentation_data_pod",
    12: "12_training_a_custom_segmentation_model",
}
...
class TestTutorials:
    ...
    def cleanup(self) -> Generator[None, None, None]:
        ...
        tutorials = (3, 4, 5, 7, 8, 9, 12)
        for num in tutorials:
            output = TUTORIALS_ROOT / f"part_{num}_model.pt"
    ...
    def test_tutorial_3(
        ...
        assert (TUTORIALS_ROOT / "part_3_model.pt").exists()
    ...
    def test_tutorial_4(
        ...
        assert (TUTORIALS_ROOT / "part_4_model.pt").exists()
    ...
    def test_tutorial_5(
        ...
        assert (TUTORIALS_ROOT / "part_5_model.pt").exists()
    ...
    def test_tutorial_7(
        ...
        assert (TUTORIALS_ROOT / "part_7_model.pt").exists()
    ...
    def test_tutorial_8(
        ...
        assert (TUTORIALS_ROOT / "part_8_model.pt").exists()
    ...
    def test_tutorial_11(
        ...
        with testbook(
            tutorial_11, execute=False, timeout=POD_TEST_TIMEOUT_PERIOD
        ) as tb:
            tb.inject(change_dir_code, pop=True)
            tb.inject(extra_imports, pop=True)
            tb.inject(patched_authentication_code, pop=True)
    ...
    def test_tutorial_12(
        ...
        tutorial_12: NotebookNode = fixed_notebooks
        with testbook(
            tutorial_12, execute=False, timeout=IMAGE_MODELLER_TIMEOUT_PERIOD
        ) as tb:
            tb.inject(change_dir_code, pop=True)
            tb.inject(extra_imports, pop=True)
            tb.inject(patched_authentication_code, pop=True)
            ...
            # Add notebook logging support
            tb.inject(
                notebook_queue_logging_code("tutorial_12"),
                after="logger_setup",
                run=False,
            )

            logger.info("Running tutorial 12")
            with patch_key_saver(tb):
                _execute(tb)

            tb.inject(patched_authentication_cleanup_code, pop=True)

        assert (TUTORIALS_ROOT / "part_12_model.pt").exists()
