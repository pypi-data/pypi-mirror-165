...
def typeof_pd_index(val, c):
    ...
    if val.inferred_type == 'integer' or pd._libs.lib.infer_dtype(val, True
        ...
        if isinstance(val.dtype, pd.core.arrays.integer._IntegerDtype):
            emhhw__yxnz = val.dtype.numpy_dtype
            dtype = numba.np.numpy_support.from_dtype(emhhw__yxnz)
        else:
            dtype = types.int64
...
class DatetimeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', fe_type.data), ('name', fe_type.name_typ), (
            'dict', types.DictType(_dt_index_data_typ.dtype, types.int64))]
        super(DatetimeIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def overload_datetime_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    dvuav__yie = idx_typ_to_format_str_map[DatetimeIndexType].format('copy()')
    check_unsupported_args('copy', nmdg__qoqb, idx_cpy_arg_defaults, fn_str
        =dvuav__yie, package_name='pandas', module_name='Index')
...
def box_dt_index(typ, val, c):
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    qds__bicgp = c.pyapi.import_module_noblock(dkah__yjr)
    niwai__jpoi = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, niwai__jpoi.data)
    phxr__hwuec = c.pyapi.from_native_value(typ.data, niwai__jpoi.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, niwai__jpoi.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, niwai__jpoi.name, c.
        env_manager)
    args = c.pyapi.tuple_pack([phxr__hwuec])
    xir__kgr = c.pyapi.object_getattr_string(qds__bicgp, 'DatetimeIndex')
    kws = c.pyapi.dict_pack([('name', sji__xag)])
    tzvn__jaqrw = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(phxr__hwuec)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(qds__bicgp)
    c.pyapi.decref(xir__kgr)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_datetime_index(typ, val, c):
    if isinstance(typ.data, DatetimeArrayType):
        ols__ydhas = c.pyapi.object_getattr_string(val, 'array')
    else:
        ols__ydhas = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, ols__ydhas).value
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    zoaq__cml.data = data
    zoaq__cml.name = name
    dtype = _dt_index_data_typ.dtype
    dbbdj__onoy, ium__lryk = c.pyapi.call_jit_code(lambda : numba.typed.
        Dict.empty(dtype, types.int64), types.DictType(dtype, types.int64)(
        ), [])
    zoaq__cml.dict = ium__lryk
    c.pyapi.decref(ols__ydhas)
    c.pyapi.decref(sji__xag)
    return NativeValue(zoaq__cml._getvalue())
...
def init_datetime_index(typingctx, data, name):
    ...
    def codegen(context, builder, signature, args):
        bql__veovp, mvtb__tiot = args
        niwai__jpoi = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        niwai__jpoi.data = bql__veovp
        niwai__jpoi.name = mvtb__tiot
        context.nrt.incref(builder, signature.args[0], bql__veovp)
        context.nrt.incref(builder, signature.args[1], mvtb__tiot)
        dtype = _dt_index_data_typ.dtype
        niwai__jpoi.dict = context.compile_internal(builder, lambda : numba
            .typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return niwai__jpoi._getvalue()
    oku__hqpa = DatetimeIndexType(name, data)
    sig = signature(oku__hqpa, data, name)
    return sig, codegen
...
def init_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    dhqpr__krts = args[0]
    if equiv_set.has_shape(dhqpr__krts):
        return ArrayAnalysis.AnalyzeResult(shape=dhqpr__krts, pre=[])
    return None
...
def gen_dti_field_impl(field):
    qcdi__shov = 'def impl(dti):\n'
    qcdi__shov += '    numba.parfors.parfor.init_prange()\n'
    qcdi__shov += '    A = bodo.hiframes.pd_index_ext.get_index_data(dti)\n'
    qcdi__shov += '    name = bodo.hiframes.pd_index_ext.get_index_name(dti)\n'
    qcdi__shov += '    n = len(A)\n'
    qcdi__shov += '    S = np.empty(n, np.int64)\n'
    qcdi__shov += '    for i in numba.parfors.parfor.internal_prange(n):\n'
    qcdi__shov += '        val = A[i]\n'
    qcdi__shov += '        ts = bodo.utils.conversion.box_if_dt64(val)\n'
    if field in ['weekday']:
        qcdi__shov += '        S[i] = ts.' + field + '()\n'
    else:
        qcdi__shov += '        S[i] = ts.' + field + '\n'
    qcdi__shov += (
        '    return bodo.hiframes.pd_index_ext.init_numeric_index(S, name)\n')
    fmi__pddy = {}
    exec(qcdi__shov, {'numba': numba, 'np': np, 'bodo': bodo}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def overload_datetime_index_is_leap_year(dti):
    ...
    def impl(dti):
        ...
        lprcj__bzba = len(A)
        S = np.empty(lprcj__bzba, np.bool_)
        for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
            val = A[i]
            pip__uxjl = bodo.utils.conversion.box_if_dt64(val)
            S[i] = bodo.hiframes.pd_timestamp_ext.is_leap_year(pip__uxjl.year)
        return S
    return impl
...
def overload_datetime_index_date(dti):
    ...
    def impl(dti):
        ...
        for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
            val = A[i]
            pip__uxjl = bodo.utils.conversion.box_if_dt64(val)
            S[i] = datetime.date(pip__uxjl.year, pip__uxjl.month, pip__uxjl.day
                )
        return S
    return impl
...
def overload_datetime_index_min(dti, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.min', wkqk__isyl, cwk__kmj,
    ...
    def impl(dti, axis=None, skipna=True):
        numba.parfors.parfor.init_prange()
        ggo__gsvy = bodo.hiframes.pd_index_ext.get_index_data(dti)
        s = numba.cpython.builtins.get_type_max_value(numba.core.types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(len(ggo__gsvy)):
            if not bodo.libs.array_kernels.isna(ggo__gsvy, i):
                val = bodo.hiframes.pd_timestamp_ext.dt64_to_integer(ggo__gsvy
                    [i])
                s = min(s, val)
                count += 1
...
def overload_datetime_index_max(dti, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.max', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(dti,
        'Index.max()')
...
def pd_datetimeindex_overload(data=None, freq=None, tz=None, normalize=
    ...
    wkqk__isyl = dict(freq=freq, tz=tz, normalize=normalize, closed=closed,
        ambiguous=ambiguous, dayfirst=dayfirst, yearfirst=yearfirst, dtype=
        dtype, copy=copy)
    cwk__kmj = dict(freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False)
    check_unsupported_args('pandas.DatetimeIndex', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')

    def f(data=None, freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False, name=None):
        mtmu__jkjhj = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_dt64ns(mtmu__jkjhj)
...
def overload_sub_operator_datetime_index(lhs, rhs):
    if isinstance(lhs, DatetimeIndexType
        ...
        ickfn__tbdm = np.dtype('timedelta64[ns]')

        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            ggo__gsvy = bodo.hiframes.pd_index_ext.get_index_data(lhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(lhs)
            lprcj__bzba = len(ggo__gsvy)
            S = np.empty(lprcj__bzba, ickfn__tbdm)
            glpva__duwqj = rhs.value
            for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    bodo.hiframes.pd_timestamp_ext.dt64_to_integer(
                    ggo__gsvy[i]) - glpva__duwqj)
    ...
    if isinstance(rhs, DatetimeIndexType
        ...
        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            ggo__gsvy = bodo.hiframes.pd_index_ext.get_index_data(rhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(rhs)
            lprcj__bzba = len(ggo__gsvy)
            S = np.empty(lprcj__bzba, ickfn__tbdm)
            glpva__duwqj = lhs.value
            for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    glpva__duwqj - bodo.hiframes.pd_timestamp_ext.
                    dt64_to_integer(ggo__gsvy[i]))
            return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
        return impl


def gen_dti_str_binop_impl(op, is_lhs_dti):
    obgxg__btp = numba.core.utils.OPERATORS_TO_BUILTINS[op]
    qcdi__shov = 'def impl(lhs, rhs):\n'
    if is_lhs_dti:
        qcdi__shov += '  dt_index, _str = lhs, rhs\n'
        tyfb__mndl = 'arr[i] {} other'.format(obgxg__btp)
    else:
        qcdi__shov += '  dt_index, _str = rhs, lhs\n'
        tyfb__mndl = 'other {} arr[i]'.format(obgxg__btp)
    qcdi__shov += (
        '  arr = bodo.hiframes.pd_index_ext.get_index_data(dt_index)\n')
    qcdi__shov += '  l = len(arr)\n'
    qcdi__shov += (
        '  other = bodo.hiframes.pd_timestamp_ext.parse_datetime_str(_str)\n')
    qcdi__shov += '  S = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    qcdi__shov += '  for i in numba.parfors.parfor.internal_prange(l):\n'
    qcdi__shov += '    S[i] = {}\n'.format(tyfb__mndl)
    qcdi__shov += '  return S\n'
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo, 'numba': numba, 'np': np}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def pd_index_overload(data=None, dtype=None, copy=False, name=None,
    ...
    if not is_overload_none(dtype):
        cito__dgy = parse_dtype(dtype, 'pandas.Index')
        hapty__wgtw = False
    else:
        cito__dgy = getattr(data, 'dtype', None)
        hapty__wgtw = True
    if isinstance(cito__dgy, types.misc.PyObject):
        raise BodoError(
    ...
    if isinstance(data, RangeIndexType):
    ...
    elif isinstance(data, DatetimeIndexType) or cito__dgy == types.NPDatetime(
        'ns'):
        ...
        ) or cito__dgy == types.NPTimedelta('ns'):
        ...
        if isinstance(cito__dgy, (types.Integer, types.Float, types.Boolean)):
            if hapty__wgtw:
                ...
                def impl(data=None, dtype=None, copy=False, name=None,
                    tupleize_cols=True):
                    mtmu__jkjhj = bodo.utils.conversion.coerce_to_array(data)
                    fzdri__cgnmk = bodo.utils.conversion.fix_arr_dtype(
                        mtmu__jkjhj, cito__dgy)
                    return bodo.hiframes.pd_index_ext.init_numeric_index(
                        fzdri__cgnmk, name)
        elif cito__dgy in [types.string, bytes_type]:
...
def overload_datetime_index_getitem(dti, ind):
    if isinstance(dti, DatetimeIndexType):
        if isinstance(ind, types.Integer):
            ...
            def impl(dti, ind):
                fzg__aajjz = bodo.hiframes.pd_index_ext.get_index_data(dti)
                val = fzg__aajjz[ind]
...
def overload_timedelta_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            rjquu__tmt = bodo.hiframes.pd_index_ext.get_index_data(I)
            return pd.Timedelta(rjquu__tmt[ind])
        return impl

    def impl(I, ind):
        rjquu__tmt = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        cmlv__hbamg = rjquu__tmt[ind]
        return bodo.hiframes.pd_index_ext.init_timedelta_index(cmlv__hbamg,
            name)
    return impl
...
def overload_categorical_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            amw__whhi = bodo.hiframes.pd_index_ext.get_index_data(I)
            val = amw__whhi[ind]
            return val
        return impl
...
def validate_endpoints(closed):
    ...
    return ddvna__cgy, yzlga__vpxas
...
def pd_date_range_overload(start=None, end=None, periods=None, freq=None,
    ...
    wkqk__isyl = dict(tz=tz, normalize=normalize, closed=closed)
    cwk__kmj = dict(tz=None, normalize=False, closed=None)
    check_unsupported_args('pandas.date_range', wkqk__isyl, cwk__kmj,
    ...
    qcdi__shov = """def f(start=None, end=None, periods=None, freq=None, tz=None, normalize=False, name=None, closed=None):
"""
    qcdi__shov += wwle__caf
    if is_overload_none(start):
        qcdi__shov += "  start_t = pd.Timestamp('1800-01-03')\n"
    else:
        qcdi__shov += '  start_t = pd.Timestamp(start)\n'
    if is_overload_none(end):
        qcdi__shov += "  end_t = pd.Timestamp('1800-01-03')\n"
    else:
        qcdi__shov += '  end_t = pd.Timestamp(end)\n'
    if not is_overload_none(freq):
        qcdi__shov += (
            '  stride = bodo.hiframes.pd_index_ext.to_offset_value(freq)\n')
        if is_overload_none(periods):
            qcdi__shov += '  b = start_t.value\n'
            qcdi__shov += (
                '  e = b + (end_t.value - b) // stride * stride + stride // 2 + 1\n'
                )
        elif not is_overload_none(start):
            qcdi__shov += '  b = start_t.value\n'
            qcdi__shov += '  addend = np.int64(periods) * np.int64(stride)\n'
            qcdi__shov += '  e = np.int64(b) + addend\n'
        elif not is_overload_none(end):
            qcdi__shov += '  e = end_t.value + stride\n'
            qcdi__shov += '  addend = np.int64(periods) * np.int64(-stride)\n'
            qcdi__shov += '  b = np.int64(e) + addend\n'
        else:
            raise_bodo_error(
                "at least 'start' or 'end' should be specified if a 'period' is given."
                )
        qcdi__shov += '  arr = np.arange(b, e, stride, np.int64)\n'
    else:
        qcdi__shov += '  delta = end_t.value - start_t.value\n'
        qcdi__shov += '  step = delta / (periods - 1)\n'
        qcdi__shov += '  arr1 = np.arange(0, periods, 1, np.float64)\n'
        qcdi__shov += '  arr1 *= step\n'
        qcdi__shov += '  arr1 += start_t.value\n'
        qcdi__shov += '  arr = arr1.astype(np.int64)\n'
        qcdi__shov += '  arr[-1] = end_t.value\n'
    qcdi__shov += '  A = bodo.utils.conversion.convert_to_dt64ns(arr)\n'
    qcdi__shov += (
        '  return bodo.hiframes.pd_index_ext.init_datetime_index(A, name)\n')
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo, 'np': np, 'pd': pd}, fmi__pddy)
    f = fmi__pddy['f']
    return f
...
def pd_timedelta_range_overload(start=None, end=None, periods=None, freq=
    ...
    def f(start=None, end=None, periods=None, freq=None, name=None, closed=None
        ...
        uif__zrl = pd.Timedelta('1 day')
        if start is not None:
            uif__zrl = pd.Timedelta(start)
        urz__udyyf = pd.Timedelta('1 day')
        if end is not None:
            urz__udyyf = pd.Timedelta(end)
        if start is None and end is None and closed is not None:
            raise ValueError(
                'Closed has to be None if not both of start and end are defined'
                )
        ddvna__cgy, yzlga__vpxas = (bodo.hiframes.pd_index_ext.
            validate_endpoints(closed))
        if freq is not None:
            rfs__ilxp = _dummy_convert_none_to_int(freq)
            if periods is None:
                b = uif__zrl.value
                dpaya__fuxo = b + (urz__udyyf.value - b
                    ) // rfs__ilxp * rfs__ilxp + rfs__ilxp // 2 + 1
            elif start is not None:
                periods = _dummy_convert_none_to_int(periods)
                b = uif__zrl.value
                jhpq__xasmk = np.int64(periods) * np.int64(rfs__ilxp)
                dpaya__fuxo = np.int64(b) + jhpq__xasmk
            elif end is not None:
                periods = _dummy_convert_none_to_int(periods)
                dpaya__fuxo = urz__udyyf.value + rfs__ilxp
                jhpq__xasmk = np.int64(periods) * np.int64(-rfs__ilxp)
                b = np.int64(dpaya__fuxo) + jhpq__xasmk
            else:
                raise ValueError(
                    "at least 'start' or 'end' should be specified if a 'period' is given."
                    )
            yjlwe__sjhi = np.arange(b, dpaya__fuxo, rfs__ilxp, np.int64)
        else:
            periods = _dummy_convert_none_to_int(periods)
            trvj__coow = urz__udyyf.value - uif__zrl.value
            step = trvj__coow / (periods - 1)
            zagoh__zxmqd = np.arange(0, periods, 1, np.float64)
            zagoh__zxmqd *= step
            zagoh__zxmqd += uif__zrl.value
            yjlwe__sjhi = zagoh__zxmqd.astype(np.int64)
            yjlwe__sjhi[-1] = urz__udyyf.value
        if not ddvna__cgy and len(yjlwe__sjhi) and yjlwe__sjhi[0
            ] == uif__zrl.value:
            yjlwe__sjhi = yjlwe__sjhi[1:]
        if not yzlga__vpxas and len(yjlwe__sjhi) and yjlwe__sjhi[-1
            ] == urz__udyyf.value:
            yjlwe__sjhi = yjlwe__sjhi[:-1]
        S = bodo.utils.conversion.convert_to_dt64ns(yjlwe__sjhi)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return f
...
def overload_pd_timestamp_isocalendar(idx):
    cco__tmrsl = ColNamesMetaType(('year', 'week', 'day'))

    def impl(idx):
        A = bodo.hiframes.pd_index_ext.get_index_data(idx)
        numba.parfors.parfor.init_prange()
        lprcj__bzba = len(A)
        hohgy__xtccc = bodo.libs.int_arr_ext.alloc_int_array(lprcj__bzba,
            np.uint32)
        vpdg__owp = bodo.libs.int_arr_ext.alloc_int_array(lprcj__bzba, np.
            uint32)
        uxqq__bop = bodo.libs.int_arr_ext.alloc_int_array(lprcj__bzba, np.
            uint32)
        for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
            if bodo.libs.array_kernels.isna(A, i):
                bodo.libs.array_kernels.setna(hohgy__xtccc, i)
                bodo.libs.array_kernels.setna(vpdg__owp, i)
                bodo.libs.array_kernels.setna(uxqq__bop, i)
                continue
            hohgy__xtccc[i], vpdg__owp[i], uxqq__bop[i
                ] = bodo.utils.conversion.box_if_dt64(A[i]).isocalendar()
        return bodo.hiframes.pd_dataframe_ext.init_dataframe((hohgy__xtccc,
            vpdg__owp, uxqq__bop), idx, cco__tmrsl)
    return impl
...
class TimedeltaIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', _timedelta_index_data_typ), ('name', fe_type.
            name_typ), ('dict', types.DictType(_timedelta_index_data_typ.
            dtype, types.int64))]
        super(TimedeltaIndexTypeModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def box_timedelta_index(typ, val, c):
    ...
    phxr__hwuec = c.pyapi.from_native_value(_timedelta_index_data_typ,
        timedelta_index.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, timedelta_index.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, timedelta_index.name,
        c.env_manager)
    args = c.pyapi.tuple_pack([phxr__hwuec])
    kws = c.pyapi.dict_pack([('name', sji__xag)])
    xir__kgr = c.pyapi.object_getattr_string(qds__bicgp, 'TimedeltaIndex')
    tzvn__jaqrw = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(phxr__hwuec)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(qds__bicgp)
...
def unbox_timedelta_index(typ, val, c):
    qibxn__mfyre = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(_timedelta_index_data_typ, qibxn__mfyre
        ).value
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    c.pyapi.decref(qibxn__mfyre)
    c.pyapi.decref(sji__xag)
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    zoaq__cml.data = data
    zoaq__cml.name = name
...
def init_timedelta_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        timedelta_index = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        timedelta_index.data = bql__veovp
        timedelta_index.name = mvtb__tiot
        context.nrt.incref(builder, signature.args[0], bql__veovp)
        context.nrt.incref(builder, signature.args[1], mvtb__tiot)
        dtype = _timedelta_index_data_typ.dtype
    ...
    oku__hqpa = TimedeltaIndexType(name)
    sig = signature(oku__hqpa, data, name)
    return sig, codegen
...
def overload_timedelta_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    dvuav__yie = idx_typ_to_format_str_map[TimedeltaIndexType].format('copy()')
    check_unsupported_args('TimedeltaIndex.copy', nmdg__qoqb,
        idx_cpy_arg_defaults, fn_str=dvuav__yie, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def overload_timedelta_index_min(tdi, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.min', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')

    def impl(tdi, axis=None, skipna=True):
        numba.parfors.parfor.init_prange()
        data = bodo.hiframes.pd_index_ext.get_index_data(tdi)
        lprcj__bzba = len(data)
        zduh__hag = numba.cpython.builtins.get_type_max_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
            ...
            count += 1
            zduh__hag = min(zduh__hag, val)
        ifzz__xgmz = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
            zduh__hag)
        return bodo.hiframes.pd_index_ext._tdi_val_finalize(ifzz__xgmz, count)
    return impl
...
def overload_timedelta_index_max(tdi, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.max', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    ...
    def impl(tdi, axis=None, skipna=True):
        ...
        data = bodo.hiframes.pd_index_ext.get_index_data(tdi)
        lprcj__bzba = len(data)
        kezv__nbca = numba.cpython.builtins.get_type_min_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
            if bodo.libs.array_kernels.isna(data, i):
...
def gen_tdi_field_impl(field):
    qcdi__shov = 'def impl(tdi):\n'
    qcdi__shov += '    numba.parfors.parfor.init_prange()\n'
    qcdi__shov += '    A = bodo.hiframes.pd_index_ext.get_index_data(tdi)\n'
    qcdi__shov += '    name = bodo.hiframes.pd_index_ext.get_index_name(tdi)\n'
    qcdi__shov += '    n = len(A)\n'
    qcdi__shov += '    S = np.empty(n, np.int64)\n'
    qcdi__shov += '    for i in numba.parfors.parfor.internal_prange(n):\n'
    qcdi__shov += (
    ...
    if field == 'nanoseconds':
        qcdi__shov += '        S[i] = td64 % 1000\n'
    elif field == 'microseconds':
        qcdi__shov += '        S[i] = td64 // 1000 % 100000\n'
    elif field == 'seconds':
        qcdi__shov += (
...
def pd_timedelta_index_overload(data=None, unit=None, freq=None, dtype=None,
    ...
    wkqk__isyl = dict(unit=unit, freq=freq, dtype=dtype, copy=copy)
    cwk__kmj = dict(unit=None, freq=None, dtype=None, copy=False)
    check_unsupported_args('pandas.TimedeltaIndex', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')

    def impl(data=None, unit=None, freq=None, dtype=None, copy=False, name=None
        ):
        mtmu__jkjhj = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_td64ns(mtmu__jkjhj)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return impl
...
class RangeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('start', types.int64), ('stop', types.int64), ('step',
            types.int64), ('name', fe_type.name_typ)]
        super(RangeIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def overload_range_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    dvuav__yie = idx_typ_to_format_str_map[RangeIndexType].format('copy()')
    check_unsupported_args('RangeIndex.copy', nmdg__qoqb,
        idx_cpy_arg_defaults, fn_str=dvuav__yie, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def box_range_index(typ, val, c):
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    jcm__gjhee = c.pyapi.import_module_noblock(dkah__yjr)
    mrb__jpmo = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    ath__jfi = c.pyapi.from_native_value(types.int64, mrb__jpmo.start, c.
        env_manager)
    mev__qul = c.pyapi.from_native_value(types.int64, mrb__jpmo.stop, c.
        env_manager)
    rzr__yokgm = c.pyapi.from_native_value(types.int64, mrb__jpmo.step, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, mrb__jpmo.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, mrb__jpmo.name, c.
        env_manager)
    args = c.pyapi.tuple_pack([ath__jfi, mev__qul, rzr__yokgm])
    kws = c.pyapi.dict_pack([('name', sji__xag)])
    xir__kgr = c.pyapi.object_getattr_string(jcm__gjhee, 'RangeIndex')
    ktq__tmh = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(ath__jfi)
    c.pyapi.decref(mev__qul)
    c.pyapi.decref(rzr__yokgm)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(jcm__gjhee)
    c.pyapi.decref(xir__kgr)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
    c.context.nrt.decref(c.builder, typ, val)
...
def init_range_index(typingctx, start, stop, step, name=None):
    name = types.none if name is None else name
    reydz__jsg = is_overload_constant_int(step) and get_overload_const_int(step
        ) == 0
    ...
    def codegen(context, builder, signature, args):
        ...
        igpnc__zvc = cgutils.is_scalar_zero(builder, args[2])
        hkxdy__fcl = context.get_python_api(builder)
        with builder.if_then(igpnc__zvc):
            hkxdy__fcl.err_format('PyExc_ValueError', 'Step must not be zero')
            val = context.get_constant(types.int32, -1)
            builder.ret(val)
        mrb__jpmo = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        mrb__jpmo.start = args[0]
        mrb__jpmo.stop = args[1]
        mrb__jpmo.step = args[2]
        mrb__jpmo.name = args[3]
        context.nrt.incref(builder, signature.return_type.name_typ, args[3])
        return mrb__jpmo._getvalue()
    return RangeIndexType(name)(start, stop, step, name), codegen


def init_range_index_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 4 and not kws
    start, stop, step, uzd__yrbd = args
...
def unbox_range_index(typ, val, c):
    ath__jfi = c.pyapi.object_getattr_string(val, 'start')
    start = c.pyapi.to_native_value(types.int64, ath__jfi).value
    mev__qul = c.pyapi.object_getattr_string(val, 'stop')
    stop = c.pyapi.to_native_value(types.int64, mev__qul).value
    rzr__yokgm = c.pyapi.object_getattr_string(val, 'step')
    step = c.pyapi.to_native_value(types.int64, rzr__yokgm).value
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    c.pyapi.decref(ath__jfi)
    c.pyapi.decref(mev__qul)
    c.pyapi.decref(rzr__yokgm)
    c.pyapi.decref(sji__xag)
    mrb__jpmo = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    mrb__jpmo.start = start
    mrb__jpmo.stop = stop
    mrb__jpmo.step = step
    mrb__jpmo.name = name
    return NativeValue(mrb__jpmo._getvalue())
...
def range_index_overload(start=None, stop=None, step=None, dtype=None, copy
    ...
    def _ensure_int_or_none(value, field):
        ...
        if not is_overload_none(value) and not isinstance(value, types.
            ...
            raise BodoError(dob__bbrx.format(value=value, field=field))
    ...
    if is_overload_none(start) and is_overload_none(stop) and is_overload_none(
        step):
        dob__bbrx = 'RangeIndex(...) must be called with integers'
        raise BodoError(dob__bbrx)
    pnvh__jygrq = 'start'
    sixv__qamv = 'stop'
    lkgf__wnj = 'step'
    if is_overload_none(start):
    ...
    if is_overload_none(step):
        lkgf__wnj = '1'
    qcdi__shov = """def _pd_range_index_imp(start=None, stop=None, step=None, dtype=None, copy=False, name=None):
"""
    qcdi__shov += '  return init_range_index({}, {}, {}, name)\n'.format(
        pnvh__jygrq, sixv__qamv, lkgf__wnj)
    fmi__pddy = {}
    exec(qcdi__shov, {'init_range_index': init_range_index}, fmi__pddy)
    jgxu__ark = fmi__pddy['_pd_range_index_imp']
    return jgxu__ark
...
def overload_range_index_getitem(I, idx):
    if isinstance(I, RangeIndexType):
        ...
        if isinstance(idx, types.SliceType):
            ...
            def impl(I, idx):
                eic__plvu = numba.cpython.unicode._normalize_slice(idx, len(I))
                name = bodo.hiframes.pd_index_ext.get_index_name(I)
                start = I._start + I._step * eic__plvu.start
                stop = I._start + I._step * eic__plvu.stop
                step = I._step * eic__plvu.step
...
class PeriodIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', bodo.IntegerArrayType(types.int64)), ('name',
            fe_type.name_typ), ('dict', types.DictType(types.int64, types.
            int64))]
        super(PeriodIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def overload_period_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    freq = A.freq
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    dvuav__yie = idx_typ_to_format_str_map[PeriodIndexType].format('copy()')
    check_unsupported_args('PeriodIndex.copy', nmdg__qoqb,
        idx_cpy_arg_defaults, fn_str=dvuav__yie, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def init_period_index(typingctx, data, name, freq):
    ...
    def codegen(context, builder, signature, args):
        bql__veovp, mvtb__tiot, uzd__yrbd = args
        ihjw__elfr = signature.return_type
        qoy__tghm = cgutils.create_struct_proxy(ihjw__elfr)(context, builder)
        qoy__tghm.data = bql__veovp
        qoy__tghm.name = mvtb__tiot
        context.nrt.incref(builder, signature.args[0], args[0])
        context.nrt.incref(builder, signature.args[1], args[1])
        qoy__tghm.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(types.int64, types.int64), types.DictType(
            types.int64, types.int64)(), [])
        return qoy__tghm._getvalue()
    err__rbbn = get_overload_const_str(freq)
    oku__hqpa = PeriodIndexType(err__rbbn, name)
    sig = signature(oku__hqpa, data, name, freq)
    return sig, codegen
...
def box_period_index(typ, val, c):
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    jcm__gjhee = c.pyapi.import_module_noblock(dkah__yjr)
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, bodo.IntegerArrayType(types.int64),
        zoaq__cml.data)
    ols__ydhas = c.pyapi.from_native_value(bodo.IntegerArrayType(types.
        int64), zoaq__cml.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, zoaq__cml.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, zoaq__cml.name, c.
        env_manager)
    she__nyiuv = c.pyapi.string_from_constant_string(typ.freq)
    args = c.pyapi.tuple_pack([])
    kws = c.pyapi.dict_pack([('ordinal', ols__ydhas), ('name', sji__xag), (
        'freq', she__nyiuv)])
    xir__kgr = c.pyapi.object_getattr_string(jcm__gjhee, 'PeriodIndex')
    ktq__tmh = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(ols__ydhas)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(she__nyiuv)
    c.pyapi.decref(jcm__gjhee)
    c.pyapi.decref(xir__kgr)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_period_index(typ, val, c):
    ...
    acmfj__bgrk = c.pyapi.object_getattr_string(val, 'asi8')
    rky__gzyh = c.pyapi.call_method(val, 'isna', ())
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    qds__bicgp = c.pyapi.import_module_noblock(dkah__yjr)
    cfceo__ewl = c.pyapi.object_getattr_string(qds__bicgp, 'arrays')
    ols__ydhas = c.pyapi.call_method(cfceo__ewl, 'IntegerArray', (
        acmfj__bgrk, rky__gzyh))
    data = c.pyapi.to_native_value(arr_typ, ols__ydhas).value
    c.pyapi.decref(acmfj__bgrk)
    c.pyapi.decref(rky__gzyh)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(qds__bicgp)
    c.pyapi.decref(cfceo__ewl)
    c.pyapi.decref(ols__ydhas)
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    zoaq__cml.data = data
    zoaq__cml.name = name
...
class CategoricalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        ifvpj__luo = get_categories_int_type(fe_type.data.dtype)
        lkuo__myj = [('data', fe_type.data), ('name', fe_type.name_typ), (
            'dict', types.DictType(ifvpj__luo, types.int64))]
        super(CategoricalIndexTypeModel, self).__init__(dmm, fe_type, lkuo__myj
            )
...
def box_categorical_index(typ, val, c):
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    qds__bicgp = c.pyapi.import_module_noblock(dkah__yjr)
    wstnq__iyzs = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, wstnq__iyzs.data)
    phxr__hwuec = c.pyapi.from_native_value(typ.data, wstnq__iyzs.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, wstnq__iyzs.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, wstnq__iyzs.name, c.
        env_manager)
    args = c.pyapi.tuple_pack([phxr__hwuec])
    kws = c.pyapi.dict_pack([('name', sji__xag)])
    xir__kgr = c.pyapi.object_getattr_string(qds__bicgp, 'CategoricalIndex')
    tzvn__jaqrw = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(phxr__hwuec)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(qds__bicgp)
...
def unbox_categorical_index(typ, val, c):
    from bodo.hiframes.pd_categorical_ext import get_categories_int_type
    qibxn__mfyre = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, qibxn__mfyre).value
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    c.pyapi.decref(qibxn__mfyre)
    c.pyapi.decref(sji__xag)
...
def init_categorical_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        bql__veovp, mvtb__tiot = args
        wstnq__iyzs = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        wstnq__iyzs.data = bql__veovp
        wstnq__iyzs.name = mvtb__tiot
        context.nrt.incref(builder, signature.args[0], bql__veovp)
        context.nrt.incref(builder, signature.args[1], mvtb__tiot)
        dtype = get_categories_int_type(signature.return_type.data.dtype)
        wstnq__iyzs.dict = context.compile_internal(builder, lambda : numba
            .typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return wstnq__iyzs._getvalue()
    oku__hqpa = CategoricalIndexType(data, name)
    sig = signature(oku__hqpa, data, name)
    return sig, codegen
...
def overload_categorical_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    dvuav__yie = idx_typ_to_format_str_map[CategoricalIndexType].format(
        'copy()')
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('CategoricalIndex.copy', nmdg__qoqb,
        idx_cpy_arg_defaults, fn_str=dvuav__yie, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
class IntervalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', fe_type.data), ('name', fe_type.name_typ), (
            ...
            dtype, 2), types.int64))]
        super(IntervalIndexTypeModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def box_interval_index(typ, val, c):
    dkah__yjr = c.context.insert_const_string(c.builder.module, 'pandas')
    qds__bicgp = c.pyapi.import_module_noblock(dkah__yjr)
    vqk__easii = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, vqk__easii.data)
    phxr__hwuec = c.pyapi.from_native_value(typ.data, vqk__easii.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, vqk__easii.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, vqk__easii.name, c.
        env_manager)
    args = c.pyapi.tuple_pack([phxr__hwuec])
    kws = c.pyapi.dict_pack([('name', sji__xag)])
    xir__kgr = c.pyapi.object_getattr_string(qds__bicgp, 'IntervalIndex')
    tzvn__jaqrw = c.pyapi.call(xir__kgr, args, kws)
    c.pyapi.decref(phxr__hwuec)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(qds__bicgp)
...
def init_interval_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        bql__veovp, mvtb__tiot = args
        vqk__easii = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        vqk__easii.data = bql__veovp
        vqk__easii.name = mvtb__tiot
        context.nrt.incref(builder, signature.args[0], bql__veovp)
        context.nrt.incref(builder, signature.args[1], mvtb__tiot)
        dtype = types.UniTuple(data.arr_type.dtype, 2)
        vqk__easii.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return vqk__easii._getvalue()
    oku__hqpa = IntervalIndexType(data, name)
    sig = signature(oku__hqpa, data, name)
    return sig, codegen
...
class NumericIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(NumericIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def overload_numeric_index_copy(A, name=None, deep=False, dtype=None, names
    =None):
    dvuav__yie = idx_typ_to_format_str_map[NumericIndexType].format('copy()')
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', nmdg__qoqb, idx_cpy_arg_defaults,
        fn_str=dvuav__yie, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def box_numeric_index(typ, val, c):
    ...
    jcm__gjhee = c.pyapi.import_module_noblock(dkah__yjr)
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, typ.data, zoaq__cml.data)
    ols__ydhas = c.pyapi.from_native_value(typ.data, zoaq__cml.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, zoaq__cml.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, zoaq__cml.name, c.
        env_manager)
    rdx__awel = c.pyapi.make_none()
    epihp__tkjlj = c.pyapi.bool_from_bool(c.context.get_constant(types.
        bool_, False))
    ktq__tmh = c.pyapi.call_method(jcm__gjhee, 'Index', (ols__ydhas,
        rdx__awel, epihp__tkjlj, sji__xag))
    c.pyapi.decref(ols__ydhas)
    c.pyapi.decref(rdx__awel)
    c.pyapi.decref(epihp__tkjlj)
    c.pyapi.decref(sji__xag)
    c.pyapi.decref(jcm__gjhee)
    c.context.nrt.decref(c.builder, typ, val)
    return ktq__tmh
...
def init_numeric_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        assert len(args) == 2
        ihjw__elfr = signature.return_type
        zoaq__cml = cgutils.create_struct_proxy(ihjw__elfr)(context, builder)
        zoaq__cml.data = args[0]
        zoaq__cml.name = args[1]
        context.nrt.incref(builder, ihjw__elfr.data, args[0])
        context.nrt.incref(builder, ihjw__elfr.name_typ, args[1])
        dtype = ihjw__elfr.dtype
        zoaq__cml.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return zoaq__cml._getvalue()
...
def create_numeric_constructor(func, func_str, default_dtype):
    ...
    def overload_impl(data=None, dtype=None, copy=False, name=None):
        txhy__epshx = dict(dtype=dtype)
        bfbp__tqf = dict(dtype=None)
        check_unsupported_args(func_str, txhy__epshx, bfbp__tqf,
            package_name='pandas', module_name='Index')
        if is_overload_false(copy):

            def impl(data=None, dtype=None, copy=False, name=None):
                mtmu__jkjhj = bodo.utils.conversion.coerce_to_ndarray(data)
                ktz__cumvq = bodo.utils.conversion.fix_arr_dtype(mtmu__jkjhj,
                    np.dtype(default_dtype))
                return bodo.hiframes.pd_index_ext.init_numeric_index(ktz__cumvq
                    , name)
        else:
...
class StringIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(StringIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
class BinaryIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', binary_array_type), ('name', fe_type.name_typ
            ), ('dict', types.DictType(bytes_type, types.int64))]
        super(BinaryIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def unbox_binary_str_index(typ, val, c):
    ...
    qibxn__mfyre = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(knrd__zsn, qibxn__mfyre).value
    sji__xag = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, sji__xag).value
    c.pyapi.decref(qibxn__mfyre)
    c.pyapi.decref(sji__xag)
...
def box_binary_str_index(typ, val, c):
    ...
    jcm__gjhee = c.pyapi.import_module_noblock(dkah__yjr)
    zoaq__cml = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, knrd__zsn, zoaq__cml.data)
    ols__ydhas = c.pyapi.from_native_value(knrd__zsn, zoaq__cml.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, zoaq__cml.name)
    sji__xag = c.pyapi.from_native_value(typ.name_typ, zoaq__cml.name, c.
        env_manager)
...
def init_binary_str_index(typingctx, data, name=None):
    ...
    sig = type(bodo.utils.typing.get_index_type_from_dtype(data.dtype))(name,
        data)(data, name)
    vjoc__zuice = get_binary_str_codegen(is_binary=data.dtype == bytes_type)
    return sig, vjoc__zuice
...
def get_binary_str_codegen(is_binary=False):
    if is_binary:
        pcbi__qsffs = 'bytes_type'
    else:
        pcbi__qsffs = 'string_type'
    qcdi__shov = 'def impl(context, builder, signature, args):\n'
    qcdi__shov += '    assert len(args) == 2\n'
    qcdi__shov += '    index_typ = signature.return_type\n'
    qcdi__shov += (
        '    index_val = cgutils.create_struct_proxy(index_typ)(context, builder)\n'
        )
    qcdi__shov += '    index_val.data = args[0]\n'
    qcdi__shov += '    index_val.name = args[1]\n'
    qcdi__shov += '    # increase refcount of stored values\n'
    qcdi__shov += (
        '    context.nrt.incref(builder, signature.args[0], args[0])\n')
    qcdi__shov += (
        '    context.nrt.incref(builder, index_typ.name_typ, args[1])\n')
    qcdi__shov += '    # create empty dict for get_loc hashmap\n'
    qcdi__shov += '    index_val.dict = context.compile_internal(\n'
    qcdi__shov += '       builder,\n'
    qcdi__shov += (
        f'       lambda: numba.typed.Dict.empty({pcbi__qsffs}, types.int64),\n'
        )
    qcdi__shov += (
        f'        types.DictType({pcbi__qsffs}, types.int64)(), [],)\n')
    qcdi__shov += '    return index_val._getvalue()\n'
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo, 'signature': signature, 'cgutils':
        cgutils, 'numba': numba, 'types': types, 'bytes_type': bytes_type,
        'string_type': string_type}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def overload_binary_string_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    typ = type(A)
    dvuav__yie = idx_typ_to_format_str_map[typ].format('copy()')
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', nmdg__qoqb, idx_cpy_arg_defaults,
        fn_str=dvuav__yie, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def _verify_setop_compatible(func_name, I, other):
    ...
    rvz__exs = I.dtype if not isinstance(I, RangeIndexType) else types.int64
    pou__rnpex = other.dtype if not isinstance(other, RangeIndexType
        ) else types.int64
    if rvz__exs != pou__rnpex:
        raise BodoError(
            f'Index.{func_name}(): incompatible types {rvz__exs} and {pou__rnpex}'
            )
...
def overload_index_union(I, other, sort=None):
    wkqk__isyl = dict(sort=sort)
    onr__igyl = dict(sort=None)
    check_unsupported_args('Index.union', wkqk__isyl, onr__igyl,
        package_name='pandas', module_name='Index')
    _verify_setop_compatible('union', I, other)
    tutil__fuo = get_index_constructor(I) if not isinstance(I, RangeIndexType
        ) else init_numeric_index

    def impl(I, other, sort=None):
        pnci__kkx = bodo.utils.conversion.coerce_to_array(I)
        qhx__qcjz = bodo.utils.conversion.coerce_to_array(other)
        gvvva__diyex = bodo.libs.array_kernels.concat([pnci__kkx, qhx__qcjz])
        prren__lydg = bodo.libs.array_kernels.unique(gvvva__diyex)
        return tutil__fuo(prren__lydg, None)
    return impl
...
def overload_index_intersection(I, other, sort=None):
    wkqk__isyl = dict(sort=sort)
    onr__igyl = dict(sort=None)
    check_unsupported_args('Index.intersection', wkqk__isyl, onr__igyl,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        pnci__kkx = bodo.utils.conversion.coerce_to_array(I)
        qhx__qcjz = bodo.utils.conversion.coerce_to_array(other)
        rpy__hggmu = bodo.libs.array_kernels.unique(pnci__kkx)
        exno__isj = bodo.libs.array_kernels.unique(qhx__qcjz)
        gvvva__diyex = bodo.libs.array_kernels.concat([rpy__hggmu, exno__isj])
        yis__rjy = pd.Series(gvvva__diyex).sort_values().values
        qiki__ojanq = bodo.libs.array_kernels.intersection_mask(yis__rjy)
        return tutil__fuo(yis__rjy[qiki__ojanq], None)
    return impl
...
def overload_index_difference(I, other, sort=None):
    wkqk__isyl = dict(sort=sort)
    onr__igyl = dict(sort=None)
    check_unsupported_args('Index.difference', wkqk__isyl, onr__igyl,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        ...
        rpy__hggmu = bodo.libs.array_kernels.unique(pnci__kkx)
        exno__isj = bodo.libs.array_kernels.unique(qhx__qcjz)
        qiki__ojanq = np.empty(len(rpy__hggmu), np.bool_)
        bodo.libs.array.array_isin(qiki__ojanq, rpy__hggmu, exno__isj, False)
        return tutil__fuo(rpy__hggmu[~qiki__ojanq], None)
    return impl
...
def overload_index_symmetric_difference(I, other, result_name=None, sort=None):
    wkqk__isyl = dict(result_name=result_name, sort=sort)
    onr__igyl = dict(result_name=None, sort=None)
    check_unsupported_args('Index.symmetric_difference', wkqk__isyl,
        onr__igyl, package_name='pandas', module_name='Index')
    ...
    def impl(I, other, result_name=None, sort=None):
        ...
        rpy__hggmu = bodo.libs.array_kernels.unique(pnci__kkx)
        exno__isj = bodo.libs.array_kernels.unique(qhx__qcjz)
        wmt__hgzfb = np.empty(len(rpy__hggmu), np.bool_)
        flsn__ksssl = np.empty(len(exno__isj), np.bool_)
        bodo.libs.array.array_isin(wmt__hgzfb, rpy__hggmu, exno__isj, False)
        bodo.libs.array.array_isin(flsn__ksssl, exno__isj, rpy__hggmu, False)
        pzj__rqpc = bodo.libs.array_kernels.concat([rpy__hggmu[~wmt__hgzfb],
            exno__isj[~flsn__ksssl]])
        return tutil__fuo(pzj__rqpc, None)
    return impl
...
def overload_index_take(I, indices, axis=0, allow_fill=True, fill_value=None):
    wkqk__isyl = dict(axis=axis, allow_fill=allow_fill, fill_value=fill_value)
    onr__igyl = dict(axis=0, allow_fill=True, fill_value=None)
    check_unsupported_args('Index.take', wkqk__isyl, onr__igyl,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, ban_unique=True):
            if len(I) > 0 and not I._dict:
                yjlwe__sjhi = bodo.utils.conversion.coerce_to_array(I)
                for i in range(len(yjlwe__sjhi)):
                    if not bodo.libs.array_kernels.isna(yjlwe__sjhi, i):
                        val = (bodo.hiframes.pd_categorical_ext.
                            get_code_for_value(yjlwe__sjhi.dtype,
                            yjlwe__sjhi[i]))
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, val):
            ...
            if not is_null_value(I._dict):
                ...
                yjlwe__sjhi = bodo.utils.conversion.coerce_to_array(I)
                odpq__ksvx = (bodo.hiframes.pd_categorical_ext.
                    get_code_for_value(yjlwe__sjhi.dtype, key))
                return odpq__ksvx in I._dict
            else:
                dob__bbrx = (
                    'Global Index objects can be slow (pass as argument to JIT function for better performance).'
                    )
                warnings.warn(dob__bbrx)
                yjlwe__sjhi = bodo.utils.conversion.coerce_to_array(I)
                ind = -1
                for i in range(len(yjlwe__sjhi)):
                    if not bodo.libs.array_kernels.isna(yjlwe__sjhi, i):
                        if yjlwe__sjhi[i] == key:
                            ind = i
            return ind != -1
        return impl
...
def overload_index_get_loc(I, key, method=None, tolerance=None):
    wkqk__isyl = dict(method=method, tolerance=tolerance)
    cwk__kmj = dict(method=None, tolerance=None)
    check_unsupported_args('Index.get_loc', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
...
def create_isna_specific_method(overload_name):
    ...
    def overload_index_isna_specific_method(I):
        tchgj__yftd = overload_name in {'isna', 'isnull'}
        if isinstance(I, RangeIndexType):

            def impl(I):
                numba.parfors.parfor.init_prange()
                lprcj__bzba = len(I)
                jwv__jtgg = np.empty(lprcj__bzba, np.bool_)
                for i in numba.parfors.parfor.internal_prange(lprcj__bzba):
                    jwv__jtgg[i] = not tchgj__yftd
                return jwv__jtgg
            return impl
        qcdi__shov = f"""def impl(I):
    numba.parfors.parfor.init_prange()
    arr = bodo.hiframes.pd_index_ext.get_index_data(I)
    n = len(arr)
    out_arr = np.empty(n, np.bool_)
    for i in numba.parfors.parfor.internal_prange(n):
       out_arr[i] = {'' if tchgj__yftd else 'not '}bodo.libs.array_kernels.isna(arr, i)
    return out_arr
"""
        fmi__pddy = {}
        exec(qcdi__shov, {'bodo': bodo, 'np': np, 'numba': numba}, fmi__pddy)
        impl = fmi__pddy['impl']
        return impl
...
def overload_index_is_montonic(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(yjlwe__sjhi, 1)
        return impl
...
def overload_index_is_montonic_decreasing(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(yjlwe__sjhi, 2)
        return impl
    elif isinstance(I, RangeIndexType):
...
def overload_index_duplicated(I, keep='first'):
    ...
    def impl(I, keep='first'):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        jwv__jtgg = bodo.libs.array_kernels.duplicated((yjlwe__sjhi,))
        return jwv__jtgg
    return impl
...
def overload_index_drop_duplicates(I, keep='first'):
    wkqk__isyl = dict(keep=keep)
    cwk__kmj = dict(keep='first')
    check_unsupported_args('Index.drop_duplicates', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        return lambda I, keep='first': I.copy()
    qcdi__shov = """def impl(I, keep='first'):
...
"""
    if isinstance(I, PeriodIndexType):
        qcdi__shov += f"""    return bodo.hiframes.pd_index_ext.init_period_index(arr, name, '{I.freq}')
"""
    else:
        qcdi__shov += (
            '    return bodo.utils.conversion.index_from_array(arr, name)')
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def get_index_data_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 1 and not kws
    dhqpr__krts = args[0]
    if isinstance(self.typemap[dhqpr__krts.name], (HeterogeneousIndexType,
        MultiIndexType)):
        return None
    if equiv_set.has_shape(dhqpr__krts):
        return ArrayAnalysis.AnalyzeResult(shape=dhqpr__krts, pre=[])
...
def overload_index_map(I, mapper, na_action=None):
    ...
    wkqk__isyl = dict(na_action=na_action)
    arf__muf = dict(na_action=None)
    check_unsupported_args('Index.map', wkqk__isyl, arf__muf, package_name=
    ...
    cbk__rdo = numba.core.registry.cpu_target.typing_context
    hew__shvm = numba.core.registry.cpu_target.target_context
    try:
        vvvr__tkk = get_const_func_output_type(mapper, (dtype,), {},
            cbk__rdo, hew__shvm)
    except Exception as dpaya__fuxo:
        raise_bodo_error(get_udf_error_msg('Index.map()', dpaya__fuxo))
    xcjp__dkzof = get_udf_out_arr_type(vvvr__tkk)
    func = get_overload_const_func(mapper, None)
    qcdi__shov = 'def f(I, mapper, na_action=None):\n'
    qcdi__shov += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    qcdi__shov += '  A = bodo.utils.conversion.coerce_to_array(I)\n'
    qcdi__shov += '  numba.parfors.parfor.init_prange()\n'
    qcdi__shov += '  n = len(A)\n'
    qcdi__shov += '  S = bodo.utils.utils.alloc_type(n, _arr_typ, (-1,))\n'
    qcdi__shov += '  for i in numba.parfors.parfor.internal_prange(n):\n'
    qcdi__shov += '    t2 = bodo.utils.conversion.box_if_dt64(A[i])\n'
    qcdi__shov += '    v = map_func(t2)\n'
    qcdi__shov += '    S[i] = bodo.utils.conversion.unbox_if_timestamp(v)\n'
    qcdi__shov += '  return bodo.utils.conversion.index_from_array(S, name)\n'
    mcsg__rbov = bodo.compiler.udf_jit(func)
    fmi__pddy = {}
    exec(qcdi__shov, {'numba': numba, 'np': np, 'pd': pd, 'bodo': bodo,
        'map_func': mcsg__rbov, '_arr_typ': xcjp__dkzof,
        'init_nested_counts': bodo.utils.indexing.init_nested_counts,
        'add_nested_counts': bodo.utils.indexing.add_nested_counts,
        'data_arr_type': xcjp__dkzof.dtype}, fmi__pddy)
    f = fmi__pddy['f']
    return f
...
def index_is(context, builder, sig, args):
    urj__fctl, vtzl__gnt = sig.args
    if urj__fctl != vtzl__gnt:
...
"""
            if rhs in [bodo.hiframes.pd_timestamp_ext.pd_timestamp_type,
                ...
                qcdi__shov += """  dt = bodo.utils.conversion.unbox_if_timestamp(rhs)
  return op(arr, dt)
"""
            else:
                qcdi__shov += """  rhs_arr = bodo.utils.conversion.get_array_if_series_or_index(rhs)
  return op(arr, rhs_arr)
"""
            fmi__pddy = {}
            exec(qcdi__shov, {'bodo': bodo, 'op': op}, fmi__pddy)
            impl = fmi__pddy['impl']
            return impl
...
"""
            if lhs in [bodo.hiframes.pd_timestamp_ext.pd_timestamp_type,
                bodo.hiframes.pd_timestamp_ext.pd_timedelta_type]:
                qcdi__shov += """  dt = bodo.utils.conversion.unbox_if_timestamp(lhs)
  return op(dt, arr)
"""
            else:
                qcdi__shov += """  lhs_arr = bodo.utils.conversion.get_array_if_series_or_index(lhs)
        ...
        if isinstance(lhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(lhs.data):
                ...
                def impl3(lhs, rhs):
                    ...
                    yjlwe__sjhi = bodo.utils.conversion.coerce_to_array(data)
                    tzaar__emmqm = (bodo.utils.conversion.
                        get_array_if_series_or_index(rhs))
                    jwv__jtgg = op(yjlwe__sjhi, tzaar__emmqm)
                    return jwv__jtgg
                return impl3
            count = len(lhs.data.types)
            qcdi__shov = 'def f(lhs, rhs):\n'
            qcdi__shov += '  return [{}]\n'.format(','.join(
                'op(lhs[{}], rhs{})'.format(i, f'[{i}]' if is_iterable_type
                (rhs) else '') for i in range(count)))
            fmi__pddy = {}
            exec(qcdi__shov, {'op': op, 'np': np}, fmi__pddy)
            impl = fmi__pddy['f']
            return impl
        if isinstance(rhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(rhs.data):
                ...
                def impl4(lhs, rhs):
                    ...
                    jwv__jtgg = op(tzaar__emmqm, yjlwe__sjhi)
                    return jwv__jtgg
                return impl4
...
class HeterogeneousIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lkuo__myj = [('data', fe_type.data), ('name', fe_type.name_typ)]
        super(HeterogeneousIndexModel, self).__init__(dmm, fe_type, lkuo__myj)
...
def overload_heter_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    dvuav__yie = idx_typ_to_format_str_map[HeterogeneousIndexType].format(
        'copy()')
    nmdg__qoqb = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', nmdg__qoqb, idx_cpy_arg_defaults,
        fn_str=dvuav__yie, package_name='pandas', module_name='Index')
...
def overload_nbytes(I):
    if isinstance(I, RangeIndexType):
        ...
        qcdi__shov = 'def _impl_nbytes(I):\n'
        qcdi__shov += '    total = 0\n'
        qcdi__shov += '    data = I._data\n'
        for i in range(I.nlevels):
            qcdi__shov += f'    total += data[{i}].nbytes\n'
        qcdi__shov += '    return total\n'
        epal__izq = {}
        exec(qcdi__shov, {}, epal__izq)
        return epal__izq['_impl_nbytes']
    else:
...
def overload_index_to_series(I, index=None, name=None):
    ...
    if is_overload_none(name):
        ybayf__rnlsm = 'bodo.hiframes.pd_index_ext.get_index_name(I)'
    else:
        ybayf__rnlsm = 'name'
    qcdi__shov = 'def impl(I, index=None, name=None):\n'
    qcdi__shov += '    data = bodo.utils.conversion.index_to_array(I)\n'
    if is_overload_none(index):
        qcdi__shov += '    new_index = I\n'
    elif is_pd_index_type(index):
        qcdi__shov += '    new_index = index\n'
    ...
    qcdi__shov += f'    new_name = {ybayf__rnlsm}\n'
    qcdi__shov += (
        '    return bodo.hiframes.pd_series_ext.init_series(data, new_index, new_name)'
        )
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo, 'np': np}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def overload_index_to_frame(I, index=True, name=None):
    ...
    qcdi__shov = 'def impl(I, index=True, name=None):\n'
    qcdi__shov += '    data = bodo.utils.conversion.index_to_array(I)\n'
    qcdi__shov += f'    new_index = {gqmk__lvyjj}\n'
    if is_overload_none(name) and I.name_typ == types.none:
        yfjpb__stldk = ColNamesMetaType((0,))
    elif is_overload_none(name):
        yfjpb__stldk = ColNamesMetaType((I.name_typ,))
    elif is_overload_constant_str(name):
        yfjpb__stldk = ColNamesMetaType((get_overload_const_str(name),))
    elif is_overload_constant_int(name):
        yfjpb__stldk = ColNamesMetaType((get_overload_const_int(name),))
    else:
        raise_bodo_error(
            f'Index.to_frame(): only constant string/int are supported for argument name'
            )
    qcdi__shov += """    return bodo.hiframes.pd_dataframe_ext.init_dataframe((data,), new_index, __col_name_meta_value)
"""
    fmi__pddy = {}
    exec(qcdi__shov, {'bodo': bodo, 'np': np, '__col_name_meta_value':
        yfjpb__stldk}, fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def overload_multi_index_to_frame(I, index=True, name=None):
    ...
    qcdi__shov = 'def impl(I, index=True, name=None):\n'
    qcdi__shov += '    data = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    qcdi__shov += f'    new_index = {gqmk__lvyjj}\n'
    fqj__fbc = len(I.array_types)
    if is_overload_none(name) and I.names_typ == (types.none,) * fqj__fbc:
        yfjpb__stldk = ColNamesMetaType(tuple(range(fqj__fbc)))
    elif is_overload_none(name):
        yfjpb__stldk = ColNamesMetaType(I.names_typ)
        ...
        if fqj__fbc != len(names):
            raise_bodo_error(
                f'MultiIndex.to_frame(): expected {fqj__fbc} names, not {len(names)}'
                )
        if all(is_overload_constant_str(bcel__qnyl) or
            is_overload_constant_int(bcel__qnyl) for bcel__qnyl in names):
            yfjpb__stldk = ColNamesMetaType(names)
        else:
            raise_bodo_error(
...
def overload_index_to_numpy(I, dtype=None, copy=False, na_value=None):
    wkqk__isyl = dict(dtype=dtype, na_value=na_value)
    cwk__kmj = dict(dtype=None, na_value=None)
    check_unsupported_args('Index.to_numpy', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
...
def overload_inferred_type(I):
    ...
    if isinstance(I, StringIndexType):
        ...
        def impl(I):
            ...
            return 'string'
        return impl
    zjz__jmk = {DatetimeIndexType: 'datetime64', TimedeltaIndexType:
...
def overload_inferred_type(I):
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        return lambda I: dtype
    wihqm__vjmos = {DatetimeIndexType: np.dtype('datetime64[ns]'),
...
def init_index_from_index(I, name):
    tkjft__avcs = {NumericIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in tkjft__avcs:
        init_func = tkjft__avcs[type(I)]
...
def get_index_constructor(I):
    wwqy__iqwkz = {NumericIndexType: bodo.hiframes.pd_index_ext.
        init_numeric_index, DatetimeIndexType: bodo.hiframes.pd_index_ext.
        init_datetime_index, TimedeltaIndexType: bodo.hiframes.pd_index_ext
        .init_timedelta_index, StringIndexType: bodo.hiframes.pd_index_ext.
        init_binary_str_index, BinaryIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in wwqy__iqwkz:
        return wwqy__iqwkz[type(I)]
    raise BodoError(
...
def overload_index_min(I, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('Index.min', wkqk__isyl, cwk__kmj, package_name=
        'pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        ...
        def impl(I, axis=None, skipna=True):
            ...
            if oer__sfr == 0:
                return np.nan
            if I._step < 0:
                return I._start + I._step * (oer__sfr - 1)
            else:
                return I._start
        return impl
    if isinstance(I, CategoricalIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_min(yjlwe__sjhi)
    return impl
...
def overload_index_max(I, axis=None, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=None, skipna=True)
    check_unsupported_args('Index.max', wkqk__isyl, cwk__kmj, package_name=
        'pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_max(yjlwe__sjhi)
    return impl
...
def overload_index_argmin(I, axis=0, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmin', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmin()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = init_numeric_index(np.arange(len(yjlwe__sjhi)))
        return bodo.libs.array_ops.array_op_idxmin(yjlwe__sjhi, index)
    return impl
...
def overload_index_argmax(I, axis=0, skipna=True):
    wkqk__isyl = dict(axis=axis, skipna=skipna)
    cwk__kmj = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmax', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmax()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = np.arange(len(yjlwe__sjhi))
        return bodo.libs.array_ops.array_op_idxmax(yjlwe__sjhi, index)
    return impl
...
def overload_index_unique(I):
    tutil__fuo = get_index_constructor(I)

    def impl(I):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        cee__qbkur = bodo.libs.array_kernels.unique(yjlwe__sjhi)
        return tutil__fuo(cee__qbkur, name)
    return impl
...
def overload_index_nunique(I, dropna=True):
    ...
    def impl(I, dropna=True):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        lprcj__bzba = bodo.libs.array_kernels.nunique(yjlwe__sjhi, dropna)
        return lprcj__bzba
    return impl
...
def overload_index_isin(I, values):
    if bodo.utils.utils.is_array_typ(values):
        ...
        def impl_arr(I, values):
            wuy__rfew = bodo.utils.conversion.coerce_to_array(values)
            A = bodo.hiframes.pd_index_ext.get_index_data(I)
            lprcj__bzba = len(A)
            jwv__jtgg = np.empty(lprcj__bzba, np.bool_)
            bodo.libs.array.array_isin(jwv__jtgg, A, wuy__rfew, False)
            return jwv__jtgg
        return impl_arr
    ...
    def impl(I, values):
        A = bodo.hiframes.pd_index_ext.get_index_data(I)
        jwv__jtgg = bodo.libs.array_ops.array_op_isin(A, values)
        return jwv__jtgg
    return impl
...
def order_range(I, ascending):
    ...
    if ascending == (step > 0):
        ...
        oer__sfr = len(I)
        lcav__mazhb = start + step * (oer__sfr - 1)
        niwxl__qla = lcav__mazhb - step * oer__sfr
        return init_range_index(lcav__mazhb, niwxl__qla, -step, name)
...
def overload_index_sort_values(I, return_indexer=False, ascending=True,
    ...
    wkqk__isyl = dict(return_indexer=return_indexer, key=key)
    cwk__kmj = dict(return_indexer=False, key=None)
    check_unsupported_args('Index.sort_values', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, RangeIndexType):
        ...
        return impl
    tutil__fuo = get_index_constructor(I)
    srlde__myvax = ColNamesMetaType(('$_bodo_col_',))

    def impl(I, return_indexer=False, ascending=True, na_position='last',
        key=None):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = get_index_name(I)
        index = init_range_index(0, len(yjlwe__sjhi), 1, None)
        nnte__tgbqj = bodo.hiframes.pd_dataframe_ext.init_dataframe((
            yjlwe__sjhi,), index, srlde__myvax)
        gcvio__btj = nnte__tgbqj.sort_values(['$_bodo_col_'], ascending=
            ascending, inplace=False, na_position=na_position)
        jwv__jtgg = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(
            gcvio__btj, 0)
        return tutil__fuo(jwv__jtgg, name)
    return impl
...
def overload_index_argsort(I, axis=0, kind='quicksort', order=None):
    wkqk__isyl = dict(axis=axis, kind=kind, order=order)
    cwk__kmj = dict(axis=0, kind='quicksort', order=None)
    check_unsupported_args('Index.argsort', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, kind='quicksort', order=None):
        yjlwe__sjhi = bodo.hiframes.pd_index_ext.get_index_data(I)
        jwv__jtgg = bodo.hiframes.series_impl.argsort(yjlwe__sjhi)
        return jwv__jtgg
    return impl
...
def overload_index_where(I, cond, other=np.nan):
    ...
    if is_overload_constant_nan(other):
        ...
        exd__ujzom = 'other'
    qcdi__shov = 'def impl(I, cond, other=np.nan):\n'
    if isinstance(I, RangeIndexType):
        qcdi__shov += '  arr = np.arange(I._start, I._stop, I._step)\n'
        tutil__fuo = 'init_numeric_index'
    else:
        qcdi__shov += '  arr = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    qcdi__shov += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    qcdi__shov += (
        f'  out_arr = bodo.hiframes.series_impl.where_impl(cond, arr, {exd__ujzom})\n'
        )
    qcdi__shov += f'  return constructor(out_arr, name)\n'
    fmi__pddy = {}
    tutil__fuo = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(qcdi__shov, {'bodo': bodo, 'np': np, 'constructor': tutil__fuo},
        fmi__pddy)
    impl = fmi__pddy['impl']
    return impl
...
def overload_index_putmask(I, cond, other):
    ...
    if is_overload_constant_nan(other):
        ...
        exd__ujzom = 'other'
    qcdi__shov = 'def impl(I, cond, other):\n'
    qcdi__shov += '  cond = ~cond\n'
    if isinstance(I, RangeIndexType):
        qcdi__shov += '  arr = np.arange(I._start, I._stop, I._step)\n'
    else:
        qcdi__shov += '  arr = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
...
def overload_index_repeat(I, repeats, axis=None):
    wkqk__isyl = dict(axis=axis)
    cwk__kmj = dict(axis=None)
    check_unsupported_args('Index.repeat', wkqk__isyl, cwk__kmj,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.repeat()')
    ...
    qcdi__shov = 'def impl(I, repeats, axis=None):\n'
    ...
    qcdi__shov += '    return constructor(out_arr, name)'
    fmi__pddy = {}
    tutil__fuo = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(qcdi__shov, {'bodo': bodo, 'np': np, 'constructor': tutil__fuo},
...
def lower_constant_time_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    dtype = ty.dtype
    tavk__bgy = context.get_constant_null(types.DictType(dtype, types.int64))
    return lir.Constant.literal_struct([data, name, tavk__bgy])
...
def lower_constant_period_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    tavk__bgy = context.get_constant_null(types.DictType(types.int64, types
        .int64))
    return lir.Constant.literal_struct([data, name, tavk__bgy])
...
def lower_constant_binary_string_index(context, builder, ty, pyval):
    ...
    data = context.get_constant_generic(builder, knrd__zsn, pyval.values)
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    tavk__bgy = context.get_constant_null(types.DictType(scalar_type, types
        .int64))
    return lir.Constant.literal_struct([data, name, tavk__bgy])
...
def getiter_range_index(context, builder, sig, args):
    [apg__xbfv] = sig.args
    [index] = args
    zfwk__yhuv = context.make_helper(builder, apg__xbfv, value=index)
    noei__pchty = context.make_helper(builder, sig.return_type)
    lgeio__ocwm = cgutils.alloca_once_value(builder, zfwk__yhuv.start)
    mdvqa__hqc = context.get_constant(types.intp, 0)
    njllu__bwpwa = cgutils.alloca_once_value(builder, mdvqa__hqc)
    noei__pchty.iter = lgeio__ocwm
    noei__pchty.stop = zfwk__yhuv.stop
    noei__pchty.step = zfwk__yhuv.step
    noei__pchty.count = njllu__bwpwa
    cjqre__bocf = builder.sub(zfwk__yhuv.stop, zfwk__yhuv.start)
    gvh__dzcl = context.get_constant(types.intp, 1)
    zqzaj__cdrm = builder.icmp_signed('>', cjqre__bocf, mdvqa__hqc)
    grkhg__conxy = builder.icmp_signed('>', zfwk__yhuv.step, mdvqa__hqc)
    mvcls__objw = builder.not_(builder.xor(zqzaj__cdrm, grkhg__conxy))
    with builder.if_then(mvcls__objw):
        rowg__vsj = builder.srem(cjqre__bocf, zfwk__yhuv.step)
        rowg__vsj = builder.select(zqzaj__cdrm, rowg__vsj, builder.neg(
            rowg__vsj))
        ijarp__fqghh = builder.icmp_signed('>', rowg__vsj, mdvqa__hqc)
        zxkdz__tuiiv = builder.add(builder.sdiv(cjqre__bocf, zfwk__yhuv.
            step), builder.select(ijarp__fqghh, gvh__dzcl, mdvqa__hqc))
        builder.store(zxkdz__tuiiv, njllu__bwpwa)
    tzvn__jaqrw = noei__pchty._getvalue()
    ypvul__japjl = impl_ret_new_ref(context, builder, sig.return_type,
        tzvn__jaqrw)
    return ypvul__japjl
...
def _install_index_unsupported():
    for lisq__mzz in index_unsupported_methods:
        for ydh__ojk, typ in index_types:
            overload_method(typ, lisq__mzz, no_unliteral=True)(
                create_unsupported_overload(ydh__ojk.format(lisq__mzz + '()')))
    for xmdn__mmce in index_unsupported_atrs:
        for ydh__ojk, typ in index_types:
            overload_attribute(typ, xmdn__mmce, no_unliteral=True)(
                create_unsupported_overload(ydh__ojk.format(xmdn__mmce)))
    chfmb__hgnev = [(StringIndexType, string_index_unsupported_atrs), (
    ...
    sof__bsc = [(CategoricalIndexType, cat_idx_unsupported_methods), (
    ...
    for typ, kxzt__khlwq in sof__bsc:
        ydh__ojk = idx_typ_to_format_str_map[typ]
        for cxlb__ejw in kxzt__khlwq:
            overload_method(typ, cxlb__ejw, no_unliteral=True)(
                create_unsupported_overload(ydh__ojk.format(cxlb__ejw + '()')))
    for typ, jll__iviqi in chfmb__hgnev:
        ydh__ojk = idx_typ_to_format_str_map[typ]
        for xmdn__mmce in jll__iviqi:
            overload_attribute(typ, xmdn__mmce, no_unliteral=True)(
                create_unsupported_overload(ydh__ojk.format(xmdn__mmce)))
