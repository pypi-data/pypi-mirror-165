...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        mtggp__gcz = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(mtggp__gcz)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        edpey__sis = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, edpey__sis)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jja__gae, = args
        hqay__tdje = signature.return_type
        bil__bxj = cgutils.create_struct_proxy(hqay__tdje)(context, builder)
        bil__bxj.obj = jja__gae
        context.nrt.incref(builder, signature.args[0], jja__gae)
        return bil__bxj._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ezuv__lbio = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        mtggp__gcz = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(mtggp__gcz)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        edpey__sis = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, edpey__sis)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jja__gae, = args
        ktshi__yknx = signature.return_type
        vlt__kwte = cgutils.create_struct_proxy(ktshi__yknx)(context, builder)
        vlt__kwte.obj = jja__gae
        context.nrt.incref(builder, signature.args[0], jja__gae)
        return vlt__kwte._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                zragw__leyc = bodo.utils.conversion.coerce_to_ndarray(idx)
                fsppl__uvvpd = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    zragw__leyc]
                fduu__jkxs = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    zragw__leyc]
                mtggp__gcz = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(fsppl__uvvpd,
                    fduu__jkxs, mtggp__gcz)
            return impl
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        zragw__leyc = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ezuv__lbio = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ezuv__lbio))
                        zragw__leyc = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            zragw__leyc] = s
            ...
            def impl_arr(I, idx, val):
                zragw__leyc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[zragw__leyc
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        mtggp__gcz = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(mtggp__gcz)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        edpey__sis = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, edpey__sis)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        jja__gae, = args
        dyr__begn = signature.return_type
        eayh__kaw = cgutils.create_struct_proxy(dyr__begn)(context, builder)
        eayh__kaw.obj = jja__gae
        context.nrt.incref(builder, signature.args[0], jja__gae)
        return eayh__kaw._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            fsppl__uvvpd = bodo.hiframes.pd_series_ext.get_series_data(S)
            fduu__jkxs = bodo.hiframes.pd_series_ext.get_series_index(S)
            wrd__ajvi = (idx - fduu__jkxs._start) // fduu__jkxs._step
            return fsppl__uvvpd[wrd__ajvi]
    ...
    if isinstance(S, SeriesType) and not (isinstance(S.index,
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    fsppl__uvvpd = bodo.hiframes.pd_series_ext.get_series_data(
                        S)
                    I = bodo.hiframes.pd_series_ext.get_series_index(S)
                    zragw__leyc = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(fsppl__uvvpd[
                        zragw__leyc])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    bfewq__qua = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    lkqnt__oios = bfewq__qua.get_loc(idx)
                    if lkqnt__oios is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(lkqnt__oios)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                zragw__leyc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[zragw__leyc
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        eag__cqrp = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            mqwtb__tpcuq, int) for mqwtb__tpcuq in eag__cqrp):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            zan__njzwa = get_literal_value(idx)
            if zan__njzwa not in eag__cqrp:
                raise_bodo_error(
                    f"Series label-based getitem: '{zan__njzwa}' not in {eag__cqrp}"
                    )
            arr_ind = eag__cqrp.index(zan__njzwa)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    bil__bxj = cgutils.create_struct_proxy(fromty)(context, builder, val)
    rucnz__ago = context.cast(builder, bil__bxj.obj, fromty.stype, toty.stype)
    vtzo__xea = cgutils.create_struct_proxy(toty)(context, builder)
    vtzo__xea.obj = rucnz__ago
    return vtzo__xea._getvalue()
