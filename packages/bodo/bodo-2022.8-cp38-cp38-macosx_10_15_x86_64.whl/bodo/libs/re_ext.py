...
def box_regex_flag(typ, val, c):
    sydi__nhjlo = c.box(types.int64, val)
    uvj__inbi = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(uvj__inbi, (sydi__nhjlo,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    sydi__nhjlo = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, sydi__nhjlo)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    pps__fxrav = context.get_python_api(builder)
    vwt__lpot = builder.icmp_signed('==', val, pps__fxrav.borrow_none())
    with builder.if_then(vwt__lpot):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    iwqoz__ggw = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(iwqoz__ggw)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        lppss__qra = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if lppss__qra > 1:
            typ = types.List(types.Tuple([string_type] * lppss__qra))
...
def overload_match_group(m, *args):
    ...
    lwhp__kfvv = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ivva__gpmbk = args[0]
            with numba.objmode(out=lwhp__kfvv):
                out = m.group(ivva__gpmbk)
            return out
        return _match_group_impl_one
    ixtx__kzcwm = 'tuple_str_{}'.format(len(args))
    setattr(types, ixtx__kzcwm, types.Tuple([lwhp__kfvv] * len(args)))
    dedo__ymk = ', '.join('group{}'.format(sgpkk__socn + 1) for sgpkk__socn in
        range(len(args)))
    upsm__mor = 'def _match_group_impl(m, *args):\n'
    upsm__mor += '  ({}) = args\n'.format(dedo__ymk)
    upsm__mor += "  with numba.objmode(out='{}'):\n".format(ixtx__kzcwm)
    upsm__mor += '    out = m.group({})\n'.format(dedo__ymk)
    upsm__mor += '  return out\n'
    ojn__qycle = {}
    exec(upsm__mor, globals(), ojn__qycle)
    xczm__nrf = ojn__qycle['_match_group_impl']
    return xczm__nrf
...
def overload_match_groups(m, default=None):
    mwli__gddot = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=mwli__gddot):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(tsw__qoxe is None for tsw__qoxe in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=lwhp__kfvv):
