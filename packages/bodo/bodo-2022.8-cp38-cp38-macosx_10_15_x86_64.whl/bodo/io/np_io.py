...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        lkwfy__vjemx = context.get_abi_sizeof(context.get_data_type(dtype.
            dtype))
        return context.get_constant(types.intp, lkwfy__vjemx)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            mpy__aix = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mpy__aix.dtype)
            file_write(unicode_to_utf8(fname), mpy__aix.ctypes, dtype_size *
                mpy__aix.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            mpy__aix = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mpy__aix.dtype)
            gei__bidb = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                mpy__aix.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), mpy__aix.ctypes,
                start, count, gei__bidb)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            aoep__phebl = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                aoep__phebl = min(aoep__phebl, count * dtype_size)
            if aoep__phebl < 0:
                return -1
            return aoep__phebl
        return impl
