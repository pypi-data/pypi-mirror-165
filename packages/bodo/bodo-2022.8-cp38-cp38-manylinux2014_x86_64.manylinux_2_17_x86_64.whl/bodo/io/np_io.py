...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        dqrzx__lhmru = context.get_abi_sizeof(context.get_data_type(dtype.
            dtype))
        return context.get_constant(types.intp, dqrzx__lhmru)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            jmpnd__mhc = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(jmpnd__mhc.dtype)
            file_write(unicode_to_utf8(fname), jmpnd__mhc.ctypes, 
                dtype_size * jmpnd__mhc.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            jmpnd__mhc = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(jmpnd__mhc.dtype)
            rzt__hvb = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                jmpnd__mhc.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), jmpnd__mhc.ctypes,
                start, count, rzt__hvb)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            zkm__oify = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                zkm__oify = min(zkm__oify, count * dtype_size)
            if zkm__oify < 0:
                return -1
            return zkm__oify
        return impl
