...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        xvhjz__nalb = []
        auahi__pancf = h5g_get_num_objs(obj_id)
        for goxcz__adrma in range(auahi__pancf):
            xsy__ztvd = h5g_get_objname_by_idx(obj_id, goxcz__adrma)
            xvhjz__nalb.append(xsy__ztvd)
        return xvhjz__nalb
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    sey__gya = parse_dtype(dtype)
    tfged__ohcbf = np.int32(numba_to_c_type(sey__gya))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, tfged__ohcbf)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            ffob__iwj = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    dzwf__vkp = tuple_to_ptr(starts)
    dzat__tkosa = tuple_to_ptr(counts)
    pjhiy__xdxaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, dzwf__vkp, dzat__tkosa,
        is_parallel, out_arr.ctypes, pjhiy__xdxaq)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    dzat__tkosa = tuple_to_ptr(counts)
    pjhiy__xdxaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, dzwf__vkp, dzat__tkosa,
        is_parallel, out_arr.ctypes, pjhiy__xdxaq)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        jilbt__fkyjb = types.Array(dtype, ndim, 'C')
        wde__rns = args[3]
        jilbt__fkyjb = self.context.resolve_function_type(operator.getitem,
            [jilbt__fkyjb, wde__rns], {}).return_type.copy(layout='C')
        return signature(jilbt__fkyjb, *args)
...
def get_filter_read_indices(bool_arr):
    gav__kcdce = bool_arr.nonzero()[0]
    imkg__bhu = bodo.libs.distributed_api.get_rank()
    nrm__hvw = bodo.libs.distributed_api.get_size()
    wcozs__kfs = np.empty(nrm__hvw, np.int64)
    dikwy__fjyzt = len(bool_arr)
    bodo.libs.distributed_api.allgather(wcozs__kfs, dikwy__fjyzt)
    ucsfa__ntb = wcozs__kfs.cumsum()[imkg__bhu] - dikwy__fjyzt
    gav__kcdce += ucsfa__ntb
    pydy__pqk = bodo.libs.distributed_api.dist_reduce(len(gav__kcdce), np.
        int32(sum_op))
    dtqak__uiv = bodo.libs.distributed_api.gatherv(gav__kcdce)
    if imkg__bhu == 0:
        brc__zjoc = dtqak__uiv
    else:
        brc__zjoc = np.empty(pydy__pqk, gav__kcdce.dtype)
    bodo.libs.distributed_api.bcast(brc__zjoc)
    lxs__wtpg = bodo.libs.distributed_api.get_start(pydy__pqk, nrm__hvw,
        imkg__bhu)
    gqnu__aineo = bodo.libs.distributed_api.get_end(pydy__pqk, nrm__hvw,
        imkg__bhu)
    return brc__zjoc[lxs__wtpg:gqnu__aineo]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        ueamu__cbqa = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], ueamu__cbqa)
        return builder.bitcast(ueamu__cbqa, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    dzat__tkosa = tuple_to_ptr(counts)
    pjhiy__xdxaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, dzwf__vkp,
        dzat__tkosa, is_parallel, out_arr.ctypes, pjhiy__xdxaq,
