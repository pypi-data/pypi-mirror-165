...
def get_new_null_mask_bool_index(old_mask, ind, n):
    wla__njtxg = n + 7 >> 3
    ejia__bst = np.empty(wla__njtxg, np.uint8)
    yni__vchoc = 0
    for wqjt__gwan in range(len(ind)):
        if ind[wqjt__gwan]:
            rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                wqjt__gwan)
            bodo.libs.int_arr_ext.set_bit_to_arr(ejia__bst, yni__vchoc,
                rrz__oey)
            yni__vchoc += 1
    return ejia__bst
...
def array_getitem_bool_index(A, ind):
    ...
    kvlgr__rlk = A._data[ind]
    n = len(kvlgr__rlk)
    ejia__bst = get_new_null_mask_bool_index(old_mask, ind, n)
    return kvlgr__rlk, ejia__bst
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    yni__vchoc = 0
    for wqjt__gwan in range(len(ind)):
        rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            wqjt__gwan])
        bodo.libs.int_arr_ext.set_bit_to_arr(ejia__bst, yni__vchoc, rrz__oey)
        yni__vchoc += 1
    return ejia__bst
...
def array_getitem_int_index(A, ind):
    ame__fbe = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    kvlgr__rlk = A._data[ame__fbe]
    n = len(kvlgr__rlk)
    ejia__bst = get_new_null_mask_int_index(old_mask, ame__fbe, n)
    return kvlgr__rlk, ejia__bst


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    ylxg__vdyun = numba.cpython.unicode._normalize_slice(ind, n)
    myh__ivihi = numba.cpython.unicode._slice_span(ylxg__vdyun)
    wla__njtxg = myh__ivihi + 7 >> 3
    ejia__bst = np.empty(wla__njtxg, np.uint8)
    yni__vchoc = 0
    for wqjt__gwan in range(ylxg__vdyun.start, ylxg__vdyun.stop,
        ylxg__vdyun.step):
        rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            wqjt__gwan)
        bodo.libs.int_arr_ext.set_bit_to_arr(ejia__bst, yni__vchoc, rrz__oey)
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    kvlgr__rlk = np.ascontiguousarray(A._data[ind])
    ejia__bst = get_new_null_mask_slice_index(old_mask, ind, n)
    return kvlgr__rlk, ejia__bst
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for wqjt__gwan in range(n):
                A._data[idx[wqjt__gwan]] = val._data[wqjt__gwan]
                rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, wqjt__gwan)
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            for wqjt__gwan in idx:
                A._data[wqjt__gwan] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            oxhwz__gpg = 0
            for wqjt__gwan in range(n):
                if not bodo.libs.array_kernels.isna(idx, wqjt__gwan) and idx[
                    wqjt__gwan]:
                    A._data[wqjt__gwan] = val._data[oxhwz__gpg]
                    rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, oxhwz__gpg)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        wqjt__gwan, rrz__oey)
                    oxhwz__gpg += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    ylxg__vdyun = numba.cpython.unicode._normalize_slice(idx, n)
    oxhwz__gpg = 0
    for wqjt__gwan in range(ylxg__vdyun.start, ylxg__vdyun.stop,
        ylxg__vdyun.step):
        rrz__oey = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            oxhwz__gpg)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, wqjt__gwan, rrz__oey)
        oxhwz__gpg += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            ylxg__vdyun = numba.cpython.unicode._normalize_slice(idx, len(A))
            for wqjt__gwan in range(ylxg__vdyun.start, ylxg__vdyun.stop,
                ylxg__vdyun.step):
                A._data[wqjt__gwan] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for wqjt__gwan in idx:
                    bodo.libs.array_kernels.setna(A, wqjt__gwan)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    rxjb__nztrr = bodo.libs.str_arr_ext.pre_alloc_string_array(
                        n, -1)
                    for wqjt__gwan in numba.parfors.parfor.internal_prange(n):
                        if idx[wqjt__gwan] or bodo.libs.array_kernels.isna(A,
                            wqjt__gwan):
                            rxjb__nztrr[wqjt__gwan] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(rxjb__nztrr,
                                wqjt__gwan)
                        else:
                            rxjb__nztrr[wqjt__gwan] = A[wqjt__gwan]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for wqjt__gwan in range(n):
                    if not bodo.libs.array_kernels.isna(idx, wqjt__gwan
                        ) and idx[wqjt__gwan]:
                        bodo.libs.array_kernels.setna(A, wqjt__gwan)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for wqjt__gwan in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, wqjt__gwan)
                        continue
                    A[wqjt__gwan] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        ryfrb__ihzu = context.make_helper(builder, val_t, args[0])
        zqrhu__wgg = ryfrb__ihzu.data
        context.nrt.incref(builder, val_t.type, zqrhu__wgg)
        return zqrhu__wgg
