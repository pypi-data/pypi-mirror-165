...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        uciq__rwtjh = self.overloads[sig]
        tria__tpqcw = uciq__rwtjh.metadata.get('distributed_diagnostics', None)
        if tria__tpqcw is None:
            kss__uribu = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(kss__uribu)
        tria__tpqcw.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        xeda__grk = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(xeda__grk)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        msahx__erwvy = options.pop('distributed')
        pipeline_class = (pipeline_class if msahx__erwvy else bodo.compiler
            .BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        lybi__aozni = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if lybi__aozni else
            pipeline_class)
    sob__unrv = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(sob__unrv, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(sob__unrv)
        else:
            return master_mode_wrapper(sob__unrv)
    else:
        return sob__unrv
