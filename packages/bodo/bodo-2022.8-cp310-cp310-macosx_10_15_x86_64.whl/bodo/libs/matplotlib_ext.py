...
def _install_mpl_types():
    vyey__vdqh = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for qlsuw__lhzs, rpwlh__llhkt in vyey__vdqh:
        install_py_obj_class(types_name=qlsuw__lhzs, python_type=
            rpwlh__llhkt, module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    xltt__osit = ', '.join(f'e{jxgu__abhkm}' for jxgu__abhkm in range(len(
        args)))
    if xltt__osit:
        xltt__osit += ', '
    mrfd__dqa = ', '.join(f"{sqoyf__aco} = ''" for sqoyf__aco in kws.keys())
    xuie__jqb = 'matplotlib_obj, ' if obj_typ is not None else ''
    pgxs__ony = f'def mpl_stub({xuie__jqb} {xltt__osit} {mrfd__dqa}):\n'
    pgxs__ony += '    pass\n'
    nfv__cdrvb = {}
    exec(pgxs__ony, {}, nfv__cdrvb)
    sevbr__lpo = nfv__cdrvb['mpl_stub']
    tzpyk__owue = numba.core.utils.pysignature(sevbr__lpo)
    sano__lmo = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(sano__lmo)).replace(pysig=
        tzpyk__owue)


def generate_axes_typing(mod_name, nrows, ncols):
    uti__hcx = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(uti__hcx.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(uti__hcx.format(mod_name, 'ncols'))
    euxew__labhs = get_overload_const_int(nrows)
    eqvw__oxs = get_overload_const_int(ncols)
    if euxew__labhs < 1:
        raise BodoError(uti__hcx.format(mod_name, 'nrows'))
    if eqvw__oxs < 1:
        raise BodoError(uti__hcx.format(mod_name, 'ncols'))
    if euxew__labhs == 1 and eqvw__oxs == 1:
        gvav__bwh = types.mpl_axes_type
    else:
        if eqvw__oxs == 1:
            cjstr__umyf = types.mpl_axes_type
        else:
            cjstr__umyf = types.Tuple([types.mpl_axes_type] * eqvw__oxs)
        gvav__bwh = types.Tuple([cjstr__umyf] * euxew__labhs)
    return gvav__bwh


def generate_pie_return_type(args, kws):
    wfl__skp = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if wfl__skp == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    ueh__zqbsl = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(ueh__zqbsl):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(ueh__zqbsl):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        xibbw__prt = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, xibbw__prt]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    xibbw__prt = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    qlsuw__lhzs = str(xibbw__prt)
    if not hasattr(types, qlsuw__lhzs):
        qlsuw__lhzs = f'objmode_type{ir_utils.next_label()}'
        setattr(types, qlsuw__lhzs, xibbw__prt)
    pgxs__ony = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{qlsuw__lhzs}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    nfv__cdrvb = {}
    exec(pgxs__ony, {'numba': numba, 'np': np}, nfv__cdrvb)
    impl = nfv__cdrvb['impl']
    return impl
