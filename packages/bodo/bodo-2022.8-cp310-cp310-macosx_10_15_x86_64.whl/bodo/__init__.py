...
def _global_except_hook(exctype, value, traceback):
    ...
    fhk__xjsaq = MPI.COMM_WORLD
    iuyv__grt = fhk__xjsaq.Get_rank()
    xti__eejjs = 3.0
    lwah__flzx = True
    qtyqd__udjs = fhk__xjsaq.Ibarrier()
    gcq__hqk = time.time()
    while time.time() - gcq__hqk < xti__eejjs:
        time.sleep(0.1)
        if qtyqd__udjs.Test():
            lwah__flzx = False
            break
    try:
        ...
        if lwah__flzx:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {iuyv__grt}. \n')
...
def set_numba_environ_vars():
    if (cvobg__yevy := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = cvobg__yevy
        os.environ['NUMBA_CACHE_DIR'] = cvobg__yevy
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    gcujs__hhjts = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(gcujs__hhjts)
