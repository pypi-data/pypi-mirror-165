...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        uhgfh__vbfq = []
        snvs__izqye = h5g_get_num_objs(obj_id)
        for xgxr__bmet in range(snvs__izqye):
            vlfha__osqu = h5g_get_objname_by_idx(obj_id, xgxr__bmet)
            uhgfh__vbfq.append(vlfha__osqu)
        return uhgfh__vbfq
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    kbjw__asg = parse_dtype(dtype)
    xlld__ntj = np.int32(numba_to_c_type(kbjw__asg))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            wftn__rfebe = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    vgttt__merqp = tuple_to_ptr(starts)
    jkq__eoc = tuple_to_ptr(counts)
    bdn__ena = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, vgttt__merqp, jkq__eoc,
        is_parallel, out_arr.ctypes, bdn__ena)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    jkq__eoc = tuple_to_ptr(counts)
    bdn__ena = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, vgttt__merqp, jkq__eoc,
        is_parallel, out_arr.ctypes, bdn__ena)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        afhu__emvy = types.Array(dtype, ndim, 'C')
        wtlg__fxr = args[3]
        afhu__emvy = self.context.resolve_function_type(operator.getitem, [
            afhu__emvy, wtlg__fxr], {}).return_type.copy(layout='C')
        return signature(afhu__emvy, *args)
...
def get_filter_read_indices(bool_arr):
    sonr__qap = bool_arr.nonzero()[0]
    ush__xurqr = bodo.libs.distributed_api.get_rank()
    cpaaf__bajl = bodo.libs.distributed_api.get_size()
    wcx__mooc = np.empty(cpaaf__bajl, np.int64)
    jwgjb__djoz = len(bool_arr)
    bodo.libs.distributed_api.allgather(wcx__mooc, jwgjb__djoz)
    rajmy__fey = wcx__mooc.cumsum()[ush__xurqr] - jwgjb__djoz
    sonr__qap += rajmy__fey
    qtu__cvvlv = bodo.libs.distributed_api.dist_reduce(len(sonr__qap), np.
        int32(sum_op))
    lzhkr__xtihw = bodo.libs.distributed_api.gatherv(sonr__qap)
    if ush__xurqr == 0:
        lcced__necre = lzhkr__xtihw
    else:
        lcced__necre = np.empty(qtu__cvvlv, sonr__qap.dtype)
    bodo.libs.distributed_api.bcast(lcced__necre)
    cdrz__jmql = bodo.libs.distributed_api.get_start(qtu__cvvlv,
        cpaaf__bajl, ush__xurqr)
    bwa__hwg = bodo.libs.distributed_api.get_end(qtu__cvvlv, cpaaf__bajl,
        ush__xurqr)
    return lcced__necre[cdrz__jmql:bwa__hwg]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        say__owvu = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], say__owvu)
        return builder.bitcast(say__owvu, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    jkq__eoc = tuple_to_ptr(counts)
    bdn__ena = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, vgttt__merqp,
        jkq__eoc, is_parallel, out_arr.ctypes, bdn__ena, read_indices.
