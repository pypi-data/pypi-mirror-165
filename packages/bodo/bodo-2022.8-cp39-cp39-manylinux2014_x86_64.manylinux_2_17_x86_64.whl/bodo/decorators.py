...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        lhxbb__yyo = self.overloads[sig]
        ngq__amtqk = lhxbb__yyo.metadata.get('distributed_diagnostics', None)
        if ngq__amtqk is None:
            klman__smixq = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(klman__smixq)
        ngq__amtqk.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        xiurt__njmos = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(xiurt__njmos)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        qlpu__jiqks = options.pop('distributed')
        pipeline_class = (pipeline_class if qlpu__jiqks else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        eiks__xse = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if eiks__xse else
            pipeline_class)
    hzli__vzycx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(hzli__vzycx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(hzli__vzycx)
        else:
            return master_mode_wrapper(hzli__vzycx)
    else:
        return hzli__vzycx
