...
def unicode_to_utf8_and_len(typingctx, str_typ):
    ...
    dzj__tpa = types.Tuple([utf8_str_type, types.int64])

    def codegen(context, builder, sig, args):
        jnjm__inwh, = args
        cii__mqo = cgutils.create_struct_proxy(string_type)(context,
            builder, value=jnjm__inwh)
        xaxko__mdcp = cgutils.create_struct_proxy(utf8_str_type)(context,
            builder)
        kixrw__hgnc = cgutils.create_struct_proxy(dzj__tpa)(context, builder)
        is_ascii = builder.icmp_unsigned('==', cii__mqo.is_ascii, lir.
            Constant(cii__mqo.is_ascii.type, 1))
        with builder.if_else(is_ascii) as (zyinr__mfldc, gmklv__hifj):
            with zyinr__mfldc:
                context.nrt.incref(builder, string_type, jnjm__inwh)
                xaxko__mdcp.data = cii__mqo.data
                xaxko__mdcp.meminfo = cii__mqo.meminfo
                kixrw__hgnc.f1 = cii__mqo.length
            with gmklv__hifj:
                pjmpm__cbwfv = lir.FunctionType(lir.IntType(64), [lir.
                    IntType(8).as_pointer(), lir.IntType(8).as_pointer(),
                    lir.IntType(64), lir.IntType(32)])
                bkeh__vzgy = cgutils.get_or_insert_function(builder.module,
                    pjmpm__cbwfv, name='unicode_to_utf8')
                mdoq__sryb = context.get_constant_null(types.voidptr)
                cjcu__fvv = builder.call(bkeh__vzgy, [mdoq__sryb, cii__mqo.
                    data, cii__mqo.length, cii__mqo.kind])
                kixrw__hgnc.f1 = cjcu__fvv
                vqu__ecze = builder.add(cjcu__fvv, lir.Constant(lir.IntType
                    (64), 1))
                xaxko__mdcp.meminfo = context.nrt.meminfo_alloc_aligned(builder
                    , size=vqu__ecze, align=32)
                xaxko__mdcp.data = context.nrt.meminfo_data(builder,
                    xaxko__mdcp.meminfo)
                builder.call(bkeh__vzgy, [xaxko__mdcp.data, cii__mqo.data,
                    cii__mqo.length, cii__mqo.kind])
                builder.store(lir.Constant(lir.IntType(8), 0), builder.gep(
                    xaxko__mdcp.data, [cjcu__fvv]))
        kixrw__hgnc.f0 = xaxko__mdcp._getvalue()
        return kixrw__hgnc._getvalue()
    return dzj__tpa(string_type), codegen
...
def memcmp(typingctx, dest_t, src_t, count_t=None):
    ...
    def codegen(context, builder, sig, args):
        pjmpm__cbwfv = lir.FunctionType(lir.IntType(32), [lir.IntType(8).
            as_pointer(), lir.IntType(8).as_pointer(), lir.IntType(64)])
        djlld__unpky = cgutils.get_or_insert_function(builder.module,
            pjmpm__cbwfv, name='memcmp')
        return builder.call(djlld__unpky, args)
...
class StrToFloat(AbstractTemplate):
    ...
    def generic(self, args, kws):
        assert not kws
        [vhn__exl] = args
        if isinstance(vhn__exl, StdStringType):
            return signature(types.float64, vhn__exl)
        if vhn__exl == string_type:
            return signature(types.float64, vhn__exl)
...
def gen_unicode_to_std_str(context, builder, unicode_val):
    cii__mqo = cgutils.create_struct_proxy(string_type)(context, builder,
        value=unicode_val)
    pjmpm__cbwfv = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
        IntType(8).as_pointer(), lir.IntType(64)])
    qsun__dajqx = cgutils.get_or_insert_function(builder.module,
        pjmpm__cbwfv, name='init_string_const')
    return builder.call(qsun__dajqx, [cii__mqo.data, cii__mqo.length])
...
def gen_std_str_to_unicode(context, builder, std_str_val, del_str=False):
    ...
    def _std_str_to_unicode(std_str):
        ...
        yds__wht = numba.cpython.unicode._empty_string(kind, length, 1)
        bodo.libs.str_arr_ext._memcpy(yds__wht._data, bodo.libs.str_ext.
...
class RandomAccessStringArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dbzo__ksbsz = [('data', types.List(string_type))]
        models.StructModel.__init__(self, dmm, fe_type, dbzo__ksbsz)
...
def alloc_random_access_string_array(typingctx, n_t=None):
    ...
    def codegen(context, builder, sig, args):
        pxn__rluo, = args
        zawmu__ovzsj = types.List(string_type)
        cehbs__cvtjz = numba.cpython.listobj.ListInstance.allocate(context,
            builder, zawmu__ovzsj, pxn__rluo)
        cehbs__cvtjz.size = pxn__rluo
        afv__ltamk = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        afv__ltamk.data = cehbs__cvtjz.value
        return afv__ltamk._getvalue()
    ...
    if isinstance(v, types.Float):
        ...
        def impl(v):
            ...
            if owv__nqy < 1:
                ...
                rereb__aqk = 1 + int(np.floor(np.log10(owv__nqy)))
            length = tfjis__reu + rereb__aqk + 1 + 6
...
def cast_str_to_float64(context, builder, fromty, toty, val):
    pjmpm__cbwfv = lir.FunctionType(lir.DoubleType(), [lir.IntType(8).
        as_pointer()])
    qsun__dajqx = cgutils.get_or_insert_function(builder.module,
        pjmpm__cbwfv, name='str_to_float64')
    res = builder.call(qsun__dajqx, (val,))
...
def cast_str_to_float32(context, builder, fromty, toty, val):
    pjmpm__cbwfv = lir.FunctionType(lir.FloatType(), [lir.IntType(8).
        as_pointer()])
    qsun__dajqx = cgutils.get_or_insert_function(builder.module,
        pjmpm__cbwfv, name='str_to_float32')
    res = builder.call(qsun__dajqx, (val,))
    bodo.utils.utils.inlined_check_and_propagate_cpp_exception(context, builder
        )
    return res
...
def cast_unicode_str_to_int64(context, builder, fromty, toty, val):
    cii__mqo = cgutils.create_struct_proxy(string_type)(context, builder,
        value=val)
    pjmpm__cbwfv = lir.FunctionType(lir.IntType(toty.bitwidth), [lir.
        IntType(8).as_pointer(), lir.IntType(64)])
    qsun__dajqx = cgutils.get_or_insert_function(builder.module,
        pjmpm__cbwfv, name='str_to_int64')
    res = builder.call(qsun__dajqx, (cii__mqo.data, cii__mqo.length))
    bodo.utils.utils.inlined_check_and_propagate_cpp_exception(context, builder
        )
    return res
...
def cast_unicode_str_to_uint64(context, builder, fromty, toty, val):
    ...
    pjmpm__cbwfv = lir.FunctionType(lir.IntType(toty.bitwidth), [lir.
        IntType(8).as_pointer(), lir.IntType(64)])
    qsun__dajqx = cgutils.get_or_insert_function(builder.module,
        pjmpm__cbwfv, name='str_to_uint64')
    res = builder.call(qsun__dajqx, (cii__mqo.data, cii__mqo.length))
    bodo.utils.utils.inlined_check_and_propagate_cpp_exception(context, builder
        )
    return res
...
class StringAttribute(AttributeTemplate):
    ...
    def resolve_format(self, string_typ, args, kws):
        kws = dict(kws)
        zgvqk__bbym = ', '.join('e{}'.format(sywgs__htcu) for sywgs__htcu in
            range(len(args)))
        if zgvqk__bbym:
            zgvqk__bbym += ', '
        iyjm__slb = ', '.join("{} = ''".format(a) for a in kws.keys())
        sdw__kkesz = f'def format_stub(string, {zgvqk__bbym} {iyjm__slb}):\n'
        sdw__kkesz += '    pass\n'
        dhmt__ewqa = {}
        exec(sdw__kkesz, {}, dhmt__ewqa)
        qujwy__wovkz = dhmt__ewqa['format_stub']
        owr__qoi = numba.core.utils.pysignature(qujwy__wovkz)
        mfw__dvr = (string_typ,) + args + tuple(kws.values())
        return signature(string_typ, mfw__dvr).replace(pysig=owr__qoi)


@numba.njit(cache=True)
def str_split(arr, pat, n):
    kcp__wkxz = pat is not None and len(pat) > 1
    if kcp__wkxz:
        chae__nkq = re.compile(pat)
        if n == -1:
            n = 0
    elif n == 0:
        n = -1
    cehbs__cvtjz = len(arr)
    eiwp__ipdyf = 0
    mpe__dwmhm = 0
    for sywgs__htcu in numba.parfors.parfor.internal_prange(cehbs__cvtjz):
        if bodo.libs.array_kernels.isna(arr, sywgs__htcu):
            continue
        if kcp__wkxz:
            zdpt__lmc = chae__nkq.split(arr[sywgs__htcu], maxsplit=n)
        elif pat == '':
            zdpt__lmc = [''] + list(arr[sywgs__htcu]) + ['']
        else:
            zdpt__lmc = arr[sywgs__htcu].split(pat, n)
        eiwp__ipdyf += len(zdpt__lmc)
        for s in zdpt__lmc:
            mpe__dwmhm += bodo.libs.str_arr_ext.get_utf8_size(s)
    nzo__vgof = bodo.libs.array_item_arr_ext.pre_alloc_array_item_array(
        cehbs__cvtjz, (eiwp__ipdyf, mpe__dwmhm), bodo.libs.str_arr_ext.
        string_array_type)
    edax__oqq = bodo.libs.array_item_arr_ext.get_offsets(nzo__vgof)
    zuj__uml = bodo.libs.array_item_arr_ext.get_null_bitmap(nzo__vgof)
    qpehf__qkka = bodo.libs.array_item_arr_ext.get_data(nzo__vgof)
    hcr__vrjsi = 0
    for qqoh__veb in numba.parfors.parfor.internal_prange(cehbs__cvtjz):
        edax__oqq[qqoh__veb] = hcr__vrjsi
        if bodo.libs.array_kernels.isna(arr, qqoh__veb):
            bodo.libs.int_arr_ext.set_bit_to_arr(zuj__uml, qqoh__veb, 0)
            continue
        bodo.libs.int_arr_ext.set_bit_to_arr(zuj__uml, qqoh__veb, 1)
        if kcp__wkxz:
            zdpt__lmc = chae__nkq.split(arr[qqoh__veb], maxsplit=n)
        elif pat == '':
            zdpt__lmc = [''] + list(arr[qqoh__veb]) + ['']
        else:
            zdpt__lmc = arr[qqoh__veb].split(pat, n)
        acxi__fts = len(zdpt__lmc)
        for qyz__plvbe in range(acxi__fts):
            s = zdpt__lmc[qyz__plvbe]
            qpehf__qkka[hcr__vrjsi] = s
            hcr__vrjsi += 1
    edax__oqq[cehbs__cvtjz] = hcr__vrjsi
    return nzo__vgof
    ...
    if isinstance(x, types.Integer):
        ...
        def impl(x):
            ...
            if x == 0:
                ...
                mjkrd__zlj = fast_ceil_log2(x + 1)
                mjkrd__zlj = (mjkrd__zlj + 3) // 4
            length = len(tunyx__phqy) + mjkrd__zlj
            output = numba.cpython.unicode._empty_string(kind, length, 1)
            bodo.libs.str_arr_ext._memcpy(output._data, tunyx__phqy._data,
                len(tunyx__phqy), 1)
            int_to_hex(output, mjkrd__zlj, len(tunyx__phqy), x)
            return output
        return impl
...
def fast_ceil_log2(x):
    fzco__goy = 0 if x & x - 1 == 0 else 1
    snypf__iqra = [np.uint64(18446744069414584320), np.uint64(4294901760),
        np.uint64(65280), np.uint64(240), np.uint64(12), np.uint64(2)]
    prvhc__tnbs = 32
    for sywgs__htcu in range(len(snypf__iqra)):
        edzyl__hjq = 0 if x & snypf__iqra[sywgs__htcu] == 0 else prvhc__tnbs
        fzco__goy = fzco__goy + edzyl__hjq
        x = x >> edzyl__hjq
        prvhc__tnbs = prvhc__tnbs >> 1
    return fzco__goy
...
def int_to_hex(typingctx, output, out_len, header_len, int_val):
    ...
    def codegen(context, builder, sig, args):
        ...
        rjaph__lej = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=output)
        pjmpm__cbwfv = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(64), lir.IntType(64)])
        tbfew__sjt = cgutils.get_or_insert_function(builder.module,
            pjmpm__cbwfv, name='int_to_hex')
        neue__pydl = builder.inttoptr(builder.add(builder.ptrtoint(
            rjaph__lej.data, lir.IntType(64)), header_len), lir.IntType(8).
            as_pointer())
        builder.call(tbfew__sjt, (neue__pydl, out_len, int_val))
