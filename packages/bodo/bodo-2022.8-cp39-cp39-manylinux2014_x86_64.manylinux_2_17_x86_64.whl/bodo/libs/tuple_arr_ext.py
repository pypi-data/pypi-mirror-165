...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(aknga__bivj.dtype for
            aknga__bivj in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qiqv__jddxq = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, qiqv__jddxq)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    sthpm__wazab = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        gmm__qxcbp, = args
        sdmi__iwj = context.make_helper(builder, sthpm__wazab)
        sdmi__iwj.data = gmm__qxcbp
        context.nrt.incref(builder, data_typ, gmm__qxcbp)
        return sdmi__iwj._getvalue()
    return sthpm__wazab(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    hbw__gge = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    gmm__qxcbp = hbw__gge.value
    sdmi__iwj = c.context.make_helper(c.builder, typ)
    sdmi__iwj.data = gmm__qxcbp
    jymhu__euhm = hbw__gge.is_error
    return NativeValue(sdmi__iwj._getvalue(), is_error=jymhu__euhm)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    sdmi__iwj = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, sdmi__iwj.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        wxndp__wqej = 'def impl(arr, ind):\n'
        ykay__qynb = ','.join(f'get_data(arr._data)[{pylt__ykabt}][ind]' for
            pylt__ykabt in range(len(arr.data)))
        wxndp__wqej += f'  return ({ykay__qynb})\n'
        cfrz__szmwk = {}
        exec(wxndp__wqej, {'get_data': bodo.libs.struct_arr_ext.get_data},
            cfrz__szmwk)
        siup__pjgiy = cfrz__szmwk['impl']
        return siup__pjgiy
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        exx__yma = len(arr.data)
        wxndp__wqej = 'def impl(arr, ind, val):\n'
        wxndp__wqej += '  data = get_data(arr._data)\n'
        wxndp__wqej += '  null_bitmap = get_null_bitmap(arr._data)\n'
        wxndp__wqej += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for pylt__ykabt in range(exx__yma):
            wxndp__wqej += f'  data[{pylt__ykabt}][ind] = val[{pylt__ykabt}]\n'
        cfrz__szmwk = {}
        exec(wxndp__wqej, {'get_data': bodo.libs.struct_arr_ext.get_data,
