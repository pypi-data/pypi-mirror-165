...
def box_regex_flag(typ, val, c):
    quqq__chz = c.box(types.int64, val)
    lupl__fvg = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(lupl__fvg, (quqq__chz,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    quqq__chz = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, quqq__chz)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    lhgf__pxm = context.get_python_api(builder)
    tlz__bkym = builder.icmp_signed('==', val, lhgf__pxm.borrow_none())
    with builder.if_then(tlz__bkym):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    yjpus__dsnsa = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(yjpus__dsnsa)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        zscly__dsca = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if zscly__dsca > 1:
            typ = types.List(types.Tuple([string_type] * zscly__dsca))
...
def overload_match_group(m, *args):
    ...
    bnb__foqv = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ilzh__qfo = args[0]
            with numba.objmode(out=bnb__foqv):
                out = m.group(ilzh__qfo)
            return out
        return _match_group_impl_one
    xjk__kmzt = 'tuple_str_{}'.format(len(args))
    setattr(types, xjk__kmzt, types.Tuple([bnb__foqv] * len(args)))
    jnae__bxwsk = ', '.join('group{}'.format(cpc__zie + 1) for cpc__zie in
        range(len(args)))
    gfw__andg = 'def _match_group_impl(m, *args):\n'
    gfw__andg += '  ({}) = args\n'.format(jnae__bxwsk)
    gfw__andg += "  with numba.objmode(out='{}'):\n".format(xjk__kmzt)
    gfw__andg += '    out = m.group({})\n'.format(jnae__bxwsk)
    gfw__andg += '  return out\n'
    vvh__kmcv = {}
    exec(gfw__andg, globals(), vvh__kmcv)
    ghwj__vnn = vvh__kmcv['_match_group_impl']
    return ghwj__vnn
...
def overload_match_groups(m, default=None):
    dwvax__bao = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=dwvax__bao):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(awr__qfdmd is None for awr__qfdmd in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=bnb__foqv):
