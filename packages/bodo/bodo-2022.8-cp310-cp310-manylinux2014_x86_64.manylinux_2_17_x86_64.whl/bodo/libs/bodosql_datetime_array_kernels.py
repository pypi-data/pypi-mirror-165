...
def makedate(year, day):
    args = [year, day]
    for qfn__jqm in range(2):
        if isinstance(args[qfn__jqm], types.optional):
...
def dayname_util(arr):
    ...
    twtf__elcq = [arr]
    obgf__vemn = [True]
    yokqn__fonm = 'res[i] = pd.Timestamp(arg0).day_name()'
    mdg__ltkxu = bodo.string_array_type
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
...
def day_timestamp_util(arr):
    ...
    mdg__ltkxu = np.dtype('datetime64[ns]')
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
...
def int_to_days_util(arr):
    ...
    mdg__ltkxu = np.dtype('timedelta64[ns]')
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
...
def monthname_util(arr):
    ...
    twtf__elcq = [arr]
    obgf__vemn = [True]
    yokqn__fonm = 'res[i] = pd.Timestamp(arg0).month_name()'
    mdg__ltkxu = bodo.string_array_type
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
...
def month_diff_util(arr0, arr1):
    ...
    iasc__tqy = ['arr0', 'arr1']
    twtf__elcq = [arr0, arr1]
    obgf__vemn = [True] * 2
    yokqn__fonm = 'A0 = bodo.utils.conversion.box_if_dt64(arg0)\n'
    yokqn__fonm += 'A1 = bodo.utils.conversion.box_if_dt64(arg1)\n'
    yokqn__fonm += 'delta = 12 * (A0.year - A1.year) + (A0.month - A1.month)\n'
    yokqn__fonm += (
        'remainder = ((A0 - pd.DateOffset(months=delta)) - A1).value\n')
    yokqn__fonm += 'if delta > 0 and remainder < 0:\n'
    yokqn__fonm += '   res[i] = -(delta - 1)\n'
    yokqn__fonm += 'elif delta < 0 and remainder > 0:\n'
    yokqn__fonm += '   res[i] = -(delta + 1)\n'
    yokqn__fonm += 'else:\n'
    yokqn__fonm += '   res[i] = -delta'
    mdg__ltkxu = bodo.libs.int_arr_ext.IntegerArrayType(types.int32)
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
...
def weekday_util(arr):
    ...
    twtf__elcq = [arr]
    obgf__vemn = [True]
    yokqn__fonm = 'dt = pd.Timestamp(arg0)\n'
    yokqn__fonm += (
...
def yearofweekiso_util(arr):
    ...
    obgf__vemn = [True]
    yokqn__fonm = 'dt = pd.Timestamp(arg0)\n'
    yokqn__fonm += 'res[i] = dt.isocalendar()[0]'
    mdg__ltkxu = bodo.libs.int_arr_ext.IntegerArrayType(types.int32)
    return gen_vectorized(iasc__tqy, twtf__elcq, obgf__vemn, yokqn__fonm,
        mdg__ltkxu)
