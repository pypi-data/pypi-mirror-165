...
def _global_except_hook(exctype, value, traceback):
    ...
    zurr__nsa = MPI.COMM_WORLD
    ucz__kcce = zurr__nsa.Get_rank()
    xjpid__tcqc = 3.0
    ickup__quq = True
    bkxw__jslol = zurr__nsa.Ibarrier()
    nam__duw = time.time()
    while time.time() - nam__duw < xjpid__tcqc:
        time.sleep(0.1)
        if bkxw__jslol.Test():
            ickup__quq = False
            break
    try:
        ...
        if ickup__quq:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {ucz__kcce}. \n')
...
def set_numba_environ_vars():
    if (hhvmj__jdwtv := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = hhvmj__jdwtv
        os.environ['NUMBA_CACHE_DIR'] = hhvmj__jdwtv
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    znl__wkkq = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(znl__wkkq)
