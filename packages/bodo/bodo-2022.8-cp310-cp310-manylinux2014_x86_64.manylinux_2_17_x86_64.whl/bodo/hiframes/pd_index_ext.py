...
def typeof_pd_index(val, c):
    ...
    if val.inferred_type == 'integer' or pd._libs.lib.infer_dtype(val, True
        ...
        if isinstance(val.dtype, pd.core.arrays.integer._IntegerDtype):
            emx__qbuzn = val.dtype.numpy_dtype
            dtype = numba.np.numpy_support.from_dtype(emx__qbuzn)
        else:
            dtype = types.int64
...
class DatetimeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', fe_type.data), ('name', fe_type.name_typ),
            ('dict', types.DictType(_dt_index_data_typ.dtype, types.int64))]
        super(DatetimeIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def overload_datetime_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    gmncc__rwyz = idx_typ_to_format_str_map[DatetimeIndexType].format('copy()')
    check_unsupported_args('copy', wriv__ytypi, idx_cpy_arg_defaults,
        fn_str=gmncc__rwyz, package_name='pandas', module_name='Index')
...
def box_dt_index(typ, val, c):
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    lldlg__srd = c.pyapi.import_module_noblock(amiow__rox)
    rtap__qbkzg = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, rtap__qbkzg.data)
    puma__kkrs = c.pyapi.from_native_value(typ.data, rtap__qbkzg.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, rtap__qbkzg.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, rtap__qbkzg.name,
        c.env_manager)
    args = c.pyapi.tuple_pack([puma__kkrs])
    ydtkw__gakm = c.pyapi.object_getattr_string(lldlg__srd, 'DatetimeIndex')
    kws = c.pyapi.dict_pack([('name', bholz__liop)])
    ysxhd__obx = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(puma__kkrs)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(lldlg__srd)
    c.pyapi.decref(ydtkw__gakm)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_datetime_index(typ, val, c):
    if isinstance(typ.data, DatetimeArrayType):
        kcwta__bxbv = c.pyapi.object_getattr_string(val, 'array')
    else:
        kcwta__bxbv = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, kcwta__bxbv).value
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    tqvfb__uegd.data = data
    tqvfb__uegd.name = name
    dtype = _dt_index_data_typ.dtype
    mrgip__dsq, lzm__prlj = c.pyapi.call_jit_code(lambda : numba.typed.Dict
        .empty(dtype, types.int64), types.DictType(dtype, types.int64)(), [])
    tqvfb__uegd.dict = lzm__prlj
    c.pyapi.decref(kcwta__bxbv)
    c.pyapi.decref(bholz__liop)
    return NativeValue(tqvfb__uegd._getvalue())
...
def init_datetime_index(typingctx, data, name):
    ...
    def codegen(context, builder, signature, args):
        eiqmw__phnkg, lmq__iws = args
        rtap__qbkzg = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        rtap__qbkzg.data = eiqmw__phnkg
        rtap__qbkzg.name = lmq__iws
        context.nrt.incref(builder, signature.args[0], eiqmw__phnkg)
        context.nrt.incref(builder, signature.args[1], lmq__iws)
        dtype = _dt_index_data_typ.dtype
        rtap__qbkzg.dict = context.compile_internal(builder, lambda : numba
            .typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return rtap__qbkzg._getvalue()
    ann__vwkrn = DatetimeIndexType(name, data)
    sig = signature(ann__vwkrn, data, name)
    return sig, codegen
...
def init_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    wbpx__jcwz = args[0]
    if equiv_set.has_shape(wbpx__jcwz):
        return ArrayAnalysis.AnalyzeResult(shape=wbpx__jcwz, pre=[])
    return None
...
def gen_dti_field_impl(field):
    xoirt__eeif = 'def impl(dti):\n'
    xoirt__eeif += '    numba.parfors.parfor.init_prange()\n'
    xoirt__eeif += '    A = bodo.hiframes.pd_index_ext.get_index_data(dti)\n'
    xoirt__eeif += (
        '    name = bodo.hiframes.pd_index_ext.get_index_name(dti)\n')
    xoirt__eeif += '    n = len(A)\n'
    xoirt__eeif += '    S = np.empty(n, np.int64)\n'
    xoirt__eeif += '    for i in numba.parfors.parfor.internal_prange(n):\n'
    xoirt__eeif += '        val = A[i]\n'
    xoirt__eeif += '        ts = bodo.utils.conversion.box_if_dt64(val)\n'
    if field in ['weekday']:
        xoirt__eeif += '        S[i] = ts.' + field + '()\n'
    else:
        xoirt__eeif += '        S[i] = ts.' + field + '\n'
    xoirt__eeif += (
        '    return bodo.hiframes.pd_index_ext.init_numeric_index(S, name)\n')
    vepol__tzpj = {}
    exec(xoirt__eeif, {'numba': numba, 'np': np, 'bodo': bodo}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def overload_datetime_index_is_leap_year(dti):
    ...
    def impl(dti):
        ...
        xreo__krwv = len(A)
        S = np.empty(xreo__krwv, np.bool_)
        for i in numba.parfors.parfor.internal_prange(xreo__krwv):
            val = A[i]
            hnlkx__uqsb = bodo.utils.conversion.box_if_dt64(val)
            S[i] = bodo.hiframes.pd_timestamp_ext.is_leap_year(hnlkx__uqsb.year
                )
        return S
    return impl
...
def overload_datetime_index_date(dti):
    ...
    def impl(dti):
        ...
        for i in numba.parfors.parfor.internal_prange(xreo__krwv):
            val = A[i]
            hnlkx__uqsb = bodo.utils.conversion.box_if_dt64(val)
            S[i] = datetime.date(hnlkx__uqsb.year, hnlkx__uqsb.month,
                hnlkx__uqsb.day)
        return S
    return impl
...
def overload_datetime_index_min(dti, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.min', gvak__vbjx, ksx__mffps,
    ...
    def impl(dti, axis=None, skipna=True):
        numba.parfors.parfor.init_prange()
        cmccy__ckkq = bodo.hiframes.pd_index_ext.get_index_data(dti)
        s = numba.cpython.builtins.get_type_max_value(numba.core.types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(len(cmccy__ckkq)):
            if not bodo.libs.array_kernels.isna(cmccy__ckkq, i):
...
def overload_datetime_index_max(dti, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.max', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(dti,
        'Index.max()')
...
def pd_datetimeindex_overload(data=None, freq=None, tz=None, normalize=
    ...
    gvak__vbjx = dict(freq=freq, tz=tz, normalize=normalize, closed=closed,
        ambiguous=ambiguous, dayfirst=dayfirst, yearfirst=yearfirst, dtype=
        dtype, copy=copy)
    ksx__mffps = dict(freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False)
    check_unsupported_args('pandas.DatetimeIndex', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')

    def f(data=None, freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False, name=None):
        cnp__wevg = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_dt64ns(cnp__wevg)
...
def overload_sub_operator_datetime_index(lhs, rhs):
    if isinstance(lhs, DatetimeIndexType
        ...
        fakq__whn = np.dtype('timedelta64[ns]')

        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            cmccy__ckkq = bodo.hiframes.pd_index_ext.get_index_data(lhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(lhs)
            xreo__krwv = len(cmccy__ckkq)
            S = np.empty(xreo__krwv, fakq__whn)
            olce__uwfsp = rhs.value
            for i in numba.parfors.parfor.internal_prange(xreo__krwv):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    bodo.hiframes.pd_timestamp_ext.dt64_to_integer(
                    cmccy__ckkq[i]) - olce__uwfsp)
    ...
    if isinstance(rhs, DatetimeIndexType
        ...
        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            cmccy__ckkq = bodo.hiframes.pd_index_ext.get_index_data(rhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(rhs)
            xreo__krwv = len(cmccy__ckkq)
            S = np.empty(xreo__krwv, fakq__whn)
            olce__uwfsp = lhs.value
            for i in numba.parfors.parfor.internal_prange(xreo__krwv):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    olce__uwfsp - bodo.hiframes.pd_timestamp_ext.
                    dt64_to_integer(cmccy__ckkq[i]))
            return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
        return impl


def gen_dti_str_binop_impl(op, is_lhs_dti):
    kxoy__zuo = numba.core.utils.OPERATORS_TO_BUILTINS[op]
    xoirt__eeif = 'def impl(lhs, rhs):\n'
    if is_lhs_dti:
        xoirt__eeif += '  dt_index, _str = lhs, rhs\n'
        fmt__wpac = 'arr[i] {} other'.format(kxoy__zuo)
    else:
        xoirt__eeif += '  dt_index, _str = rhs, lhs\n'
        fmt__wpac = 'other {} arr[i]'.format(kxoy__zuo)
    xoirt__eeif += (
        '  arr = bodo.hiframes.pd_index_ext.get_index_data(dt_index)\n')
    xoirt__eeif += '  l = len(arr)\n'
    xoirt__eeif += (
        '  other = bodo.hiframes.pd_timestamp_ext.parse_datetime_str(_str)\n')
    xoirt__eeif += '  S = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    xoirt__eeif += '  for i in numba.parfors.parfor.internal_prange(l):\n'
    xoirt__eeif += '    S[i] = {}\n'.format(fmt__wpac)
    xoirt__eeif += '  return S\n'
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo, 'numba': numba, 'np': np}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def pd_index_overload(data=None, dtype=None, copy=False, name=None,
    ...
    if not is_overload_none(dtype):
        djohy__nnri = parse_dtype(dtype, 'pandas.Index')
        tfucm__yxse = False
    else:
        djohy__nnri = getattr(data, 'dtype', None)
        tfucm__yxse = True
    if isinstance(djohy__nnri, types.misc.PyObject):
        raise BodoError(
    ...
    if isinstance(data, RangeIndexType):
        ...
        if isinstance(djohy__nnri, (types.Integer, types.Float, types.Boolean)
            ):
            if tfucm__yxse:
                ...
                def impl(data=None, dtype=None, copy=False, name=None,
                    tupleize_cols=True):
                    cnp__wevg = bodo.utils.conversion.coerce_to_array(data)
                    wkn__kle = bodo.utils.conversion.fix_arr_dtype(cnp__wevg,
                        djohy__nnri)
                    return bodo.hiframes.pd_index_ext.init_numeric_index(
                        wkn__kle, name)
        elif djohy__nnri in [types.string, bytes_type]:
...
def overload_datetime_index_getitem(dti, ind):
    if isinstance(dti, DatetimeIndexType):
        if isinstance(ind, types.Integer):
            ...
            def impl(dti, ind):
                mnbd__yuios = bodo.hiframes.pd_index_ext.get_index_data(dti)
                val = mnbd__yuios[ind]
...
def overload_timedelta_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            ixm__amjrk = bodo.hiframes.pd_index_ext.get_index_data(I)
            return pd.Timedelta(ixm__amjrk[ind])
        return impl

    def impl(I, ind):
        ixm__amjrk = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        zwur__sbpqe = ixm__amjrk[ind]
        return bodo.hiframes.pd_index_ext.init_timedelta_index(zwur__sbpqe,
            name)
    return impl
...
def overload_categorical_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            msgk__dnew = bodo.hiframes.pd_index_ext.get_index_data(I)
            val = msgk__dnew[ind]
            return val
        return impl
...
def validate_endpoints(closed):
    ...
    return pigze__thpmd, uvs__dwrig
...
def pd_date_range_overload(start=None, end=None, periods=None, freq=None,
    ...
    gvak__vbjx = dict(tz=tz, normalize=normalize, closed=closed)
    ksx__mffps = dict(tz=None, normalize=False, closed=None)
    check_unsupported_args('pandas.date_range', gvak__vbjx, ksx__mffps,
    ...
    xoirt__eeif = """def f(start=None, end=None, periods=None, freq=None, tz=None, normalize=False, name=None, closed=None):
"""
    xoirt__eeif += pcde__rkfj
    if is_overload_none(start):
        xoirt__eeif += "  start_t = pd.Timestamp('1800-01-03')\n"
    else:
        xoirt__eeif += '  start_t = pd.Timestamp(start)\n'
    if is_overload_none(end):
        xoirt__eeif += "  end_t = pd.Timestamp('1800-01-03')\n"
    else:
        xoirt__eeif += '  end_t = pd.Timestamp(end)\n'
    if not is_overload_none(freq):
        xoirt__eeif += (
            '  stride = bodo.hiframes.pd_index_ext.to_offset_value(freq)\n')
        if is_overload_none(periods):
            xoirt__eeif += '  b = start_t.value\n'
            xoirt__eeif += (
                '  e = b + (end_t.value - b) // stride * stride + stride // 2 + 1\n'
                )
        elif not is_overload_none(start):
            xoirt__eeif += '  b = start_t.value\n'
            xoirt__eeif += '  addend = np.int64(periods) * np.int64(stride)\n'
            xoirt__eeif += '  e = np.int64(b) + addend\n'
        elif not is_overload_none(end):
            xoirt__eeif += '  e = end_t.value + stride\n'
            xoirt__eeif += '  addend = np.int64(periods) * np.int64(-stride)\n'
            xoirt__eeif += '  b = np.int64(e) + addend\n'
        else:
            raise_bodo_error(
                "at least 'start' or 'end' should be specified if a 'period' is given."
                )
        xoirt__eeif += '  arr = np.arange(b, e, stride, np.int64)\n'
    else:
        xoirt__eeif += '  delta = end_t.value - start_t.value\n'
        xoirt__eeif += '  step = delta / (periods - 1)\n'
        xoirt__eeif += '  arr1 = np.arange(0, periods, 1, np.float64)\n'
        xoirt__eeif += '  arr1 *= step\n'
        xoirt__eeif += '  arr1 += start_t.value\n'
        xoirt__eeif += '  arr = arr1.astype(np.int64)\n'
        xoirt__eeif += '  arr[-1] = end_t.value\n'
    xoirt__eeif += '  A = bodo.utils.conversion.convert_to_dt64ns(arr)\n'
    xoirt__eeif += (
        '  return bodo.hiframes.pd_index_ext.init_datetime_index(A, name)\n')
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo, 'np': np, 'pd': pd}, vepol__tzpj)
    f = vepol__tzpj['f']
    return f
...
def pd_timedelta_range_overload(start=None, end=None, periods=None, freq=
    ...
    def f(start=None, end=None, periods=None, freq=None, name=None, closed=None
        ...
        glnz__rqia = pd.Timedelta('1 day')
        if start is not None:
            glnz__rqia = pd.Timedelta(start)
        rmo__kmq = pd.Timedelta('1 day')
        if end is not None:
            rmo__kmq = pd.Timedelta(end)
        if start is None and end is None and closed is not None:
            raise ValueError(
                'Closed has to be None if not both of start and end are defined'
                )
        pigze__thpmd, uvs__dwrig = (bodo.hiframes.pd_index_ext.
            validate_endpoints(closed))
        if freq is not None:
            rawxo__ezug = _dummy_convert_none_to_int(freq)
            if periods is None:
                b = glnz__rqia.value
                vcy__bttv = b + (rmo__kmq.value - b
                    ) // rawxo__ezug * rawxo__ezug + rawxo__ezug // 2 + 1
            elif start is not None:
                periods = _dummy_convert_none_to_int(periods)
                b = glnz__rqia.value
                detz__fky = np.int64(periods) * np.int64(rawxo__ezug)
                vcy__bttv = np.int64(b) + detz__fky
            elif end is not None:
                periods = _dummy_convert_none_to_int(periods)
                vcy__bttv = rmo__kmq.value + rawxo__ezug
                detz__fky = np.int64(periods) * np.int64(-rawxo__ezug)
                b = np.int64(vcy__bttv) + detz__fky
            else:
                raise ValueError(
                    "at least 'start' or 'end' should be specified if a 'period' is given."
                    )
            gasb__lzfuv = np.arange(b, vcy__bttv, rawxo__ezug, np.int64)
        else:
            periods = _dummy_convert_none_to_int(periods)
            poq__viq = rmo__kmq.value - glnz__rqia.value
            step = poq__viq / (periods - 1)
            pzet__xkp = np.arange(0, periods, 1, np.float64)
            pzet__xkp *= step
            pzet__xkp += glnz__rqia.value
            gasb__lzfuv = pzet__xkp.astype(np.int64)
            gasb__lzfuv[-1] = rmo__kmq.value
        if not pigze__thpmd and len(gasb__lzfuv) and gasb__lzfuv[0
            ] == glnz__rqia.value:
            gasb__lzfuv = gasb__lzfuv[1:]
        if not uvs__dwrig and len(gasb__lzfuv) and gasb__lzfuv[-1
            ] == rmo__kmq.value:
            gasb__lzfuv = gasb__lzfuv[:-1]
        S = bodo.utils.conversion.convert_to_dt64ns(gasb__lzfuv)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return f
...
def overload_pd_timestamp_isocalendar(idx):
    aeefo__rlc = ColNamesMetaType(('year', 'week', 'day'))

    def impl(idx):
        A = bodo.hiframes.pd_index_ext.get_index_data(idx)
        numba.parfors.parfor.init_prange()
        xreo__krwv = len(A)
        iaxvb__tmppr = bodo.libs.int_arr_ext.alloc_int_array(xreo__krwv, np
            .uint32)
        uir__jxj = bodo.libs.int_arr_ext.alloc_int_array(xreo__krwv, np.uint32)
        jqz__gatqz = bodo.libs.int_arr_ext.alloc_int_array(xreo__krwv, np.
            uint32)
        for i in numba.parfors.parfor.internal_prange(xreo__krwv):
            if bodo.libs.array_kernels.isna(A, i):
                bodo.libs.array_kernels.setna(iaxvb__tmppr, i)
                bodo.libs.array_kernels.setna(uir__jxj, i)
                bodo.libs.array_kernels.setna(jqz__gatqz, i)
                continue
            iaxvb__tmppr[i], uir__jxj[i], jqz__gatqz[i
                ] = bodo.utils.conversion.box_if_dt64(A[i]).isocalendar()
        return bodo.hiframes.pd_dataframe_ext.init_dataframe((iaxvb__tmppr,
            uir__jxj, jqz__gatqz), idx, aeefo__rlc)
    return impl
...
class TimedeltaIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', _timedelta_index_data_typ), ('name',
            fe_type.name_typ), ('dict', types.DictType(
            _timedelta_index_data_typ.dtype, types.int64))]
        super(TimedeltaIndexTypeModel, self).__init__(dmm, fe_type, oikf__xbbcj
            )
...
def box_timedelta_index(typ, val, c):
    ...
    puma__kkrs = c.pyapi.from_native_value(_timedelta_index_data_typ,
        timedelta_index.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, timedelta_index.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, timedelta_index.
        name, c.env_manager)
    args = c.pyapi.tuple_pack([puma__kkrs])
    kws = c.pyapi.dict_pack([('name', bholz__liop)])
    ydtkw__gakm = c.pyapi.object_getattr_string(lldlg__srd, 'TimedeltaIndex')
    ysxhd__obx = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(puma__kkrs)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(lldlg__srd)
...
def unbox_timedelta_index(typ, val, c):
    eyg__ywktv = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(_timedelta_index_data_typ, eyg__ywktv).value
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    c.pyapi.decref(eyg__ywktv)
    c.pyapi.decref(bholz__liop)
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    tqvfb__uegd.data = data
    tqvfb__uegd.name = name
...
def init_timedelta_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        timedelta_index = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        timedelta_index.data = eiqmw__phnkg
        timedelta_index.name = lmq__iws
        context.nrt.incref(builder, signature.args[0], eiqmw__phnkg)
        context.nrt.incref(builder, signature.args[1], lmq__iws)
        dtype = _timedelta_index_data_typ.dtype
    ...
    ann__vwkrn = TimedeltaIndexType(name)
    sig = signature(ann__vwkrn, data, name)
    return sig, codegen
...
def overload_timedelta_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    gmncc__rwyz = idx_typ_to_format_str_map[TimedeltaIndexType].format('copy()'
        )
    check_unsupported_args('TimedeltaIndex.copy', wriv__ytypi,
        idx_cpy_arg_defaults, fn_str=gmncc__rwyz, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def overload_timedelta_index_min(tdi, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.min', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    ...
    def impl(tdi, axis=None, skipna=True):
        ...
        xreo__krwv = len(data)
        wwfg__ksyf = numba.cpython.builtins.get_type_max_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(xreo__krwv):
            ...
            count += 1
            wwfg__ksyf = min(wwfg__ksyf, val)
        jwol__idd = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
            wwfg__ksyf)
        return bodo.hiframes.pd_index_ext._tdi_val_finalize(jwol__idd, count)
    return impl
...
def overload_timedelta_index_max(tdi, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.max', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    ...
    def impl(tdi, axis=None, skipna=True):
        ...
        data = bodo.hiframes.pd_index_ext.get_index_data(tdi)
        xreo__krwv = len(data)
        ijy__hqftw = numba.cpython.builtins.get_type_min_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(xreo__krwv):
            if bodo.libs.array_kernels.isna(data, i):
...
def gen_tdi_field_impl(field):
    xoirt__eeif = 'def impl(tdi):\n'
    xoirt__eeif += '    numba.parfors.parfor.init_prange()\n'
    xoirt__eeif += '    A = bodo.hiframes.pd_index_ext.get_index_data(tdi)\n'
    xoirt__eeif += (
    ...
    if field == 'nanoseconds':
        xoirt__eeif += '        S[i] = td64 % 1000\n'
    elif field == 'microseconds':
        xoirt__eeif += '        S[i] = td64 // 1000 % 100000\n'
    elif field == 'seconds':
        xoirt__eeif += (
...
def pd_timedelta_index_overload(data=None, unit=None, freq=None, dtype=None,
    ...
    gvak__vbjx = dict(unit=unit, freq=freq, dtype=dtype, copy=copy)
    ksx__mffps = dict(unit=None, freq=None, dtype=None, copy=False)
    check_unsupported_args('pandas.TimedeltaIndex', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')

    def impl(data=None, unit=None, freq=None, dtype=None, copy=False, name=None
        ):
        cnp__wevg = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_td64ns(cnp__wevg)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return impl
...
class RangeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('start', types.int64), ('stop', types.int64), (
            'step', types.int64), ('name', fe_type.name_typ)]
        super(RangeIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def overload_range_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    gmncc__rwyz = idx_typ_to_format_str_map[RangeIndexType].format('copy()')
    check_unsupported_args('RangeIndex.copy', wriv__ytypi,
        idx_cpy_arg_defaults, fn_str=gmncc__rwyz, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def box_range_index(typ, val, c):
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    jwf__crgx = c.pyapi.import_module_noblock(amiow__rox)
    lif__gmfvz = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    axpep__afre = c.pyapi.from_native_value(types.int64, lif__gmfvz.start,
        c.env_manager)
    rczol__qco = c.pyapi.from_native_value(types.int64, lif__gmfvz.stop, c.
        env_manager)
    ojs__etl = c.pyapi.from_native_value(types.int64, lif__gmfvz.step, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, lif__gmfvz.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, lif__gmfvz.name,
        c.env_manager)
    args = c.pyapi.tuple_pack([axpep__afre, rczol__qco, ojs__etl])
    kws = c.pyapi.dict_pack([('name', bholz__liop)])
    ydtkw__gakm = c.pyapi.object_getattr_string(jwf__crgx, 'RangeIndex')
    vbzfz__wdttl = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(axpep__afre)
    c.pyapi.decref(rczol__qco)
    c.pyapi.decref(ojs__etl)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(jwf__crgx)
    c.pyapi.decref(ydtkw__gakm)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
    c.context.nrt.decref(c.builder, typ, val)
...
def init_range_index(typingctx, start, stop, step, name=None):
    name = types.none if name is None else name
    kiv__yoq = is_overload_constant_int(step) and get_overload_const_int(step
        ) == 0
    ...
    def codegen(context, builder, signature, args):
        ...
        vmwgt__fwejh = cgutils.is_scalar_zero(builder, args[2])
        knt__knqvp = context.get_python_api(builder)
        with builder.if_then(vmwgt__fwejh):
            knt__knqvp.err_format('PyExc_ValueError', 'Step must not be zero')
            val = context.get_constant(types.int32, -1)
            builder.ret(val)
        lif__gmfvz = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        lif__gmfvz.start = args[0]
        lif__gmfvz.stop = args[1]
        lif__gmfvz.step = args[2]
        lif__gmfvz.name = args[3]
        context.nrt.incref(builder, signature.return_type.name_typ, args[3])
        return lif__gmfvz._getvalue()
    return RangeIndexType(name)(start, stop, step, name), codegen


def init_range_index_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 4 and not kws
    start, stop, step, ebe__dxiu = args
...
def unbox_range_index(typ, val, c):
    axpep__afre = c.pyapi.object_getattr_string(val, 'start')
    start = c.pyapi.to_native_value(types.int64, axpep__afre).value
    rczol__qco = c.pyapi.object_getattr_string(val, 'stop')
    stop = c.pyapi.to_native_value(types.int64, rczol__qco).value
    ojs__etl = c.pyapi.object_getattr_string(val, 'step')
    step = c.pyapi.to_native_value(types.int64, ojs__etl).value
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    c.pyapi.decref(axpep__afre)
    c.pyapi.decref(rczol__qco)
    c.pyapi.decref(ojs__etl)
    c.pyapi.decref(bholz__liop)
    lif__gmfvz = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    lif__gmfvz.start = start
    lif__gmfvz.stop = stop
    lif__gmfvz.step = step
    lif__gmfvz.name = name
    return NativeValue(lif__gmfvz._getvalue())
...
def range_index_overload(start=None, stop=None, step=None, dtype=None, copy
    ...
    def _ensure_int_or_none(value, field):
        ...
        if not is_overload_none(value) and not isinstance(value, types.
            ...
            raise BodoError(aberv__wvmy.format(value=value, field=field))
    ...
    if is_overload_none(start) and is_overload_none(stop) and is_overload_none(
        step):
        aberv__wvmy = 'RangeIndex(...) must be called with integers'
        raise BodoError(aberv__wvmy)
    ooq__vor = 'start'
    tfp__wdaua = 'stop'
    khxc__vlnm = 'step'
    if is_overload_none(start):
    ...
    if is_overload_none(step):
        khxc__vlnm = '1'
    xoirt__eeif = """def _pd_range_index_imp(start=None, stop=None, step=None, dtype=None, copy=False, name=None):
"""
    xoirt__eeif += '  return init_range_index({}, {}, {}, name)\n'.format(
        ooq__vor, tfp__wdaua, khxc__vlnm)
    vepol__tzpj = {}
    exec(xoirt__eeif, {'init_range_index': init_range_index}, vepol__tzpj)
    dthyf__vhbmo = vepol__tzpj['_pd_range_index_imp']
    return dthyf__vhbmo
...
def overload_range_index_getitem(I, idx):
    if isinstance(I, RangeIndexType):
        ...
        if isinstance(idx, types.SliceType):
            ...
            def impl(I, idx):
                ntfjr__rextv = numba.cpython.unicode._normalize_slice(idx,
                    len(I))
                name = bodo.hiframes.pd_index_ext.get_index_name(I)
                start = I._start + I._step * ntfjr__rextv.start
                stop = I._start + I._step * ntfjr__rextv.stop
                step = I._step * ntfjr__rextv.step
...
class PeriodIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', bodo.IntegerArrayType(types.int64)), (
            'name', fe_type.name_typ), ('dict', types.DictType(types.int64,
            types.int64))]
        super(PeriodIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def overload_period_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    freq = A.freq
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    gmncc__rwyz = idx_typ_to_format_str_map[PeriodIndexType].format('copy()')
    check_unsupported_args('PeriodIndex.copy', wriv__ytypi,
        idx_cpy_arg_defaults, fn_str=gmncc__rwyz, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def init_period_index(typingctx, data, name, freq):
    ...
    def codegen(context, builder, signature, args):
        eiqmw__phnkg, lmq__iws, ebe__dxiu = args
        ixo__hgwp = signature.return_type
        axmn__nmgd = cgutils.create_struct_proxy(ixo__hgwp)(context, builder)
        axmn__nmgd.data = eiqmw__phnkg
        axmn__nmgd.name = lmq__iws
        context.nrt.incref(builder, signature.args[0], args[0])
        context.nrt.incref(builder, signature.args[1], args[1])
        axmn__nmgd.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(types.int64, types.int64), types.DictType(
            types.int64, types.int64)(), [])
        return axmn__nmgd._getvalue()
    hgswy__jqd = get_overload_const_str(freq)
    ann__vwkrn = PeriodIndexType(hgswy__jqd, name)
    sig = signature(ann__vwkrn, data, name, freq)
    return sig, codegen
...
def box_period_index(typ, val, c):
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    jwf__crgx = c.pyapi.import_module_noblock(amiow__rox)
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, bodo.IntegerArrayType(types.int64),
        tqvfb__uegd.data)
    kcwta__bxbv = c.pyapi.from_native_value(bodo.IntegerArrayType(types.
        int64), tqvfb__uegd.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, tqvfb__uegd.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, tqvfb__uegd.name,
        c.env_manager)
    bek__yoz = c.pyapi.string_from_constant_string(typ.freq)
    args = c.pyapi.tuple_pack([])
    kws = c.pyapi.dict_pack([('ordinal', kcwta__bxbv), ('name', bholz__liop
        ), ('freq', bek__yoz)])
    ydtkw__gakm = c.pyapi.object_getattr_string(jwf__crgx, 'PeriodIndex')
    vbzfz__wdttl = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(kcwta__bxbv)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(bek__yoz)
    c.pyapi.decref(jwf__crgx)
    c.pyapi.decref(ydtkw__gakm)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_period_index(typ, val, c):
    ...
    jooi__sircd = c.pyapi.object_getattr_string(val, 'asi8')
    nwzqf__oghvn = c.pyapi.call_method(val, 'isna', ())
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    lldlg__srd = c.pyapi.import_module_noblock(amiow__rox)
    bzw__oipp = c.pyapi.object_getattr_string(lldlg__srd, 'arrays')
    kcwta__bxbv = c.pyapi.call_method(bzw__oipp, 'IntegerArray', (
        jooi__sircd, nwzqf__oghvn))
    data = c.pyapi.to_native_value(arr_typ, kcwta__bxbv).value
    c.pyapi.decref(jooi__sircd)
    c.pyapi.decref(nwzqf__oghvn)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(lldlg__srd)
    c.pyapi.decref(bzw__oipp)
    c.pyapi.decref(kcwta__bxbv)
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    tqvfb__uegd.data = data
    tqvfb__uegd.name = name
...
class CategoricalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        mzdp__fdk = get_categories_int_type(fe_type.data.dtype)
        oikf__xbbcj = [('data', fe_type.data), ('name', fe_type.name_typ),
            ('dict', types.DictType(mzdp__fdk, types.int64))]
...
def box_categorical_index(typ, val, c):
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    lldlg__srd = c.pyapi.import_module_noblock(amiow__rox)
    jzan__elg = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, jzan__elg.data)
    puma__kkrs = c.pyapi.from_native_value(typ.data, jzan__elg.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, jzan__elg.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, jzan__elg.name, c
        .env_manager)
    args = c.pyapi.tuple_pack([puma__kkrs])
    kws = c.pyapi.dict_pack([('name', bholz__liop)])
    ydtkw__gakm = c.pyapi.object_getattr_string(lldlg__srd, 'CategoricalIndex')
    ysxhd__obx = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(puma__kkrs)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(lldlg__srd)
...
def unbox_categorical_index(typ, val, c):
    from bodo.hiframes.pd_categorical_ext import get_categories_int_type
    eyg__ywktv = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, eyg__ywktv).value
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    c.pyapi.decref(eyg__ywktv)
    c.pyapi.decref(bholz__liop)
...
def init_categorical_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        eiqmw__phnkg, lmq__iws = args
        jzan__elg = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        jzan__elg.data = eiqmw__phnkg
        jzan__elg.name = lmq__iws
        context.nrt.incref(builder, signature.args[0], eiqmw__phnkg)
        context.nrt.incref(builder, signature.args[1], lmq__iws)
        dtype = get_categories_int_type(signature.return_type.data.dtype)
        jzan__elg.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return jzan__elg._getvalue()
    ann__vwkrn = CategoricalIndexType(data, name)
    sig = signature(ann__vwkrn, data, name)
    return sig, codegen
...
def overload_categorical_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    gmncc__rwyz = idx_typ_to_format_str_map[CategoricalIndexType].format(
        'copy()')
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('CategoricalIndex.copy', wriv__ytypi,
        idx_cpy_arg_defaults, fn_str=gmncc__rwyz, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
class IntervalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', fe_type.data), ('name', fe_type.name_typ),
            ...
            dtype, 2), types.int64))]
        super(IntervalIndexTypeModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def box_interval_index(typ, val, c):
    amiow__rox = c.context.insert_const_string(c.builder.module, 'pandas')
    lldlg__srd = c.pyapi.import_module_noblock(amiow__rox)
    kcul__aubd = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, kcul__aubd.data)
    puma__kkrs = c.pyapi.from_native_value(typ.data, kcul__aubd.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, kcul__aubd.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, kcul__aubd.name,
        c.env_manager)
    args = c.pyapi.tuple_pack([puma__kkrs])
    kws = c.pyapi.dict_pack([('name', bholz__liop)])
    ydtkw__gakm = c.pyapi.object_getattr_string(lldlg__srd, 'IntervalIndex')
    ysxhd__obx = c.pyapi.call(ydtkw__gakm, args, kws)
    c.pyapi.decref(puma__kkrs)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(lldlg__srd)
...
def init_interval_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        eiqmw__phnkg, lmq__iws = args
        kcul__aubd = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        kcul__aubd.data = eiqmw__phnkg
        kcul__aubd.name = lmq__iws
        context.nrt.incref(builder, signature.args[0], eiqmw__phnkg)
        context.nrt.incref(builder, signature.args[1], lmq__iws)
        dtype = types.UniTuple(data.arr_type.dtype, 2)
        kcul__aubd.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return kcul__aubd._getvalue()
    ann__vwkrn = IntervalIndexType(data, name)
    sig = signature(ann__vwkrn, data, name)
    return sig, codegen
...
class NumericIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(NumericIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def overload_numeric_index_copy(A, name=None, deep=False, dtype=None, names
    =None):
    gmncc__rwyz = idx_typ_to_format_str_map[NumericIndexType].format('copy()')
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', wriv__ytypi, idx_cpy_arg_defaults,
        fn_str=gmncc__rwyz, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def box_numeric_index(typ, val, c):
    ...
    jwf__crgx = c.pyapi.import_module_noblock(amiow__rox)
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, typ.data, tqvfb__uegd.data)
    kcwta__bxbv = c.pyapi.from_native_value(typ.data, tqvfb__uegd.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, tqvfb__uegd.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, tqvfb__uegd.name,
        c.env_manager)
    qjb__pwbu = c.pyapi.make_none()
    uik__gwc = c.pyapi.bool_from_bool(c.context.get_constant(types.bool_, 
        False))
    vbzfz__wdttl = c.pyapi.call_method(jwf__crgx, 'Index', (kcwta__bxbv,
        qjb__pwbu, uik__gwc, bholz__liop))
    c.pyapi.decref(kcwta__bxbv)
    c.pyapi.decref(qjb__pwbu)
    c.pyapi.decref(uik__gwc)
    c.pyapi.decref(bholz__liop)
    c.pyapi.decref(jwf__crgx)
    c.context.nrt.decref(c.builder, typ, val)
    return vbzfz__wdttl
...
def init_numeric_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        assert len(args) == 2
        ixo__hgwp = signature.return_type
        tqvfb__uegd = cgutils.create_struct_proxy(ixo__hgwp)(context, builder)
        tqvfb__uegd.data = args[0]
        tqvfb__uegd.name = args[1]
        context.nrt.incref(builder, ixo__hgwp.data, args[0])
        context.nrt.incref(builder, ixo__hgwp.name_typ, args[1])
        dtype = ixo__hgwp.dtype
        tqvfb__uegd.dict = context.compile_internal(builder, lambda : numba
            .typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return tqvfb__uegd._getvalue()
...
def create_numeric_constructor(func, func_str, default_dtype):
    ...
    def overload_impl(data=None, dtype=None, copy=False, name=None):
        irj__wldvi = dict(dtype=dtype)
        wwb__qvbnt = dict(dtype=None)
        check_unsupported_args(func_str, irj__wldvi, wwb__qvbnt,
            package_name='pandas', module_name='Index')
        if is_overload_false(copy):

            def impl(data=None, dtype=None, copy=False, name=None):
                cnp__wevg = bodo.utils.conversion.coerce_to_ndarray(data)
                augf__dycn = bodo.utils.conversion.fix_arr_dtype(cnp__wevg,
                    np.dtype(default_dtype))
                return bodo.hiframes.pd_index_ext.init_numeric_index(augf__dycn
                    , name)
        else:
...
class StringIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(StringIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
class BinaryIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', binary_array_type), ('name', fe_type.
            name_typ), ('dict', types.DictType(bytes_type, types.int64))]
        super(BinaryIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj)
...
def unbox_binary_str_index(typ, val, c):
    imxcp__umbx = typ.data
    scalar_type = typ.data.dtype
    eyg__ywktv = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(imxcp__umbx, eyg__ywktv).value
    bholz__liop = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, bholz__liop).value
    c.pyapi.decref(eyg__ywktv)
    c.pyapi.decref(bholz__liop)
...
def box_binary_str_index(typ, val, c):
    ...
    jwf__crgx = c.pyapi.import_module_noblock(amiow__rox)
    tqvfb__uegd = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, imxcp__umbx, tqvfb__uegd.data)
    kcwta__bxbv = c.pyapi.from_native_value(imxcp__umbx, tqvfb__uegd.data,
        c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, tqvfb__uegd.name)
    bholz__liop = c.pyapi.from_native_value(typ.name_typ, tqvfb__uegd.name,
        c.env_manager)
...
def init_binary_str_index(typingctx, data, name=None):
    ...
    sig = type(bodo.utils.typing.get_index_type_from_dtype(data.dtype))(name,
        data)(data, name)
    hvt__fgfv = get_binary_str_codegen(is_binary=data.dtype == bytes_type)
    return sig, hvt__fgfv
...
def get_binary_str_codegen(is_binary=False):
    if is_binary:
        ...
        ofe__xrh = 'string_type'
    xoirt__eeif = 'def impl(context, builder, signature, args):\n'
    xoirt__eeif += '    assert len(args) == 2\n'
    xoirt__eeif += '    index_typ = signature.return_type\n'
    xoirt__eeif += (
        '    index_val = cgutils.create_struct_proxy(index_typ)(context, builder)\n'
        )
    xoirt__eeif += '    index_val.data = args[0]\n'
    xoirt__eeif += '    index_val.name = args[1]\n'
    xoirt__eeif += '    # increase refcount of stored values\n'
    xoirt__eeif += (
        '    context.nrt.incref(builder, signature.args[0], args[0])\n')
    xoirt__eeif += (
        '    context.nrt.incref(builder, index_typ.name_typ, args[1])\n')
    xoirt__eeif += '    # create empty dict for get_loc hashmap\n'
    xoirt__eeif += '    index_val.dict = context.compile_internal(\n'
    xoirt__eeif += '       builder,\n'
    xoirt__eeif += (
        f'       lambda: numba.typed.Dict.empty({ofe__xrh}, types.int64),\n')
    xoirt__eeif += f'        types.DictType({ofe__xrh}, types.int64)(), [],)\n'
    xoirt__eeif += '    return index_val._getvalue()\n'
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo, 'signature': signature, 'cgutils':
        cgutils, 'numba': numba, 'types': types, 'bytes_type': bytes_type,
        'string_type': string_type}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def overload_binary_string_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    typ = type(A)
    gmncc__rwyz = idx_typ_to_format_str_map[typ].format('copy()')
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', wriv__ytypi, idx_cpy_arg_defaults,
        fn_str=gmncc__rwyz, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def _verify_setop_compatible(func_name, I, other):
    ...
    oldvz__kihqj = I.dtype if not isinstance(I, RangeIndexType
        ) else types.int64
    elvv__quk = other.dtype if not isinstance(other, RangeIndexType
        ) else types.int64
    if oldvz__kihqj != elvv__quk:
        raise BodoError(
            f'Index.{func_name}(): incompatible types {oldvz__kihqj} and {elvv__quk}'
            )
...
def overload_index_union(I, other, sort=None):
    gvak__vbjx = dict(sort=sort)
    jcnac__bem = dict(sort=None)
    check_unsupported_args('Index.union', gvak__vbjx, jcnac__bem,
        package_name='pandas', module_name='Index')
    _verify_setop_compatible('union', I, other)
    phd__gbg = get_index_constructor(I) if not isinstance(I, RangeIndexType
        ) else init_numeric_index

    def impl(I, other, sort=None):
        yro__kld = bodo.utils.conversion.coerce_to_array(I)
        xrtng__ykszl = bodo.utils.conversion.coerce_to_array(other)
        khfgh__mbnx = bodo.libs.array_kernels.concat([yro__kld, xrtng__ykszl])
        lckc__pco = bodo.libs.array_kernels.unique(khfgh__mbnx)
        return phd__gbg(lckc__pco, None)
    return impl
...
def overload_index_intersection(I, other, sort=None):
    gvak__vbjx = dict(sort=sort)
    jcnac__bem = dict(sort=None)
    check_unsupported_args('Index.intersection', gvak__vbjx, jcnac__bem,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        yro__kld = bodo.utils.conversion.coerce_to_array(I)
        xrtng__ykszl = bodo.utils.conversion.coerce_to_array(other)
        nrm__isl = bodo.libs.array_kernels.unique(yro__kld)
        igzl__xcawq = bodo.libs.array_kernels.unique(xrtng__ykszl)
        khfgh__mbnx = bodo.libs.array_kernels.concat([nrm__isl, igzl__xcawq])
        nsg__oiaur = pd.Series(khfgh__mbnx).sort_values().values
        rqp__vfyt = bodo.libs.array_kernels.intersection_mask(nsg__oiaur)
        return phd__gbg(nsg__oiaur[rqp__vfyt], None)
    return impl
...
def overload_index_difference(I, other, sort=None):
    gvak__vbjx = dict(sort=sort)
    jcnac__bem = dict(sort=None)
    check_unsupported_args('Index.difference', gvak__vbjx, jcnac__bem,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        ...
        nrm__isl = bodo.libs.array_kernels.unique(yro__kld)
        igzl__xcawq = bodo.libs.array_kernels.unique(xrtng__ykszl)
        rqp__vfyt = np.empty(len(nrm__isl), np.bool_)
        bodo.libs.array.array_isin(rqp__vfyt, nrm__isl, igzl__xcawq, False)
        return phd__gbg(nrm__isl[~rqp__vfyt], None)
    return impl
...
def overload_index_symmetric_difference(I, other, result_name=None, sort=None):
    gvak__vbjx = dict(result_name=result_name, sort=sort)
    jcnac__bem = dict(result_name=None, sort=None)
    check_unsupported_args('Index.symmetric_difference', gvak__vbjx,
        jcnac__bem, package_name='pandas', module_name='Index')
    ...
    def impl(I, other, result_name=None, sort=None):
        ...
        nrm__isl = bodo.libs.array_kernels.unique(yro__kld)
        igzl__xcawq = bodo.libs.array_kernels.unique(xrtng__ykszl)
        hkdb__lqb = np.empty(len(nrm__isl), np.bool_)
        euv__ahafq = np.empty(len(igzl__xcawq), np.bool_)
        bodo.libs.array.array_isin(hkdb__lqb, nrm__isl, igzl__xcawq, False)
        bodo.libs.array.array_isin(euv__ahafq, igzl__xcawq, nrm__isl, False)
        cmjh__oev = bodo.libs.array_kernels.concat([nrm__isl[~hkdb__lqb],
            igzl__xcawq[~euv__ahafq]])
        return phd__gbg(cmjh__oev, None)
    return impl
...
def overload_index_take(I, indices, axis=0, allow_fill=True, fill_value=None):
    gvak__vbjx = dict(axis=axis, allow_fill=allow_fill, fill_value=fill_value)
    jcnac__bem = dict(axis=0, allow_fill=True, fill_value=None)
    check_unsupported_args('Index.take', gvak__vbjx, jcnac__bem,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, ban_unique=True):
            if len(I) > 0 and not I._dict:
                gasb__lzfuv = bodo.utils.conversion.coerce_to_array(I)
                for i in range(len(gasb__lzfuv)):
                    if not bodo.libs.array_kernels.isna(gasb__lzfuv, i):
                        val = (bodo.hiframes.pd_categorical_ext.
                            get_code_for_value(gasb__lzfuv.dtype,
                            gasb__lzfuv[i]))
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, val):
            ...
            if not is_null_value(I._dict):
                ...
                gasb__lzfuv = bodo.utils.conversion.coerce_to_array(I)
                lbjs__woda = (bodo.hiframes.pd_categorical_ext.
                    get_code_for_value(gasb__lzfuv.dtype, key))
                return lbjs__woda in I._dict
            else:
                aberv__wvmy = (
                    'Global Index objects can be slow (pass as argument to JIT function for better performance).'
                    )
                warnings.warn(aberv__wvmy)
                gasb__lzfuv = bodo.utils.conversion.coerce_to_array(I)
                ind = -1
                for i in range(len(gasb__lzfuv)):
                    if not bodo.libs.array_kernels.isna(gasb__lzfuv, i):
                        if gasb__lzfuv[i] == key:
                            ind = i
            return ind != -1
        return impl
...
def overload_index_get_loc(I, key, method=None, tolerance=None):
    gvak__vbjx = dict(method=method, tolerance=tolerance)
    ksx__mffps = dict(method=None, tolerance=None)
    check_unsupported_args('Index.get_loc', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
...
def create_isna_specific_method(overload_name):
    ...
    def overload_index_isna_specific_method(I):
        mbdbu__bnwt = overload_name in {'isna', 'isnull'}
        if isinstance(I, RangeIndexType):

            def impl(I):
                numba.parfors.parfor.init_prange()
                xreo__krwv = len(I)
                oiyp__qir = np.empty(xreo__krwv, np.bool_)
                for i in numba.parfors.parfor.internal_prange(xreo__krwv):
                    oiyp__qir[i] = not mbdbu__bnwt
                return oiyp__qir
            return impl
        xoirt__eeif = f"""def impl(I):
    numba.parfors.parfor.init_prange()
    arr = bodo.hiframes.pd_index_ext.get_index_data(I)
    n = len(arr)
    out_arr = np.empty(n, np.bool_)
    for i in numba.parfors.parfor.internal_prange(n):
       out_arr[i] = {'' if mbdbu__bnwt else 'not '}bodo.libs.array_kernels.isna(arr, i)
    return out_arr
"""
        vepol__tzpj = {}
        exec(xoirt__eeif, {'bodo': bodo, 'np': np, 'numba': numba}, vepol__tzpj
            )
        impl = vepol__tzpj['impl']
        return impl
...
def overload_index_is_montonic(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(gasb__lzfuv, 1)
        return impl
...
def overload_index_is_montonic_decreasing(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(gasb__lzfuv, 2)
        return impl
    elif isinstance(I, RangeIndexType):
...
def overload_index_duplicated(I, keep='first'):
    ...
    def impl(I, keep='first'):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        oiyp__qir = bodo.libs.array_kernels.duplicated((gasb__lzfuv,))
        return oiyp__qir
    return impl
...
def overload_index_drop_duplicates(I, keep='first'):
    gvak__vbjx = dict(keep=keep)
    ksx__mffps = dict(keep='first')
    check_unsupported_args('Index.drop_duplicates', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        return lambda I, keep='first': I.copy()
    xoirt__eeif = """def impl(I, keep='first'):
...
"""
    if isinstance(I, PeriodIndexType):
        xoirt__eeif += f"""    return bodo.hiframes.pd_index_ext.init_period_index(arr, name, '{I.freq}')
"""
    else:
        xoirt__eeif += (
            '    return bodo.utils.conversion.index_from_array(arr, name)')
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def get_index_data_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 1 and not kws
    wbpx__jcwz = args[0]
    if isinstance(self.typemap[wbpx__jcwz.name], (HeterogeneousIndexType,
        MultiIndexType)):
        return None
    if equiv_set.has_shape(wbpx__jcwz):
        return ArrayAnalysis.AnalyzeResult(shape=wbpx__jcwz, pre=[])
...
def overload_index_map(I, mapper, na_action=None):
    ...
    gvak__vbjx = dict(na_action=na_action)
    tbfr__kjfl = dict(na_action=None)
    check_unsupported_args('Index.map', gvak__vbjx, tbfr__kjfl,
        package_name='pandas', module_name='Index')
    dtype = I.dtype
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'DatetimeIndex.map')
    ...
    gtot__hzne = numba.core.registry.cpu_target.typing_context
    kzg__xwfk = numba.core.registry.cpu_target.target_context
    try:
        kmjt__plr = get_const_func_output_type(mapper, (dtype,), {},
            gtot__hzne, kzg__xwfk)
    except Exception as vcy__bttv:
        raise_bodo_error(get_udf_error_msg('Index.map()', vcy__bttv))
    rhxn__rrc = get_udf_out_arr_type(kmjt__plr)
    func = get_overload_const_func(mapper, None)
    xoirt__eeif = 'def f(I, mapper, na_action=None):\n'
    xoirt__eeif += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    xoirt__eeif += '  A = bodo.utils.conversion.coerce_to_array(I)\n'
    xoirt__eeif += '  numba.parfors.parfor.init_prange()\n'
    xoirt__eeif += '  n = len(A)\n'
    xoirt__eeif += '  S = bodo.utils.utils.alloc_type(n, _arr_typ, (-1,))\n'
    xoirt__eeif += '  for i in numba.parfors.parfor.internal_prange(n):\n'
    xoirt__eeif += '    t2 = bodo.utils.conversion.box_if_dt64(A[i])\n'
    xoirt__eeif += '    v = map_func(t2)\n'
    xoirt__eeif += '    S[i] = bodo.utils.conversion.unbox_if_timestamp(v)\n'
    xoirt__eeif += '  return bodo.utils.conversion.index_from_array(S, name)\n'
    ehsl__zmdq = bodo.compiler.udf_jit(func)
    vepol__tzpj = {}
    exec(xoirt__eeif, {'numba': numba, 'np': np, 'pd': pd, 'bodo': bodo,
        'map_func': ehsl__zmdq, '_arr_typ': rhxn__rrc, 'init_nested_counts':
        bodo.utils.indexing.init_nested_counts, 'add_nested_counts': bodo.
        utils.indexing.add_nested_counts, 'data_arr_type': rhxn__rrc.dtype},
        vepol__tzpj)
    f = vepol__tzpj['f']
    return f
...
def index_is(context, builder, sig, args):
    ama__plt, kzr__znj = sig.args
    if ama__plt != kzr__znj:
...
"""
            if rhs in [bodo.hiframes.pd_timestamp_ext.pd_timestamp_type,
                ...
                xoirt__eeif += """  dt = bodo.utils.conversion.unbox_if_timestamp(rhs)
  return op(arr, dt)
"""
            else:
                xoirt__eeif += """  rhs_arr = bodo.utils.conversion.get_array_if_series_or_index(rhs)
  return op(arr, rhs_arr)
"""
            vepol__tzpj = {}
            exec(xoirt__eeif, {'bodo': bodo, 'op': op}, vepol__tzpj)
            impl = vepol__tzpj['impl']
            return impl
...
"""
            if lhs in [bodo.hiframes.pd_timestamp_ext.pd_timestamp_type,
                bodo.hiframes.pd_timestamp_ext.pd_timedelta_type]:
                xoirt__eeif += """  dt = bodo.utils.conversion.unbox_if_timestamp(lhs)
  return op(dt, arr)
"""
            else:
                xoirt__eeif += """  lhs_arr = bodo.utils.conversion.get_array_if_series_or_index(lhs)
        ...
        if isinstance(lhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(lhs.data):
                ...
                def impl3(lhs, rhs):
                    ...
                    gasb__lzfuv = bodo.utils.conversion.coerce_to_array(data)
                    ygunr__sopn = (bodo.utils.conversion.
                        get_array_if_series_or_index(rhs))
                    oiyp__qir = op(gasb__lzfuv, ygunr__sopn)
                    return oiyp__qir
                return impl3
            count = len(lhs.data.types)
            xoirt__eeif = 'def f(lhs, rhs):\n'
            xoirt__eeif += '  return [{}]\n'.format(','.join(
                'op(lhs[{}], rhs{})'.format(i, f'[{i}]' if is_iterable_type
                (rhs) else '') for i in range(count)))
            vepol__tzpj = {}
            exec(xoirt__eeif, {'op': op, 'np': np}, vepol__tzpj)
            impl = vepol__tzpj['f']
            return impl
        if isinstance(rhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(rhs.data):
                ...
                def impl4(lhs, rhs):
                    ...
                    oiyp__qir = op(ygunr__sopn, gasb__lzfuv)
                    return oiyp__qir
                return impl4
...
class HeterogeneousIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oikf__xbbcj = [('data', fe_type.data), ('name', fe_type.name_typ)]
        super(HeterogeneousIndexModel, self).__init__(dmm, fe_type, oikf__xbbcj
            )
...
def overload_heter_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    gmncc__rwyz = idx_typ_to_format_str_map[HeterogeneousIndexType].format(
        'copy()')
    wriv__ytypi = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', wriv__ytypi, idx_cpy_arg_defaults,
        fn_str=gmncc__rwyz, package_name='pandas', module_name='Index')
...
def overload_nbytes(I):
    if isinstance(I, RangeIndexType):
        ...
        xoirt__eeif = 'def _impl_nbytes(I):\n'
        xoirt__eeif += '    total = 0\n'
        xoirt__eeif += '    data = I._data\n'
        for i in range(I.nlevels):
            xoirt__eeif += f'    total += data[{i}].nbytes\n'
        xoirt__eeif += '    return total\n'
        hbkf__iikbr = {}
        exec(xoirt__eeif, {}, hbkf__iikbr)
        return hbkf__iikbr['_impl_nbytes']
    else:
...
def overload_index_to_series(I, index=None, name=None):
    ...
    if is_overload_none(name):
        psq__iqw = 'bodo.hiframes.pd_index_ext.get_index_name(I)'
    else:
        psq__iqw = 'name'
    xoirt__eeif = 'def impl(I, index=None, name=None):\n'
    xoirt__eeif += '    data = bodo.utils.conversion.index_to_array(I)\n'
    if is_overload_none(index):
        xoirt__eeif += '    new_index = I\n'
    elif is_pd_index_type(index):
        xoirt__eeif += '    new_index = index\n'
    ...
    xoirt__eeif += f'    new_name = {psq__iqw}\n'
    xoirt__eeif += (
        '    return bodo.hiframes.pd_series_ext.init_series(data, new_index, new_name)'
        )
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo, 'np': np}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def overload_index_to_frame(I, index=True, name=None):
    ...
    xoirt__eeif = 'def impl(I, index=True, name=None):\n'
    xoirt__eeif += '    data = bodo.utils.conversion.index_to_array(I)\n'
    xoirt__eeif += f'    new_index = {plafw__fewc}\n'
    if is_overload_none(name) and I.name_typ == types.none:
        zthvi__hum = ColNamesMetaType((0,))
    elif is_overload_none(name):
        zthvi__hum = ColNamesMetaType((I.name_typ,))
    elif is_overload_constant_str(name):
        zthvi__hum = ColNamesMetaType((get_overload_const_str(name),))
    elif is_overload_constant_int(name):
        zthvi__hum = ColNamesMetaType((get_overload_const_int(name),))
    else:
        raise_bodo_error(
            f'Index.to_frame(): only constant string/int are supported for argument name'
            )
    xoirt__eeif += """    return bodo.hiframes.pd_dataframe_ext.init_dataframe((data,), new_index, __col_name_meta_value)
"""
    vepol__tzpj = {}
    exec(xoirt__eeif, {'bodo': bodo, 'np': np, '__col_name_meta_value':
        zthvi__hum}, vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def overload_multi_index_to_frame(I, index=True, name=None):
    ...
    xoirt__eeif = 'def impl(I, index=True, name=None):\n'
    xoirt__eeif += '    data = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    xoirt__eeif += f'    new_index = {plafw__fewc}\n'
    fkc__hqs = len(I.array_types)
    if is_overload_none(name) and I.names_typ == (types.none,) * fkc__hqs:
        zthvi__hum = ColNamesMetaType(tuple(range(fkc__hqs)))
    elif is_overload_none(name):
        zthvi__hum = ColNamesMetaType(I.names_typ)
        ...
        if fkc__hqs != len(names):
            raise_bodo_error(
                f'MultiIndex.to_frame(): expected {fkc__hqs} names, not {len(names)}'
                )
        if all(is_overload_constant_str(qvt__ahh) or
            is_overload_constant_int(qvt__ahh) for qvt__ahh in names):
            zthvi__hum = ColNamesMetaType(names)
        else:
            raise_bodo_error(
...
def overload_index_to_numpy(I, dtype=None, copy=False, na_value=None):
    gvak__vbjx = dict(dtype=dtype, na_value=na_value)
    ksx__mffps = dict(dtype=None, na_value=None)
    check_unsupported_args('Index.to_numpy', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
...
def overload_inferred_type(I):
    ...
    if isinstance(I, StringIndexType):
        ...
        def impl(I):
            ...
            return 'string'
        return impl
    nhcha__rsggi = {DatetimeIndexType: 'datetime64', TimedeltaIndexType:
    ...
    inferred_type = nhcha__rsggi[type(I)]
...
def overload_inferred_type(I):
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        return lambda I: dtype
    iyj__vsyh = {DatetimeIndexType: np.dtype('datetime64[ns]'),
...
def init_index_from_index(I, name):
    kmojs__gtp = {NumericIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in kmojs__gtp:
        init_func = kmojs__gtp[type(I)]
...
def get_index_constructor(I):
    wsuoz__hwrlh = {NumericIndexType: bodo.hiframes.pd_index_ext.
        init_numeric_index, DatetimeIndexType: bodo.hiframes.pd_index_ext.
        init_datetime_index, TimedeltaIndexType: bodo.hiframes.pd_index_ext
        .init_timedelta_index, StringIndexType: bodo.hiframes.pd_index_ext.
        init_binary_str_index, BinaryIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in wsuoz__hwrlh:
        return wsuoz__hwrlh[type(I)]
    raise BodoError(
...
def overload_index_min(I, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('Index.min', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        ...
        def impl(I, axis=None, skipna=True):
            ...
            if ejo__yjrb == 0:
                return np.nan
            if I._step < 0:
                return I._start + I._step * (ejo__yjrb - 1)
            else:
                return I._start
        return impl
    if isinstance(I, CategoricalIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_min(gasb__lzfuv)
    return impl
...
def overload_index_max(I, axis=None, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=None, skipna=True)
    check_unsupported_args('Index.max', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_max(gasb__lzfuv)
    return impl
...
def overload_index_argmin(I, axis=0, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmin', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmin()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = init_numeric_index(np.arange(len(gasb__lzfuv)))
        return bodo.libs.array_ops.array_op_idxmin(gasb__lzfuv, index)
    return impl
...
def overload_index_argmax(I, axis=0, skipna=True):
    gvak__vbjx = dict(axis=axis, skipna=skipna)
    ksx__mffps = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmax', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmax()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = np.arange(len(gasb__lzfuv))
        return bodo.libs.array_ops.array_op_idxmax(gasb__lzfuv, index)
    return impl
...
def overload_index_unique(I):
    phd__gbg = get_index_constructor(I)

    def impl(I):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        zkowr__srxvu = bodo.libs.array_kernels.unique(gasb__lzfuv)
        return phd__gbg(zkowr__srxvu, name)
    return impl
...
def overload_index_nunique(I, dropna=True):
    ...
    def impl(I, dropna=True):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        xreo__krwv = bodo.libs.array_kernels.nunique(gasb__lzfuv, dropna)
        return xreo__krwv
    return impl
...
def overload_index_isin(I, values):
    if bodo.utils.utils.is_array_typ(values):
        ...
        def impl_arr(I, values):
            apxxo__fkcyl = bodo.utils.conversion.coerce_to_array(values)
            A = bodo.hiframes.pd_index_ext.get_index_data(I)
            xreo__krwv = len(A)
            oiyp__qir = np.empty(xreo__krwv, np.bool_)
            bodo.libs.array.array_isin(oiyp__qir, A, apxxo__fkcyl, False)
            return oiyp__qir
        return impl_arr
    ...
    def impl(I, values):
        A = bodo.hiframes.pd_index_ext.get_index_data(I)
        oiyp__qir = bodo.libs.array_ops.array_op_isin(A, values)
        return oiyp__qir
    return impl
...
def order_range(I, ascending):
    ...
    if ascending == (step > 0):
        ...
        ejo__yjrb = len(I)
        lnpr__tltmi = start + step * (ejo__yjrb - 1)
        mea__aikpk = lnpr__tltmi - step * ejo__yjrb
        return init_range_index(lnpr__tltmi, mea__aikpk, -step, name)
...
def overload_index_sort_values(I, return_indexer=False, ascending=True,
    ...
    gvak__vbjx = dict(return_indexer=return_indexer, key=key)
    ksx__mffps = dict(return_indexer=False, key=None)
    check_unsupported_args('Index.sort_values', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, RangeIndexType):
        ...
        return impl
    phd__gbg = get_index_constructor(I)
    lhfqz__bagkk = ColNamesMetaType(('$_bodo_col_',))

    def impl(I, return_indexer=False, ascending=True, na_position='last',
        key=None):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = get_index_name(I)
        index = init_range_index(0, len(gasb__lzfuv), 1, None)
        gdvur__kqeyy = bodo.hiframes.pd_dataframe_ext.init_dataframe((
            gasb__lzfuv,), index, lhfqz__bagkk)
        nmoof__svdoy = gdvur__kqeyy.sort_values(['$_bodo_col_'], ascending=
            ascending, inplace=False, na_position=na_position)
        oiyp__qir = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(
            nmoof__svdoy, 0)
        return phd__gbg(oiyp__qir, name)
    return impl
...
def overload_index_argsort(I, axis=0, kind='quicksort', order=None):
    gvak__vbjx = dict(axis=axis, kind=kind, order=order)
    ksx__mffps = dict(axis=0, kind='quicksort', order=None)
    check_unsupported_args('Index.argsort', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, kind='quicksort', order=None):
        gasb__lzfuv = bodo.hiframes.pd_index_ext.get_index_data(I)
        oiyp__qir = bodo.hiframes.series_impl.argsort(gasb__lzfuv)
        return oiyp__qir
    return impl
...
def overload_index_where(I, cond, other=np.nan):
    ...
    if is_overload_constant_nan(other):
        ...
        bnx__oru = 'other'
    xoirt__eeif = 'def impl(I, cond, other=np.nan):\n'
    if isinstance(I, RangeIndexType):
        xoirt__eeif += '  arr = np.arange(I._start, I._stop, I._step)\n'
        phd__gbg = 'init_numeric_index'
    else:
        xoirt__eeif += '  arr = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    xoirt__eeif += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    xoirt__eeif += (
        f'  out_arr = bodo.hiframes.series_impl.where_impl(cond, arr, {bnx__oru})\n'
        )
    xoirt__eeif += f'  return constructor(out_arr, name)\n'
    vepol__tzpj = {}
    phd__gbg = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(xoirt__eeif, {'bodo': bodo, 'np': np, 'constructor': phd__gbg},
        vepol__tzpj)
    impl = vepol__tzpj['impl']
    return impl
...
def overload_index_putmask(I, cond, other):
    ...
    if is_overload_constant_nan(other):
        ...
        bnx__oru = 'other'
    xoirt__eeif = 'def impl(I, cond, other):\n'
    xoirt__eeif += '  cond = ~cond\n'
    if isinstance(I, RangeIndexType):
        xoirt__eeif += '  arr = np.arange(I._start, I._stop, I._step)\n'
    else:
        xoirt__eeif += '  arr = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
...
def overload_index_repeat(I, repeats, axis=None):
    gvak__vbjx = dict(axis=axis)
    ksx__mffps = dict(axis=None)
    check_unsupported_args('Index.repeat', gvak__vbjx, ksx__mffps,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.repeat()')
    ...
    xoirt__eeif = 'def impl(I, repeats, axis=None):\n'
    ...
    xoirt__eeif += '    return constructor(out_arr, name)'
    vepol__tzpj = {}
    phd__gbg = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(xoirt__eeif, {'bodo': bodo, 'np': np, 'constructor': phd__gbg},
...
def lower_constant_time_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    dtype = ty.dtype
    hzoq__mcxo = context.get_constant_null(types.DictType(dtype, types.int64))
    return lir.Constant.literal_struct([data, name, hzoq__mcxo])
...
def lower_constant_period_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    hzoq__mcxo = context.get_constant_null(types.DictType(types.int64,
        types.int64))
    return lir.Constant.literal_struct([data, name, hzoq__mcxo])
...
def lower_constant_binary_string_index(context, builder, ty, pyval):
    ...
    data = context.get_constant_generic(builder, imxcp__umbx, pyval.values)
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    hzoq__mcxo = context.get_constant_null(types.DictType(scalar_type,
        types.int64))
    return lir.Constant.literal_struct([data, name, hzoq__mcxo])
...
def getiter_range_index(context, builder, sig, args):
    [aamz__nptd] = sig.args
    [index] = args
    krotm__wifrl = context.make_helper(builder, aamz__nptd, value=index)
    fxrc__pfvlh = context.make_helper(builder, sig.return_type)
    oxt__wmke = cgutils.alloca_once_value(builder, krotm__wifrl.start)
    bvx__apesa = context.get_constant(types.intp, 0)
    tapd__qso = cgutils.alloca_once_value(builder, bvx__apesa)
    fxrc__pfvlh.iter = oxt__wmke
    fxrc__pfvlh.stop = krotm__wifrl.stop
    fxrc__pfvlh.step = krotm__wifrl.step
    fxrc__pfvlh.count = tapd__qso
    kpf__pwumq = builder.sub(krotm__wifrl.stop, krotm__wifrl.start)
    dawgl__syza = context.get_constant(types.intp, 1)
    ouj__sqvl = builder.icmp_signed('>', kpf__pwumq, bvx__apesa)
    xxk__baqn = builder.icmp_signed('>', krotm__wifrl.step, bvx__apesa)
    galyp__rik = builder.not_(builder.xor(ouj__sqvl, xxk__baqn))
    with builder.if_then(galyp__rik):
        wza__wvyrq = builder.srem(kpf__pwumq, krotm__wifrl.step)
        wza__wvyrq = builder.select(ouj__sqvl, wza__wvyrq, builder.neg(
            wza__wvyrq))
        jxgqi__gzk = builder.icmp_signed('>', wza__wvyrq, bvx__apesa)
        mqld__spiyg = builder.add(builder.sdiv(kpf__pwumq, krotm__wifrl.
            step), builder.select(jxgqi__gzk, dawgl__syza, bvx__apesa))
        builder.store(mqld__spiyg, tapd__qso)
    ysxhd__obx = fxrc__pfvlh._getvalue()
    vsvk__anct = impl_ret_new_ref(context, builder, sig.return_type, ysxhd__obx
        )
    return vsvk__anct
...
def _install_index_unsupported():
    for lfwhi__rcgdm in index_unsupported_methods:
        for ccguw__nikf, typ in index_types:
            overload_method(typ, lfwhi__rcgdm, no_unliteral=True)(
                create_unsupported_overload(ccguw__nikf.format(lfwhi__rcgdm +
                '()')))
    for dxcu__tujw in index_unsupported_atrs:
        for ccguw__nikf, typ in index_types:
            overload_attribute(typ, dxcu__tujw, no_unliteral=True)(
                create_unsupported_overload(ccguw__nikf.format(dxcu__tujw)))
    yoid__afvh = [(StringIndexType, string_index_unsupported_atrs), (
    ...
    ycz__mgi = [(CategoricalIndexType, cat_idx_unsupported_methods), (
    ...
    for typ, kmf__djzb in ycz__mgi:
        ccguw__nikf = idx_typ_to_format_str_map[typ]
        for zdso__kih in kmf__djzb:
            overload_method(typ, zdso__kih, no_unliteral=True)(
                create_unsupported_overload(ccguw__nikf.format(zdso__kih +
                '()')))
    for typ, mme__cvlpj in yoid__afvh:
        ccguw__nikf = idx_typ_to_format_str_map[typ]
        for dxcu__tujw in mme__cvlpj:
            overload_attribute(typ, dxcu__tujw, no_unliteral=True)(
                create_unsupported_overload(ccguw__nikf.format(dxcu__tujw)))
