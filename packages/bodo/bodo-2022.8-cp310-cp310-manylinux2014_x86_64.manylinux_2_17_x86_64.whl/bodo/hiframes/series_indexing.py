...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        fzxj__gkcs = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(fzxj__gkcs)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        yxg__zfj = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, yxg__zfj)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        uqui__xarpw, = args
        wqp__ozz = signature.return_type
        yegk__bkz = cgutils.create_struct_proxy(wqp__ozz)(context, builder)
        yegk__bkz.obj = uqui__xarpw
        context.nrt.incref(builder, signature.args[0], uqui__xarpw)
        return yegk__bkz._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ojoek__ohs = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        fzxj__gkcs = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(fzxj__gkcs)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        yxg__zfj = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, yxg__zfj)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        uqui__xarpw, = args
        ldpfm__ieuvt = signature.return_type
        moq__qmq = cgutils.create_struct_proxy(ldpfm__ieuvt)(context, builder)
        moq__qmq.obj = uqui__xarpw
        context.nrt.incref(builder, signature.args[0], uqui__xarpw)
        return moq__qmq._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                odkz__dqad = bodo.utils.conversion.coerce_to_ndarray(idx)
                ifyyq__zwee = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    odkz__dqad]
                rmknw__jrkjy = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    odkz__dqad]
                fzxj__gkcs = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(ifyyq__zwee,
                    rmknw__jrkjy, fzxj__gkcs)
            return impl
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        odkz__dqad = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ojoek__ohs = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ojoek__ohs))
                        odkz__dqad = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            odkz__dqad] = s
            ...
            def impl_arr(I, idx, val):
                odkz__dqad = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[odkz__dqad
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        fzxj__gkcs = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(fzxj__gkcs)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        yxg__zfj = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, yxg__zfj)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        uqui__xarpw, = args
        iwh__puxfd = signature.return_type
        gvmbn__ssewi = cgutils.create_struct_proxy(iwh__puxfd)(context, builder
            )
        gvmbn__ssewi.obj = uqui__xarpw
        context.nrt.incref(builder, signature.args[0], uqui__xarpw)
        return gvmbn__ssewi._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            ifyyq__zwee = bodo.hiframes.pd_series_ext.get_series_data(S)
            rmknw__jrkjy = bodo.hiframes.pd_series_ext.get_series_index(S)
            ktj__rqm = (idx - rmknw__jrkjy._start) // rmknw__jrkjy._step
            return ifyyq__zwee[ktj__rqm]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    odkz__dqad = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(ifyyq__zwee[
                        odkz__dqad])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    wwf__xfn = bodo.hiframes.pd_series_ext.get_series_index(S)
                    cjyhz__uit = wwf__xfn.get_loc(idx)
                    if cjyhz__uit is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(cjyhz__uit)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                odkz__dqad = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[odkz__dqad
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        jehs__qbntc = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(hpyx__xmbv,
            int) for hpyx__xmbv in jehs__qbntc):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            dofer__qdxq = get_literal_value(idx)
            if dofer__qdxq not in jehs__qbntc:
                raise_bodo_error(
                    f"Series label-based getitem: '{dofer__qdxq}' not in {jehs__qbntc}"
                    )
            arr_ind = jehs__qbntc.index(dofer__qdxq)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    yegk__bkz = cgutils.create_struct_proxy(fromty)(context, builder, val)
    fgzq__asopf = context.cast(builder, yegk__bkz.obj, fromty.stype, toty.stype
        )
    rkloj__qsbp = cgutils.create_struct_proxy(toty)(context, builder)
    rkloj__qsbp.obj = fgzq__asopf
    return rkloj__qsbp._getvalue()
