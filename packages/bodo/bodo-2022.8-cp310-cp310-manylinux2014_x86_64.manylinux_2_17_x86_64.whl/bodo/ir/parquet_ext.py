...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    dkxe__axag = pq_node.out_vars[0].name
    rtrr__xuuu = pq_node.out_vars[1].name
    if dkxe__axag not in lives and rtrr__xuuu not in lives:
        return None
    elif dkxe__axag not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
        pq_node.is_live_table = False
    elif rtrr__xuuu not in lives:
