...
def nornir_init(cloud_index, region):
    """ 
    NORNIR INITIALIZE
    Takes:
        cloud_index: index of cloud provider, currently GCP or AWS only
    ...
    if cloud_index == 0:    # GCP
        berglas_nornir_passwords(nr)
    elif cloud_index == 1:    # AWS
...
def load_secrets(norn):
    ...
    for host in norn.inventory.hosts:
        ...
        if norn.inventory.hosts[host].data['cloud'] == "gcp":
            password = get_berglas_passwords(host)
            norn.inventory.hosts[host].password = password
        elif norn.inventory.hosts[host].data['cloud'] == "aws":
            password = get_aws_passwords(host, norn.inventory.hosts[host].data['region'])
            norn.inventory.hosts[host].password = password
    
    return norn


def get_berglas_passwords(host):
    """ 
...
def get_aws_passwords(host, region):
    """ 
    GET AWS PASSWORD
    Takes:
        host: Singular host from an initialized Nornir object
