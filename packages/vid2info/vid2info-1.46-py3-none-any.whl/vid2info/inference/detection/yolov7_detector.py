...
class YoloV7Detector(__YoloDetector):
    def __init__(self, weights : str = WEIGTHS_PATH, dataset_yml : str | None = DATASET_YML,
        ...
        self.model = torch.load(weights.replace('yolov6n', 'yolov7'), map_location=self.inference_device)  # load
        self.model = self.model['ema' if self.model.get('ema') else 'model'].float().eval()
        self.prepare_model()
