...
class StateBuffer:
    ...
    def get_element_state_machine(self, track_id: int, current_class: str | None = None, bbox_xyxy: np.ndarray | None = None,
                                  recover_default_machines: bool = True) -> ElementFiniteStateMachine:
        """
    ...
    def recover_track_id(self, new_track_id: int, class_name: str, bbox_xyxy: np.ndarray,
                         max_time_difference_in_seconds: float | int = 10.0) -> int | None:
        """
        ...
        """
        current_time = time()
        bbox_center = (bbox_xyxy[0] + bbox_xyxy[2]) / 2, (bbox_xyxy[1] + bbox_xyxy[3]) / 2
        ...
        for track_id, element_info in self.elements_history.items():
            ...
            if element_info[OUT_TIMESTAMP] > current_time - max_time_difference_in_seconds \
                    and element_info[OUT_CLASS_NAME] == class_name:
        ...
        if len(suspicious_track_ids) > 0:
            ...
            suspicious_bbox_centers = []
            for suspicious_track_id in suspicious_track_ids:
                x1, y1, x2, y2 = self.get_last_detection_of_element(track_id=suspicious_track_id).bbox_xyxy
                suspicious_center = ((x1 + x2) / 2, (y1 + y2) / 2)
                suspicious_bbox_centers.append(suspicious_center)
            # Get the closest bbox center
            suspicious_bbox_centers = np.array(suspicious_bbox_centers) - bbox_center
            closest_track_id = np.argmin(np.linalg.norm(suspicious_bbox_centers, axis=1))
            old_track_id = suspicious_track_ids[closest_track_id]
            return old_track_id
