...
def startRSS():
    ...
    def get(name):
        return SC(host="127.0.0.1").request(command="get", data=((name,), {}))
    def update(name, value):
        return SC(host="127.0.0.1").request(command="update", data=((name, value), {}))
    def add_job(t, v):
        return SC(host="127.0.0.1").request(command="add_job", data=((t, v), {}))
    def get_job_result(t):
        r = SC(host="127.0.0.1").request(command="get_job_result", data=((t,), {}))
        if isinstance(r, Exception):
            raise KeyError
        return r
    def get_tabs(*args, **kwargs):
        return SC(host="127.0.0.1").request(command="get_tabs", data=(args, kwargs))
    def set_tab(*args, **kwargs):
        return SC(host="127.0.0.1").request(command="set_tab", data=(args, kwargs))
    def close_tab(*args, **kwargs):
        return SC(host="127.0.0.1").request(command="close_tab", data=(args, kwargs))
    def screenshot(
