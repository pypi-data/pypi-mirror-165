...
def ShowAC(mountNam, cache_path):
    ...
    # noinspection PyUnresolvedReferences
    display(HTML("<br>"), MakeButton("mount 挂载", lambda: MountCMD(mountNam, cache_path), "primary"),
            MakeButton("unmount 卸载", lambda: unmountCMD(mountNam), "danger"))


def MountCMD(mountNam, cache_path, verbose=True):
    mPoint = "/mnt/rdrive/{}".format(mountNam.value)
    try:
        os.makedirs(mPoint)
    except:
        if verbose:
            print("\nfailed to mount 不能挂载 {}\n{} is using 正在使用".format(mountNam.value, mPoint))
        return
    cmd = ("rclone mount {}: {}".format(mountNam.value, mPoint) +
    ...
    if runSh(cmd, shell=True) == 0:
        if verbose:
            print("\nmounted 已挂载 {} on 在 {}".format(mountNam.value, mPoint))
    else:
        if verbose:
            print("\nfailed to mount 不能挂载 {} on 在 {}".format(mountNam.value, mPoint))


def unmountCMD(mountNam, verbose=True):
    mPoint = "/mnt/rdrive/{}".format(mountNam.value)
    ...
    if os.system("fusermount -uz {}".format(mPoint)) == 0:
        ...
        if verbose:
            print("\nunmounted 已卸载 {} on 在 {}".format(mountNam.value, mPoint))
    else:
        runSh("fusermount -uz {}".format(mPoint), output=True)


def mount_unmount_gui():
