...
class Csv():
	...
	def __str__(self):
		return self.lineterminator.join([self.delimiter.join([str(j) for j in i]) for i in self.__main])
	def __len__(self):
