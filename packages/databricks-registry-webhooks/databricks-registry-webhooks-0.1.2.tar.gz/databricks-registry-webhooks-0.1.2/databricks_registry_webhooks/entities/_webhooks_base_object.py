...
class _WebhooksBaseObject(object):
    def __iter__(self):
...
def to_string(obj):
    return _WebhooksBaseObjectPrinter().to_string(obj)
...
class _WebhooksBaseObjectPrinter(object):
    def __init__(self):
        super().__init__()
        self.printer = pprint.PrettyPrinter()

    def to_string(self, obj):
        if isinstance(obj, _WebhooksBaseObject):
