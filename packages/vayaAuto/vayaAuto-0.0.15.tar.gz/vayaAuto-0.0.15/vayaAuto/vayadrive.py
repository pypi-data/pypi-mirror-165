...
class VayaDrive(object):
    ...
    def set_exe_path(self, path):
        ...
        if os.path.isdir(os.path.join(path, build_folder)):
        ...
        if not os.path.isfile(self.exe_path):
            logger.error(f'found exe - {self.exe_path}')
    ...
    def enabled_cameras(self):
        for name, option in self.configuration['Pipe-0'].items():
            # if 'SENSORS__Table' in name and (option.value.split(','))[1] == 'true':
            #     print()
    ...
    def extract_dependencies_linux(self):
        print('extract_dependencies_linux')
        ...
        with open(run_sh, 'r') as f:
            ...
            for line in lines:
                ...
                if 'EXPORT' in line and line.index('EXPORT') == 0:
                    ...
                    line = line.replace('\n', '')
                    depends += line + ':'
        depends += '../Libs/PCL/PCL-1.8.1/lib/ubuntu1804:'
        return depends
