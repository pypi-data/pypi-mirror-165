...
class Metric(ABC):
    ...
    def __call__(self, doc):
        if isinstance(doc, str):
            log.error("Passed string instead of doc.")
        try:
            value, debug = self.count(doc)
        except (LookupError, ArithmeticError) as e:
            value = -1
            debug = {}
            log.error(f"Error calculating metric {self.code}")
...
class CustomMetric(Metric):
    ...
    def __init__(self, name=None):
        if isinstance(name, Callable):
            raise TypeError("Missing decorator parameters.")

        self.name = name
        self.category = "Custom Metrics"
    ...
    def __call__(self, arg):
        if isinstance(arg, Callable):
            ...
            if not self.name:
                self.name = f"Custom Metric {self.code}"
            return self
