...
class RegValue(object):
    ...
    def set(self,value,type=None):
        ...
        If not provided, determines the type by examining the type of `value` - see _determine_value_type
        """
        handle=winreg.CreateKey(self.path.hkey_constant,self.path.path)
        if type is None: type=self._determine_value_type(value)
        value=self._coerce_value_to_type(value,type)
        try:
    ...
    @classmethod
    def _coerce_value_to_type(cls,value,type):
