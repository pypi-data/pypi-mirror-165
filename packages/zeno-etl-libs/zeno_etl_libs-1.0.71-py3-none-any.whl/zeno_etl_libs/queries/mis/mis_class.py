...
class Mis:
    ...
    def power_cons_bills(self, Sales, Stores, power_consumer_value,fofo_tag = 'no'):
        ...
        if fofo_tag == 'no':
            ...
            power_cons_aug = sales.groupby(['store_id', 'order_source','fofo_distributor', 'patient_id'])[['value']].sum().reset_index()
            power_cons_aug1 = power_cons_aug[power_cons_aug['value'] > power_consumer_value]

            df_lp = pd.merge(left=power_cons_aug1[['store_id', 'order_source','fofo_distributor','patient_id']],
                             right=sales,
