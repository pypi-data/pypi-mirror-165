...
parser.add_argument('-met', '--mis_email_to', default="mis.zeno@zeno.health,rupa.sharma@zeno.health,vaibhav.bhawar@zeno.health,saurav.maskar@zeno.health,vijay.pratap@zeno.health", type=str, required=False)
...
try:
    ...
    power_cons_bills = mis.power_cons_bills(sales,stores,power_consumer_value,fofo_tag='yes')
    power_cons_bills_unified = mis.power_cons_bills(sales_unified,stores,power_consumer_value,fofo_tag='yes')

    logger.info('21. - Power consumers - Bills ran successfully')

    breakup_master_fofo = pd.concat([breakup_master_fofo, power_cons_bills], sort=True)
    unified_master_fofo = pd.concat([unified_master_fofo, power_cons_bills_unified], sort=True)
    ...
    cols_to_move = ['tag_flag', 'type1', 'age_bracket_',
                    'payment', 'fofo_distributor','order_source']
    breakup_master_fofo = breakup_master_fofo[ cols_to_move + [col for col in breakup_master_fofo.columns
                                             if col not in cols_to_move]]

    cols_to_move = ['tag_flag', 'type1', 'age_bracket_',
                    'payment', 'fofo_distributor']
