...
patient_data_final = pd.DataFrame()
...
for store in stores:
    ...
    if patient_data.shape[0] == 0:
        continue
    unique_patients = patient_data['patient-id'].unique()

    if len(unique_patients) <= 1:
        unique_patients = np.append(unique_patients, [0, 0])

    logger.info(f'Final Patient promo data size {patient_data.shape} for store {store}')
    patient_data_final = pd.concat([patient_data_final, patient_data])

logger.info(patient_data_final.info())
# data type correction
patient_data_final['bill-value'] = patient_data_final['bill-value'].astype(float)
patient_data_final['promo-discount'] = patient_data_final['promo-discount'].astype(float)
patient_data_final['flat-discount'] = patient_data_final['flat-discount'].fillna(0).astype(int)
patient_data_final['percent-discount'] = patient_data_final['percent-discount'].fillna(0).astype(int)
patient_data_final['max-discount'] = patient_data_final['max-discount'].fillna(0).astype(int)
patient_data_final['min-purchase'] = patient_data_final['min-purchase'].fillna(0).astype(int)
patient_data_final['max-time'] = patient_data_final['max-time'].fillna(0).astype(int)
patient_data_final['max-per-patient'] = patient_data_final['max-per-patient'].fillna(0).astype(int)
patient_data_final['campaign-id'] = patient_data_final['campaign-id'].fillna(0).astype(int)

patient_data_final['hd-flag'] = patient_data_final['hd-flag'].astype(int)
patient_data_final['pr-flag'] = patient_data_final['pr-flag'].astype(int)
patient_data_final['is-generic'] = patient_data_final['is-generic'].astype(int)
patient_data_final['is-chronic'] = patient_data_final['is-chronic'].astype(int)
patient_data_final['is-repeatable'] = patient_data_final['is-repeatable'].astype(int)

patient_data_final['previous-total-trips'] = patient_data_final['previous-total-trips'].fillna(0).astype(int)
patient_data_final['previous-90days-trips'] = patient_data_final['previous-90days-trips'].fillna(0).astype(int)
patient_data_final['pre3trips-promo'] = patient_data_final['pre3trips-promo'].fillna(0).astype(int)
patient_data_final['next-90days-trips'] = patient_data_final['next-90days-trips'].fillna(0).astype(int)
patient_data_final['post3trips-promo'] = patient_data_final['post3trips-promo'].fillna(0).astype(int)
patient_data_final['last-visit-in-days'] = patient_data_final['last-visit-in-days'].fillna(-1).astype(int)
patient_data_final['bill-date'] = pd.to_datetime(patient_data_final['bill-date']).dt.date

# etl
patient_data_final['created-at'] = dt.now(tz=gettz('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S')
patient_data_final['created-by'] = 'etl-automation'
patient_data_final['updated-at'] = dt.now(tz=gettz('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M:%S')
patient_data_final['updated-by'] = 'etl-automation'

# Write to csv
s3.save_df_to_s3(df=patient_data_final[table_info['column_name']], file_name='Shubham_G/43/campaign_prepost.csv')
s3.write_df_to_db(df=patient_data_final[table_info['column_name']], table_name=table_name, db=rs_db, schema=schema)
