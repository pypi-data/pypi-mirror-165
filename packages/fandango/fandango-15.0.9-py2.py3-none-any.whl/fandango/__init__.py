...
deprecated = [
    ...
    p + os.path.sep + "tango.pyo",
    p + os.path.sep + "device.pyc",
    p + os.path.sep + "device.pyo",
]
for p in deprecated:
    if os.path.exists(p):
        try:
...
try:
    ...
    try:
        ...
        deprecated = [
            f
            for f in (
                os.path.join(path, "interface" + ext)
                for ext in (".py", ".pyc", ".pyo", "py~")
            )
            if os.path.exists(f)
        ]
        if deprecated:
            print(
                "fandango.interface: deprecated files still exist: %s"
                % ",".join(deprecated)
            )
            try:
                [os.remove(f) for f in deprecated]
            except:
                print("... and should be removed manually!")
