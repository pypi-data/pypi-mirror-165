...
class Distiller:
    def __init__(
        ...
        self.alpha_act = params.alpha_act
    ...
    def step(
        ...
        if self.alpha_act > 0.0:
            cls_S = s_hidden_states[-1][:, 0, :]
            cls_T = t_hidden_states[-1][:, 0, :]

            margin = 1.0
            loss_alter = self.criterion_alternative_L2(cls_S, cls_T, margin) / 1000
            loss += self.alpha_act * loss_alter
    ...
    def criterion_alternative_L2(self, source, target, margin):
        loss = (source + margin) ** 2 * ((source > -margin) & (target <= 0)).float() + (
            source - margin
        ) ** 2 * ((source <= margin) & (target > 0)).float()
