# -*- coding: utf-8 -*-
"""
@author: Iuliia Baglaeva and Bogdan detrendingIaparov
"""
import numpy as np
import pandas as pd
   ...
   **Linear Gibbs kernel** of covariance matrix. Numpy Arrays of signals and time are an input to the class.
   ...
   The covariance matrix *k(x, y)* defines the mutual relationship between all points of the true signal.
    ...
    def _FitSingleTransient(self, idx):
        ...
        Yfit2 = Yfit[::]
        Yfit = Yfit2[(Xfit > self.dt * self._n_samples_offset) | (Yfit2 >= baseline)] #too low values (which appear at high noise) at the start can break the GP approximation
        ...
        Peak_sig = minimize(self._MinFunc,x0 = [t0_sig + 1.55*self._n_samples_offset*self.dt], args = (self.transients[idx],True))
        ...
        try:
            q1_rise = bisect(self._FindTimeFraction,t0_sig,TTP_sig,args=(self.quantile1,-Peak_sig.fun,base_sig,self.transients[idx]))
        except:
            q1_rise = None
        try:
            q2_rise = bisect(self._FindTimeFraction,t0_sig,TTP_sig,args=(self.quantile2,-Peak_sig.fun,base_sig,self.transients[idx]))
        except:
            q2_rise = None
        try:
            half_rise = bisect(self._FindTimeFraction,t0_sig,TTP_sig,args=(0.5,-Peak_sig.fun,base_sig,self.transients[idx]))
        except:
            half_rise = None
        try:
            q1m_rise = bisect(self._FindTimeFraction,t0_sig,TTP_sig,args=(1 - self.quantile1,-Peak_sig.fun,base_sig,self.transients[idx]))
        except:
            q1m_rise = None
        try:
            q2m_rise = bisect(self._FindTimeFraction,t0_sig,TTP_sig,args=(1 - self.quantile2,-Peak_sig.fun,base_sig,self.transients[idx]))
        except:
            q2m_rise = None
        q1_decay = bisect(self._FindTimeFraction,TTP_sig,t[-1],args=(self.quantile1,-Peak_sig.fun,base_sig,self.transients[idx]))
        q2_decay = bisect(self._FindTimeFraction,TTP_sig,t[-1],args=(self.quantile2,-Peak_sig.fun,base_sig,self.transients[idx]))
        half_decay = bisect(self._FindTimeFraction,TTP_sig,t[-1],args=(0.5,-Peak_sig.fun,base_sig,self.transients[idx]))
        q1m_decay = bisect(self._FindTimeFraction,TTP_sig,t[-1],args=(1 - self.quantile1,-Peak_sig.fun,base_sig,self.transients[idx]))
        q2m_decay = bisect(self._FindTimeFraction,TTP_sig,t[-1],args=(1 - self.quantile2,-Peak_sig.fun,base_sig,self.transients[idx]))
    ...
    def GetTransientsTable(self,x_label="x",y_label="y"):
        ...
        sig_spline = CubicSpline(t_model_out, sig_model_out)
