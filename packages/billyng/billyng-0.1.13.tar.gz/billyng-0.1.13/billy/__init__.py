...
class Customer:
    ...
    @staticmethod
    @checkUrlIsSet
    def charge(customerId: str, paymentMethodId: str, amount: float, currency: str, description: str = None, descriptor: str = None):
        """Set default payment method for customer"""
        assert isinstance(customerId, str) and customerId.startswith("cus_"), "invalid customer id"
        assert isinstance(paymentMethodId, str) and paymentMethodId.startswith("pm_"), "invalid payment method id"
        ...
        return _post(f"/customer/{customerId}/charge", json={ "paymentMethodId": paymentMethodId, "amount": amount, "currency": currency, "description": description, "descriptor": descriptor })
