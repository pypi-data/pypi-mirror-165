...
class AtmosphereGrid:
    def __init__(
        ...
        *gas: str,
        ...
        """
        Args:
            alt_range: Altitude range tuple in meters.
            wave_no_range: Wave number range tuple in cm^{-1}
            *gas: 
            verbose:
            db_connection:
        """
    ...
    def surface_flux(self) -> pd.DataFrame:
        """
        Returns an array of spectral radiance as a function of flux.
        Returns:
            Dataframe of spectral Radiance.
        """
        ...
        # Transmission
        for i, alt in enumerate(self.alt_list):
            if i != 0:
                transmission_grid.iloc[:, i] \
                    *= transmission_grid.iloc[:, i-1]
        (transmission_grid.T * ground_flux).T
    ...
    def flux_up(self) -> pd.DataFrame:
        ...
        return flux_from_ground - atm_flux