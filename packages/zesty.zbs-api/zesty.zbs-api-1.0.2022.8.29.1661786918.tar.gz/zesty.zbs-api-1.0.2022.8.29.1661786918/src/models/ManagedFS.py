...
try:
    from sqlalchemy import Column, engine, cast, String, case, or_
...
class ManagedFsMixin:
    ...
    # Custom query
    @staticmethod
    def custom_query(session: Union[Session, sessionmaker]) -> Query:
        managedFsB = aliased(ManagedFs)

        q = session.query(ManagedFs, case((or_(managedFsB.policies == None, cast(managedFsB.policies, String) == 'null'), ''), else_=cast(managedFsB.policies, String).regexp_replace(r'.+"name":\s"([^"]+).+', "\\1"))
                          .label("policies_name")) \
            .outerjoin(managedFsB, ManagedFs.fs_id == managedFsB.fs_id)

        return q

    @staticmethod
    def custom_order_by(sorting_column: str, sorting_order: str) -> str:
        actual_sorting_column = sorting_column
        if sorting_column == "policies":
            actual_sorting_column = "policies_name"
        else:
            actual_sorting_column = f"managed_filesystems.{actual_sorting_column}"
