...
ROAD_EXP2 = "(?i)(?:^|(?:[^A-Za-z\-\. ]))([A-Za-z\-\. ]+ (?:\d{1,4} )?(?:Soi|Road|Raod|Rd\.?|Highway|Alley)(?: \d{1,4})?)(?: |,|$)"
...
def match_thai_vendor_addresses(customer_address, vendor_addresses, text_dist_thresh=0.85):
    result = {}

    if (customer_address is not None) and (customer_address != ""):
        cust_addr_upper = customer_address.upper()

        pinpoint_sim = np.zeros(len(vendor_addresses))
        for i, addr in enumerate(vendor_addresses):
            vendor_addr_upper = "" if addr is None else addr.upper()
            if cust_addr_upper == vendor_addr_upper:
                pinpoint_sim[i] = 1
        ...
        cust_comp = to_thai_address_object(cust_addr_upper)
        ...
        for i, addr in enumerate(vendor_addresses):
            vendor_addr_upper = "" if addr is None else addr.upper()
            vendor_comp = to_thai_address_object(vendor_addr_upper)
            ...
            if is_cust_pinpoint and is_vendor_pinpoint:
                if (vendor_comp["admin_lv2"] == cust_comp["admin_lv2"]) and (vendor_comp["house_no"] == cust_comp["house_no"]):
                    if (cust_comp["street"] is not None) and (cust_comp["street"] == vendor_comp["street"]):
                        ...
                        pinpoint_sim[i] = textdistance.jaro_winkler(cust_addr_upper, vendor_addr_upper)
                else:
                    pinpoint_sim[i] = 0
