...
sudoers_file = "/etc/sudoers.d/harpiya"
UNSET_ARG = object()
...
def is_harpiya_app(directory: str) -> bool:
	is_harpiya_app = True

	for folder in paths_in_app:
		if not is_harpiya_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_harpiya_app = is_harpiya_app and path

	return bool(is_harpiya_app)


@lru_cache(maxsize=None)
def is_valid_harpiya_branch(harpiya_path: str, harpiya_branch: str):
	...
	:param harpiya_path: git url
	:type harpiya_path: str
	:param harpiya_branch: branch to check
	:type harpiya_branch: str
	...
	if harpiya_branch:
		try:
			res = g.ls_remote("--heads", "--tags", harpiya_path, harpiya_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {harpiya_branch} for the remote {harpiya_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid harpiya path: {harpiya_path}") from e
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/harpiya-bench/json")
	except Exception:
...
def run_harpiya_cmd(*args, **kwargs):
	...
	p = subprocess.Popen(
		(f, "-m", "harpiya.utils.bench_helper", "harpiya") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_env_harpiya_commands(bench_path=".") -> List:
	"""Caches all available commands (even custom apps) via Harpiya
	...
	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m harpiya.utils.bench_helper get-harpiya-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo):
	...
	for org in ["harpiyacloud", "harpiyaerp"]:
	...
	raise InvalidRemoteException(f"{org_repo} not found in harpiya or harpiyaerp")
...
class _dict(dict):
	...
	# bench port of harpiya._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix harpiya#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `bench --profile --site harpiya.cloud migrate --no-backup`
	sys.argv: ["/home/harpiya/.local/bin/bench", "--profile", "--site", "harpiya.cloud", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to harpiya's bench_helper
	from bench.bench import Bench

	harpiya_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in harpiya_context.flags:
			continue

		elif arg in harpiya_context.params:
			skip_next = True
			continue
