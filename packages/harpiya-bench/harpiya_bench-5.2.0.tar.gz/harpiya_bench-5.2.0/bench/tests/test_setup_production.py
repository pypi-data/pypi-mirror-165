...
class TestSetupProduction(TestBenchBase):
	...
	def assert_supervisor_config(self, bench_name, use_rq=True):
		...
		with open(conf_src) as f:
			...
			tests = [
				f"program:{bench_name}-harpiya-web",
			...
			if use_rq:
				tests.extend(
					[
						f"program:{bench_name}-harpiya-schedule",
						f"program:{bench_name}-harpiya-default-worker",
						f"program:{bench_name}-harpiya-short-worker",
						f"program:{bench_name}-harpiya-long-worker",
					]
				)
				...
				tests.extend(
					[
						f"program:{bench_name}-harpiya-workerbeat",
						f"program:{bench_name}-harpiya-worker",
						f"program:{bench_name}-harpiya-longjob-worker",
						f"program:{bench_name}-harpiya-async-worker",
					]
				)
	...
	def assert_supervisor_process(self, bench_name, use_rq=True, disable_production=False):
		...
		tests = [
			r"{bench_name}-web:{bench_name}-harpiya-web[\s]+RUNNING",
		...
		if use_rq:
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-harpiya-schedule[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-default-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-short-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-long-worker-0[\s]+RUNNING",
				]
			)
			...
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-harpiya-workerbeat[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-longjob-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-harpiya-async-worker[\s]+RUNNING",
				]
			)
