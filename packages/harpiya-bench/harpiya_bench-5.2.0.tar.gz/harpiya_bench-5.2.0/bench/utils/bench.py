...
def update_node_packages(bench_path=".", apps=None):
	...
	v = LooseVersion(get_develop_version("harpiya", bench_path=bench_path))

	# After rollup was merged, harpiya_version = 10.1
...
def restart_supervisor_processes(bench_path=".", web_workers=False):
	...
	if cmd:
		...
		bench.run(f"sudo supervisorctl restart all")
...
def handle_version_upgrade(version_upgrade, bench_path, force, reset, conf):
	...
	if version_upgrade[0]:
		if force:
			log(
				"""Force flag has been used for a major version change in Harpiyaand it's apps.
This will take significant time to migrate and might break custom apps.""",
				level=3,
			)
		else:
			print(
				f"""This update will cause a major version change in Harpiya/HarpiyaERP from {version_upgrade[1]} to {version_upgrade[2]}.
...
def update(
	...
	print(
		"_" * 80 + "\nBench: Deployment tool for Harpiyaand HarpiyaApplications"
		" (https://harpiya.cloud/bench).\nOpen source depends on your contributions, so do"
...
def remove_backups_crontab(bench_path="."):
	...
	user = Bench(bench_dir).conf.get("harpiya_user")
