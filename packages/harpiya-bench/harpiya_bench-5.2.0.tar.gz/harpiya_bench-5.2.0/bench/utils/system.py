...
def init(
	...
	* setup backups crontab
	...
	# remote apps
		...
		harpiya_path = harpiya_path or "https://harpiyacloud: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@github.com/harpiyacloud/harpiya.git"
		is_valid_harpiya_branch(harpiya_path=harpiya_path, harpiya_branch=harpiya_branch)
		get_app(
			harpiya_path,
			branch=harpiya_branch,
			bench_path=path,
			skip_assets=True,
			verbose=verbose,
			resolve_deps=False,
...
def setup_sudoers(user):
	...
	template = bench.config.env().get_template("harpiya_sudoers")
	harpiya_sudoers = template.render(
		**{
			"user": user,
...
def migrate_site(site, bench_path="."):
	run_harpiya_cmd("--site", site, "migrate", bench_path=bench_path)


def backup_site(site, bench_path="."):
	run_harpiya_cmd("--site", site, "backup", bench_path=bench_path)
...
def fix_prod_setup_perms(bench_path=".", harpiya_user=None):
	from glob import glob
	from bench.bench import Bench

	harpiya_user = harpiya_user or Bench(bench_path).conf.get("harpiya_user")

	if not harpiya_user:
		print("harpiya user not set")
		sys.exit(1)

	globs = ["logs/*", "config/*"]
	for glob_name in globs:
		for path in glob(glob_name):
			uid = pwd.getpwnam(harpiya_user).pw_uid
			gid = grp.getgrnam(harpiya_user).gr_gid
...
def setup_fonts():
	...
	exec_cmd("git clone https://github.com/harpiyacloud/fonts.git", cwd="/tmp")
