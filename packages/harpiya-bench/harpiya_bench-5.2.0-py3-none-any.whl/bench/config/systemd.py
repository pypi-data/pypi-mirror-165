...
def generate_systemd_config(
	...
	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-harpiya-default-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-harpiya-short-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-harpiya-long-worker@" + str(i + 1) + ".service"
		)
...
def setup_main_config(bench_info, bench_path):
	# Main config
	bench_template = bench.config.env().get_template("systemd/harpiya-bench.target")
...
def setup_workers_config(bench_info, bench_path):
	...
	bench_workers_target_template = bench.config.env().get_template(
		"systemd/harpiya-bench-workers.target"
	)
	bench_default_worker_template = bench.config.env().get_template(
		"systemd/harpiya-bench-harpiya-default-worker.service"
	)
	bench_short_worker_template = bench.config.env().get_template(
		"systemd/harpiya-bench-harpiya-short-worker.service"
	)
	bench_long_worker_template = bench.config.env().get_template(
		"systemd/harpiya-bench-harpiya-long-worker.service"
	)
	bench_schedule_worker_template = bench.config.env().get_template(
		"systemd/harpiya-bench-harpiya-schedule.service"
	)
	...
	bench_default_worker_config_path = os.path.join(
		...
		"config",
		"systemd",
		bench_info.get("bench_name") + "-harpiya-default-worker@.service",
	)
	bench_short_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-harpiya-short-worker@.service",
	)
	bench_long_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-harpiya-long-worker@.service",
	)
	bench_schedule_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-harpiya-schedule.service",
	)
...
def setup_web_config(bench_info, bench_path):
	...
	bench_web_target_template = bench.config.env().get_template(
		"systemd/harpiya-bench-web.target"
	)
	bench_web_service_template = bench.config.env().get_template(
		"systemd/harpiya-bench-harpiya-web.service"
	)
	bench_node_socketio_template = bench.config.env().get_template(
		"systemd/harpiya-bench-node-socketio.service"
	)
	...
	bench_web_service_config_path = os.path.join(
		bench_path, "config", "systemd", bench_info.get("bench_name") + "-harpiya-web.service"
	)
...
def get_unit_files(bench_path):
	...
	unit_files = [
		...
		[bench_name + "-harpiya-default-worker@", ".service"],
		[bench_name + "-harpiya-short-worker@", ".service"],
		[bench_name + "-harpiya-long-worker@", ".service"],
		[bench_name + "-harpiya-schedule", ".service"],
		[bench_name + "-harpiya-web", ".service"],
