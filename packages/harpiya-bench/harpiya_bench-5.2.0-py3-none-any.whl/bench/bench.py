...
class Bench(Base, Validator):
	...
	def build(self):
		# build assets & stuff
		run_harpiya_cmd("build", bench_path=self.name)
...
class BenchApps(MutableSequence):
	...
	def update_apps_states(
		...
		if self.apps and not os.path.exists(self.states_path):
			...
			# Keeping harpiya as the first app.
			if "harpiya" in self.apps:
				self.apps.remove("harpiya")
				self.apps.insert(0, "harpiya")
	...
	def initialize_apps(self):
		try:
			self.apps = [
				...
				if is_harpiya_app(os.path.join(self.bench.name, "apps", x))
			]
			self.apps.remove("harpiya")
			self.apps.insert(0, "harpiya")
		except FileNotFoundError:
...
class BenchSetup(Base):
	...
	def env(self, python="python3"):
		...
		- upgrade env pip
		- install harpiya python dependencies
		"""
		import bench.cli
		import click
		...
		harpiya = os.path.join(self.bench.name, "apps", "harpiya")
		...
		if os.path.exists(harpiya):
			self.run(
				f"{self.bench.python} -m pip install {quiet_flag} --upgrade -e {harpiya}",
				cwd=self.bench.name,
			)
	...
	def backups(self):
		...
		user = self.bench.conf.get("harpiya_user")
