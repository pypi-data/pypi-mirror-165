...
HARPIYA_BRANCH = "version-12"
if PYTHON_VER.major == 3:
	if PYTHON_VER.minor >= 10:
		HARPIYA_BRANCH = "develop"
	if 7 >= PYTHON_VER.minor >= 9:
		HARPIYA_BRANCH = "version-13"
...
class TestBenchBase(unittest.TestCase):
	...
	def assert_folders(self, bench_name):
		...
		self.assert_exists(bench_name, "apps", "harpiya")
	...
	def init_bench(self, bench_name, **kwargs):
		...
		harpiya_tmp_path = "/tmp/harpiya"

		if not os.path.exists(harpiya_tmp_path):
			exec_cmd(
				f"git clone https://github.com/harpiyacloud/harpiya -b {HARPIYA_BRANCH} --depth 1 --origin upstream {harpiya_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.benches_path, bench_name)):
			...
			exec_cmd(
				"git remote set-url upstream https://github.com/harpiyacloud/harpiya",
				cwd=os.path.join(self.benches_path, bench_name, "apps", "harpiya"),
			)
