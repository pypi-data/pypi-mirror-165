...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/harpiyacloud/healthcare.git
		        2. git@github.com:harpiyacloud/healthcare.git
		        3. harpiyacloud/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"harpiya/harpiyaerp": "~13.17.0"}]
		"""
...
class App(AppMeta):
	...
	def install(
		...
		if not resolved and self.repo != "harpiya" and not ignore_resolution:
			click.secho(
...
def make_resolution_plan(app: App, bench: "Bench"):
	...
	for app_name in app._get_dependencies():
		...
		is_valid_harpiya_branch(dep_app.url, dep_app.branch)
...
def add_to_excluded_apps_txt(app, bench_path="."):
	if app == "harpiya":
		raise ValueError("Harpiyaapp cannot be excludeed from update")
...
def get_app(
	...
	"""bench get-app clones a HarpiyaApp from remote (GitHub or any other git server),
	...
	harpiya_path, harpiya_branch = None, None
	...
	if resolve_deps:
		...
		if "harpiya" in resolution:
			# Todo: Make harpiya a terminal dependency for all harpiya apps.
			harpiya_path, harpiya_branch = resolution["harpiya"].url, resolution["harpiya"].tag
	...
	if not is_bench_directory(bench_path):
		...
		init(
			path=bench_path,
			harpiya_path=harpiya_path,
			harpiya_branch=harpiya_branch or branch,
		)
		os.chdir(bench_path)
		bench_setup = True
...
def new_app(app, no_git=None, bench_path="."):
	if bench.HARPIYA_VERSION in (0, None):
		raise NotInBenchDirectoryError(
			f"{os.path.realpath(bench_path)} is not a valid bench directory."
		)
	...
	if no_git:
		if bench.HARPIYA_VERSION < 14:
			click.secho("Harpiyav14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_harpiya_cmd(*args, bench_path=bench_path)
...
def use_rq(bench_path):
	...
	celery_app = os.path.join(bench_path, "apps", "harpiya", "harpiya", "celery_app.py")
