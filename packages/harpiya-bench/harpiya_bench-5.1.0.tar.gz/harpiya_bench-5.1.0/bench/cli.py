...
def cli():
	...
	if (
		...
		log(
			...
			" with: `pip install harpiya-bench`\n",
			level=3,
		)
	...
	if in_bench:
		if cmd_from_sys in get_harpiya_commands():
			harpiya_cmd()
		else:
			app_cmd()
...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/harpiya_bench_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		harpiya_user = bench_config.get("harpiya_user")
		if harpiya_user:
			drop_privileges(uid_name=harpiya_user, gid_name=harpiya_user)
			os.environ["HOME"] = pwd.getpwnam(harpiya_user).pw_dir
		else:
...
def app_cmd(bench_path="."):
	...
	os.execv(f, [f] + ["-m", "harpiya.utils.bench_helper"] + sys.argv[1:])


def harpiya_cmd(bench_path="."):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
	os.execv(f, [f] + ["-m", "harpiya.utils.bench_helper", "harpiya"] + sys.argv[1:])


def get_harpiya_commands():
	if not is_bench_directory():
		return set()

	return set(get_env_harpiya_commands())


def get_harpiya_help(bench_path="."):
	python = get_env_cmd("python", bench_path=bench_path)
	sites_path = os.path.join(bench_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -m harpiya.utils.bench_helper get-harpiya-help", cwd=sites_path
		)
