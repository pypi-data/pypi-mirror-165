...
from bench.tests.test_base import HARPIYA_BRANCH, TestBenchBase
from bench.bench import Bench


# changed from harpiya_theme because it wasn't maintained and incompatible,
# chat app & wiki was breaking too. hopefully harpiya_docs will be maintained
# for longer since docs.harpiya.cloud is powered by it ;)
TEST_HARPIYA_APP = "harpiya_docs"
...
class TestBenchInit(TestBenchBase):
	...
	def test_get_app(self):
		...
		exec_cmd(f"bench get-app {TEST_HARPIYA_APP} --skip-assets", cwd=bench_path)
		self.assertTrue(os.path.exists(os.path.join(bench_path, "apps", TEST_HARPIYA_APP)))
		app_installed_in_env = TEST_HARPIYA_APP in subprocess.check_output(
			["bench", "pip", "freeze"], cwd=bench_path
		).decode("utf8")
		self.assertTrue(app_installed_in_env)

	@unittest.skipIf(HARPIYA_BRANCH != "develop", "only for develop branch")
	def test_get_app_resolve_deps(self):
		HARPIYA_APP = "healthcare"
		self.init_bench("test-bench")
		bench_path = os.path.join(self.benches_path, "test-bench")
		exec_cmd(f"bench get-app {HARPIYA_APP} --resolve-deps --skip-assets", cwd=bench_path)
		self.assertTrue(os.path.exists(os.path.join(bench_path, "apps", HARPIYA_APP)))
		...
		self.assertTrue(HARPIYA_APP in states)
	...
	def test_install_app(self):
		...
		self.init_bench(bench_name)
		exec_cmd(
			f"bench get-app {TEST_HARPIYA_APP} --branch master --skip-assets", cwd=bench_path
		)
		...
		installed_app = not exec_cmd(
			f"bench --site {site_name} install-app {TEST_HARPIYA_APP}",
			cwd=bench_path,
			_raise=False,
		)
		...
		if installed_app:
			...
			self.assertTrue(TEST_HARPIYA_APP in app_installed_on_site)
	...
	def test_remove_app(self):
		...
		exec_cmd(
			f"bench get-app {TEST_HARPIYA_APP} --branch master --overwrite --skip-assets",
			cwd=bench_path,
		)
		exec_cmd(f"bench remove-app {TEST_HARPIYA_APP}", cwd=bench_path)

		with open(os.path.join(bench_path, "sites", "apps.txt")) as f:
			self.assertFalse(TEST_HARPIYA_APP in f.read())
		self.assertFalse(
			TEST_HARPIYA_APP
			in subprocess.check_output(["bench", "pip", "freeze"], cwd=bench_path).decode("utf8")
		)
		self.assertFalse(os.path.exists(os.path.join(bench_path, "apps", TEST_HARPIYA_APP)))

	def test_switch_to_branch(self):
		self.init_bench("test-bench")
		bench_path = os.path.join(self.benches_path, "test-bench")
		app_path = os.path.join(bench_path, "apps", "harpiya")

		# * chore: change to 14 when avalible
		prevoius_branch = "version-13"
		if HARPIYA_BRANCH != "develop":
			# assuming we follow `version-#`
			prevoius_branch = f"version-{int(HARPIYA_BRANCH.split('-')[1]) - 1}"

		successful_switch = not exec_cmd(
			f"bench switch-to-branch {prevoius_branch} harpiya --upgrade",
			cwd=bench_path,
			_raise=False,
		)
		...
		successful_switch = not exec_cmd(
			f"bench switch-to-branch {HARPIYA_BRANCH} harpiya --upgrade",
			cwd=bench_path,
			_raise=False,
		)
		if successful_switch:
			app_branch_after_second_switch = str(git.Repo(path=app_path).active_branch)
			self.assertEqual(HARPIYA_BRANCH, app_branch_after_second_switch)
