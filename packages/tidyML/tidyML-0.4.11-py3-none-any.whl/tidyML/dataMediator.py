...
class DataMediator:
    ...
    def __balance(
        ...
        splitAssigner = (
            lambda dataFrame: largeSplit
            if largeSplit.index.isin(dataFrame).all()
            else smallSplit
        )
        self.experimental = splitAssigner(self.experimental)
        self.control = splitAssigner(self.control)
