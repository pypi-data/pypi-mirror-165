...
from cryton_core.lib.util import constants, logger, states, util, exceptions
from cryton_core.lib.models import stage, plan, step, run, event
...
class Listener:
    ...
    @staticmethod
    def get_correlation_event(correlation_id: str) -> None or CorrelationEventModel:
        """
        ...
        for i in range(3):
            try:
                correlation_event_obj = CorrelationEventModel.objects.get(correlation_id=correlation_id)
    ...
    def step_resp_callback(self, message: amqpstorm.Message) -> None:
        ...
        # TODO: This is a hotfix for the slow STARTING -> RUNNING state transition.
        #  It will be resolved in the Listener rework ticket.
        #  It seems like a problem on the Worker side, as if the send_message method is blocked when the module finishes
        #  before it was sent.
        while step_exec_obj.state == states.STARTING:
            time.sleep(0.2)
