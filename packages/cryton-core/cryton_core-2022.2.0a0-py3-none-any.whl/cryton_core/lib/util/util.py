...
from cryton_core.cryton_app.models import WorkerModel, PlanModel
...
def rabbit_connection() -> amqpstorm.Connection:
    ...
    try:
        connection = amqpstorm.Connection(rabbit_host, rabbit_username, rabbit_password, port=rabbit_port)
...
def save_plan_evidence_file(file_name: str, file_content: str, evidence_dir: str) -> str:
    ...
    if not check_path(file_path, config.EVIDENCE_DIRECTORY):
...
def validate_attack_module_args(
    ...
    event_info = {constants.EVENT_T: constants.EVENT_VALIDATE_MODULE,
                  constants.EVENT_V: {"module": module_name, "module_arguments": module_args}}
    with Rpc() as rpc:
...
def parse_delta_to_datetime(time_str: str) -> timedelta:  # TODO: update to format YY-MM-DD hh:mm:ss
    try:
...
def fill_template(plan_template: str, inventory_variables: dict) -> str:
    """
    ...
    :param plan_template: Plan template to fill
...
def parse_inventory_file(inventory_file: str) -> dict:
    ...
    """
    try:  # JSON
        parsed_inventory = json.loads(inventory_file)
    except json.decoder.JSONDecodeError:
        try:  # YAML
            parsed_inventory = yaml.safe_load(inventory_file)
        except yaml.YAMLError:
            try:  # INI
                config_parser = configparser.ConfigParser()
                config_parser.read_string(inventory_file)
                parsed_inventory = {section: dict(config_parser.items(section)) for section in config_parser.sections()}
            except configparser.Error:
                raise ValueError(f"Invalid inventory file provided.")

    if parsed_inventory is None or isinstance(parsed_inventory, str):
        raise ValueError("Input file cannot be empty and must contain JSON, YAML, or INI structure.")
...
class Rpc:
    ...
    def call(self, queue_name: str, msg_body: dict = None, time_limit: float = config.RPC_TIMEOUT) -> dict:
        """
    ...
    def wait_for_response(self, time_limit: float, sleep_time: float = 0.01) -> None:
        """
        Wait for response.
        :param time_limit: Time limit for response
        ...
        :return: None
        """
        # TODO: Compare timestamps instead of changing the time_limit since it doubles the time_out in reality
