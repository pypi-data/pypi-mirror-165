...
#                     return pd.DataFrame(columns=results)
    ...
    async def get_picklist_values_by_object_record_type(sf, object: str):
        object_record_type_ids = sf.query(f"Select Id, Name From RecordType Where SobjectType = '{object}'").apply(lambda row: {"Id": row['Id'], "Name": row['Name']}, axis=1).to_list()
        object_describe = sf.sf_api_call(f"/services/data/{sf.api_version}/ui-api/object-info/{object}")['fields']
        controlling_fields = []
        ...
        def record_type_picklist_retriever(sf, object, record_type_id, record_type_name):
            result = []
            for key, value in sf.sf_api_call(f"/services/data/{sf.api_version}/ui-api/object-info/{object}/picklist-values/{record_type_id}")['picklistFieldValues'].items():
        ...
        results = pd.concat(await asyncio.gather(*[async_picklist_rt_retrieve(sf, object, record_type_id['Id'], record_type_id['Name']) for record_type_id in object_record_type_ids]))
