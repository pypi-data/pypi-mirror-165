...
from .utils import addcomma, canonical_path, compact_json, progress, pattern_to_regex
...
TRIGGERS = [
    ...
    BEGIN
        INSERT INTO nexus(nexus, refs, totalsize) VALUES(NEW.nexus, 1, (min(NEW.refs, 1) * NEW.blocksize))
        ON CONFLICT(nexus) DO UPDATE SET refs = refs + 1, totalsize = totalsize + (min(NEW.refs, 1) * NEW.blocksize);
    END
    """,
    ...
    WHEN OLD.blocksize != NEW.blocksize OR OLD.nexus != NEW.nexus OR OLD.refs != NEW.refs
    BEGIN
        INSERT INTO nexus(nexus, refs, totalsize) VALUES(NEW.nexus, 1, (min(NEW.refs, 1) * NEW.blocksize))
        ON CONFLICT(nexus) DO UPDATE SET refs = refs + 1, totalsize = totalsize + (min(NEW.refs, 1) * NEW.blocksize);

        UPDATE nexus SET refs = refs - 1, totalsize = totalsize - (min(OLD.refs, 1) * OLD.blocksize) WHERE nexus = OLD.nexus;
    END
    """,
    """
    ...
    BEGIN
        INSERT INTO object(hash, size, priority, maxcopies, refs, last_path, last_modtime)
          VALUES(NEW.hash, NEW.size, NEW.priority, NEW.maxcopies, NEW.virtual_path, 1, NEW.last_modified)
...
def prefix_sql(column, prefix):
    return f'({column} >= ? AND {column} < ?)', (prefix, prefix[:-1] + chr(1 + ord(prefix[-1:])))
...
class BackupDB(SQLiteDB):
    ...
    def load_nexus(self):
        rtn = {}
        with self.execute('SELECT nexus, level, refs, totalsize, disks FROM nexus') as curs:
            for nexus, level, refs, totalsize, disks in curs:
                rtn[nexus] = Nexus(nexus, level, refs, totalsize, disks)
        return rtn

    def create_empty_nexus(self, nexusid:str):
        return Nexus(nexusid, nexus_level(nexusid), 0, 0, ','.join(self.nexus_disks(nexusid)))

    def get_used_space(self, nexus_index):
        return self.query_scalar('SELECT sum(totalsize) FROM nexus WHERE disk_in_nexus(nexus, ?)', (nexus_index,), 0)
    ...
    def set_disk_params(self, size=None, relpath=None, path=None, fstype=None, fsuuid=None):
        ...
        if path is not None:
            ...
            self.execute_nonquery(
                'UPDATE disks SET relative_path= ?, fstype = ?, fsuuid = ? WHERE uuid = ?',
                (disk.relative_path, disk.fstype, disk.fsuuid, disk.uuid)
            )

        if size is not None:
            self.execute_nonquery('UPDATE disks SET size = ? WHERE uuid = ?', (size, disk.uuid))
            disk.size = size

        if relpath is not None:
            disk.relative_path = relpath or None
            self.execute_nonquery('UPDATE disks SET relative_path = ? WHERE uuid = ?', (disk.relative_path, disk.uuid))

        if fstype is not None:
            disk.fstype = fstype or None
            self.execute_nonquery('UPDATE disks SET fstype = ? WHERE uuid = ?', (disk.fstype, disk.uuid))

        if fsuuid is not None:
            disk.fsuuid = fsuuid or None
            self.execute_nonquery('UPDATE disks SET fsuuid = ? WHERE uuid = ?', (disk.fsuuid, disk.uuid))
