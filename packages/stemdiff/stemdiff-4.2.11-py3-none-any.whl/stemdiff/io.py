...
def show_datafile(filename, intensity_cut=300):
    ...
    intensity_cut : integer, optinal, default is 300
        Cut off too high intensities.
        If intensity_cut = 300, all intensities > 300 will be equal to 300.
        This reduces the strongest intensity of the central/primary spot.
        
    Returns
    -------
    Nothing
        The output are the files and entropies shown on the screen.

    '''
    print('Datafile:', filename.name)
    # a) Read datafile
    arr = read_datafile(filename)
    # b) Calculate and print Shannon entropy of the datafile
...
def save_datafile(filename, output_image, intensity_cut=300, itype='8bit'):
    ...
    -------
    Nothing
        The output is the saved PNG-image in active directory.

    '''
    # a) Read datafile
    arr = read_datafile(filename)
    # b) Cut intensity and save datafile as PNG-image using matplotlib
    arr = np.where(arr>intensity_cut, intensity_cut, arr)
...
def show_datafiles(datafiles, intensity_cut=300):
    ...
    * [Enter] = next file, [Ctrl+C] = end of show (hardcore, but working).
...
def prepare_array_for_saving(arr, icut=None, itype='8bit', R=None):
    '''
...
def show_array(arr, icut=None, itype='8bit', R=None, cmap='gray'):
    '''
    ...
    cmap : str, name of colormap, optional, default is 'gray'
        ...
        * Matplotlib colormaps are in module `matplotlib.pyplot.cm`.
        * Simple way to see matplotlib colormaps:
          `import matplotlib; dir(matplotlib.pyplot.cm)`
        * More details about matplotlib colormaps:
          <https://matplotlib.org/stable/tutorials/colors/colormaps.html>

    Returns
    -------
    Nothing
        The output is *arr* shown as image on the screen.
    '''
    # Prepare array for saving
    arr = prepare_array_for_saving(arr, icut, itype, R)
    # show  array as image
...
def show_image(image_name, itype='8bit', cmap='gray'):
    ...
    -------
    Nothing
        The output is the image with *image_name* shown on screen.
    '''
