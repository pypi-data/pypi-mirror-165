...
class Conv(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        attr["channels"] = infer_shape(inputs[1])[0]
        ...
        if dimension_picker("conv")(attr) == "conv1d" and attr.get("group") != 1:
            # if dimension_picker("conv")(attr) == "conv1d":
            group_conv1d = True
...
class Reshape(OnnxOpConverter):
    ...
    def _impl_v5(cls, inputs, attr, params):
        if get_name(inputs[1]) in params:
            ...
            inputs[1] = infer_value(inputs[1], params).asnumpy()
...
class RoiAlign(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        if mode not in [b"avg", f"avg", b"max"]:
    ...
    def from_onnx(self, graph, opset, get_output_expr=False):
        ...
        for node in graph.node:
            ...
            if op_name == "MaxPool" and len(node_output) == 2:
                indices = self._convert_operator("MaxPoolLocation", inputs, attr, opset)
        ...
        output_names = [self._parse_value_proto(i) for i in graph.output]
        # reset output name
        self._nodes = reset_output_name(self._nodes, output_names)
        # now return the outputs
        outputs = [self._nodes[i] for i in output_names]
