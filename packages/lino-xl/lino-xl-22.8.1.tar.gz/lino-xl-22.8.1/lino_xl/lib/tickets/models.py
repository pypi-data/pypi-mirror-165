...
class LinkChecker(Checker):
    ...
    verbose_name = _("Find dependencies that will get lost")
    model = 'tickets.Link'

    def get_checkdata_problems(self, obj, fix=False):
        if obj.child is None or obj.parent is None:
            return
        if obj.child.parent != obj.parent:
            yield (True, _("{} but parent is {}").format(obj, obj.child.parent))
            if fix:
                obj.child.parent = obj.parent
                obj.child.full_clean()
                obj.child.save()


LinkChecker.activate()
