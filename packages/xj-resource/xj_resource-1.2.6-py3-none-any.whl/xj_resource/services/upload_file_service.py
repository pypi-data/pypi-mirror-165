...
main_config = JConfig().get_section(path=str(BASE_DIR) + "/config.ini", section="xj_resource")
module_config = JConfig().get_section(path=str(Path(__file__).resolve().parent.parent) + "/config.ini", section="xj_resource")
main_config = JDict(main_config)
module_config = JDict(module_config)
...
class UploadFileService(object):
    base_path = str(BASE_DIR)  # 根目录
    to_month = time.strftime('%Y-%m', time.localtime(time.time()))
    folder_path = (main_config.file_upload_dir or module_config.file_upload_dir or "/upload/file/") + to_month + "/"  # 存放路径，不含文件名
    ...
    def validate(self):
        ...
        file_format_list = main_config.file_formats or module_config.file_formats or None
        if file_format_list:
    ...
    def save_to_db(self, save_data):
        ...
        try:
            ...
            if md5 is None:
                return False
            instance = ResourceFile.objects.filter(md5=md5)
            if not instance:
                serializer = UploadFileSerializer(data=save_data)
                return serializer.save()
            else:
                instance.update(**save_data)
                return ResourceFile.objects.filter(md5=md5).first().to_json()
        except Exception as e:
            self.__set_error("保存数据库异常：" + str(e))
            return False

    def info_detail(self):
        try:
            file_md5 = self.get_md5()
            instance = ResourceFile.objects.filter(md5=file_md5).first()
            if instance and os.path.exists(self.base_path + instance.url):
            ...
            # 获取MD5
            self.file_info = {
                'url': (main_config.file_upload_host or module_config.file_upload_host or "") + (self.folder_path + self.new_filename).replace('//', '/'),
