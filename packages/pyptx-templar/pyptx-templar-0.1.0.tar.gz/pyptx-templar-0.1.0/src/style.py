...
from pyptx_templar.xml import SubElement
...
def set_cell_borders(cell, border_color=RGB(0, 0, 0), border_width=Pt(1),
                     border_style=MSO_LINE.DASH, left=False, top=False,
                     right=False, bot=False):
    tc = cell._tc
    ...
    for idx, (side, do) in enumerate(borders):
        ...
        ln = SubElement(tcPr, tag, before=before, w=str(border_width),
...
def set_rect_borders(table, left, top, right, bot, outer=False, inner=False,
                     **kwargs):
    assert left >= 0
    assert top >= 0
    ...
    for x in range(left, right + 1):
        for y in range(top, bot + 1):
            ...
            set_cell_borders(table.cell(y, x), **kwargs, left=leftb, top=topb,
                             right=rightb, bot=botb)
...
def font_style(ft, size=None, font=None, bold=None, italic=None, font_rgb=None,
               font_brightness=None, underline=None, language=None):
    if size is not None:
        ft.size = size
    if font is not None:
        ft.name = font
...
# https://python-pptx.readthedocs.io/en/latest/api/util.html
def cell_style(cell, valign=None, back_rgb=None, back_brightness=None,
               margin_bottom=None, margin_top=None, margin_left=None,
               margin_right=None, **kwargs):
