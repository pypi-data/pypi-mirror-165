...
def generate_hmac_sql(column: str, key: str = None):
    """
    Generate a SQL string with HMAC-SHA256 implemented. If key is not set, this function will check env for "EMAIL_HASH_SALT". 
    Function will break if a column with a space is passed.
    For more info on how to use see https://madecom.atlassian.net/browse/AT-3783

    Args:
        column: column to be hashed, typically email. Include alias if required, e.g. generate_hmac_sql("table1.email").
        key: hashing key
    ...
    """
    if " " in column:
        raise ValueError("Column must not contain spaces")

    if not key:
        key = get_env_var("EMAIL_HASH_SALT", break_if_missing=True)

    key_pad = rf'CONCAT(CAST("{key}" AS BYTES), REPEAT(b"\x00", 64 - BYTE_LENGTH(CAST("{key}" AS BYTES))))'
    opad = r'REPEAT(b"\x5c", 64)'
    ipad = r'REPEAT(b"\x36", 64)'
    sql = rf"""SUBSTRING(TO_HEX(SHA256(CONCAT({key_pad} ^ {opad}, SHA256(CONCAT({key_pad} ^ {ipad}, CAST({column} AS BYTES)))))), 0, 16)"""
    return sql
