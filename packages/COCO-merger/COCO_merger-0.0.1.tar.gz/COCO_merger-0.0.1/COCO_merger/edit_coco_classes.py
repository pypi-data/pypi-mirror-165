...
def mm_red(a1, aa):
    ...
    if len(testt(d1['images'], qqqq=False)) != 0:
        ...
        if len(testt(d1['images'], qqqq=False)) != 0:
            print("\t\t\tImages couldn't fix: {} duplicate id are still present".format(
                len(testt(d1['images'], qqqq=False))))
            return {}
    ...
    if len(testt(d1['annotations'], qqqq=False)) != 0:
        ...
        if len(testt(d1['annotations'], qqqq=False)) != 0:
            print("\t\t\tAnnotations couldn't fix: {} duplicate id are still present".format(
                len(testt(d1['annotations'], qqqq=False))))
            return {}
    ...
    asd = input(
        'Please enter images filepath to clean or "NA" to return deleted images files as text file ("temp_mm_delete.txt"):')
    if asd != "NA":
    ...
    d1['images'] = res3
    print("Annotations Summary: Initial {}  Final {} Removed {}".format(len(d1['annotations']), len(ann),
                                                                        len(d1['annotations']) - len(ann)))
    d1['annotations'] = ann
    return d1
