...
// %%EXPORT px
uint32_t mm_op15_norm_A(uint64_t *v)
{
    uint64_t w;
...
/**
  @brief Auxiliary function for function ``mm_op15_check_in_Gx0``

  Inputs ``v``, ``tags``, and ``v0``, and the return value are as in
  function ``mm_op15_check_in_Gx0``. 
  ...
  array ``g`` is the same way as function ``mm_op15_check_in_Gx0``
  writes its result int that array.
  
  The function does not change the inputs ``v``, ``tags``, and ``v0``.  
*/
// %%EXPORT px
int32_t mm_op15_find_in_Gx0(uint64_t *v, uint32_t *tags, uint64_t *v0, uint32_t *g)
{
    uint64_t a[48], w3;
    int32_t res, v_y;
    uint_fast32_t w_type4, i, len, perm_num, y;

    if (mm_op15_norm_A(v) != tags[OFS_NORM_A]) return 0x101;

    w3 = leech3matrix_kernel_vector(15, v, tags[OFS_DIAG_VA]);
    ...
    if (mm_op15_compare_len(v0, a, 48)) return 0x106;
...
/**
  ...
  Inputs ``v``, ``tags``, and ``v0``, and the return value are as in
  function ``mm_op15_check_in_Gx0``.  Parameter ``g`` must contain 
  the corrsponding output of function ``mm_op15_check_in_Gx0``, when
  ...
  The function does not change the inputs ``tags``, and ``v0``. 
...
*/
// %%EXPORT px
uint32_t mm_op15_find_in_Qx0(uint64_t *v, uint32_t *tags, uint64_t *v0, uint32_t *g, uint64_t *work)
{
...
/**
  ...
  computed in module ``mmgroup.mm_order.py``.
...
*/
// %%EXPORT px
int32_t mm_op15_check_in_Gx0(uint64_t *v, uint32_t *tags, uint64_t *v0, uint32_t *g)
{
    int32_t res;
    uint64_t *w, *work;

    res = mm_op15_find_in_Gx0(v, tags, v0, g);
    ...
    work = w + 15468;
    mm_op15_copy(v, w);    
    res = mm_op15_find_in_Qx0(w, tags, v0, g, work);
    free(w);
    return res;    
}
...
/**
  ...
  Input parameters ``v0`` and ``tags`` are as in 
  function ``mm_op15_check_in_Gx0``; they are not changed.

  A negative return value indicates an error. 
*/
// %%EXPORT px
int32_t mm_op15_order_Gx0(uint32_t *g, uint32_t n, uint32_t *tags, uint64_t *v0, uint32_t *h, uint32_t o)
{
    int32_t res;
    uint_fast32_t i;
    ...
    for (i = 1; i <= o; ++i) {   
        ...
        if (res < 0) goto done;
        res = mm_op15_find_in_Gx0(w, tags, v0, h);
        if (res >= 0x100) continue;
        if (res < 0) goto done;
        mm_op15_copy(w, w1);    
        res = mm_op15_find_in_Qx0(w1, tags, v0, h, work);
        if (res < 0) goto done;
        if (res < 0x100) {
            res += i << 8;
            goto done;
...
*/
// %%EXPORT px
int32_t mm_op15_order(uint32_t *g, uint32_t n, uint32_t *tags, uint64_t *v0, uint32_t o)
{
    int32_t res, e;
    uint32_t h[10];

    res = mm_op15_order_Gx0(g, n, tags, v0, h, o);
    if (res <= 0) return res;
    e =  res >> 8;
