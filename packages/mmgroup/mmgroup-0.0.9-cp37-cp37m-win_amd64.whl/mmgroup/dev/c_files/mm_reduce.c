...
/**
  @brief Load the image of a 2A axis into a vector in \f$\rho_{15}\f$

  Let \f$v = v^+\f$ if ``s = 0`` and \f$v = v^-\f$ if ``s = 1``,
  where \f$v^+\f$ and \f$v^-\f$ defined as in [Sey22], Section 6. Any
  other values of ``s`` are illegal.  Let \f$g\f$ be the element of
  the monster given as a word of generators in the buffer ``a`` of
  length ``n``. Then the function loads the 2A axis \f$v \cdot g\f$
  into  the vector referred  by ``v``. That vector will store an
  element of the  representation \f$\rho_{15}\f$ of the monster group
...
// %%EXPORT px
MM_REDUCE_API
int32_t mm_reduce_map_axis(uint_mmv_t *v, uint32_t s, uint32_t *a, uint32_t n, uint_mmv_t *work)
{
    uint32_t axis = 0x200 + ((s & 1) << 24);
    int32_t res = gen_leech2_op_word_many(&axis, 1, a, n);
    uint32_t n0 = res;
    if (res < 0) return res;
    res = mm_op15_store_axis(axis, v);
    if (res < 0) return res;
    res = mm_op15_word(v, a + n0, n - n0, 1, work);
    return 0 - (res != 0);
}
...
// Preprocessing is omitted if A_BUFSITE is set to 0.
#define A_BUFSITE 0
/// @endcond  
...
int32_t mm_reduce_M(uint32_t *a,  uint32_t n, uint32_t mode, uint32_t *r)
{
  ...
  #if A_BUFSIZE > 0
    res = gt_word_shorten(a, n, p_work->buf, A_BUFSITE, 0);
    if (res >= 0) {n = res; a = p_work->buf;}
  #endif

    if ((res = mm_reduce_map_axis(v, 0, a, n, work)) < 0) goto done;
    if ((res = mm_reduce_vector_vp(v, mode, r, work)) < 0) goto done;    

    if ((res = mm_reduce_map_axis(v, 1, a, n, work)) < 0) goto done;
