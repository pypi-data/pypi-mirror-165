...
def load_internal_params(inference_context,update_internal_params=None):
    internal_params = {    
            "data":{
                "data_method":"filepath" #the method to read the input data
    ...
    if inference_context=='internal' and update_internal_params is not None:
        for k,v in update_internal_params.items():
            internal_params[k]=v
...
def load_data_batch(params):
    ...
    if path_input_data=='':
        path_input_data,path_output,path_artifact = get_datapath(context,run_name=None)
    
    path_input_datafile = path_input_data+input_filename
    print(f"input file to load: {path_input_datafile}",'\n')
    
    print('load data')
    data = retrieve_data(path_input_datafile, index_column,method=params['data']['data_method'])
...
def inference(params):
    ...
    path_data,path_output,path_artifact = get_datapath(context,run_name=None)

    #get best results
    runs_df = get_wandb_runs(entity_name=params['project']['entity_name']
                             ,project_name=params['project']['project_name']
                             ,read_from=params['project']['read_from']
                             ,path_filename=path_output+'model_metrics.csv')
    ...
    device_type = identify_tensor_device()
    ...
    for att in attributes_original:
        ...
        if att!='specialty':
            ...
            #get the paths for the artifacts
            path_data,path_output,path_artifact = get_datapath(context,run_name=run_name)
            ...
            #load the pretrained model
            print(path_artifact)
            models=pickle.load(open(f'{path_artifact}.pkl', 'rb'))
