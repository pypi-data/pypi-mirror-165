...
def retrieve_data(path,filename,drop_null_index=False,index_column=None,method='filepath',use_cache=False,save_file=False,aws_credentials=None):
...
def load_data_batch(params):
    ...
    if path_input_data=="":
        path_input_data,path_output,path_artifact = get_datapath(read_from=read_data_from,run_name=None,override_path=override_project_path)
...
def inference(params):
    ...
    entity_name = params['project']['entity_name']
    ...
    path_data,path_output,path_artifact = get_datapath(read_from=read_data_from,run_name=None,override_path=override_project_path)

    #get best results
    runs_df = get_wandb_runs(entity_name=entity_name
    ...
    for att in attributes_original:
        ...
        if att!='specialty':
            ...
            #get the paths for the artifacts
            path_data,path_output,path_artifact = get_datapath(read_from=read_data_from,run_name=run_name,override_path=override_project_path)
