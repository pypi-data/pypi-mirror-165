...
def get_data_path(read_from='s3',override_path=None,project=None):
...
def load_data_batch(params):
    ...
    path_input_data = get_data_path(read_from=read_data_from,override_path=override_data_path,project=project_name)
...
def check_params(params):
    if params['inference_type']=='batch':
        ...
        if output_filename is None:
            output_filename = ''
        elif '.csv' not in output_filename:
            output_filename+='.csv'
...
def inference(params,data=None):
    ...
    if params['inference_type']=='batch' and save_output_csv:
        save_inference_output(params,data)

    #return data
    data=return_inference_output(params,data,data_input)
...
def return_inference_output(params,data,data_input):

    output_format = params['run']['output_format']
    attributes = params['external_params']['labels']
...
def save_inference_output(params,data):
