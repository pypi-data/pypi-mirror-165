    ...
    def paintEvent(self, event):
        ...
        for conn in self.connectors:
            ...
            if no_selection or is_selected:
                if conn not in active_connectors:
                    if self.highlighted is not None: #  and not self.highlighted.ishelper:
        ...
        for conn in self.connectors:
            ...
            if conn in active_connectors:
                ...
                self.curved_connector(qp,self.gox + int(conn.a.x+12.5), self.goy + int(conn.a.y+12.5),self.gox + int(conn.b.x+12.5), self.goy + int(conn.b.y+12.5), self.gox + int(conn.c.x+5), self.goy + int(conn.c.y+5))
                ...
                if self.parent().checkBoxLabels.isChecked():
            ...
            if box.ishelper:
                ...
                if box in active_boxes:
                    if self.parent().checkBoxLabels.isChecked():
                        ...
                        qp.drawEllipse(QtCore.QPoint(self.zoom_factor*(self.gox + x+4),self.zoom_factor*(self.goy + y+4)),3, 3)
                else:
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.mode == "drag":
            ...
            if self.selected is not None:
                ...
                self.maxx = self.parent().frameGeometry().width()-50 # 800
                self.maxy = self.parent().frameGeometry().height()-50 # 700

                self.selected.x = self.mousex + self.offx # + self.offx,1-self.gox,1/self.zoom_factor*self.maxx-self.gox)
                self.selected.y = self.mousey + self.offy # + self.offy, 1-self.goy, 1/self.zoom_factor*self.maxy-self.goy)
