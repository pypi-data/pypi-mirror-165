...
def get_gfs_from_ncep(date:str,creation_time:str,forecast_time:int,bbox=[115,38,136,54],save_dir='.',cover=False):
    ...
    url=url_.relpace('YYYYMMDD',date).replace('CC',creation_time.zfill(2)).replace('FFF',str(forecast_time).zfill(3))
    file_name=file_name_.relpace('YYYYMMDD',date).replace('CC',creation_time.zfill(2)).replace('FFF',str(forecast_time).zfill(3))

    file_path=os.path.join(save_dir,url.path)
