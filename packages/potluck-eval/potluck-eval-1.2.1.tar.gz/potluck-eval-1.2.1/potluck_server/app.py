...
@app.route('/<course>/<semester>/deliver', methods=['POST'])
def route_deliver(course, semester):
    ...
    for i, outcome in enumerate(outcomes):
        if (
         ...
         or not isinstance(outcome[2], str)
        ):
            return (
    ...
    for i, block in enumerate(codeBlocks):
        if (
         ...
         or len(block) != 2
         or not all(isinstance(part, str) for part in block)
        ):
            return (
    ...
    einfo = None
    for group in task_info["exercises"]:
        emap = group["exercises"]
        if exercise in emap:
            einfo = emap[exercise]
            break
...
def exercise_credit(exercise_info, outcomes):
    ...
    if n_outcomes != exercise_info.get(
        'count',
        len(exercise_info.get('per_outcome', []))
    ):
        # If the number of outcomes isn't what we expected,
        # we won't know which outcomes might be passive vs.
        # active, and we don't know how to map outcomes onto
        # per-outcome concepts... so we just ignore the
        # whole exercise and count it as not-yet-submitted.
        # TODO: Less fragile way to associate outcomes w/
        # concepts!
...
def set_concept_statuses(concepts, task_info, outcomes):
    ...
    for group in task_info["exercises"]:
        ...
        for eid in group["exercises"]:
            # Add to max credit
            group_max += 1
            # Get info, tag, and outcomes
            einfo = group["exercises"][eid]
            ...
            # concepts.
            for concept_path in einfo["concepts"]:
        ...
        credit_fraction = group_credit / group_max
...
def note_exercise_status(concepts, eid, exercise_status, concept_path):
    ...
    """
    try:
        parents = all_parents(concepts, concept_path)
    except ValueError as e:
        flask.flash(
            "Error recording exercise status: {}".format(e.message)
        )
        parents = []
...
def attach_outcome(concepts, outcome_info, concept_path):
    ...
    try:
        ...
        flask.flash(
            "Error attaching exercise outcome: {}".format(e.message)
        )
        parents = []
...
def get_submission_filename(
    ...
    return werkzeug.utils.safe_join(
