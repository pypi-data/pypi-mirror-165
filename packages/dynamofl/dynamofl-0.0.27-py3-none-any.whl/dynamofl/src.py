...
class DynamoFL(_Base):
    ...
    def _on_message(self, ws, res):
        ...
        if j['event'] == 'client-info':
            ...
            # self.initiate_project_participants(should_fetch_bridges=True, should_spawn_threads=True, reset=True)
    ...
    def initiate_project_participants(self, should_fetch_bridges=False, should_spawn_threads=False, ds=None, reset=False):
        if reset:
            self.project_participants = []
