...
class Model:
    ...
    def estimate(self, sample:Sample, print_equation:bool=True, motor:str='statsmodels'):
        if self.formula != '':
            ...
            if print_equation:
                print(table)
            return Equation(dep_name, indep_names, indep_coefs, cov_var_coefs, df_resid, df_model, table, sample,y,x)
    ...
    def estimate_constrained(self, sample: Sample, vars_no:int, min_obs:int=10, max_obs:int=100,
        ...
        with warnings.catch_warnings():
            ...
            for cor in cors_sorted:
                if no >= vars_no:
                    ...
                    if Variable(var).stats.count(sample) >= min_obs:
...
class Equation:
    def __init__(self, dep_var:str, indep_vars:list[str], indep_coefs:np.ndarray, 
                    cov_var_coefs:np.ndarray, df_resid:int, df_model:int, table:str, sample:Sample,y,x) -> None:
        self.dep_var = dep_var
