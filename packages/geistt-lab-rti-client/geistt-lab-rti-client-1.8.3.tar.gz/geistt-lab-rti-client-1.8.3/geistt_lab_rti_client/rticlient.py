...
class RTIClient(Emitter):
    ...
    def execute_command(self, name: str, entity_id: str = None, transaction_id: str = None, **kwargs):
        message = proto.Commands()
        if transaction_id: message.execute.transaction_id = transaction_id
        message.execute.name = name
        for key, value in kwargs.items():
            message.execute.arguments[key] = value
        self.publish(constants.commands_channel + (f"/{entity_id}" if entity_id else ""), message)
