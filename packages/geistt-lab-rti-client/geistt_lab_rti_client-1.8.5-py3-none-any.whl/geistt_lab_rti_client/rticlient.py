...
class RTIClient(Emitter):
    ...
    def execute_command(self, name: str, entity_id: str = None, transaction_id: str = None, wait: bool = False, timeout: float = 5, **kwargs):
        message = proto.Commands()
        if wait and not transaction_id: transaction_id = str(uuid4())
        if transaction_id: message.execute.transaction_id = transaction_id
        message.execute.name = name
        for key, value in kwargs.items():
            message.execute.arguments[key] = str(value)
        subscription = None
        channel = constants.commands_channel + (f"/{entity_id}" if entity_id else "")
        if wait: 
            response = None
