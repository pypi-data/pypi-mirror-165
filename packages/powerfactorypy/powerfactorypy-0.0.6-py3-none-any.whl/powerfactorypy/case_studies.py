...
class PFStudyCases(powerfactorypy.PFBaseInterface):
  ...
  def __init__(self,app):
    ...
    self.hierarchy = None
    self.study_cases = []
    # ToDo base case/scen/var to copy
    self.parent_folder_study_cases = powerfactorypy.PFTranslator.get_default_study_case_folder_name(
      self.language)
    self.parent_folder_scenarios = powerfactorypy.PFTranslator.get_default_operation_scenario_folder_path(
      self.language)
    self.parent_folder_variations = powerfactorypy.PFTranslator.get_default_variations_folder_path(
      self.language)
  ...
  def create_cases(self):
    ...
    for case_num in range(number_of_cases):
      ...
      if self.add_scenario_to_each_case:
        self.create_scenario(parameter_values_string,folder_path)
  ...
  def get_folder_path(self,case_num):
    ...
    if self.hierarchy:
      ...
      for par_name in self.hierarchy:
        folder_path += par_name + "_" + str(
          self.get_value_of_parameter_for_case(par_name,case_num)) + "\\"
      return folder_path[:-1] # discard last "\\""
    else:
      return None

  def get_value_of_parameter_for_case(self,par_name,case_num):
    """Returns a parameter value for a certain case number.
    The values in 'parameter_values' can be 
      - a list where each element corresponds to a case number
  ...
  def get_case_params_value_string(self,case_num,omitted_parameters=None):
    ...
    for par_name in self.parameter_values:
      if omitted_parameters is None or par_name not in omitted_parameters:
        parameter_values_string += par_name + "_" + str(
          self.get_value_of_parameter_for_case(par_name,case_num)) + self.delimiter
    return parameter_values_string[:-1] # discard last delimiter
