...
class PFDynSimInterface(powerfactorypy.PFBaseInterface):
  ...
  def format_csv(self,file_path):
    ...
    Example first row of some column before formating: 
      '\\username.IntUser\\powerfactorypy_base.IntPrj\\Network Model.IntPrjfolder\\Network Data.IntPrjfolder\\Grid.ElmNet\\AC Voltage Source.ElmVac\\s:u0'
    ...
    with open(file_path) as read_file, open(file_path + ".temp", "w") as write_file:
      ...
      for col,path in enumerate(full_paths):
          if col > 0:
              formated_path = powerfactorypy.PFStringManipuilation.format_full_path(path,self)
