        ...
        if matches:
            ...
            if isinstance(today_matches, list):
                ...
                return MessageSegment.image(
                    await md_to_pic(md=day_analyze_builder(date, today_matches)))
        else:
            return MessageSegment.text("今日无比赛.")

    @classmethod
    async def analyze_daily(cls, date: str, inf: dict) -> MessageSegment:
        """
        内部函数，格式化今日比赛信息
        :param date: 日期
        :param inf: 比赛信息
        :return:
        """
        return MessageSegment.image(await md_to_pic(md=day_analyze_builder(date, inf)))
            ...
            for date, matches in matches.items():
                if isinstance(matches, list):
                    ...
                    _msg += MessageSegment.image(
                        await md_to_pic(md=day_analyze_builder(date, matches)))
            return _msg
        else:
