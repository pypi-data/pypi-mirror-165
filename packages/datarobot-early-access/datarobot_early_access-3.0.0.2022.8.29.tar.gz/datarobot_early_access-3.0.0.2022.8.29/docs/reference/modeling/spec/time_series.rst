...
.. code-block:: python
   ...
   # As of v3.0, can use ``Project.set_datetime_partitioning`` instead of passing the spec into ``Project.set_target`` via ``partitioning_method``.
...
.. code-block:: python
    ...
    # start the project, specifying the partitioning method (if ``Project.set_datetime_partitioning`` and ``Project.set_advanced_options`` were not used)
    project.set_target(
        target='target',
        partitioning_method=datetime_spec
