...
def _is_compatible_version(version_response: Response) -> bool:
    ...
    if server_version_string is None:
        ...
        return True
    client_version_string = _get_client_version()
    server_major, server_minor = _get_major_minor_version(server_version_string)
    client_major, client_minor = _get_major_minor_version(client_version_string)
    if server_major > client_major:
        err_msg = (
            f"Client and server versions incompatible. Server version: {server_version_string} - "
            f"Client version: {client_version_string}"
        )
        warnings.warn(err_msg)
        return False
    if server_major < client_major or server_minor < client_minor:
        warn_msg = (
            "Client version is ahead of server version, you may have incompatibilities. "
            f"Server version: {server_version_string} - Client version: {client_version_string}"
        )
        warnings.warn(warn_msg)
    return True
