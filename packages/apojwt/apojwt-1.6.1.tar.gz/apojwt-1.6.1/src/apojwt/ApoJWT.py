...
class ApoJWT():
    ...
    def __init__(self, secret: str, exp_period: int=900, iss: str="", asynch: bool=False, server_audience: str="default", algorithm: str="HS256", admin_permission: str=None, token_finder=None, permission_formatter=None, exception_handler=None):        
                ...
                async def wrapper(*args, **kwargs):
                    ...
                    if formatted_perm not in decoded_permissions:
                        if self.__exception_handler is None:
                            ...
                            kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=403, exception_type="unauthorized", msg="JWT is not authorized for this action")
    ...
    def refresh(self, fn):
        ...
        if self.asynch is True:
            ...
            async def wrapper(*args, **kwargs):
                ...
                if "token_subject" in kwargs.keys():
                    kwargs.pop("token_subject")
                if "token_data" in kwargs.keys():
                    kwargs.pop("token_data")
                ...
                kwarg_dict = dict(token_data=token_payload["data"], token_subject=token_payload["sub"], access_token=new_token)
                self.__form_kwargs(fn, kwarg_dict, kwargs)
                return await fn(*args, **kwargs)
            return wrapper
        else:
    ...
    def __decode(self, token, secret=None, verify_exp=True, *args, **kwargs):
        ...
        try:
            ...
            if self.__exception_handler is None:
                ...
                kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=401, exception_type="signature", msg="JWT signature is invalid")
                self.__form_kwargs(self.__exception_handler, kwarg_dict, kwargs)
                self.__exception_handler(*args, **kwargs)
            ...
            if self.__exception_handler is None:
                ...
                kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=403, exception_type="expired", msg="JWT signature has expired")
                self.__form_kwargs(self.__exception_handler, kwarg_dict, kwargs)
                self.__exception_handler(*args, **kwargs)
                raise not_implemented
            ...
            if self.__exception_handler is None:
                ...
                kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=401, exception_type="issuer", msg="JWT issuer is invalid")
                self.__form_kwargs(self.__exception_handler, kwarg_dict, kwargs)
                self.__exception_handler(*args, **kwargs)
                raise not_implemented
            ...
            if self.__exception_handler is None:
                ...
                kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=401, exception_type="audience", msg="JWT audience is invalid")
                self.__form_kwargs(self.__exception_handler, kwarg_dict, kwargs)
                self.__exception_handler(*args, **kwargs)
                raise not_implemented
            ...
            if self.__exception_handler is None:
                ...
                kwarg_dict = dict(token_data=dict(), token_subject="", access_token="", code=401, exception_type="invalid", msg="JWT is invalid")
                self.__form_kwargs(self.__exception_handler, kwarg_dict, kwargs)
                self.__exception_handler(*args, **kwargs)
                raise not_implemented
