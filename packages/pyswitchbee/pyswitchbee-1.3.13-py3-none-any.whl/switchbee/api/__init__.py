            ...
            for item in zone[ApiAttribute.ITEMS]:
                ...
                # add group switch only of hardware type != VIRTUAL as we can't read their statuses
                    ...
                    self._devices_map[item[ApiAttribute.ID]] = SwitchBeeThermostat(
                        ...
                        unit=ThermostatTemperatureUnit(
                            item[ApiAttribute.TEMPERATURE_UNITS]
                        ),
                    )
    ...
    async def fetch_states(
        ...
        for device_state in states[ApiAttribute.DATA]:
            ...
            if self._devices_map[device_id].type == DeviceType.Dimmer:
                ...
                self._devices_map[device_id].mode = ThermostatMode(
                    device_state[ApiAttribute.STATE][ApiAttribute.MODE]
                )
                self._devices_map[device_id].fan = ThermostatFanSpeed(
                    device_state[ApiAttribute.STATE][ApiAttribute.FAN]
                )
