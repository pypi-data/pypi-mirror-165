...
class ApiClient(object):
    ...
    def __getattr(self, name):
        if name in self.DUPLICATE_SCHEMA_MAPPING:
            return getattr(ntnx_vmm_py_client.Ntnx, self.DUPLICATE_SCHEMA_MAPPING[name])
        if hasattr(ntnx_vmm_py_client.Ntnx, name.split('.')[-1]):
            return getattr(ntnx_vmm_py_client.Ntnx, name.split('.')[-1])
        else:
            return getattr(ntnx_vmm_py_client.Ntnx, name)
