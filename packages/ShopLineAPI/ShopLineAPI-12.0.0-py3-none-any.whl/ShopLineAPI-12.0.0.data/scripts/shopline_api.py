...
def start_interpreter(**variables):
    ...
    console = type("shopify " + shopline.version.VERSION, (code.InteractiveConsole, object), {})
    import readline
...
class Tasks(object):
    ...
    def console(cls, connection=None):
        ...
        shopline.ShopifyResource.activate_session(session)

        start_interpreter(shopify=shopline)

    @classmethod
    @usage("version")
    def version(cls):
        """output the shopify library version"""
        print(shopline.version.VERSION)
    ...
    def _session_from_config(cls, config):
        session = shopline.Session(config.get("domain"), config.get("api_version", cls._default_api_version))
