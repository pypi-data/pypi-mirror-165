                ...
                if data[cause].dtype == 'category':
                    ...
                    while True:
                        ...
                        if data[cause].value_counts()[selected_state].sum() > m * data.shape[0] or len(states) == 1:
                            ...
                            m_l = max((data.shape[0] * m - data[cause].isin(selected_state).sum() * m_h) / data[cause].isin(
                                states).sum(), 0)
                            break
                    data_missing[var][
...
def cor_test(data, cols):
    data = pandas.DataFrame(data)
    if len(cols) == 2:
        return pg.partial_corr(data, cols[0], cols[1])['p-val'][0]
    elif len(cols) > 2:
        return pg.partial_corr(data, cols[0], cols[1], cols[2:])['p-val'][0]
    else:
...
# orient v-structrues
def orient_v_structure(data, varnames, pc, sepset, ci_test, threshold=0.05):
    dag = {}
    v_candidate = {}
    for var in pc:
        dag[var] = {}
