...
class ReleaseManager:
    ...
    def build_ecommerce_pack(self, bitbucket_workspace, bitbucket_repo_slug):
        ...
        for full_module_name, module_item in module_dependencies.items():
            ...
            self.file_manager.add_license_to_php_files(
                ...
                self.json_parser.get_variable_from_file('version', f"{modulepath}/composer.json")
            )
            os.system(f"echo See https://aheadworks.com/end-user-license-agreement/ >> {module_path}/license.txt")

            module_zip_filepath = f"{artifacts_dir}/{module_name}.zip"
            print(f"\t\u21E8 Packing {module_path} into {module_zip_filepath}\n")
            self._pack_module(module_path, module_zip_filepath)
            artifacts.append(module_zip_filepath)
        ...
        for filename in self._get_store_package_names(magento_module_info[core_module_name], composer['version']):
            store_package = f"{artifacts_dir}/{filename}"
            print(f"Packing app from {build_dir} into {store_package}")
            self._pack_module(build_dir, store_package, "app")
            artifacts.append(store_package)

        print(f"\nUploading artifacts\n{artifacts}...\n")
        # TODO: move to bitbucket api manager along with the fileupload code
        data = { 'grant_type': 'client_credentials' }
        response = requests.post('https://bitbucket.org/site/oauth2/access_token', data=data, auth=(os.getenv('BITBUCKET_KEY'), os.getenv('BITBUCKET_SECRET')))
        bb_token = json.loads(response.text)['access_token']
        bb_auth_headers = {'Authorization': f"Bearer {bb_token}", }
        self._upload_artifacts(bitbucket_workspace, bitbucket_repo_slug, bb_auth_headers, artifacts)
    ...
    def build_metapackage_pack(self, bitbucket_workspace, bitbucket_repo_slug, artifacts_dir):
        ...
        for full_module_name in composer["require"]:
            ...
            print(f"\t\u21E8 Packing {module_path} into {zip_package_path}\n")
            self._pack_module(module_path, zip_package_path)
            artifacts.append(zip_package_path)

            upload_as = f"{metapackage_module_name}-{module_composer['version']}.zip"
            os.system(
                f"echo '{upload_env} deploy comp -p send-package --filename={upload_as} {zip_package_path}' >> {artifacts_dir}/upload")
        ...
        marketplace_package_path = f"{artifacts_dir}/{bitbucket_repo_slug}.zip"
        self._pack_module("/tmp", marketplace_package_path, "composer.json")
        artifacts.append(marketplace_package_path)
    ...
    def _pack_module(self, source_module_path, target_package_path, local_path="./"):
        pwd = os.getcwd()
        with cd(f"{source_module_path}"):
            os.system(f"zip -qr {target_package_path} {local_path}")
        os.system(f"cd {pwd}")
    ...
    def _get_magento_package_name(self, modulepath):
        with open(f"{modulepath}/registration.php") as reg:
            l = reg.readlines()
            for line in l:
    ...
    def _upload_artifacts(self, bitbucket_workspace, bitbucket_repo_slug, bb_auth_headers, filepath):
        request = f"https://api.bitbucket.org/2.0/repositories/{bitbucket_workspace}/{bitbucket_repo_slug}/downloads"
        if type(filepath) is not list:
            filepath = [filepath, ]
        for f in filepath:
            files = {'files': open(f, 'rb')}
            response = requests.post(request, files=files, headers=bb_auth_headers)
            print(response)
