...
__all__ = ['json_load', 'json_dump', 'make_reproducible', 'get_logging_level', 'delete_logger', 'set_logger', 'set_empty_logger',
           ...
           'check_last_part', 'get_action', 'get_norun', 'argnames', 'store_attr', 'get_specific_dict_param',
    ...
    for file_path in file_paths:
        try:
            ...
            if copy_no_repo_files:
                ...
                out_of_repo_path.mkdir (parents=True, exist_ok=True)
                shutil.copy (file_path, str(out_of_repo_path))
                out_of_repo_path = out_of_repo_path / Path(file_path).name
                git.add (str(out_of_repo_path))
                if len(git.status ('-s', '--', str(out_of_repo_path)))>0:
                    changed_files = True
            else:
