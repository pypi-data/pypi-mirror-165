...
def submit_test_colab(key, ipynb_name, func):
  ...
  pipes2 = subprocess.Popen(['pipreqs','--ignore', './drive,./train', './'], cwd=current_cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
  std_out, std_err = pipes2.communicate()

  # sed -i '/aifactory/d' requirements.txt
  ...
  pipes4 = subprocess.Popen(['sed','-i', 's/main()/#main()/g2', './' + filename + '.py'], cwd=current_cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
  ...
  # res = requests.post(api_url_test, files = {'file': open("./submit.zip",'rb', )}, data=values)  
  # if res.status_code == 200 or res.status_code == 201: 
  #   print("success")
  #   return
  # print(res.status_code)  
