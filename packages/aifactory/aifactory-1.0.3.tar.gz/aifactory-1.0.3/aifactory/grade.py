...
def submit_test_new(key, func):
  values = {"key": key}
  # pipes1 = subprocess.Popen(['jupiter','nbconvert', '--to','script', 'task3_new.ipynb'], cwd=current_cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
  ...
  for (path, dir, files) in os.walk("./"):
    for file in files:        
      if "train" not in path and "submit.zip" not in file:
