...
class SceneEngineClient(object):
    ...
    def add_session(self,
        ...
        status:
            Session status. By default it is READY but can be IN PROGRESS as well
    ...
    def __check_entity_durations(self, session_uid: str, timestamps: List[Union[str, datetime, None]]):
        ...
        """
        session_metadata = self.get_asset_manager(asset_type=AssetsConstants.SESSIONS_ASSET_ID).get_metadata(id=session_uid)
        ...
        if clean_timestamps:
            ...
            if duration > max_duration:
                raise SessionError(f"Expected length {duration} sec is larger than max {max_duration} sec for session {session_uid}.")
