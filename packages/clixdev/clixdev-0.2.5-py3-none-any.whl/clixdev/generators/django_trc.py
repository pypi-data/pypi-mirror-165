...
def {misc.get('token')}(request, *args, **kwargs):
\tif request.method == '{req.get('method')}':
\t\ttry:
\t\t\theaders = request.headers
{body_code if endpoint.get('body').get('type') == 'json' else ''}
    ...
    for model in app.get('models'):
        ...
        for field in fields.values():
            ...
            if type == 'UUIDField':
                ...
                _class += f"\t{field[1]} = models.ForeignKey({field[2].split('.')[0]}, on_delete=models.SET_NULL, null={field[5]}, blank={field[6]}, unique={field[8]})\n"
            else:
