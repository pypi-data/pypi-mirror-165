...
class NumTensorSeq(ABC):
    """
    This is base class for numeric tensor sequences.
    """
    ...
    def split(self, splitter: TrainValTestSpliter) -> Tuple['NumTensorSeq', 'NumTensorSeq', 'NumTensorSeq']:
    ...
    @abstractmethod
    def scale(self, scaler: Optional[Scaler]) -> 'ScaledNumTensorSeq':
        raise NotImplementedError()


class NumScalarVecSeq(NumTensorSeq):
    """
    This is class represents sequence of numeric vectors of scalar features.
    ...
    def scale(self, scaler: Optional[Scaler]) -> 'ScaledNumScalarVecSeq':
        return ScaledNumScalarVecSeq.from_seq(name=self.name, seq=self, scaler=scaler)


class Num1DVecSeq(NumTensorSeq):
    """
    ...
    def scale(self, scaler: Optional[Scaler]) -> 'ScaledNum1DVecSeq':
        return ScaledNum1DVecSeq.from_seq(name=self.name, seq=self, scaler=scaler)


class ScaledNumTensorSeq(ABC):
    """
    ...
    @classmethod
    def from_seq(cls, name: str, seq: NumTensorSeq, scaler: Optional[Scaler]) -> 'ScaledNumTensorSeq':
        if scaler is not None:
    ...
    def __add__(self, other: 'ScaledNumTensorSeq') -> 'ScaledNumTensorSeq':
...
class ScaledNumScalarVecSeq(ScaledNumTensorSeq):
    """
    This is class represents sequence of numeric vectors of scalar features.
    It can be served as input for Dense NN layers.
...
class ScaledNum1DVecSeq(ScaledNumTensorSeq):
    """
    This is class represents sequence of multi-feature time-series numeric data,
        where each feature is a 1D vector representing one time-series.
    It can be served as input for Conv1D or RNN sequntial types of layers.
