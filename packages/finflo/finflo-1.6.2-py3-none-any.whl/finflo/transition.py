    ...
    def intermediator_transition_handler(self, type, action, t_id):
        ...
        if action == FinFlotransition.gets_default_return_action().lower():
            ...
            wf = workflowitems.objects.update_or_create( transitionmanager=gets_model or gets_model.id, defaults= {"initial_state" : gets_wf.initial_state, "interim_state" : gets_action.to_state.description ,
                "final_state" : gets_action.to_state.description, "action" : action, "subaction" : sign_list[1 + gets_model.sub_sign], "model_type" : type.upper(), "event_user" : get_current_user() , "current_from_party" : gets_action.from_party , "current_to_party" : gets_action.to_party})
        ...
        # len action and sub_sign
            ...
            def Transition_Handler():
                    ...
                    if len(sign_lists)-1 != gets_model.sub_sign:
                        ws = workflowitems.objects.update_or_create( transitionmanager=gets_model or gets_model.id, defaults= {"initial_state" : gets_action.from_state.description, "interim_state" :  sign_lists[1 + gets_model.sub_sign], 
                            "final_state" : gets_action.to_state.description, "action" : action, "subaction" : sign_list[1 + gets_model.sub_sign], "model_type" : type.upper(), "event_user" : get_current_user() , "current_from_party" : gets_action.from_party , "current_to_party" : gets_action.to_party})
                        we = workevents.objects.create(workflowitems=gets_wf, event_user=get_current_user(),  initial_state=gets_action.from_state.description,
                                                  interim_state = sign_lists[1 + gets_model.sub_sign], final_state=gets_action.to_state.description, action=action, subaction=sub_action[0], type=type.upper(), from_party = gets_action.from_party , to_party = gets_action.to_party)
                        gets_model.sub_sign += 1
                        gets_model.save()

                    elif len(sign_lists)-1 == int(gets_model.sub_sign):
