    ...
    if v_latest > v_current:
        ...
        if chat:
            ...
            if 'CHAT_WEBHOOK_URL' in os.environ:
                ...
                payload = payload.replace('{repository}', control['sources'])
                if 'CI' in os.environ:
                    payload = payload.replace('{branch}', os.environ['CI_COMMIT_BRANCH'])
                else:
                    payload = payload.replace('{branch}', 'unknowni')
...
def versionPackage():
    """ Return package version define by WAPT/control
...
def versionLatest():
    ...
    results: string
    """
    latest_version = latestVersion(True)
    v_latest = version.parse(latest_version)
    return v_latest
