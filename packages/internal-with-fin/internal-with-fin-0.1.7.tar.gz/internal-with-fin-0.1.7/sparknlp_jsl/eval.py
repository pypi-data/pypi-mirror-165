...
class NerDLMetrics(ExtendedJavaWrapper):
    ...
    def __init__(self, mode="full_chunk", dropO=False):
        ExtendedJavaWrapper.__init__(self, "com.johnsnowlabs.nlp.eval.ner.NerDLMetrics", mode, dropO)

    def computeMetricsFromDF(self, df, prediction_col="ner", label_col="label"):
        jdf = self._java_obj.computeMetricsFromDF(df._jdf, prediction_col, label_col)
