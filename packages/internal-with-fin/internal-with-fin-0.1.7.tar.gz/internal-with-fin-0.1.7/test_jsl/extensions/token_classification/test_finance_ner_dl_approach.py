...
class FinanceNerSuite(unittest.TestCase):
    ...
    def buildAndFit(self, annoToFit) -> PipelineModel:
        ...
        # jsl_ner = sparknlp_jsl.extensions.finance.FinanceNerModel.pretrained("finner_roles", "en", "finance/models") \
        #     .setInputCols(["sentence", "token", "embeddings"]).setOutputCol("ner")
        #
        # jsl_ner.write().overwrite().save("./tmp_clinical_ner")
        # emb.transform(self.test_data).write.mode("overwrite").parquet(self.test_data_parquet_path)

        return Pipeline().setStages([emb, annoToFit]).fit(self.train_data)

    def test_pretrained_model(self):
        model = sparknlp_jsl.extensions.finance.FinanceNerApproach() \
        ...
        trainedModel = pipelineModel.stages[1]
