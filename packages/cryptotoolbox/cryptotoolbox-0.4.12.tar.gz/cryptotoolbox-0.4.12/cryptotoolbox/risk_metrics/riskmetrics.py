...
def compute_plot_excel_kpis_meterics_metrics(data, title, local_root_directory='./', plot_html=False, write_file=True):
    ...
    for me_strat in daily_df.columns:
        if not 'drawdown' in me_strat:
            ...
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'advance_kpis_details_{title}.xlsx',
                                        engine='xlsxwriter')
            ...
            if write_file:
                ...
                writer.save()
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'perf_dd_{me_strat_name}_{title}.xlsx',
                                        engine='xlsxwriter')
            ...
            epochkpis_df = epochkpis_df.drop(columns=['date', 'epoch_number'])

            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'epoch_{me_strat_name}_{title}.xlsx',
                                        engine='xlsxwriter')
