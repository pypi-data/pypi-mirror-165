...
def compute_plot_excel_kpis_meterics_metrics(data, title, local_root_directory='./', plot_html=False, write_file=True):
    ...
    for me_strat in daily_df.columns:
        if not 'drawdown' in me_strat:
            ...
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'weekly_lo_epochs_details_{title}.xlsx',
                                        engine='xlsxwriter')
            ...
            if write_file:
                writer.save()
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'{me_strat_name}_kpi_godel_{title}.xlsx',
                                        engine='xlsxwriter')
                data.to_excel(writer, sheet_name='godel')
                writer.save()
            ################ computing the metrics per epoch (monthly !)

            daily_df['epoch_number'] = daily_df.index.year.astype(str) + '_' + daily_df.index.month.astype(str)
            ...
            epochkpis_df = daily_df[['epoch_number', me_strat]].groupby(['epoch_number']).apply(go_comp_kpi)

            epochkpis_df = epochkpis_df.sort_index()
            epochkpis_df = pd.DataFrame.from_records(epochkpis_df.to_dict()).T
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'monthly_{me_strat_name}_epochs_godel_{title}.xlsx',
                                        engine='xlsxwriter')
