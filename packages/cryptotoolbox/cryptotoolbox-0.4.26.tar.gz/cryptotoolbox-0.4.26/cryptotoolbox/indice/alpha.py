...
def compute_alpha_metrics(data_df = None, core_tokens=['BTC', 'ETH'], extra_tokens=['AVAX'], moderating_factor = 0.1, frequency='weekly', generate_kpis = True, plot_html=True, stop_loss_quantile = 0.75,take_profit_quantile = 0.75,write_excel_file = True, local_root_directory = None, plot_metrics = True,plot_html_save = True):
    ...
    if generate_kpis:
        ...
        btd_kpis, ytd_kpis, past_returns = riskmetrics.compute_extensive_kpis(
            data_df=merged_df.copy(), strat='COMBO_STRAT')
        ...
        merged_df['COMBO_STRAT_compo'] = merged_df.apply(comp_alloc, axis=1)

        ############# strategy drawdown
        dd_data_df = merged_df.copy() #pd.merge(merged_df.copy(), sol_ohlc_df.copy(), left_index=True,
                              #right_index=True)
        ...
        if plot_html_save:
            ...
            dollar_df = merged_df[['DOLLAR_COMBO_STRAT']].copy()
            ...
            fig = realtime_plotting_utility.plot_multiple_time_series(
                data_df=merged_df[['long_expo', 'short_expo']],
        ...
        whole_period_kpi_df = riskmetrics.get_kpi(merged_df[[me_strat]])
        whole_period_kpi_df = whole_period_kpi_df.iloc[1:]

        kpi_df = riskmetrics.get_kpi(merged_df[[me_strat]])
        stub = f'mf_{moderating_factor}_strat_{me_strat}'
        kpi_df.columns = [stub]
        to_store_df = merged_df.copy()
        ...
        if write_excel_file:
            writer = pd.ExcelWriter(local_root_directory + 'weekly_ls_epochs_details_ls.xlsx',
                                    engine='xlsxwriter')
    ...
    btd_kpis, ytd_kpis, past_returns = riskmetrics.compute_extensive_kpis(data_df=merged_df.copy(),strat='COMBO_STRAT')
