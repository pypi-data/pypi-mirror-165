...
parser = argparse.ArgumentParser(prog='mDELTA', description=' Multifuricating Developmental cEll Lineage Tree Alignment(mDELTA) ',
                                 epilog='More details on https://github.com/Chenjy0212/modelta')
parser.add_argument('TreeSeqFile', type=str,
                    help='[path/filename] A text file storing cell lineage tree #1 in newick format. Tips can be labeled by name or cell type. Branch lengths should be removed.')
parser.add_argument('TreeSeqFile2', type=str,
                    help='[path/filename] A text file storing cell lineage tree #2 in newick format. Tips can be labeled by name or cell type. Branch lengths should be removed.')
parser.add_argument('-nt', '--Name2TypeFile', type=str, default='',
                    help='[path/filename] List of correspondance between tip name and cell type for cell lineage tree #1.')
parser.add_argument('-nt2', '--Name2TypeFile2', type=str, default='',
                    help='[path/filename] List of correspondance between tip name and cell type for cell lineage tree #2.')
...
parser.add_argument('-t', '--top', type=int, default=0,
                    help='[int > 0] Performs local (instead of global) alignment, and output the top NUM local alignments with the highest score (e.g. `-t 10`). In the case of global alignment, this parameter should be omitted.')
parser.add_argument('-ma', '--mav', type=float,
                    default=2., help=' [float] Default=2.')
parser.add_argument('-mi', '--miv', type=float, default=-1.,
                    help=' [float] Default=-1. Shorthand for a simple matching score scheme, where the matching score between a pair of the same cell types is MAV and all other pairs are MIV. (e.g. `-ma 2 -mi -2`). Overridden by `-sd`.')
parser.add_argument('-p', '--pv', type=float, default=-1.,
                    help=' [float] The score for pruning a tip of the tree (e.g. `-p -2`). Default to -1.')
parser.add_argument('-T', '--Tqdm', type=int, default=1,
                    help=' [0(off) or 1(on)] Toggle for the jupyter notebook environment.')
parser.add_argument('-n', '--notebook', type=int, default=0,
                    help='[0(off) or 1(on)] Toggle for the jupyter notebook environment.')
parser.add_argument('-P', '--PERM', type=int, default=0,
                    help='[int > 0] Toggle for the statistical significance. For each observed alignment, the aligned trees will be permuted PERM times to generate a null distribution of alignment scores, with which a P value can be calculated for the observed alignment score.')
parser.add_argument('-a', '--Alg', type=str, default='KM',
                    help='[KM / GA] Use Kuhn-Munkres or Greedy Algorithm to find the optimal alignment score.')
parser.add_argument('-c', '--CPUs', type=int, default=50,
                    help='[int > 0] Number of threads for multi-processing. Default to 50., it can reach the maximum number of local CPU cores - 1.')
parser.add_argument('-o', '--output', type=str, default='KM',
                    help='[path/filename] Output filename')
parser.add_argument('-x', '--diff', type=int, default=0,
                    help=' [int > 0] Alignment must consist of a minimal of DIFF% aligned cell pairs that are different from previous(better) local alignments in order to be considered as another new alignment (e.g. `-x 20` means 20 persent).')
parser.add_argument('-mg', '--merge', type=float, default=10,
                    help='[float] This is the scaling factor for calculating the score of merging an internal node (e.g. -mg -1), which is multiplied by the number of tips of the internal node to be merged. Default to 0.')
        ...
        if(self.nodeobj[0] == '('):  # 存在括号意味着还没达到叶子结点
            ...
            for index, item in enumerate(node_list):
                if index == 0:
                    ...
                    # 赋予label
                    label = str(
                        index) if self.label == 'root' else self_tmp.label+'_'+str(index)
                    self.left = MultiTree(item, label=label)
                    self = self.left
                    self.CreatTree()
                else:
                    label = str(
                        index) if not '_' in self.label else self_tmp.label+'_'+str(index)
...
def scoremat(TreeSeqFile: str,
    ...
    if ScoreDictFile == '':
        ...
        score_dict = QuantitativeScoreFile(root1.leaves(
            []), root2.leaves([]), mav, miv, ScoreDictFile)
                ...
                for i in range(lllloop[loop_index[0]]):
                    for j in range(llllloop[loop_index[1]]):
                        ...
                        matrix_values[i_index][j_index] = GetMaxScore(trace=trace_value,
                                                                      ...
                                                                      prune=pv,
                                                                      lll_label=[
                                                                          i.label for i in lll[0]],
                                                                      llll_label=[
                                                                          i.label for i in llll[0]],
                                                                      merge=merge,)
                        pbar.update(1)
        # print(ttrace)
        ...
        for i in tracemat_value[tuple(rac)]:
            if isinstance(i[0], int) and isinstance(i[1], int):
                ...
                if i == [] or (i[0] < root1.leaf_count() and i[1] < root2.leaf_count()):
                    ...
                    mat[tuple(i)] = -99999.
                    # print(i)
                    changemat(i, tracemat, tracemat_value,
                              mat, list_tmp1, list_tmp2)
        # return mat
            ...
            if isinstance(i[0], int) and isinstance(i[1], int):
                if i[0] < root1.leaf_count() and i[1] < root2.leaf_count():
                    ...
                    if tree_tmp1[-1] == '(':
                        ...
                        tree_tmp2.append(',')
                        tree_tmp2.append('(')
                    getmatchtree(i, lllnode_obj, llllnode_obj,
                                 tracemat_value, mat, tree_tmp1, tree_tmp2)
                    tree_tmp1.append(')')
                    tree_tmp2.append(')')
            else:
    ...
    if top == 0:  # 默认情况下
        ...
        tree_tmp2 = ['(']
        mat_tmp2[-1, -1] = -99999.
        getmatchtree([-1, -1], lllnode_obj, llllnode_obj,
                     trace_value, mat_tmp2, tree_tmp1, tree_tmp2)
        tree_tmp1.append(');')
        tree_tmp2.append(');')
        ...
        tree_tmp4 = ['(']
        mat_tmp3[-1, -1] = -99999.
        getmatchtree([-1, -1], lllnode_label, llllnode_label,
                     trace_value, mat_tmp3, tree_tmp3, tree_tmp4)
        tree_tmp3.append(');')
        tree_tmp4.append(');')
        ...
        for jjj in range(top):
            ...
            changemat([del_i_index, del_j_index], ttrace,
                      trace_value, mat_tmp, list_tmp1, list_tmp2)

            tree_tmp1 = ['(']
            tree_tmp2 = ['(']
            mat_tmp2[del_i_index, del_j_index] = -99999.
            getmatchtree([del_i_index, del_j_index], lllnode_obj,
                         llllnode_obj, trace_value, mat_tmp2, tree_tmp1, tree_tmp2)

            tree_tmp3 = ['(']
            tree_tmp4 = ['(']
            mat_tmp3[del_i_index, del_j_index] = -99999.
            getmatchtree([del_i_index, del_j_index], lllnode_label,
                         llllnode_label, trace_value, mat_tmp3, tree_tmp3, tree_tmp4)
            ...
            if jjj > 0:
                if len(scorelist[-1]['Root1_match']) == 0:
                    ...
                    percent = (len(list(set(list_tmp1) - set(scorelist[-1]['Root1_match'])) + list(set(
                        scorelist[-1]['Root1_match']) - set(list_tmp1))) / len(scorelist[-1]['Root1_match']))*100.
