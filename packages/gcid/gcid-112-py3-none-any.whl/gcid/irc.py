...
# pylint: disable=E1101,C0115,C0116,R0902,R0903,R0904,R0912,R0915,C0411,W0212
...
from gcid.database import Class, find, last
from gcid.default import Default
from gcid.event import Event
from gcid.json import save
from gcid.object import Object, edit, keys, update
from gcid.table import Callbacks
from gcid.thread import launch
from gcid.utils import elapsed, fntime, locked, printable
...
def cfg(event):
    ...
    last(config)
    if not event.sets:
        event.reply(printable(config, keys(config), skip="realname,sleep,username"))
    else:
