...
from wedeliver_core.wedeliver_core.helpers.exceptions import AppMicroFetcherError, AppFetchServiceDataError
import requests
import json

from wedeliver_core.wedeliver_core.helpers.get_obj_value import get_obj_value
...
class MicroFetcher(object):
    ...
    def __init__(self, service_name):
        self.app = WeDeliverCore.getApp()
        self.service_url = self.app.config.get(service_name)
        self.service_name = service_name
        if not self.service_url:
            raise AppMicroFetcherError('Service {} not defined on Env'.format(self.service_url))
