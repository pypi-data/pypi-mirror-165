...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        if config.lora_only == 1:
            ...
            self.c_attn = adamixlib.AdamixLoRAMergedLinear(
                in_features=nx, out_features=n_state * 3,
                r=config.lora_attn_dim, 
                lora_alpha=config.lora_attn_alpha, 
                lora_dropout=config.lora_dropout, 
                ...
                fan_in_fan_out=True,
                merge_weights=False,
                share_down=config.share_down,
                share_up=config.share_up,
...
class MLP(nn.Module):
    def __init__(self, n_state, config):  # in MLP: n_state=3072 (4 * n_embd)
        ...
        if config.adamix_only == 1:
            self.ExpertSoup = ExpertSoup(nx, config.lora_dim, 'gelu', num_expert=config.n_experts, sharing_down=config.share_down, sharing_up=config.share_up)
