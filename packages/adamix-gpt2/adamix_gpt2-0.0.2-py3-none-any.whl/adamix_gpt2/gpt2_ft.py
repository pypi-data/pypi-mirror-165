...
parser.add_argument('--adamix_checkpoint', default=None, help='adamix trained checkpoint path')
...
parser.add_argument('--share_down', type=int, default=0, help='share adamix downsample')

parser.add_argument('--share_up', type=int, default=0, help='share adamix upsample')
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                ...
                if hasattr(lm_net, 'module'):
                    torch.save({'model_state_dict': adamixlib.adamix_state_dict(model.module.state_dict())}, model_path)
                else:
                    torch.save({'model_state_dict': adamixlib.adamix_state_dict(model.state_dict())}, model_path)
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.sm':
        config = GPT2Config(
            ...
            share_down=args.share_down,
            share_up=args.share_up,
    ...
    if args.adamix_checkpoint is not None:
        print('loading adamix checkpoint weights.')
        adamix_state_dict = torch.load(args.adamix_checkpoint, map_location='cpu')
        if 'model_state_dict' in adamix_state_dict:
            adamix_state_dict = adamix_state_dict['model_state_dict']
        print(lm_net.load_state_dict(adamix_state_dict, strict=False))

    lm_net = lm_net.cuda()

    if args.lora_dim > 0:
        adamixlib.mark_only_lora_as_trainable(lm_net)
    ...
    if args.rank == 0:
        ...
        if hasattr(lm_net, 'module'):
            torch.save({'model_state_dict': adamixlib.adamix_state_dict(lm_net.module.state_dict())}, model_path)
        else:
            torch.save({'model_state_dict': adamixlib.adamix_state_dict(lm_net.state_dict())}, model_path)
