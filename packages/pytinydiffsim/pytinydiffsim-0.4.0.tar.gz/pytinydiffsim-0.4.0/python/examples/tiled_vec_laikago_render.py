...
if 1:
    ...
    viz = g.OpenGLUrdfVisualizer(width=width, height=height, window_type=2)
    ...
    cuda_num_bytes = width*height*4*4 #4 component float, each float 4 bytes
    print("cuda_num_bytes=", cuda_num_bytes)

    ttensor = torch.zeros(width*height*4, dtype=torch.float32, device="cuda")
