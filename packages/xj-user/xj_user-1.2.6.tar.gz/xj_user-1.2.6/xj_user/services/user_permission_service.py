...
class PermissionService():
    ...
    def get_group_user(user_id):
        inner_group = BaseInfo.objects.filter(id=user_id) \
            .annotate(user_permission_id=F("user_group__permission_id")) \
            .annotate(parent_group_id=F("user_group__parent_group")) \
            .values('user_group_id', "user_permission_id", "parent_group_id", ).first()
        user_group_id = int(inner_group['user_group_id'])
        parent_group_id = int(inner_group['parent_group_id'])
        child_group_ids = [i['id'] for i in list(Group.objects.filter(parent_group=parent_group_id).values("id"))]
        res_set = {}
        res_set['GROUP_INSIDE'] = [i['id'] for i in list(BaseInfo.objects.filter(user_group_id=user_group_id).values("id"))]
        res_set['GROUP_PARENT'] = [i['id'] for i in list(BaseInfo.objects.filter(user_group_id=parent_group_id).values("id"))]
        res_set['GROUP_CHILDREN'] = [i['id'] for i in list(BaseInfo.objects.filter(user_group_id__in=child_group_ids).values("id"))]
        res_set['GROUP_OUTSIDE'] = []
        return res_set, None
    ...
    def get_user_group_permission(user_id, module=None, feature="USER_GROUP", type=None):
        ...
        permission_dict = BaseInfo.objects.filter(id=user_id).annotate(user_permission_id=F("user_group__permission_id")).values("user_permission_id").first()
        params.setdefault("permission_id", permission_dict.get("user_permission_id", 0))
