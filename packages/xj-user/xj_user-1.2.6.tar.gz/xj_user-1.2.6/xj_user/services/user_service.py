...
path = str(Path(__file__).resolve().parent.parent) + "\\concig.ini"
config = Config(path=path)


# PS：config.get_items("xj_user") 可以直接获取所有参数并转换自带你对象
# config_dict_obj = JDict(config.get_items("xj_user"))
# print(config_dict_obj.asd)
...
class UserService:
    ...
    def check_login(user_id, password, account):
        ...
        token = jwt.encode(payload={'account': account, 'user_id': user_id, "exp": expire_timestamp}, key=config.get('xj_user', 'JWT_SECRET_KEY', "@zxmxy2021!"))
    ...
    def check_login_wechat(user_id, phone):
        ...
        # 过期时间
        expire_timestamp = datetime.utcnow() + timedelta(days=int(config.get('xj_user', 'DAY', 7)), seconds=int(config.get('xj_user', 'SECOND', 1)))
        # 为本次登录生成Token并记录
        # todo 漏洞，当用户修改用户名时，并可能导致account失效，是否存用户ID更好
        token = jwt.encode(payload={'account': phone, 'user_id': user_id, "exp": expire_timestamp}, key=config.get('xj_user', 'JWT_SECRET_KEY', "@zxmxy2021!"))
        # payload = jwt.decode(token, key=Config.getIns().get('xj_user', 'JWT_SECRET_KEY'), verify=True, algorithms=["RS256", "HS256"])
        # print("> payload:", payload)
        auth_set.token = token
        auth_set.save()
    ...
    def check_token(token):
        ...
        try:
            ...
            jwt_payload = jwt.decode(token, key=config.get('xj_user', 'JWT_SECRET_KEY', "@zxmxy2021!"), verify=True, algorithms=["RS256", "HS256"])
