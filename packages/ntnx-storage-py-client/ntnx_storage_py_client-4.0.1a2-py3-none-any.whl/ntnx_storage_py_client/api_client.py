...
from ntnx_storage_py_client.configuration import Configuration
import ntnx_storage_py_client.Ntnx
from ntnx_storage_py_client import rest
...
class ApiClient(object):
    ...
    def __call_api(
        ...
        if response_data.status == 401:
            ...
            if response_data.status == 401:
                raise ntnx_storage_py_client.rest.ApiException(http_resp=response_data)
    ...
    def __getattr(self, name):
        if name in self.DUPLICATE_SCHEMA_MAPPING:
            return getattr(ntnx_storage_py_client.Ntnx, self.DUPLICATE_SCHEMA_MAPPING[name])
        if hasattr(ntnx_storage_py_client.Ntnx, name.split('.')[-1]):
            return getattr(ntnx_storage_py_client.Ntnx, name.split('.')[-1])
        else:
            return getattr(ntnx_storage_py_client.Ntnx, name)
