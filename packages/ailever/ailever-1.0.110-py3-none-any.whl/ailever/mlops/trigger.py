...
class Trigger:
    ...
    def reg_trigger(self, train, validation, inference=None, predict=lambda x:x, fit=lambda x:x, target=list(), refresh=False, store=True, note=None):
        ...
        X_train = train[train.columns[~train.columns.isin(target)]].values.ravel()
        y_train = train[train.columns[train.columns.isin(target)]].values.squeeze().ravel()
        X_validation = validation[validation.columns[~validation.columns.isin(target)]].values.ravel()
        y_validation = validation[validation.columns[validation.columns.isin(target)]].values.squeeze().ravel()
        X_test = inference[inference.columns[~inference.columns.isin(target)]].values.ravel()
