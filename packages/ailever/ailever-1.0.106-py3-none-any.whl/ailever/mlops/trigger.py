...
class Trigger:
    ...
    def reg_trigger(self, train, validation, inference=None, predict=lambda x:x, fit=lambda x:x, target=list(), refresh=False, store=True, note=None):
        ...
        X_train = train[train.columns[~train.columns.isin(target)]].values.squeeze().ravel()
        y_train = train[train.columns[train.columns.isin(target)]].values.squeeze().ravel()
        X_validation = validation[validation.columns[~validation.columns.isin(target)]].values.squeeze().ravel()
        y_validation = validation[validation.columns[validation.columns.isin(target)]].values.squeeze().ravel()
        X_test = inference[inference.columns[~inference.columns.isin(target)]].values.squeeze().ravel()
        y_test = inference[inference.columns[inference.columns.isin(target)]].values.squeeze().ravel()
    ...
    def feature_importance(self, **kwargs):
        explanation_columns = kwargs['feature_name'] if 'feature_names' in kwargs.keys() else list(range(len(self.__model__.feature_importances_)))
        feature_importance = pandas.DataFrame(data=self.__model__.feature_importances_[np.newaxis,:], columns=explanation_columns).T.rename(columns={0:'FeatureImportance'})
        ...
        barplot_table.plot.barh(figsize=(30, height), title='Feature Importance', color='red', ax=axes[0])
