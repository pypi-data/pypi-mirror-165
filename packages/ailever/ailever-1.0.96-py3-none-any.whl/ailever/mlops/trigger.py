...
class Trigger:
    def __init__(self, model):
        ...
        if hasattr(model, '__name__'):
            if model.__name__ in ['DecisionTreeClassifier', 'DecisionTreeRegressor']:
                from sklearn.tree import export_graphviz
                import graphviz
                self.__tree__ = lambda class_names, feature_names: graphviz.Source(
                        export_graphviz(
                            self.__model__,
                            out_file=None,
    ...
    def tree(self, **kwargs):
        if 'class_names' in kwargs.keys():
            kwargs['class_names'] = None
        elif 'feature_names' in kwargs.keys():
            kwargs['feature_names'] = None
        return self.__tree__(**kwargs)
