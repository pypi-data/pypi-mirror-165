...
def simple_line_highlighting(line, line_parts, theme):
    ...
    for begin, end in line_parts:
        ...
        elif has_line_number and begin < end_lineno_info:
            begin_line = line[begin : end_lineno_info + 1]
            arrow_position = begin_line.find("-->")
            if arrow_position != -1:
                part = Text(line[begin : arrow_position + 3], style=operator_style)
                part.append(
                    Text(line[arrow_position + 3 : end_lineno_info], style=number_style)
                )
            else:
                part = Text(line[begin:end_lineno_info], style=number_style)
            part.append(
                Text(line[end_lineno_info : end_lineno_info + 1], style=operator_style)
            )
            part.append(Text(line[end_lineno_info + 1 : end], style=code_style))
        elif line.strip() == "(...)":
