...
def get_rgb_gradient_function(min_value: T, max_value: T, color_map: str) -> Callable[[T], np.ndarray]:
...
def move_along_vector_field(mobject: Mobject, func: Callable[[np.ndarray], np.ndarray]) -> Mobject:
    mobject.add_updater(lambda m, dt: m.shift(func(m.get_center()) * dt))
    return mobject


def move_submobjects_along_vector_field(mobject: Mobject, func: Callable[[np.ndarray], np.ndarray]) -> Mobject:
...
def move_points_along_vector_field(
    mobject: Mobject, func: Callable[[float, float], Iterable[float]], coordinate_system: CoordinateSystem
) -> Mobject:
...
class VectorField(VGroup):
    ...
    def __init__(self, func: Callable[[float, float], Sequence[float]], coordinate_system: CoordinateSystem, **kwargs):
...
class ShowPassingFlashWithThinningStrokeWidth(AnimationGroup):
    ...
    def __init__(self, vmobject: VMobject, **kwargs):
        ...
        AnimationGroup.__init__(
            self,
            *[
                VShowPassingFlash(vmobject.copy().set_stroke(width=stroke_width), time_width=time_width, **kwargs)
                for stroke_width, time_width in zip(
                    np.linspace(0, max_stroke_width, self.n_segments), np.linspace(max_time_width, 0, self.n_segments)
                )
            ],
        )
