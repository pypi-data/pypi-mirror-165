...
class GraphScene(Scene):
        ...
        for x, color in zip(x_range, colors):
            ...
            points = VGroup(
                *list(
                    map(
                        VectorizedPoint,
                        [self.coords_to_point(x, 0), self.coords_to_point(x + width_scale_factor * dx, 0), graph_point],
                    )
                )
            )
    ...
    def get_vertical_line_to_graph(self, x, graph, line_class=Line, **line_kwargs):
        ...
        return line_class(self.coords_to_point(x, 0), self.input_to_graph_point(x, graph), **line_kwargs)
    ...
    def add_T_label(self, x_val, side=RIGHT, label=None, color=WHITE, animated=False, **kwargs):
        ...
        if animated:
            self.play(DrawBorderThenFill(triangle), ShowCreation(v_line), Write(T_label, run_time=1), **kwargs)
    ...
    def animate_secant_slope_group_change(
        ...
        self.play(UpdateFromAlphaFunc(secant_slope_group, update_func, run_time=run_time, **anim_kwargs), *added_anims)
