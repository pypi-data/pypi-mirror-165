...
class VMobject(Mobject):
    ...
    def set_rgba_array(self, rgba_array: npt.ArrayLike, name: str = None, recurse: bool = False):
        if name is None:
    ...
    def align_stroke_width_data_to_points(self, recurse: bool = True) -> None:
        for mob in self.get_family(recurse):
            mob.data["stroke_width"] = resize_with_interpolation(mob.data["stroke_width"], len(mob.get_points()))
    ...
    # Points
    def set_anchors_and_handles(self, anchors1: np.ndarray, handles: np.ndarray, anchors2: np.ndarray):
    ...
    def add_cubic_bezier_curve(
        self, anchor1: npt.ArrayLike, handle1: npt.ArrayLike, handle2: npt.ArrayLike, anchor2: npt.ArrayLike
    ):
        new_points = get_quadratic_approximation_of_cubic(anchor1, handle1, handle2, anchor2)
        self.append_points(new_points)

    def add_cubic_bezier_curve_to(self, handle1: npt.ArrayLike, handle2: npt.ArrayLike, anchor: npt.ArrayLike):
        """
        Add cubic bezier curve to the path.
        """
        self.throw_error_if_no_points()
        quadratic_approx = get_quadratic_approximation_of_cubic(self.get_last_point(), handle1, handle2, anchor)
    ...
    def add_line_to(self, point: np.ndarray):
        ...
        if self.long_lines:
            ...
            points = [interpolate(end, halfway, a) for a in alphas] + [interpolate(halfway, point, a) for a in alphas]
        else:
    ...
    def subdivide_sharp_curves(self, angle_threshold: float = 30 * DEGREES, recurse: bool = True):
        ...
        for vmob in vmobs:
            ...
            for tup in vmob.get_bezier_tuples():
                ...
                if angle > angle_threshold:
                    ...
                    new_points.extend(
                        [partial_quadratic_bezier_points(tup, a1, a2) for a1, a2 in zip(alphas, alphas[1:])]
                    )
                else:
    ...
    def set_points_as_corners(self, points: Iterable[np.ndarray]):
        ...
        self.set_anchors_and_handles(*[interpolate(points[:-1], points[1:], a) for a in np.linspace(0, 1, nppc)])
        return self

    def set_points_smoothly(self, points: Iterable[np.ndarray], true_smooth: bool = False):
    ...
    def get_subpaths_from_points(self, points: Sequence[np.ndarray]) -> list[Sequence[np.ndarray]]:
        nppc = self.n_points_per_curve
        ...
        return [points[i1:i2] for i1, i2 in zip(split_indices, split_indices[1:]) if (i2 - i1) >= nppc]
    ...
    def point_from_proportion(self, alpha: float) -> np.ndarray:
        ...
        i = next((i for i, x in enumerate(partials) if x >= full * alpha), len(partials))  # Default
    ...
    def get_points_without_null_curves(self, atol: float = 1e-9) -> np.ndarray:
        ...
        distinct_curves = reduce(
            op.or_, [(abs(points[i::nppc] - points[0::nppc]) > atol).any(1) for i in range(1, nppc)]
        )
    ...
    @triggers_refreshed_triangulation
    def apply_function(self, function: Callable[[np.ndarray], np.ndarray], make_smooth: bool = False, **kwargs):
    ...
    def get_shader_wrapper_list(self) -> list[ShaderWrapper]:
        ...
        # Combine data lists
        wrapper_lists = [back_stroke_shader_wrappers, fill_shader_wrappers, stroke_shader_wrappers]
        result = []
