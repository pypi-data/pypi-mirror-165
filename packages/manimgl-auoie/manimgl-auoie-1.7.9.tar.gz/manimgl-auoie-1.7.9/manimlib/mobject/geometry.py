...
class Arc(TipableVMobject):
    ...
    def init_points(self) -> None:
        self.set_points(
            Arc.create_quadratic_bezier_points(
                angle=self.angle, start_angle=self.start_angle, n_components=self.n_components
            )
        )
    ...
    @staticmethod
    def create_quadratic_bezier_points(angle: float, start_angle: float = 0, n_components: int = 8) -> np.ndarray:
        samples = np.array(
            [
                [np.cos(a), np.sin(a), 0]
...
class ArcBetweenPoints(Arc):
    def __init__(self, start: np.ndarray, end: np.ndarray, angle: float = TAU / 4, **kwargs):
...
class Dot(Circle):
    CONFIG = {"radius": DEFAULT_DOT_RADIUS, "stroke_width": 0, "fill_opacity": 1.0, "color": WHITE}
...
class Line(TipableVMobject):
    ...
    def set_points_by_ends(self, start: np.ndarray, end: np.ndarray, buff: float = 0, path_arc: float = 0):
        vect = end - start
        dist = get_norm(vect)
    ...
    def pointify(self, mob_or_point: Mobject | np.ndarray, direction: np.ndarray | None = None) -> np.ndarray:
        """
...
class DashedLine(Line):
    ...
    def __init__(self, *args, **kwargs):
        ...
        dashes = DashedVMobject(self, num_dashes=num_dashes, positive_space_ratio=ps_ratio)
        self.clear_points()
        self.add(*dashes)
...
class Arrow(Line):
    ...
    def create_tip_with_stroke_width(self):
        ...
        if len(widths_array) > nppc:
            widths_array[-nppc:] = [a * self.tip_width_ratio * width for a in np.linspace(1, 0, nppc)]
...
class FillArrow(Line):
    ...
    def set_points_by_ends(self, start: np.ndarray, end: np.ndarray, buff: float = 0, path_arc: float = 0) -> None:
    ...
    def reset_points_around_ends(self):
        self.set_points_by_ends(self.get_start().copy(), self.get_end().copy(), path_arc=self.path_arc)
        return self
...
class CubicBezier(VMobject):
    def __init__(self, a0: np.ndarray, h0: np.ndarray, h1: np.ndarray, a1: np.ndarray, **kwargs):
        VMobject.__init__(self, **kwargs)
...
class Polygon(VMobject):
    ...
    def round_corners(self, radius: float | None = None):
        if radius is None:
            ...
            min_edge_length = min(get_norm(v1 - v2) for v1, v2 in zip(verts, verts[1:]) if not np.isclose(v1, v2).all())
