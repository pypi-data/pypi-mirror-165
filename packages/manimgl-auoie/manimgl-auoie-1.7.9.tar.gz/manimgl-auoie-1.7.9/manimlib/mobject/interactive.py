...
class Checkbox(ControlMobject):
    ...
    def get_cross(self) -> VGroup:
        cross = VGroup(
            Line(UP + LEFT, DOWN + RIGHT, **self.cross_kwargs), Line(UP + RIGHT, DOWN + LEFT, **self.cross_kwargs)
        )
...
class LinearNumberSlider(ControlMobject):
    CONFIG = {
        ...
        "circle_kwargs": {"radius": 0.1, "stroke_color": GREY_A, "fill_color": GREY_A, "fill_opacity": 1.0},
    }
    ...
    def __init__(self, value: float = 0, **kwargs):
        ...
        self.slider_axis = Line(start=self.bar.get_bounding_box_point(LEFT), end=self.bar.get_bounding_box_point(RIGHT))
...
class ColorSliders(Group):
    CONFIG = {
        ...
        "background_grid_kwargs": {"colors": [GREY_A, GREY_C], "single_square_len": 0.1},
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(Group(self.background, self.selected_color_box).fix_in_frame(), self.sliders, **kwargs)
...
class ControlPanel(Group):
    CONFIG = {
        ...
        "opener_kwargs": {"width": FRAME_WIDTH / 8, "height": 0.5, "fill_color": GREY_C, "fill_opacity": 1.0},
