...
class Surface(Mobject):
    ...
    def get_surface_points_and_nudged_points(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def pointwise_become_partial(self, smobject: "Surface", a: float, b: float, axis: np.ndarray | None = None):
    ...
    def get_partial_points_array(
        self, points: np.ndarray, a: float, b: float, resolution: npt.ArrayLike, axis: int
    ) -> np.ndarray:
        if len(points) == 0:
            return points
        nu, nv = resolution[:2]
    ...
    def always_sort_to_camera(self, camera: Camera):
        self.add_updater(lambda m: m.sort_faces_back_to_front(camera.get_location() - self.get_center()))
...
class TexturedSurface(Surface):
    ...
    def init_points(self):
        ...
        self.data["im_coords"] = np.array(
            [[u, v] for u in np.linspace(0, 1, nu) for v in np.linspace(1, 0, nv)]  # Reverse y-direction
        )
    ...
    def pointwise_become_partial(self, tsmobject: "TexturedSurface", a: float, b: float, axis: int = 1):
