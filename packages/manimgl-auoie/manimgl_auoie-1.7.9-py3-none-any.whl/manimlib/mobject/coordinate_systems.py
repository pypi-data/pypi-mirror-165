...
class CoordinateSystem(ABC):
    ...
    def get_x_axis_label(self, label_tex: str, edge: np.ndarray = RIGHT, direction: np.ndarray = DL, **kwargs) -> Tex:
        return self.get_axis_label(label_tex, self.get_x_axis(), edge, direction, **kwargs)

    def get_y_axis_label(self, label_tex: str, edge: np.ndarray = UP, direction: np.ndarray = DR, **kwargs) -> Tex:
        return self.get_axis_label(label_tex, self.get_y_axis(), edge, direction, **kwargs)

    def get_axis_label(
        self, label_tex: str, axis: np.ndarray, edge: np.ndarray, direction: np.ndarray, buff: float = MED_SMALL_BUFF
    ) -> Tex:
        label = Tex(label_tex)
    ...
    # Useful for graphing
    def get_graph(
        self, function: Callable[[float], float], x_range: Sequence[float] | None = None, **kwargs
    ) -> ParametricCurve:
        ...
        graph = ParametricCurve(lambda t: self.c2p(t, function(t)), t_range=t_range, **kwargs)
        graph.underlying_function = function
        graph.x_range = x_range
        return graph

    def get_parametric_curve(self, function: Callable[[float], np.ndarray], **kwargs) -> ParametricCurve:
        dim = self.dimension
        graph = ParametricCurve(lambda t: self.coords_to_point(*function(t)[:dim]), **kwargs)
        graph.underlying_function = function
        return graph

    def input_to_graph_point(self, x: float, graph: ParametricCurve) -> np.ndarray | None:
        if hasattr(graph, "underlying_function"):
            return self.coords_to_point(x, graph.underlying_function(x))
        else:
            alpha = binary_search(
                function=lambda a: self.point_to_coords(graph.quick_point_from_proportion(a))[0],
                target=x,
    ...
    def get_tangent_line(self, x: float, graph: ParametricCurve, length: float = 5, line_func: Callable[[np.ndarray, np.ndarray], T] = Line) -> T:
    ...
    def get_riemann_rectangles(
        ...
        for x0, x1 in zip(xs, xs[1:]):
            ...
            rect = Rectangle(width=self.x_axis.n2p(x1)[0] - self.x_axis.n2p(x0)[0], height=height)
...
class Axes(VGroup, CoordinateSystem):
    ...
    def create_axis(self, range_terms: Sequence[float], axis_config: dict[str], length: float) -> NumberLine:
    ...
    def add_coordinate_labels(
        self, x_values: Iterable[float] | None = None, y_values: Iterable[float] | None = None, **kwargs
    ) -> VGroup:
        axes = self.get_axes()
...
class ComplexPlane(NumberPlane):
    ...
    def add_coordinate_labels(self, numbers: list[complex] | None = None, skip_first: bool = True, **kwargs):
        if numbers is None:
