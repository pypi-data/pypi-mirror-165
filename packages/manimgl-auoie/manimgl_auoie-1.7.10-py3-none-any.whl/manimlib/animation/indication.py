...
from manimlib.constants import GREY, ORIGIN, RIGHT, SMALL_BUFF, TAU, UP, YELLOW
...
class AnimationOnSurroundingRectangle(AnimationGroup):
    ...
    def get_rect(self) -> SurroundingRectangle:
        return SurroundingRectangle(self.mobject_to_surround, **self.surrounding_rectangle_config)
...
class WiggleOutThenIn(Animation):
    ...
    def interpolate_submobject(self, submobject: Mobject, starting_sumobject: Mobject, alpha: float) -> None:
        ...
        submobject.scale(
            interpolate(1, self.scale_value, there_and_back(alpha)), about_point=self.get_scale_about_point()
        )
        submobject.rotate(
            wiggle(alpha, self.n_wiggles) * self.rotation_angle, about_point=self.get_rotate_about_point()
        )
