...
class FlowPlot:
    ...
    def marker_choice(self):
        ...
        if True:
            marker = pandas.read_csv('~/.local/scpantheon-dir/data/' + self.marker_file.filename)
...
def upload_callback(upload_button):
    ...
    def load():
        ...
        if filetype == 'csv':
            adata = anndata.read_csv('~/.local/scpantheon-dir/data/'+upload_button.filename)
            print('csv')
        elif filetype == 'h5ad':
            adata = anndata.read('~/.local/scpantheon-dir/data/'+upload_button.filename)
            print('h5ad')
        elif filetype == 'mtx':
            adata = sc.read_10x_mtx(
                '~/.local/scpantheon-dir/data/hg19/',  # the directory with the `.mtx` file
