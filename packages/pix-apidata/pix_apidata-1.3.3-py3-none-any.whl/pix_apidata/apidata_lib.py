...
class ApiData():
    ...
    async def unsubscribeGreeks(self, syms):
        await connection.invoke("UnsubscribeGreeks", [syms])

    async def subscribeSegments(self, needSnapshot=False):
        res = await connection.invoke("SubscribeSegments", [needSnapshot])
        return res

    async def subscribeOptionChainRange(self, spotName, expiryDate, numberOfStrikes):
