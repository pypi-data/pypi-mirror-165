...
class BaseApplication(ApplicationInterface, metaclass=BaseApplicationMeta):
    ...
    def _init_parameters(self, parameters: Union[Sequence[Any], Mapping[str, Any]]) -> bool:
        """Init application with new parameters. Does not trigger callbacks."""

        self.logger.debug("init_parameter")
        ...
        # Update application state with parameters. there is a collision with "schema" key and pydantic
        for p in parameters:
            app_param = parse_obj_as(Parameter, p)
            self.state.params[p["name"]] = app_param
    ...
    def _on_parameter(self, assets: Union[Sequence[Any], Mapping[str, Any]]) -> bool:
        ...
        for a in assets:
            asset_name = a.get("name", "anonymous_parameter")
            params: List[Parameter] = []
            all_params = {}
            for name, param in self.state.params.items():
                all_params.update({name: param})

            if "parameters" in a.keys():
                for p in a["parameters"]:
                    state_p = copy(self.state.params.get(p, None))
                    if state_p is None:
                        self.logger.error(
                            f"Encountered unconfigured parameter: {p} - aborting parameter change"
                        )
                        return False
                    if a["parameters"].get(p, None):
                        value = a["parameters"][p].get("value", None)
                        if value is not None:
                            state_p.value = value

                    checker = state_p.check()
                    if checker and not checker.is_valid:
                        self.logger.error(
                            f"Parameter {p} encountered validation error: {checker.error}"
                        )
                        return False
                    all_params.pop(state_p.name)
                    params.append(state_p)
            for name, state_p in all_params.items():
                checker = state_p.check()
                if checker and not checker.is_valid:
                    self.logger.error(
                        f"Parameter {name} encountered validation error: {checker.error}"
                    )
                    return False

                params.append(copy(state_p))
            # Call application on_parameter
            ret = self.on_parameter({asset_name: params})
        return ret
