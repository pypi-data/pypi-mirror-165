...
class PTWEPIDQC(ImagePhantomBase):
    ...
    # note there is a typo in the PTW EPID QC manual which lists one of the lp/mm as
    # 0.59 which is clearly wrong based on visual inspection.  Based on a report
    # found here  (https://www.wienkav.at/kav/kfj/91033454/physik/icheck/epidSoft_6X%20Highres.pdf)
    # as well as physical measurements communicated by personal email the correct value is 1.429
    _high_contrast_roi_settings_diag = {
        ...
        'roi d4': {'distance from center': 0.45, 'angle': 300, 'roi radius': 0.0775, 'lp/mm': 0.333},
    }

    _high_contrast_roi_settings_horiz = {
        'roi h1': {'distance from center': 0.52, 'angle': -2.8, 'roi radius': 0.018, 'lp/mm': 0.5},
        'roi h3': {'distance from center': 0.52, 'angle': 2.8, 'roi radius': 0.018, 'lp/mm': 0.667},
        'roi h4': {'distance from center': 0.35, 'angle': -2.6, 'roi radius': 0.009, 'lp/mm': 1.0},
        'roi h2': {'distance from center': 0.35, 'angle': 2.0, 'roi radius': 0.009, 'lp/mm': 1.429},
        ...
        'roi h7': {'distance from center': 0.17, 'angle': 180, 'roi radius': 0.007, 'lp/mm': 3.33},
    }

    _high_contrast_roi_settings_vert = {
        'roi v1': {'distance from center': 0.90, 'angle': 268.5, 'roi radius': 0.018, 'lp/mm': 0.5},
        'roi v3': {'distance from center': 0.90, 'angle': 271.5, 'roi radius': 0.018, 'lp/mm': 0.667},
        'roi v4': {'distance from center': 0.72, 'angle': 269, 'roi radius': 0.009, 'lp/mm': 1.0},
        'roi v2': {'distance from center': 0.72, 'angle': 271, 'roi radius': 0.009, 'lp/mm': 1.429},
        ...
        'roi v7': {'distance from center': 0.17, 'angle': 270, 'roi radius': 0.007, 'lp/mm': 3.33},
    }
        ...
        if image:
            ...
            if self.distance_results:
                for region in self.distance_results['regions'].values():
                    img_ax.plot(*zip(*region['contour']), color='yellow')
                    x, y = region['point'].x, region['point'].y
                    img_ax.plot([x], [y], 'o', color='yellow', markersize=2)
