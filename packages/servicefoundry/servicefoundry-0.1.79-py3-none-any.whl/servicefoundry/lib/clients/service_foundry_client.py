...
class ServiceFoundryServiceClient:
    ...
    def _tail_logs(
        ...
        sio.emit(
            ...
            json.dumps(
                {
                    # TODO: server should standardize this handshake
                    "pipelineRunName": resource_name,
                    "startTs": start_ts,
                }
            ),
        )
    ...
    def tail_build_logs(
        ...
        self._tail_logs(
            resource_name=build_response.name,
            tail_logs_url=build_response.tailLogsUrl,
            start_ts=build_response.logsStartTs,
            callback=callback,
            wait=wait,
        )
    ...
    def tail_logs(
        ...
        self._tail_logs(
            resource_name=pipeline_run.pipelineName,
            tail_logs_url=pipeline_run.tailLogsUrl,
            start_ts=pipeline_run.logsStartTs,
            callback=callback,
            wait=wait,
        )
