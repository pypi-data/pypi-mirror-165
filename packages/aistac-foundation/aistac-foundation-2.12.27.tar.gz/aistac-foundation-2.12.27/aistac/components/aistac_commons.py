...
class AistacCommons(object):
    ...
    def label_gen(limit: int=None) -> str:
        """generates a sequential headers. if limit is set will return at that limit. To use get the generator with
        gen = Commons.label_gen() then use next(gen) to retrieve the label"""
...
class DataAnalytics(object):
    ...
    @staticmethod
    def from_root(analytics_blob: dict, root: str) -> dict:
        ...
        return DataAnalytics(analysis=result).to_dict()
