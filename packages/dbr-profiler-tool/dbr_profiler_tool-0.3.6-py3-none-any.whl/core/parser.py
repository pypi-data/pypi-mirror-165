...
#'''{"profile": "", "url":"", "account_id":"dcdbb945-e659-4e8c-b108-db6b3ac3d0eb", "export_db": "logs", "verify_ssl": "False", "verbosity":"info", 
...
def parse_input_jsonargs(jsonargs):
    ...
    args['url']=url_validation(args['url'])
    ...
    cloudtype=''
    if 'azuredatabricks.net' in args['url']:
        cloudtype='azure'
    if 'cloud.databricks' in args['url']:
        cloudtype='aws'
    if 'gcp.databricks' in args['url']:
        cloudtype='gcp'
    else:
        raise ValueError(f'Unknown Cloud type detected {args["url"]}')
    args.update({'cloud_type':cloudtype})
    return args
